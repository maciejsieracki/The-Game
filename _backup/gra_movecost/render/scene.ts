 ...
 * ORAZ ukrywa/przywraca nakładki (las, śnieg, krzewy, oazy, rzekę).
 */
...
export function buildScene(map: GameMap, canvas: HTMLCanvasElement): SceneResult {
  ...
  // Klucze hexów wzdłuż trasy rzeki -- dla fog-of-war
...

  {
    ...
    // Zbierz klucze hexów rzeki dla fog-of-war
    riverHexKeys = riverPath.map(([q, r]) => `${q},${r}`);
  ...
  function setFog(visible: Set<string>, explored: Set<string>): void {
    ...
    // -- Rzeka: pokaż jeśli choć jeden hex trasy jest widziany lub odkryty
    if (riverTubeMesh !== null) {
      riverTubeMesh.visible = riverHexKeys.some(k => visible.has(k) || explored.has(k));
    }
  }
