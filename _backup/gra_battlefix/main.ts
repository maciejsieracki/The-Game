...
function boot(): void {
  try {
     ...
     *
     * Canned battle: FOUR Legionista (Rome) vs FOUR Falanga (Greece).
    ...
    function launchTestBattle(): void {
      ...
      // --- Look up Legionista and Falanga from data.units ---
      // The 'Jednostka' field in the JSON is plain ASCII for these two units.
      let legDef: any = (data.units as any[]).find((u: any) => u['Jednostka'] === 'Legionista');
      ...
      if (!legDef) {
        ...
        console.warn('[launchTestBattle] Legionista not found, using fallback:', legDef?.['Jednostka'] ?? 'NONE');
      }
      ...
      // --- Build 4 attacker BattleUnit (Legionista) ---
      const legHp  = unitHealth(legDef);
      const legAtk = unitAtak(legDef);
      // categoryOf uses normalised name -- 'Legionista' matches 'legionist' -> 'miecznik'
