...
export function generateMap(
  ...
  // ---------------------------------------------------------------------------
  // Rzeki -- deterministyczne sledzenie od Gory/Wzgorza do morza
  // ---------------------------------------------------------------------------
  //
  // Logika odzwierciedla ta z scene.ts (descend lowest elevation rank).
  // Uzywamy rand() (mulberry32) do deterministycznego wyboru zrodel.
  // Wynik: map.riverPaths + hex.rzeka.obecna = true dla kazdego hexa trasy.
  // ---------------------------------------------------------------------------

  /** Ranking wysokosci terenu (nizszy = nizej nad poziomem morza). */
  const ELEVATION_RANK: Record<TerenBazowy, number> = {
    [TerenBazowy.Morze]:    0,
  ...
  /** Szesc sasiadow aksjalnych (pointy-top). */
  const RIVER_NEIGHBORS: [number, number][] = [
    [+1,  0], [+1, -1], [0, -1],
    [-1,  0], [-1, +1], [0, +1],
  ];
  ...
  /** Sledzi rzeke od (sq, sr) do morza/krawedzi; zwraca sciezke wspolrzednych. */
  function traceRiver(sq: number, sr: number): Array<{ q: number; r: number }> {
    const path: Array<{ q: number; r: number }> = [{ q: sq, r: sr }];
    const visited = new Set<string>([`${sq},${sr}`]);
    let cq = sq, cr = sr;

    for (let step = 0; step < MAX_RIVER_LEN; step++) {
      const curHex = hexes[`${cq},${cr}`];
      if (!curHex) break;
      if (curHex.terenBazowy === TerenBazowy.Morze) break;

      // Znajdz sasiada z najnizszym rankingiem terenu (schodzi w dol)
      let bestRank = ELEVATION_RANK[curHex.terenBazowy];
      let bestNeighbors: Array<[number, number]> = [];

      for (const [dq, dr] of RIVER_NEIGHBORS) {
        const nq = cq + dq;
        const nr = cr + dr;
        const key = `${nq},${nr}`;
      ...
      if (bestNeighbors.length === 0) break; // utknelismy
      // Deterministyczny wybor wsrod roworzednych: bierzemy pierwszy
      ...
      // Sprawdz czy dotarlismy do morza
      const nHex = hexes[`${cq},${cr}`];
      if (nHex && nHex.terenBazowy === TerenBazowy.Morze) break;
    }
    return path;
  }

  // Zbierz kandydatow na zrodla rzek: Gory i Wzgorza z interiorow mapy
  ...
  // Szukamy 1-2 rzek; dla kazdej robimy max 8 prob wyboru zrodla
  ...
  if (total > 0) {
    ...
    for (let river = 0; river < 2; river++) {
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        // Deterministyczny wybor kandydata oparty na rand() i offsecie rzeki
        const seedOffset = Math.floor(rand() * total);
        const idx = (seedOffset + attempt * step) % total;
        const src = riverSources[idx];
        if (!src) continue;
        const srcKey = `${src[0]},${src[1]}`;
        if (usedHexKeys.has(srcKey)) continue;

        const candidate = traceRiver(src[0], src[1]);
        if (candidate.length > best.length) best = candidate;
      }

      // Akceptuj trase jesli ma wystarczajaca dlugosc
      if (best.length >= MIN_RIVER_LEN) {
        riverPaths.push(best);
        // Oznacz heksy rzeki: rzeka.obecna = true
  ...
  // Fallback: jesli zadna rzeka nie zostala wygenerowana, dodaj falista linie
  if (riverPaths.length === 0) {
    ...
    if (fallback.length >= MIN_RIVER_LEN) {
      riverPaths.push(fallback);
    }
  }

  return { szerokoscQ: width, wysokoscR: height, hexes, seed, riverPaths };
}
