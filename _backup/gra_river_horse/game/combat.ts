...
export function terrainDefenseMultiplier(
  ...
  const terrLow = defenderTerrain.toLowerCase();

  const entry = terrainData.find((t) => {
    const tLow = t['Teren'].toLowerCase();
    return tLow.includes(terrLow) || terrLow.includes(tLow.split(' ')[0] ?? '');
  });
  ...
  const eName = entry['Teren'].toLowerCase();

  // Las: +50% ONLY vs ranged / cavalry
  if (eName.includes('las')) {
    const aLow = attackerRola.toLowerCase();
    const isRangedOrCav =
  ...
  if (eName.includes('gory') || eName.includes('góry')) {
    return 1.75;
  }

  // Wzgorza (hills)
  if (eName.includes('wzg') || eName.includes('wzgó')) {
    return 1.5;
  }
...
export function terrainRiverAttackMultiplier(
  ...
  if (!defenderTerrain || terrainData.length === 0) return 1.0;
  const terrLow = defenderTerrain.toLowerCase();
  const isRiver = terrLow.includes('rzek') || terrLow.includes('river');
  if (!isRiver) return 1.0;
  const entry = terrainData.find((t) => t['Teren'].toLowerCase().includes('rzek'));
