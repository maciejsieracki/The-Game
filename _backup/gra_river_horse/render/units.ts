...
export function buildUnitModel(category: string, ownerColor_: number): THREE.Group {
  switch (category) {
    ...
    case 'konnica': {
      ...
      const RSPEAR_X = AV_ARM_OFFSET_X * 0.85 + AV_ARM_W * 0.4 + 0.015 * HEX_R;
      ...
      mRSpearTip.position.set(RSPEAR_X, RIDER_BOT + 0.40 * HEX_R + 0.028 * HEX_R, 0);
      group.add(mRSpearTip);

      group.userData['mats'] = mats;
      group.userData['perTokenGeos'] = [gRSash, gRArmL, gRArmR, gRHead, gREyeL, gREyeR];
      return group;
    }
    ...
    case 'rydwan': {
      ...
      const mWood   = mat(COLOR_CHARIOT,  0.05, 0.85);  // warm wood brown
      const mOwner  = mat(ownerColor_,    0.10, 0.70);  // owner-color front panel
      const mSteel  = mat(COLOR_STEEL,    0.40, 0.50);  // metal fittings / axle
      const mBronze = mat(COLOR_BRONZE,   0.30, 0.55);  // bronze wheel rim / yoke
      const mHorse  = mat(COLOR_HORSE,    0.10, 0.80);  // brown horse body
      const mMane   = mat(0x1a0e06,       0.05, 0.85);  // near-black mane / tail
      const mSkin   = mat(COLOR_SKIN,     0.05, 0.80);  // driver skin
      const mCloth  = mat(COLOR_LEATHER,  0.05, 0.85);  // driver clothing
      ...
      group.userData['perTokenGeos'] = [
        ...
        gPole, gYoke, gRein,
        gDLegL, gDLegR, gDTorso, gDSash, gDArmL, gDArmR, gDHead, gDEyeL, gDEyeR, gHelm
      ];
      return group;
    }
