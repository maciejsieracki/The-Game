...
const RIVER_BANK_COLOR = 0x5a3e1e;  // ciemniejszy, ziemisty kolor brzegu
...
/** Wpis nakładki oazy — mesh + klucz heksa. */
interface OasisItem {
  obj: THREE.Mesh;
  key: string;
}
...
export function buildScene(map: GameMap, canvas: HTMLCanvasElement): SceneResult {
  ...
  // Tablica oaz z kluczem heksa -- dla fog-of-war
  const oasisItems: OasisItem[] = [];
  ...
  // ---------------------------------------------------------------------------
  // Tablice nakładek dla fog-of-war (las, śnieg, krzewy)
  ...
  for (const hex of Object.values(map.hexes)) {
    ...
    if (hex.nakladka === Nakladka.Las) {
      ...
      for (let ti = 0; ti < treeCount && forestIdx < forestMesh.count; ti++) {
        ...
        // Zapisz klucz heksa i oryginalną macierz dla fog-of-war
    ...
    if (t === TerenBazowy.Pustynia) {
      ...
      if (oasisRoll < 1.0 / 6.0) {
        ...
        oasisItems.push({ obj: pool, key: hexKey });
        ...
        for (let pi = 0; pi < palmCount; pi++) {
          ...
          oasisItems.push({ obj: trunk, key: hexKey });
          ...
          for (let fi = 0; fi < frondCount; fi++) {
            ...
            oasisItems.push({ obj: frond, key: hexKey });
          }
        }
      }
    }
  ...
  //
  // Każda ścieżka map.riverPaths[i] to tablica {q,r} od źródła do ujścia.
  // Dla każdej ścieżki budujemy DWA płaskie ribbon-meshe:
  //
  //   1. WODA (riverWaterMat, kolor ~0x3a86b5):
  //      Płaska wstęga szerokości R*0.38, leżąca PONIŻEJ szczytu terenu
  //      (y = terrainTop - 0.04*R). Wygląda jak wycięty kanał wodny.
  //
  //   2. BRZEGI (riverBankMat, kolor ciemnoziemisty ~0x5a3e1e):
  //      Szersza wstęga (halfWidth = R*0.52) po obu stronach, na tym samym Y
  //      co woda (albo o 0.01*R niżej). Dzięki temu woda jest "w kanale"
  //      obramowanym ciemniejszymi brzegami — efekt wyrzeźbionej doliny.
  //
  // Obie geometrie budujemy funkcją buildRibbonGeometry():
  //   - interpolacja CatmullRom przez środki hexów ścieżki
  //   - wstęga z trójkątów, normalne skierowane w górę
  //   - brak rotacji/unoszenia -- siada płasko na terenie
  //
  // Mgła wojenna:
  //   - riverHexKeys: unia kluczy hex wszystkich ścieżek
  //   - entry.mesh.visible = some hex seen/explored (jak poprzednio)
  //   - obydwa mesze (woda + brzeg) chowane razem
  //
  // Jeśli map.riverPaths jest puste lub niezdefiniowane, nie renderujemy nic.
  // ---------------------------------------------------------------------------

  const paths = map.riverPaths ?? [];

  // Parametry wyglądu rzeki
  const RIVER_WATER_HALF_WIDTH = R * 0.38; // szerokość połówkowa wody
  const RIVER_BANK_HALF_WIDTH  = R * 0.52; // szerokość połówkowa z brzegiem
  const RIVER_DEPTH_BELOW      = R * 0.04; // zagłębienie poniżej szczytu terenu
  const RIVER_BANK_EXTRA_DEPTH = R * 0.01; // brzeg jeszcze nieco niżej (opcjonalne)

  for (const path of paths) {
    if (path.length < 2) continue; // za krótka ścieżka -- pomijamy

    // Zbierz hex-klucze tej ścieżki
    const pathHexKeys = new Set<string>();
    for (const { q, r } of path) {
      pathHexKeys.add(`${q},${r}`);
    }

    // Zbuduj punkty 3D ze środków hexów wzdłuż ścieżki
    // Y ustawiamy PONIŻEJ szczytu terenu: terrainTop - RIVER_DEPTH_BELOW
    const riverPoints: THREE.Vector3[] = path.map(({ q, r }) => {
      const hex = map.hexes[`${q},${r}`];
      const t = hex ? hex.terenBazowy : TerenBazowy.Laka;
      const vis = TERRAIN_VISUALS[t];
      const { x, z } = axialToWorld(q, r, R);
      const yTop = vis.height + vis.yOffset - RIVER_DEPTH_BELOW; // wycięty kanał
      return new THREE.Vector3(x, yTop, z);
    });

    // -- Woda: płaska wstęga (halfWidth = RIVER_WATER_HALF_WIDTH)
    const waterGeo = buildRibbonGeometry(riverPoints, RIVER_WATER_HALF_WIDTH, 8);
    const waterMesh = new THREE.Mesh(waterGeo, riverWaterMat);
    waterMesh.castShadow = false;
    waterMesh.receiveShadow = true;
    scene.add(waterMesh);

    // -- Brzegi: szersza wstęga (halfWidth = RIVER_BANK_HALF_WIDTH), nieco niżej
    // Brzegi leżą na tym samym Y co woda lub o RIVER_BANK_EXTRA_DEPTH niżej.
    // Budujemy je z tych samych punktów (lekko obniżonych).
    const bankPoints: THREE.Vector3[] = riverPoints.map(p =>
      new THREE.Vector3(p.x, p.y - RIVER_BANK_EXTRA_DEPTH, p.z),
    );
    const bankGeo = buildRibbonGeometry(bankPoints, RIVER_BANK_HALF_WIDTH, 8);
    const bankMesh = new THREE.Mesh(bankGeo, riverBankMat);
    bankMesh.castShadow = false;
    bankMesh.receiveShadow = true;
    scene.add(bankMesh);

    riverEntries.push({ waterMesh, bankMesh, waterGeo, bankGeo, hexKeys: pathHexKeys });

    // Dołącz hex-klucze tej rzeki do globalnej listy (fog-of-war)
    for (const k of pathHexKeys) {
      riverHexKeys.push(k);
    }
  }
  ...
  // Macierz zerowa (skala 0,0,0) — ukrywa instancję InstancedMesh
  ...
  function setFog(visible: Set<string>, explored: Set<string>): void {
    ...
    // -- Baza: przelicz kolory prizmów terenu
    ...
    // -- Nakładki instancji: las (stożki)
    const forestCount2 = forestMesh.count;
    for (let i = 0; i < forestCount2; i++) {
      const k = forestHexKey[i];
      if (k === undefined) continue;
      if (isHidden(k)) {
        forestMesh.setMatrixAt(i, ZERO_MATRIX);
      } else {
        const m = forestOrigMatrix[i];
        if (m) forestMesh.setMatrixAt(i, m);
      }
    }
    forestMesh.instanceMatrix.needsUpdate = true;

    // -- Nakładki instancji: śnieg na górach
    const snowCount2 = snowMesh.count;
    for (let i = 0; i < snowCount2; i++) {
      const k = snowHexKey[i];
      if (k === undefined) continue;
      if (isHidden(k)) {
        snowMesh.setMatrixAt(i, ZERO_MATRIX);
      } else {
        const m = snowOrigMatrix[i];
        if (m) snowMesh.setMatrixAt(i, m);
      }
    }
    snowMesh.instanceMatrix.needsUpdate = true;

    // -- Nakładki instancji: krzewy na wzgórzach
    const shrubCount2 = shrubMesh.count;
    for (let i = 0; i < shrubCount2; i++) {
      const k = shrubHexKey[i];
      if (k === undefined) continue;
      if (isHidden(k)) {
        shrubMesh.setMatrixAt(i, ZERO_MATRIX);
      } else {
        const m = shrubOrigMatrix[i];
        if (m) shrubMesh.setMatrixAt(i, m);
      }
    }
    shrubMesh.instanceMatrix.needsUpdate = true;

    // -- Oazy: pokaż/ukryj per-mesh
    for (const { obj, key } of oasisItems) {
      obj.visible = !isHidden(key);
    }

    // -- Rzeki: każda siatka (woda + brzegi) widoczna, gdy choć jeden jej hex jest widziany lub odkryty
    for (const entry of riverEntries) {
