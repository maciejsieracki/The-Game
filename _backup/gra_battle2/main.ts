...
import { canFoundCity, foundCity, cityName } from './game/cities';
import { CityRenderer } from './render/cities';
...
function boot(): void {
  try {
    ...
    const cities: City[] = [];
    const cityRenderer = new CityRenderer(scene, map);
     ...
     *
     * Builds attacker (player units, ownerId===0) and defender (up to 3 nearest
     * enemy units, ownerId!==0) arrays.  If either side has no combat-capable
     * units, a synthetic sample army is used.  Then opens the pre-battle screen.
     */
    function launchTestBattle(): void {
      const PLAYER_COLOR  = 0xffd54a;
      const ENEMY_COLOR   = 0xc84040;

      // --- ATTACKER: player units with Atak > 0 ---
      const playerRaw = units.filter(u => u.ownerId === 0);
      let attackerUnits: BattleUnit[] = playerRaw
        .map(u => {
          const def = lookupUnitDef(u.typeId);
          return runtimeToBattleUnit(u, def, PLAYER_COLOR);
        })
        .filter(bu => unitAtak(bu.stats as any) > 0);

      if (attackerUnits.length === 0) {
        // Synthesise: pick first combat-capable entry from data.units
        const sample = (data.units as any[]).find(
          (u: any) => normFieldVal(u['Atak'], 0) > 0 && u['Health'],
        );
        if (sample) {
          attackerUnits = [defToBattleUnit(sample, 0, PLAYER_COLOR)];
        } else {
          attackerUnits = [defToBattleUnit({
            'Jednostka': 'Wojownik',
            'Atak': 6, 'Obrona': 6, 'Uderzenie': 4,
            'Pancerz': 4, 'Przebicie': 0, 'Health': 30,
            'Rola (linia)': 'Wrecz',
            'Prog dezercji (% health)': 0.25,
            'Atak dystansowy': 0, 'Zasieg ataku (hex)': null,
            'Ilosc pociskow': null, 'Ruch w bitwie (heksy)': 4,
            'Kara obrony z flanki (%)': 15, 'Kara obrony z tylu (%)': 30,
          }, 0, PLAYER_COLOR)];
        }
      }

      // --- DEFENDER: up to 3 nearest enemy units ---
      const enemyRaw = units.filter(u => u.ownerId !== 0);
      const refUnit = playerRaw[0];
      const refQ = refUnit ? refUnit.q : 0;
      const refR = refUnit ? refUnit.r : 0;

      const sortedEnemies = enemyRaw
        .slice()
        .sort((a, b) => hexDistance(refQ, refR, a.q, a.r) - hexDistance(refQ, refR, b.q, b.r))
        .slice(0, 3);

      let defenderUnits: BattleUnit[] = sortedEnemies.map(u => {
        const def = lookupUnitDef(u.typeId);
        return runtimeToBattleUnit(u, def, ENEMY_COLOR);
      });

      if (defenderUnits.length === 0) {
        // Synthesise 2 sample defenders from data.units
        const candidates: any[] = (data.units as any[]).filter(
          (u: any) => normFieldVal(u['Atak'], 0) > 0 && u['Health'],
        );
        // Try to find Wlocznik and Lucznik by normalised name match
        function nameNorm(s: string): string {
          return s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\u0141\u0142]/g, 'l').toLowerCase();
        }
        const wlocznik = candidates.find((u: any) =>
          nameNorm((u['Jednostka'] as string) ?? '').includes('wloczn'),
        ) ?? candidates[0];
        const lucznik = candidates.find((u: any) =>
          nameNorm((u['Jednostka'] as string) ?? '').includes('luczn'),
        ) ?? candidates[1];
        if (wlocznik) defenderUnits.push(defToBattleUnit(wlocznik, 10, ENEMY_COLOR));
        if (lucznik && lucznik !== wlocznik) defenderUnits.push(defToBattleUnit(lucznik, 11, ENEMY_COLOR));
        if (defenderUnits.length === 0) {
          defenderUnits = [defToBattleUnit({
            'Jednostka': 'W\u0142\u00f3cznik',
            'Atak': 6, 'Obrona': 6, 'Uderzenie': 2,
            'Pancerz': 2, 'Przebicie': 0, 'Health': 80,
            'Rola (linia)': 'Wrecz',
            'Prog dezercji (% health)': 0.25,
            'Atak dystansowy': 0, 'Zasieg ataku (hex)': null,
            'Ilosc pociskow': null, 'Ruch w bitwie (heksy)': 2,
            'Kara obrony z flanki (%)': 30, 'Kara obrony z tylu (%)': 50,
          }, 10, ENEMY_COLOR)];
        }
      }

      // --- TERRAIN: hex of the first player unit ---
      let teren = 'Laka';
      if (playerRaw.length > 0) {
        const firstUnit = playerRaw[0]!;
        const hexKey = keyOf(firstUnit.q, firstUnit.r);
        const hex = map.hexes[hexKey];
        if (hex) teren = hex.terenBazowy as string;
      }

      // --- szanseAtkPct: rough 50+delta estimate (clamped 10..90) ---
      const sumAtkAtk = attackerUnits.reduce((s, bu) => s + unitAtak(bu.stats as any), 0);
      const sumDefDef = defenderUnits.reduce((s, bu) => s + normFieldVal((bu.stats as any)['Obrona'], 5), 0);
      const szanseAtkPct = Math.max(10, Math.min(90, 50 + (sumAtkAtk - sumDefDef) * 5));

      // --- PreBattleInfo ---
      const pbInfo: PreBattleInfo = {
        atakujacy: {
          nazwa: 'Gracz',
          cywilizacja: 'Tw\u00f3j lud',
          units: attackerUnits.map((bu): PreBattleUnit => ({
            nazwa:     bu.nazwa,
            kategoria: bu.kategoria,
            hp:        bu.hp,
            maxHp:     bu.maxHp,
            atak:      unitAtak(bu.stats as any),
          })),
        },
        obronca: {
          nazwa: 'Wr\u00f3g',
          cywilizacja: 'Barbarzy\u0144cy',
      ...
      showPreBattle(pbInfo, {
        ...
        onBattlefield: () => {
          ...
          bs.play((res) => {
            // res.winner is 'atakujacy' | 'obronca'
            const winnerLabel =
    ...
    window.addEventListener('keydown', (e: KeyboardEvent) => {
      ...
      // --- B: Found a city (settler only) ---
      if (e.key.toLowerCase() === 'b') {
        // Gallery mode: ignore.
        if (galleryOn) return;

        const u = selectedId !== null ? units.find(x => x.id === selectedId) : undefined;
        if (u && u.ownerId === 0 && u.category === 'osadnik') {
