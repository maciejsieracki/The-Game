 ...
 *         jedna TubeGeometry CatmullRom per ścieżkę; wiele rzek obsługiwane.
 *
...
/** Jedna siatka rzeki + zbiór kluczy hex wzdłuż jej trasy (dla fog-of-war). */
...
export function buildScene(map: GameMap, canvas: HTMLCanvasElement): SceneResult {
  ...
  // ---------------------------------------------------------------------------
  // Rzeka z danych mapy -- wczytywana z map.riverPaths
  // ---------------------------------------------------------------------------

  // Wspólny materiał dla wszystkich rzek
  const riverMat = new THREE.MeshLambertMaterial({ color: RIVER_COLOR });

  // Tablica wszystkich siatek rzek z ich hex-kluczami (fog-of-war)
  const riverEntries: RiverEntry[] = [];

  // Zbiór wszystkich hex-kluczy wzdłuż rzek (unia wszystkich ścieżek) -- dla fog-of-war
  // Używamy tablicy łańcuchów dla zgodności z istniejącą logiką fog (riverHexKeys.some(...))
  ...
  // Dla każdej ścieżki:
  //   1. Przeliczamy środki hexów na punkty 3D (axialToWorld + y na szczycie terenu + offset).
  //   2. Budujemy CatmullRomCurve3 → TubeGeometry → Mesh.
  //   3. Zbieramy klucze hex dla fog-of-war.
  // Jeśli map.riverPaths jest puste lub niezdefiniowane, nie renderujemy żadnej rzeki.
  // ---------------------------------------------------------------------------
  ...
  for (const path of paths) {
    ...
    const riverPoints: THREE.Vector3[] = path.map(({ q, r }) => {
      ...
      const yTop = vis.height + vis.yOffset + 0.05; // tuż nad powierzchnią terenu
    ...
    const geo = new THREE.TubeGeometry(curve, segments, R * 0.12, 8, false);
    const mesh = new THREE.Mesh(geo, riverMat);
    mesh.castShadow = false;
    mesh.receiveShadow = true;
    scene.add(mesh);

    riverEntries.push({ mesh, geo, hexKeys: pathHexKeys });
  ...
  function dispose() {
    ...
    // Rzeki
    for (const entry of riverEntries) { entry.geo.dispose(); }
    riverMat.dispose();
    renderer.dispose();
  }
  ...
  function setFog(visible: Set<string>, explored: Set<string>): void {
    ...
    // -- Rzeki: każda siatka widoczna, gdy choć jeden jej hex jest widziany lub odkryty
    for (const entry of riverEntries) {
      entry.mesh.visible = Array.from(entry.hexKeys).some(k => visible.has(k) || explored.has(k));
    }
  }
