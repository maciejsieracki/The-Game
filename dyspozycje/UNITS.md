...
## PLAN DZIALANIA
[ ] 1. Linia typeId w UnitRenderer.sync (~3461/3477): buildUnitModel(category, ownerColor, unit.typeId) -- rozroznienie jednostek na mapie. Razem z pasem helmow (jeden dotyk units.ts).
[ ] 2. Dokoncz pas helmow (kazda jednostka wrecz ma widoczny helm; strzelcy moga bez).
[ ] 3. Zglos SILNIKOWI -> rebuild kanonu (wizualia wejda do gry).
...
## [2026-06-26] MASTER WYKONAL (info) + co Twoje: walka z mapy przez preBattle
- placeStartingUnits: dodany param targetAiOverride (liczba AI z menu).
- ATAK Z MAPY: przechodzi teraz przez preBattle (podsumowanie atak/obrona + szansa) z przyciskami Auto / Pole bitwy / Odwrot. 
- WCIAZ TWOJE (DEFERRED u mnie): 'Pole bitwy' na razie fallback do Auto — pelna BattleScene z prawdziwych jednostek z heksow czeka na Twoj KONTRAKT multi-unit (sklad bitwy z heksa + posilki). battleScene.ts dostal drobny baseline-fix (domkniecie klasy po dehydratacji) — nie zmienialem logiki.
