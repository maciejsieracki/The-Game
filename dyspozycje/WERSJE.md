...
## ROBOCZA bc76a12e - 2026-09-26 16:17 UTC - zywnosc + automanage + ai-miasta (3 tematy)

||- md5 (pelne): bc76a12ed28947153b7ec32ceeb6b9d3 · sha256: a80e6a62b998aa23a18dc9524e47dda2dfae392b1c0dc366b486c42cb83198b8 · stempel label `bc76a12e` · integration/publish commit: `27fe22fc92770490ea119ce22bda45ad1643a453` (bundle commit: `c1adee26`) · base: `f057729a` (origin/main)
||- **Trzy rownolegle tematy scalone razem** na wyrazne polecenie wlasciciela: (1) `R-EKONOMIA-ZYWNOSC-NADWYZKA-PO-LIMICIE-Q1-20260925` (fa20567c) -- nadwyzka zywnosci po osiagnieciu limitu populacji (popCapByCityId, gra/src/main.ts +55/-1). (2) `R-AUTOMANAGE-ULEPSZENIA-NIE-BUDOWANE-Q1-20260925` (0361c8b9) -- autonomiczna kolejka ulepszen: pickAutoBuildItem dostaje fallback do ulepszen dla tryb='priorytet', tozsamy z istniejacym dla tryb='lista' (gra/src/game/auto-manage.ts). (3) `R-AI-ZAKLADANIE-MIAST-TELEPORTACJA-Q1-20260925` (bcc11fe5) -- major AI Trudny nie zaklada juz bonusowych miast poza wlasnym promieniem terytorium stolicy (pickBonusCityHex, gra/src/game/ai-difficulty-bonus.ts).
||- Kazdy temat: Operator -> niezalezny Evaluator -> Final Control, wszystkie PASS. Zero konfliktu plikowego miedzy tematami (kazdy dotyka innych plikow produkcyjnych, potwierdzone `git diff --name-only` per temat). Integracja: 3x `git merge --no-ff` z czystego `origin/main` (f057729a) w swiezym worktree, zero konfliktow merge.
||- Bramki: `tsc --noEmit` PASS. Testy celowane: `auto-wyzywienie-live-recalc-test` 60/60, `auto-manage-ulepszenia-fallback-test` 11/11, `auto-manage-test` 45/45, `ai-founding-territory-test` 29/29, `cities-buildings-production-owner-contract-test` (regresja wspolna niezwiazana) 53/53 -- wszystkie zielone. 4 legacy zywnosc-testy (`spichlerz-widocznosc-test` 37/8, `akwedukt-popcap-test` 6/1, `spichlerz-deficyt-scalenie-test` 54/4, `spichlerz-wzrost-test` 2/7) potwierdzone **identyczne liczbowo** na scalonym drzewie i na osobnym czystym `origin/main` worktree (`/tmp/clean-baseline-check`) -- rozstrzyga to ostatecznie wczesniej otwarty punkt: zero regresji wprowadzonej przez te trzy tematy. `git diff --check` PASS. `vite build` 892 modulow PASS (bezposrednio, bez npm run).
||- `node gra/tools/verify-robocza-bundle.cjs` -> `VERIFY OK`, manifest match OK (stamp match WARN oczekiwany na Linuksie, znany nieszkodliwy false-positive). Smoke test headless Chromium na zbudowanym bundlu: tytul `The Game — 4X`, canvas obecny, pelny HUD renderuje sie (Skarbiec/Praca/Spichlerz/Nauka/Armia/Miasta/Obywatele/Kultura/Religia), zero bledow strony, zero bledow konsoli.
||- **Poza tym deployem**: `ai_profil_obronna` (lokalny commit `b4c9582f`, niezmergowany), pakiet Dyplomacja 8 pol (implementacja w toku), PR #142 rally-point UI (`2cca1748`, OPEN/MERGEABLE, niezalezny od tej integracji) -- wszystkie czekaja na wlasny cykl.
||- Push do zdalnego repo: WYKONANY -- patrz commit `c1adee26` na `origin/main` (readback w KANAL-PRACA.md).
||- **AKTUALNA**

## ROBOCZA 706bf94a - 2026-09-25 19:00 UTC - civ-matrix consolidated integration (PR #140) -- ZASTĄPIONA przez ROBOCZA bc76a12e (2026-09-26)
...
|- Publish commit: `5b9f38143e5f61d5e78c0392905fc08ac0d298f3`; source/product release remains `8eba87007a866edc80c4776df77697af65e17f2d`; product `main` at publish: `4331f04b8464275824b38f1e50c9d34d564cb908`; docs close: `79790811c89a9bd0fc2666042b26539587138ff0`.
...
|- `node gra/tools/verify-robocza-bundle.cjs` → `VERIFY OK`; manifest match `OK`; Linux stamp match `WARN` jest znanym zachowaniem portu Node. Product release commit `104fce6d5002ff3e0d64083268dd4434dbb0452e`; docs-only republish closure and remote readback `PASS`; byte-identical no-op republish. Brak promocji do KANON/FINALNA.
...
|- Status: **DEPLOYED — remote branch and origin/main read back as `2aeb52743476bd1175789049b350a34f1888dda3`**.
...
|- **`R-MIASTA-ZDOBYCIE-RAPORT-TROFEA-Q1`** (`6d082b9a`, 2 rundy) — modal zdobycia miasta i eliminacji pokazuje strukturalny raport (złoto/nauka/technologie/Power/budynki) zamiast ogólników, plus wpis w panelu wydarzeń. Ekonomia przejęcia nietknięta. Runda 1 `DECISION_REQUIRED` (sprzeczność w dispatchu: bramka asercjonowała literał który GOAL kazał usunąć) rozwiązana rozszerzeniem allowlisty; runda 2 przepisała bramkę na strukturę `reportRows` bez osłabienia (24→35 asercji). Nowa bramka `miasto-zdobycie-raport-test.cjs` 95/0.
|- **`R-WYCINKA-DREWNO-50-I-KOMUNIKAT-CAP-Q1`** (`4e38cece`, 1 runda) — wyrąb lasu: 25→50 Drewna. Naprawiony przy okazji komunikat gracza — pokazywał wartość SPRZED przycięcia limitem magazynu, teraz pokazuje faktycznie zapisaną ilość (jak ścieżka AI). Nowa bramka `wycinka-drewno-cap-test.cjs` 15/0.
|- **`R-WOJNA-WYMUSZONA-PAROWANIE-ZAMIAST-DOMINA-Q1`** (`1a7a48e9`, 2 rundy) — mechanizm „domino" (dodawał gracza jako trzeci front do istniejącej wojny AI↔AI, zostawiał sojusznika gracza bez ŻADNEJ wojny gdy sojusz blokował całą parę) zastąpiony parowaniem 1v1: cywilizacje bez wojny parowane pierwsze (gracz jak każde AI, bez limitu), nieparzysta reszta dołącza jako trzeci do istniejącej pary. Runda 1: Evaluator znalazł zachłanny algorytm parowania niegwarantujący maksymalnego dopasowania (adwersaryjna topologia blokad) — naprawiony na dokładny max-matching (DP na bitmasce), zweryfikowany fuzzem 4000+ prób. Nowa bramka `wojna-wymuszona-parowanie-test.cjs` 47/0.
|- **`R-RELIGIA-KONWERSJA-PO-PODBOJU-Q1`** (`1da7a6bd`, 1 runda) — obca religia po podboju nigdy nie stawała się dominująca (zdobywca dostawał 0% obecności religijnej w mieście z różnym okręgiem kulturowym, `convertViaTemple` nigdy się nie odpalał). Naprawione analogicznie do już istniejącego mechanizmu kultury (`onCityCapturedCulture`) — nowa `onCityCapturedReligion`, ten sam okrąg = 100% jak dziś, różny okrąg = inwersja udziału. Evaluator własną kontrolą znalazł i Obrona naprawiła dwa błędy zachowania populacji (suma > populacja przy 3+ religiach, suma < populacja przy braku danych o poprzedniej religii). Nowa bramka `religia-konwersja-po-podboju-test.cjs` 12/0.
...
|- **`R-SZCZESCIE-PRZEBUDOWA-SKALI-Q1`** (`04660f9d`, **cztery rundy**, Final Control PASS bez zastrzeżeń) — **najgłębsza zmiana balansu od początku projektu.** Wszystkie liczby od właściciela, żadna nie strojona; zweryfikowane niezależnie przez Evaluatora i dwukrotnie przez Final Control, pozycja po pozycji z JSON.
    Rdzeń: lista budynków szczęściodajnych przeniesiona do DANYCH (`dajeSzczescie` per budynek, 19 tak / 22 nie) · Spichlerz +5, Świątynia/Teatr/Akademia 3/4/4, sześć cudów po +6 · Kultura i Religia proporcjonalnie, skala 10/16/23 per epoka · Wealth z epoką (cap +10 w każdej epoce, rosnący próg) · podatki liniowo −10..+10 przy progu 90%, punkt obojętny 45% · zaopatrzenie symetryczne ±2 na surowiec, wprost w danych · wojna −5 · bonus osiedla [15,12,8,5] · zagęszczenie USUNIĘTE · `szczescie_max_epoka` **per trudność** 20/40/60 · 30/50/70 · 35/55/80 · `szczescie_max_pop_wspolczynnik` 0,04 płasko · **siedem martwych parametrów usuniętych na stałe**.
    **Dwie naprawy strukturalne znalezione po drodze, obie przez Evaluatora i Final Control, nie przez Operatora:** (1) panel miasta nie przekazywał `haCuda` i zamrażał `atWar:false` — luka do **41 punktów** wobec silnika; teraz jeden tor, pilnowany bramką, która realnie buduje i URUCHAMIA panel w jsdom, zamiast porównywać funkcję z samą sobą. (2) Dwa fallbacki (`SZMAX_DEFAULTS`, `SZ_MAX_POP_WSP_DEFAULT`) rozjechane z danymi, przy czym drugi był **całkowicie niepilnowany** — mutacja 0,048 → 0,5, czyli dziesięciokrotna, zostawiała **sześć bramek zielonych**, bo jedyna asercja porównywała stałą z samą sobą. Tautologia przeżyła trzy rundy, Evaluatora i Obronę; złapał ją dopiero Final Control własną mutacją. **Lekcja do zapamiętania: mutacja jest jedynym testem asercji.**
    Bramki: nowa `szczescie-przebudowa-skali-test.cjs` **519/0** · osiem istniejących przepisanych na nowy kontrakt, **liczba asercji wzrosła w każdym pliku i nigdzie nie spadła** (logic 213/213, society-breakdown 43→53, zamożność 60→88, normalizacja 132→147, building-happiness 8→14, r-wzrost 52→59, war-parity 18→21, wealth 28→36) · `tsc --noEmit` czysty · cała rodzina szczęścia/porządku zielona.
...
|- md5 (pełne): df1c0a8724d25af6e12acf160fb63410 · stempel: ROBOCZA · label df1c0a87 · źródłowe commity integracji: `d618a8ae` + `7dab640b` + `8080f792` + `e2b88238` + `ec8b2e8e` + `0079853f` + `0afbb5d5` + `68f1f089`
|- **`P-AI-NIE-STAWIA-BUDYNKOW-Q1`** (`0afbb5d5`, 2 rundy) — **zdobyte miasta i miasta AI wreszcie się rozbudowują.** Przyczyna była węższa niż podejrzenie właściciela: `seedCityOwnerDefaults` resetował `budowaTryb` na `'reczny'` przy KAŻDYM założeniu i KAŻDYM przejęciu miasta, kasując `'zrownowazone'` z `foundCityAt`. Skutek: `pickAutoBuildItem` zwracał `null` dla miast AI, a gałąź `isAutoBudowaTryb` nigdy nie odpalała. Miasta-państwa nie miały drugiej drogi do kolejki, więc były puste — i takie właśnie miasto właściciel zdobył. Naprawa: profil automatyczny dla właścicieli AI + `upgradeBudowaProfilAutoDefaultsOnLoad` migrująca stare zapisy, z jawnym pominięciem ownera 0. **Pomiar: przed naprawą 2 z 9 długo trzymanych miast dużego AI miało jakikolwiek budynek, po naprawie 5 z 5.** Nowa bramka `ai-buduje-budynki-test.cjs` 42/0 — realny build vite, realny headless Chromium, realne `doStartGame`/`endTurn()`, roundtrip zapisu i trzy mutacje kontrolne. Ustalenie Final Control: sumy imperium (`A1`/`A2`) zostają ZIELONE pod mutacją, bo duże AI ma drugą drogę do kolejki — defekt łapie **wyłącznie** asercja pokrycia per-miasto `A7` i nie wolno jej osłabiać.
|- **`P-BRAMKI-INFRA-CRASH-DWIE-Q1`** (`68f1f089`) — dwie bramki wywalały się PRZED pierwszą asercją, więc mimo obecności w kryteriach końca tematów **nie mierzyły niczego** i generowały fałszywe `PASS-WITH-NOTES`. `map-field-battle-test.cjs`: łańcuch `mapFieldBattle.ts:33` → `audio/muzyka-antyczna.ts` → `audio/filePlayer.ts` z **ośmioma** wywołaniami `import.meta.glob` (funkcja Vite, nieznana pod esbuildem) — stub całego modułu na granicy `esbuild onResolve`, wzorem 60+ istniejących stubów. `entity-card-contract-test.cjs`: `unitMiniPreview` wchodził na `requestAnimationFrame`, którego jsdom domyślnie nie rejestruje — `pretendToBeVisual` daje **prawdziwą implementację**, nie zaślepkę. Efekt: **20 i 75 realnych asercji** tam, gdzie było zero. Dowód anty-maskujący wykonany TRZY razy niezależnie, za każdym razem na innym celu. Zero zmian w `gra/src` i `gra/data`. Odsłonił przy okazji realny defekt: `collectBattleRoster` nie wyklucza sąsiadującego zwiadowcy, choć siostrzana `collectAtkRosterNearCity` to robi — zarejestrowany jako `P-ROSTER-ZWIADOWCA-DRIFT-DWOCH-FUNKCJI-Q1`.
|- **`R-AUTOWYZYWIENIE-ROWNY-WZROST-Q1-A`** (`d618a8ae`) — hipoteza orkiestratora „autowyżywienie jest wszystko-albo-nic" została **ODRZUCONA pomiarem Operatora**. Realna przyczyna to asymetria: obniżanie działało per miasto i zależało od kolejności iteracji (`maxSafePoziomRacjiForCity` pytał „jak nisko musi zejść TO JEDNO miasto, żeby CAŁE imperium się zbilansowało"), a podnoszenie szło lockstepem i było globalnie cofane. Zmierzone: maxSafe **0 dla Sparty, 2 dla Jin**; po naprawie **4 dla wszystkich 12 miast**. Nowe: `resolveEqualGrowthRationPlan`, `simulateCityFoodAllFed`, sprawiedliwy kontrfaktyk `min(poziom, level)`.
|- **`R-AUTOWYZYWIENIE-STAN-PRZYCISKU-Q1-B`** (`7dab640b`) — cztery kontrolki panelu miasta (obie połówki Żywności oraz komponent współdzielony przez Skarbiec, Naukę i Pracę) pokazują wreszcie, czy są włączone: klasa `.civ-cs .hbtn.off` i `data-stan="wl|wyl"`. Kryterium orkiestratora żądające zrzutów Spichlerza w dwóch stanach zostało **uchylone** — te stany nie istnieją, bo Spichlerz jest akcją jednorazową (wcześniejsza decyzja właściciela).
|- **`R-NAZWY-MIAST-AUDYT-STOLICE-I-PANSTWA-Q1`** (`e2b88238`) — Asyria dostaje **Aszur**, Fenicja **Byblos**, wymienione 140 nazw miast-państw. `nazwyKlastra` zsynchronizowane dla 14 cywilizacji, bo wymaga tego `validateCityNamesPools`. Recon orkiestratora mówił o 126 kolizjach; realna liczba to **125** — `Závist` i `Zavist` różnią się diakrytykiem.
|- **`R-ENTITYCARD-JEDNA-KARTA-CZY-STOS-Q1`** (`ec8b2e8e`, 2 rundy) — rozstrzygnięta sprzeczność dwóch zatwierdzonych tematów (jedna karta naraz kontra stos zagnieżdżony). ECHO właściciela: **stos, ale maksymalnie dwie karty**; trzecia zamyka NAJSTARSZĄ. Karta B przesunięta 72 px w lewo i 56 px w dół, żeby brzeg A wystawał. Obrona obaliła moją ratyfikację kontreksperymentem: przy `pointer-events:none` kliknięcie w widoczny brzeg A nic nie robi — moje zdanie o klikalności brzegu było **inferencją, nie słowami właściciela**, i przeczyło ECHO 2.
|- **`P-AUTO-BATTLE-SYMULATOR-PY-ROZJAZD-Q1`** (`0079853f`) — `auto-battle-power.py` doprowadzony do parytetu z runtime TS po naprawie W1. Symulator liczył starą podłogę na jednostce przy nowym wykładniku 1,2, więc odtwarzał krzywą-pułapkę r=10 → 0,500 i r=20 → 1,000. To narzędzie, którym właściciel stroi balans — rozjechane doradzałoby wartości z nieaktualnego wzoru. Nowa bramka parytetu porównuje SUMĘ składu z tolerancją względną 1e-9.
|- **`R-MAPA-ETYKIETA-STOLICY-NAZWA-MIASTA-Q1`** (`8080f792`) — przeniesiony z FALI 348, gdzie wszedł do `main` po zbudowaniu bundla.
|- ZASTĄPIONA przez FALA 350
...
|- `R-BADANIA-KOSZT-PODWOJENIE-Q1` (`gra/data/tech.json`) — koszt nauki podwojony dla wszystkich 32 technologii poza czterema wskazanymi przez właściciela (Obróbka drewna, Rolnictwo, Łowiectwo, Oswojenie zwierząt — wszystkie startowo 5). Powód: badania kończyły się szybciej niż budowa odblokowanych budynków.
|- `R-HANDEL-SZLAKI-DOCHOD-PODZIEL5-Q1` (`gra/src/game/trade-routes.ts`) — dochód z każdego szlaku handlowego (per para miast, bez zmiany liczby szlaków) obniżony **pięciokrotnie**, zaokrąglony do liczby całkowitej, **nie mniej niż 1** (ECHO właściciela: wariant A — zostaje „każde miasto z każdym", nie agregacja per cywilizacja). Regresja w `cuda-handel-test.cjs` (8/25) wychwycona przez Final Control INNEGO tematu przy pełnym przebiegu bramek — naprawiona bezpośrednio (funkcja testu i dystans próbki dopasowane do nowej reguły ÷5), 25/25 PASS.
|- `R-DYPLOMACJA-BILANS-UNIFIKACJA-Q1` (-a/-b, `gra/src/ui/diplomacyAcceptanceBalance.ts`, `gra/src/game/diplomacy-proposals.ts`, `gra/src/game/diplomacy-ai-offer-balance.ts`) — panel „Bilans (netto)" dla kierunku „gracz proponuje" czytał teraz **dokładnie tę samą wartość**, której używa bramka akceptacji (`evaluateProposal`), zamiast liczyć własną, niespójną sumę. Recon potwierdził realną niespójność (bilans widoczny jako dodatni, deal mimo to odrzucany) — naprawa „jedno źródło prawdy" zamiast kolejnej punktowej łaty. Nowy test `diplomacy-bilans-unifikacja-test.cjs` (27/27) dowodzi zgodności obu zrzutów ekranu właściciela z nową bramką.
|- `R-DYPLOMACJA-BILANS-UNIFIKACJA-Q1-C` (kontynuacja, `gra/src/game/diplomacy-acceptance-points.ts`, `gra/src/game/diplomacy-proposals.ts`) — właściciel: „tak otwórz" na rozszerzenie zakresu. (a) recon kierunku „oni proponują" (`previewIncomingPlayerAccept`) — mechanizm świadomie inny niż `evaluateProposal`, brak realnej sprzeczności, zamknięte bez zmian, wniosek jawnie opisany (`diplomacy-incoming-own-gate-recon-test.cjs`, 15/15). (b) `generateCounterOffer` miał rozjazd ról proponent/respondent — naprawiony, wynik teraz spójny niezależnie od tego, kto proponuje; 2 zmiany balansu AI ujawnione i zaakceptowane przez właściciela („Tak, zaakceptuj oba"). `diplomacy-counter-offer-role-symmetry-test.cjs` 17/17 (Operator+Obrona), z porównaniem liczbowym PRZED/PO.
|- `P-PRACA-IMPERIUM-AI-ULEPSZENIA-MIESZANE-Q1` (`gra/src/main.ts`, `gra/src/ui/hud.ts`, `gra/src/ui/empireDetailPanel.ts`) — żeton „Pula Pracy" pokazywał mylącą wartość netto (np. „Praca 39 -10"), bo koszt automatycznego rozdysponowania ulepszeń przez AI był wliczany w to samo saldo co przychód produkcji. Koszt auto-ulepszeń AI wydzielony do osobnego pola i pokazany w podsumowaniu panelu imperium („AUTO-ULEPSZENIA (AI)"), główny żeton bez zmian w liczeniu (`_lastPracaRate` — chroniony przez `praca-pula-rate-parity-test.cjs`, celowo nietknięty). Runda 1 — model sędziego złapał realny defekt layoutu (nowy box renderował się w połowie szerokości w gridzie 2-kolumnowym) i dwie luki testowe pokrywające regex zamiast realnego wykonania funkcji — Obrona naprawiła wszystko w tej samej rundzie. `praca-auto-ulepszenia-koszt-split-test.cjs` 20/20.
|- `P-ULEPSZENIA-INFO-IKONA-POZYCJA-ETYKIETA-KOSZTU-Q1` (`gra/src/ui/buildModeHud.ts`) — ikonka „ⓘ" w panelu „Ulepszenia terenu" przeniesiona całkowicie na koniec wiersza (nie kolidowała już z klikiem na nazwę ulepszenia), etykieta kosztu uproszczona z „E1 · 40 P" do „40 P" (prefiks ery usunięty jako mylący). Pierwszy pełny przebieg modelu sędziego zakończony **zerem zarzutów** — trywialna integracja.
|- Pierwszy pełny sezon nowego modelu sędziego (§3c, `R-PROC-AUTOBOT.md`, od 2026-08-29): Evaluator wydaje ponumerowane zarzuty zamiast jednego PASS/FAIL, Operator broni się (PRZYJMUJĘ/ODRZUCAM) w TEJ SAMEJ rundzie, Final Control orzeka per zarzut ślepo (bez etykiet kto co napisał). Złapał realny defekt layoutu i dwie luki testowe w temacie Pracy, które płaski model PASS/FAIL prawdopodobnie by przepuścił.
...
## ROBOCZA 13b771f4 - 2026-08-17 18:08 UTC (17.08.2026 20:08 PL) - FALA 291: **osiem tematów zamkniętych w sesji „skończ wszystkie tematy"** — trzy limity ekonomiczne (praca / nauka / miasta), dwa domyślne ustawienia nowego miasta, pula cywilizacji per mapa, toggle chatek w kreatorze, model 3D kopalni cyny i obniżka kosztu Wyrębu. (1) **`R-PRACA-LIMIT-50-PROC-WSPOLNY-WOREK-Q1`** (`6f7466d2` + rundy 3-4 `2b4dca6d`, `6c137129`) — suwak przydziału Pracy na ulepszenia terenu ograniczony do 50% budżetu, reszta trafia do wspólnego worka imperium; runda 2 wykryła i **cofnęła realną regresję** (`14b19d03`), rundy 3-4 domknęły łącznie 4 luki obejścia limitu, ostatnia w `ensureCitySaveDefaults` (migracja starych zapisów — save sprzed zmiany nie mógł wnieść wartości powyżej capa). (2) **`R-NAUKA-LIMIT-60-PROC-BUDZETU-Q1`** (`7eaf1158` + rundy 2-4 `5d6018d5`, `32886279`) — analogiczny limit 60% budżetu na Naukę, **gracz i AI tą samą ścieżką** (parytet); runda 2 naprawiła bug sumy 100% przy rozdziale, runda 4 ujemne wartości w `adjustHandelSplit`, gdy gałąź Nauki stała na capie. (3) **`R-MIASTA-LIMIT-PER-EPOKA-Q1`** (`a874f456`, dopięcie UI `58e6d31c`) — limit liczby miast rośnie z epoką: Kamień = baza, Brąz = baza+5, Żelazo = baza+10 (`cityLimitBase` podpięte do UI). (4) **`P-CHATKI-NAGRODY-TOGGLE-USTAWIENIA-Q1`** (`e0abf972`, fix `58e6d31c`) — chatki z nagrodami dają się wyłączyć w kreatorze nowej gry. (5) **`R-CYWILIZACJE-EPOKA-PULA-Q1=A` / `R-CYWILIZACJE-DOSTEPNE-PER-MAPA-PLUS-JEDEN`** (`73cf79c4` częściowe → `48246469` dokończenie wg decyzji ABC) — pula cywilizacji per rozmiar mapy = max+1, 17/18 wpisów. (6) **`R-NOWE-MIASTO-AUTOWYZYWIENIE-DOMYSLNIE` + `R-NOWE-MIASTO-AUTOBUDOWA-ZROWNOWAZONA-DOMYSLNIE`** (`eb03cb94`) — nowo założone miasto startuje z autoWyżywieniem WŁ. i trybem budowy „zrównoważone". (7) **`P-CYNA-BRAK-WIZUALIZACJI-3D-NA-MAPIE`** (`34700709`, Opus 5 zgodnie z wyjątkiem render) — dedykowany model 3D kopalni cyny (`kopalnia-cyny-opus5.ts`, 352 linie) + naprawa całkowitego braku wizualizacji tego ulepszenia na mapie. (8) **`R-WYRAB-KOSZT-PRACA-5P-Q1`** (`a7b16859`, Evaluator PASS-WITH-NOTES) — `koszt_praca` Wyrębu 5 → 2.5, czyli widoczny koszt w grze 10P → 5P. Dodatkowo dokumentacja procesu: `CLAUDE.md` + `civ-autobot/SKILL.md` zaktualizowane o nowy przydział modeli (Operator Haiku 4.5 / Evaluator Sonnet 5) i mechanizm dispatchu przez Workflow (`35d1159a`, `5f80d5c9`). ⚠️ **`gra/src/map/**` dotknięte 2 plikami** (`generator.ts` +5/−? — ścieżka toggle chatek, `newGameMapDefaults.ts` +11 — domyślne kreatora), **generator terenu strukturalnie nietknięty**; `gra/data` 3 pliki (`e-start-params.json`, `terrain-improvements.json`, `ui-params.json`). - **AKTUALNA**
|- md5 (pelne): 13b771f42dfebafea8da0cb3561c77b6 · stempel: `ROBOCZA · 90eb1054 · 2026-08-17 18:08` (UTC) · stamp md5 90eb10544c302c84f849ab557a809596 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `32886279`
|- Build z HEAD `32886279` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **24 commity** od commitu deployu FALI 290 (`3786cedc`); pod `gra/src`+`gra/data` **20 plików** (`+739/−71`), największe: `render/kopalnia-cyny-opus5.ts` +352 (nowy), `game/cities.ts` +119, `data/e-start-params.json` +48/−?, `render/ulepszenia-modele-p3b.ts` +44, `main.ts` +34, `render/styleResources.ts` +34, `ui/newGameFlow.ts` +32. `git status --porcelain` pusty przed buildem.
|- `npx tsc --noEmit` **0 błędów** · build `vite` **825 modułów, 26,40 s, 37,15 MB, exit 0** · `verify-robocza-bundle.cjs` → **VERIFY OK** (manifest match: OK, stamp match: WARN — normalne).
|- Bramki tematyczne tej fali (uruchomione w drzewie głównym po scaleniu): `praca-limit-50-test` **23/0** · `kopalnia-cyny-render-test` **23/0** · `hex-tooltip-stadnina-kopalnia-cyny-test` **29/0** · `logic-test` **213/213** (punkt odniesienia `R-BRAMKA-MINDIST-Q1=A` zachowany).
|- ⚠️ **`map-gen-regression-test.cjs` NIE uruchomiony w oknie tego deployu** — znany limit wydajności sandboksa (`P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`, >95 min CPU przy FALI 290). Zmiany pod `gra/src/map/**` w tej fali dotyczą wyłącznie ścieżki toggle chatek i domyślnych kreatora nowej gry, **nie algorytmu generowania terenu ani rzek** — ryzyko regresji generatora ocenione jako strukturalnie znikome. Do uzupełnienia wstecznie, tak jak przy FALI 290.
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **suwak Pracy na ulepszenia nie przekracza 50% budżetu**, nadwyżka idzie do wspólnego worka — sprawdź też na **wczytanym starym zapisie** (migracja miała czwartą lukę); (b) ⭐ **suwak Nauki nie przekracza 60% budżetu**, suma przydziału zawsze 100%, **żadnych ujemnych wartości** przy przesuwaniu Handlu gdy Nauka stoi na capie; (c) **limit miast rośnie z epoką** (Kamień/Brąz+5/Żelazo+10) i jest widoczny w UI; (d) **kreator nowej gry: przełącznik chatek z nagrodami działa** (wyłączone = brak chatek na mapie) i **pula cywilizacji zmienia się z rozmiarem mapy** (max+1); (e) **nowe miasto startuje z autoWyżywieniem WŁ. i budową „zrównoważone"**; (f) ⭐ **kopalnia cyny ma teraz widoczny model 3D na mapie** (wcześniej nie renderowała się wcale); (g) **Wyręb kosztuje 5P zamiast 10P**.
|- **Scalenie do `main`: FALA 290 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 290 `3786cedc`**, nie w czubek gałęzi. **FALA 291 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 292.

## ROBOCZA 67fc4b0b - 2026-08-17 10:56 UTC (17.08.2026 12:56 PL) - FALA 290: **dwa tematy — podświetlenie heksów pod kopalnie + potrojenie rzadkości kopalni miedzi/cyny**, oba z werdyktem Evaluatora (Opus 5) **PASS-WITH-NOTES**. (1) **`R-KOPALNIA-PODSWIETLENIE-HEKSOW-Q1`** (`b0f9bcb9`, decyzja bezpośrednia właściciela) — w momencie wyboru kopalni do budowy, kwalifikujące się heksy w zasięgu miasta podświetlają się na jasnoniebiesko (`0x66ccff`, opacity 0.30); nowe pole `RangeOverlayStyle.alwaysOnTop` + `depthTest:false` na warstwie (bo bryła kopalni na Wzgórzu/Górze wystaje ponad heks i ogólny highlight tonął pod nią); WYŁĄCZNIE 4 kopalnie (miedź/żelazo/cyna/złoto — sprostowanie N1 Evaluatora, rejestr chwilowo błędnie podawał „węgiel"), zero dodatkowych skanów terytorium ponad istniejący `getQualifyingHexes`, `qualifies()`/`createQualifier()` nietknięte. Evaluator: 4 niezależne mutacje (opacity, alwaysOnTop, ścieżka gaszenia, scope creep na kamieniołom) — wszystkie złapane przez nowy test. Noty nieblokujące wydzielone do osobnych tematów (`P-CUD-WONDER-EARLY-RETURN-STALE-HIGHLIGHT` — pre-istniejące, `P-KOPALNIA-PODSWIETLENIE-KOSMETYKA` — kosmetyka). (2) **`R-SUROWCE-KOPALNIE-MIEDZ-CYNA-3X-Q1`** (`d0fc4b94`, ECHO dispatch bezpośredni) — `miedz.rarity` 0.10→0.30, `cyna.rarity` 0.02→0.06 (proporcja 5:1 zachowana), jedyne źródło prawdy w runtime to `gra/data/map-gen-params.json` (`BASE_DEPOSIT_RULES`/`FALLBACK_DEPOSIT_RARITY` w kodzie to martwe ścieżki fallback, świadomie nietknięte). ⚠️ **`gra/src/map/**` dotknięte tylko przez (1)** — jeden plik `improvement-build.ts` (klucze kopalni), generator terenu/mapy nietknięty. - **ZASTĄPIONA** (→ `13b771f4`, FALA 291)
|- md5 (pelne): 67fc4b0bc30802bbc75ccc31f110f471 · stempel: `ROBOCZA · 8bfd6668 · 2026-08-17 10:56` (UTC) · stamp md5 8bfd66688428a52e112f299daab5d710 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `d152be56`
|- Build z HEAD `d152be56` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **20 commitów** od commitu deployu FALI 289 (`0528dccc`); pod `gra/src`+`gra/data` **4 pliki** (`+146/−6`: `main.ts` +52, `render/rangeOverlay.ts` +76/−2, `map/improvement-build.ts` +20, `data/map-gen-params.json` +2/−2) + 1 nowy plik testu (`kopalnia-podswietlenie-heksow-test.cjs`, 358 linii). Zmierzone `git diff --name-only 0528dccc..HEAD -- gra/src/map` → wyłącznie `improvement-build.ts` (klucze kopalni, NIE generator). `git status --porcelain` pusty przed buildem i po nim (poza oczekiwanymi plikami deployu pod `gra-robocza/`).
|- `npx tsc --noEmit` **0 błędów** · build `vite` **824 moduły, exit 0** · `verify-robocza-bundle.cjs` → **VERIFY OK** (manifest match: OK, stamp match: WARN — normalne).
|- Bramki tematu (1): `kopalnia-podswietlenie-heksow-test` **71/0** (nowy) · `map-improvement-qualify-test` **112/0** · `improvement-territory-gate-test` **6/0** · `logic-test` **213/213**.
|- Bramki tematu (2): `cyna-surowiec-test` **78/0** (potwierdza proporcję 5:1 dynamicznie) · `map-deposits-era-test` **16/0** · `surowce-katalog-kolejnosc-test` **62/0** · `diplomacy-deposit-trade-test` **9/0** · `surowce-dostep-test` **13/0** · `fair-play-grid-test` **8/0** (realna bramka rozmieszczenia złóż) · `deposit-coast-test` **20/0** (potwierdzone dwukrotnie, w tym bezpośrednio w drzewie głównym).
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły**: `zloto-test` **38/7** · `tarasy-cywilizacje-test` **16/1** · `deposit-building-gate-test` **46/1** — wszystkie identyczne z punktem odniesienia, zero wzrostu liczby porażek.
|- ✅ **`map-gen-regression-test.cjs` DOKOŃCZONY po deployu (uzupełnienie wsteczne) — exit 1, ale WYŁĄCZNIE z powodu znanego, pre-istniejącego limitu wydajności sandboksa (`P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`, ten sam problem co w FALI 287), nie regresji tej zmiany.** Wszystkie asercje POPRAWNOŚCI zielone: `0 sierot w sieci` PASS, `dopływy domknięte` PASS, `hash A=fdb5e82c B=fdb5e82c → IDENTYCZNY` (determinizm PASS), `0 tras bez ujścia` PASS (0 złych), `0 main bez REALNEGO morza` PASS, `Pangea nieregularna` PASS, `chat ze skarbami` PASS ×3. Jedyne 2 FAIL to progi czasowe generacji: `standard <7s` (zmierzone 85,84s) i `duża <15s` (zmierzone 1291,92s) — ta maszyna jest wolniejsza niż referencyjna przy ustalaniu progów, niezwiązane z `rarity` złóż. Potwierdza: **potrojenie rzadkości miedzi/cyny nie wprowadziło regresji generatora**.
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **przy wyborze kopalni do budowy heksy, na których można ją postawić, podświetlają się na jasnoniebiesko** — sprawdź że dotyczy WYŁĄCZNIE kopalni (miedź/żelazo/cyna/złoto), nie innych ulepszeń terenu; (b) **kopalni miedzi i cyny na mapie powinno być teraz zauważalnie więcej** (3× gęstość złóż) — sprawdź subiektywnie czy rozmieszczenie na nowych/regenerowanych mapach wygląda sensownie, nie stłoczone.
|- **Scalenie do `main`: FALA 289 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 289 `0528dccc`**, nie w czubek gałęzi. **FALA 290 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 291.

## ROBOCZA 3d37608e - 2026-08-16 19:40 UTC (16.08.2026 21:40 PL) - FALA 288: **saga 4-rundowa `P-BITWA-ATAK-DYSTANSOWY-BRAK-NA-MAPIE`** (AI potrafi teraz realnie zdobywać miasta przy ataku dystansowym) **+ toast przejścia epoki + 3 tematy panelu imperium**, wszystkie z werdyktem Evaluatora **PASS-WITH-NOTES**. (1) **`P-BITWA-ATAK-DYSTANSOWY-BRAK-NA-MAPIE`, runda 4** (`6826b16c`, ECHO `WEJSCIE-Q1=A`) — egzekutor AI po wejściu na pusty heks miasta woła `tryAutoCaptureEmptyCityAt` (ta sama ścieżka co barbarzyńcy), naprawia wieczną oscylację evict i trwałe uwięzienie na heksie ze rundy 3; `isWithinCityAttackRange` zrównany z regułą egzekucyjną (mur LUB obrońcy blokuje też adiacencję, nie tylko zasięg), naprawia zamrożenie przy mieście bronionym. Evaluator (Opus 5, `a4380802596aad26a`) potwierdził naprawę **trzema własnymi mutacjami cofającymi kod — wszystkie czerwone**, nie tylko odczytem. Nota N1 (teleport jednostki na pełny zasięg przy zdobyciu, do 6 heksów w jednej turze) wydzielona do osobnego tematu `P-BITWA-ATAK-DYSTANSOWY-TELEPORT-Q1`, czeka na ABC, nie blokuje. (2) **`P-EPOKA-BRAK-INFO`** (`6936d4d3`) — toast przejścia epoki przestał być połykany przez `deferredEotHints` na końcu tury. (3) **`P-PANEL-MIASTA-VS-SPICHLERZ-WZROST-ROZJAZD`** (ECHO `Q1=B`, `0b46cdbd`) — SUMA wzrostu w Spichlerzu Centralnym liczona efektywnie (uwzględnia głód), spójnie z już wcześniej ujednoliconą sekcją Miasta. (4) **`P-PANEL-MIASTO-OBYWATELE-TRESC-NIEPELNA`, runda 2** (N1-N7, `01b89eec`) — SZLAKI liczone poprawnie (`paired`), przywrócone/dodane jednostki miar, poprawki JSDoc. (5) **`P-ARMIA-PANEL-BRAK-INFO-PRODUKCJA-JEDNOSTEK`, runda 2** (N1/N2, `b22bc5ce`) — wstrzymana produkcja pokazuje tekst zamiast fikcyjnej liczby tur, `etaTurns()` dostał twarde asercje brzegowe. ⚠️ **`gra/src/map/**` i `gra/data` NIE ruszone** (0 plików obu) — regresja generatora i danych rozgrywki strukturalnie niemożliwa. - **ZASTĄPIONA** (→ `e457e054`, FALA 289)

## ROBOCZA e457e054 - 2026-08-16 21:37 UTC (16.08.2026 23:37 PL) - FALA 289: **cofnięcie mechaniki „atak bez adiacencji"** (`P-BITWA-ATAK-DYSTANSOWY-BRAK-NA-MAPIE`, ECHO `COFNIECIE-Q1=A`) — właściciel sprostował, że 4-rundowa saga zaimplementowana w FALI 281-288 (jednostki z zasięgiem >1 heksa strzelające bez podejścia, AI zdobywające miasta z dystansu) była **nieporozumieniem zakresu** — nigdy nie chciał prawdziwego strzelania bez ruchu, wyłącznie mechanizmu marsz-potem-atak (osobny, już wcześniej poprawnie wdrożony temat `P-BITWA-MARSZ-POTEM-ATAK-NIE-KOLEJKUJE`, FALA 284-285, nietknięty tym cofnięciem). **Cofnięte do stanu funkcjonalnie równoważnego commitowi sprzed rundy 1** (`9e96370a`) — `main.ts` (`tryLaunchMarchAttack` wraca do wymogu adiacencji, usunięte 3 funkcje zasięgu które stały się martwym kodem, usunięta gałąź AI wejścia na miasto z dystansu), `game/ai.ts` (`isWithinAttackRange`/`isWithinCityAttackRange` wracają do `hexDistance===1`), `game/city-hex-movement.ts` i `map/map-attack-city.ts` (oba **bajt-w-bajt identyczne** z kodem sprzed sagi, zweryfikowane `git diff` pustym). Evaluator (Opus 5) potwierdził kompletność cofnięcia i **żywym uruchomieniem** (33/0, własny harness) że marsz-potem-atak działa identycznie dla zwarcia/dystansu/mieszanych stosów. Usunięte 2 testy tej mechaniki (1554 linie), nowy strażnik regresyjny `atak-adiacencja-wymagana-test.cjs` (57/0) pilnujący że adiacencja jest znowu wymagana. Przy okazji: nowa żywa bramka regresyjna `era-change-toast-live-test.cjs` (17/17) dla toastu przejścia epoki (`P-EPOKA-BRAK-INFO`, FALA 288) — potwierdza mechanizm działa poprawnie, brak zmiany w logice, wyłącznie dodana infrastruktura testowa. ⚠️ **`gra/data` NIE ruszone** (0 plików) — regresja danych rozgrywki strukturalnie niemożliwa. `gra/src/map/map-attack-city.ts` dotknięte (część cofnięcia, logika rozstrzygania ataku na miasto — NIE generator terenu; pozostałe pliki `gra/src/map/**`, w tym cały generator, nietknięte). - **ZASTĄPIONA** (→ `67fc4b0b`, FALA 290)
|- md5 (pelne): e457e054027aaec49b288d82e744c1dd · stempel: `ROBOCZA · 5798a6ef · 2026-08-16 21:37` (UTC) · stamp md5 5798a6ef (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `2ce69b8f`
|- Build z HEAD `2ce69b8f` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **11 commitów** od commitu deployu FALI 288 (`5b471f32`); pod `gra/` **9 plików** (`+578/−1969`: `main.ts` −200 netto, `game/city-hex-movement.ts` −42, `game/combat.ts` −24, `map/map-attack-city.ts` +/−49, 2 nowe pliki testowe `+453`, 2 usunięte pliki testowe `−1554`, `march-attack-queue-persist-test.cjs` dostosowany). Zmierzone `git diff --name-only 5b471f32..HEAD -- gra/data` → **pusto**. `git status --porcelain` pusty przed buildem i po nim (poza oczekiwanymi plikami deployu pod `gra-robocza/`).
|- `npx tsc --noEmit` **0 błędów** · build `vite` **824 moduły, 21,52 s, 37,14 MB, exit 0** · `verify-robocza-bundle.cjs` → **VERIFY OK** (manifest match: OK, stamp match: WARN — normalne).
|- Bramki tematyczne tej fali (uruchomione w drzewie głównym po scaleniu, nie przepisane z raportu subagenta): `combat-test` **6/6** · `map-attack-city-test` **8/0** · `barb-city-capture-cluster-test` **94/0** · `march-attack-queue-persist-test` **57/0** · `atak-adiacencja-wymagana-test` **57/0** · `era-change-toast-live-test` **17/17**.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły**: `ai-test` **285/8** · `unit-power-test` **4/2** — obie identyczne z punktem odniesienia `CLAUDE.md` §BRAMKI, zero wzrostu liczby porażek.
|- **Nowe znalezisko Evaluatora rundy 4 (FALA 288), zarejestrowane w `PYTANIA-OTWARTE.md`, świadomie NIE naprawione w tej fali, teraz nieaktualne po cofnięciu**: `P-BITWA-ATAK-DYSTANSOWY-TELEPORT-Q1` — dotyczyło mechaniki cofniętej tą falą, temat traci aktualność wraz z cofnięciem.
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **atak na jednostkę lub miasto znowu wymaga fizycznego podejścia na sąsiedni heks** — łucznicy/procarze/kusznicy/katapulty NIE strzelają już bez ruchu, tak jak przed FALĄ 281; (b) **klik na odległego wroga nadal poprawnie planuje marsz + automatyczny atak po dotarciu** (mechanizm marsz-potem-atak, niezmieniony) — sprawdź czy to działa płynnie dla mieszanych armii (zwarcie+dystans w jednym stosie); (c) **AI nie zdobywa już miast z dystansu** — miasta bez muru/obrońców w adiacencji AI nadal zdobywa normalnie (barbarzyńska ścieżka `tryAutoCaptureEmptyCityAt` nietknięta).
|- **Scalenie do `main`: FALA 288 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 288 `5b471f32`**, nie w czubek gałęzi. **FALA 289 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 290.

## ROBOCZA 3d37608e - 2026-08-16 19:40 UTC (16.08.2026 21:40 PL) - FALA 288 (ZASTĄPIONA, patrz wyżej):
|- md5 (pelne): 3d37608eaca97bbcff050548f7ed4bca · stempel: `ROBOCZA · e24d9773 · 2026-08-16 19:40` (UTC) · stamp md5 e24d9773 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `189b2b0f`
|- Build z HEAD `189b2b0f` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **34 commity** od commitu deployu FALI 287 (`17baa179`); pod `gra/` **10 plików źródłowych** (`+983/−80`: `main.ts` +202, `empireDetailPanel.ts` +441/−cały reskin poprzedniej fali kontynuowany, `empireDetailTypes.ts` +219, `ai.ts` +121, `city-hex-movement.ts` +42, `production.ts` +16, `eot-event-defer.ts` +16/−10, `cityPanel.ts` +6/−4) + **8 plików bramek**. Zmierzone `git diff --name-only 17baa179..HEAD -- gra/src/map` → **pusto**; `git diff --name-only … -- gra/data` → **pusto**. `git status --porcelain` pusty przed buildem i po nim (poza oczekiwanymi plikami deployu pod `gra-robocza/`).
|- `npx tsc --noEmit` **0 błędów** · build `vite` **824 moduły, 16,75 s, 37,14 MB, exit 0** · `verify-robocza-bundle.cjs` → **VERIFY OK** (manifest match: OK, stamp match: WARN — normalne).
|- Bramki tematyczne tej fali (uruchomione w drzewie głównym po scaleniu każdego tematu, nie przepisane z raportu subagenta): `atak-dystansowy-mapa-test` **102/0** · `atak-dystansowy-egzekucja-test` **52/0** · `combat-test` **6/6** · `map-attack-city-test` **8/0** · `barb-city-capture-cluster-test` **94/0** · `empire-miasta-table-test` **96/0** · `empire-panel-miasto-obywatele-content-test` **99/0** · `empire-armia-produkcja-test` **51/0** · `era-change-toast-defer-test` zielony.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły**: `ai-test` **285/8** · `unit-power-test` **4/2** — obie identyczne z punktem odniesienia `CLAUDE.md` §BRAMKI, zero wzrostu liczby porażek.
|- **Nowe znalezisko Evaluatora, zarejestrowane w `PYTANIA-OTWARTE.md`, świadomie NIE naprawione w tej fali** (nie blokuje deployu): `P-BITWA-ATAK-DYSTANSOWY-TELEPORT-Q1` — jednostka atakująca z dystansu teleportuje się na pełny zasięg (do 6 heksów) przy wejściu na heks miasta, co po rundzie 4 ma teraz realny skutek własnościowy (zmiana właściciela miasta w jednej turze); zarezerwowane dla właściciela od rundy 1 tematu, nigdy nie zadane — czeka na ABC, nie pilne.
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **AI potrafi teraz faktycznie zdobywać miasta atakiem dystansowym** — jednostka AI (np. Katapulta, Balista) w zasięgu pustego lub niebronionego miasta wchodzi na jego heks i przejmuje je, zamiast zamierać lub oscylować; (b) miasto **bronione murem lub garnizonem** nadal wymaga zwykłego szturmu — AI nie próbuje już wejść i się nie zawiesza; (c) **toast przejścia epoki** pokazuje się zawsze, nie ginie już przy zbiegu z innymi zdarzeniami końca tury; (d) **Spichlerz Centralny** — wiersz SUMA/WZROST% liczy teraz efektywnie (miasto głodujące nie zawyża sumy); (e) **panel Miasto/Obywatele i Armia** — drobne poprawki treści (SZLAKI, jednostki miar, tekst „wstrzymana" zamiast fikcyjnej liczby tur).
|- **Scalenie do `main`: FALA 287 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 287 `17baa179`**, nie w czubek gałęzi. **FALA 288 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 289.

## ROBOCZA a2afa359 - 2026-08-16 16:04 UTC (16.08.2026 18:04 PL) - FALA 287: **domknięcie `R-DESIGN-11-ZAKLADEK` (faza 3) — reskin SZEŚCIU ostatnich zakładek panelu imperium wg makiety Designera** + **jedna zmiana danych rozgrywki (leczenie jednostek)**. Wszystkie tematy z werdyktem Evaluatora (Opus 5) **PASS-WITH-NOTES**, część po 2–3 rundach poprawek. (1) **`R-DESIGN-11-ZAKLADEK` — Kultura, Surowce, Armia, Handel** (`a6ed0553` + runda 2 `1f431aa7`): reskin na wspólny język wizualny paneli (`civ-emp-hero`/`civ-emp-eyebrow`/`civ-emp-box`), a runda 2 po Evaluatorze naprawiła **6 realnych usterek czytelności**: N2 — do zakładki Armia dołożona **brakująca para boxów KOSZT ZŁOTA / ZAOPATRZENIE** z klatki 7 makiety (dane były już w silniku: `bogactwoUtrzymanieJednostek`, `kosztWojska`, panel ich po prostu nie pokazywał); N3 — podpis „14 rośnie" w Surowcach **kłamał przy tempie 0** (`resStateOf()` zwraca `good` także dla tempa zerowego), rozbite na „rośnie" / „bez zmian" wg `ratePerTurn>0`; N8 — **zerowy przyrost Kultury renderował się na zielono** jak wzrost, dodany trzeci, neutralny przypadek `rate===0`; N10 — dwa różne poziomy pilności głodu wojska (**już się dzieje** vs **za N tur**) były spłaszczone do jednego czerwonego stylu, przywrócony wariant ostrzegawczy; N4/N6 — odmiana przymiotnika „pełny" i martwa gałąź znaku przy zaszytym prefiksie `+`. (2) **`R-DESIGN-11-ZAKLADEK` — Spichlerz Centralny** (`7a413462` + runda 2 `a98f63f8` + runda 3 `f1847031`, jedyny temat z realną makietą): reskin + **wiersz SUMA/WZROST% wreszcie pokazuje wyliczoną średnią zamiast „-"** (zweryfikowane na dwóch klatkach makiety: 4A `(7+5+3)/3=5`, 4B `(4+0+0)/3=1,33→1`), naprawiona **podwójna klasa `fill fill`** na pasku zapełnienia w stanie zdrowym; rundy 2–3 zamknęły ponadto **trzy dziury w samej bramce** (regex łapiący `chip-warning` gdziekolwiek dalej w funkcji, martwa asercja pilnująca wyłącznie starej klasy `civ-emp-note`, oraz — runda 3 — bramka pilnująca **deklaracji** zmiennej `fedMark`, a nie tego, czy jest faktycznie **wstawiona do wiersza tabeli**: usunięcie `${fedMark}` z interpolacji przechodziło jako 50/0), każda naprawa potwierdzona mutacyjnie. (3) **`R-DESIGN-11-ZAKLADEK` — Miasto + Obywatele** (`98345aa5` + runda `eef657d3`): **rozbicie strukturalne, nie sam reskin** — wspólna zakładka `econ-miasta` rozdzielona na dwa niezależne bloki top-level `miasto` (kategoria produkcyjna) i `obywatele` (społeczna); **chipy HUD „miasta" i „ludność" prowadziły dotąd do tej samej funkcji `cityMiastaMiniDetail()`, mieszając dane produkcyjne ze społecznymi**. Stare id `econ-miasta` celowo zostaje jako kotwica scrollowania w bloku Mocy. (4) **`R-MANPOWER-LECZENIE-PROC-TRUDNOSC` — ZMIANA DANYCH ROZGRYWKI, decyzja bezpośrednia właściciela** (`afb51098` + rundy `007fd3d4`/`6ecf6c02`): **leczenie jednostki wojskowej z puli Manpower imperium** (`manpower_uzupelnienie_hp_proc_max_tura`, jednostka **% maxHP na turę**) — **łatwy 25→40 %, normalny 20→30 %, trudny 15→20 %**; rundy 2–3 wyczyściły **trzy niezależne miejsca w repo**, które nadal podawały nieaktualne 25/20/15 (JSDoc `ManpowerReplenishParams`, fallback `DEFAULT_REPLENISH_PCT` w `manpower.ts` — nieosiągalny przy poprawnym JSON, ale będący sprzeczną drugą „prawdą" w pliku, oraz tabela w dokumentacji ekonomii). ⚠️ **`gra/src/map/**` NIE ruszone** (0 plików) — regresja generatora strukturalnie niemożliwa. ⚠️ **`gra/data` RUSZONE, w odróżnieniu od kilku ostatnich fal** — dokładnie 1 plik i 1 blok wartości: `miasto-params.json`, celowa zmiana leczenia z pkt (4), nic ponadto. - **ZASTĄPIONA** (→ `3d37608e`, FALA 288)
|- Build z HEAD `3e36fcc5` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **18 commitów** od commitu deployu FALI 286 (`3ea531c6`, źródłowy HEAD tamtej fali `8cdf6664`); pod `gra/` **8 plików** (`+1328/−131`), w `gra/src` **3** (`ui/empireDetailPanel.ts` `+1268` — trzon reskinu, `ui/empirePanelSectionMap.ts` `+46` — routing chipów HUD po rozbiciu Miasto/Obywatele, `game/manpower.ts` `+12` — wyłącznie komentarze i fallback, **bez zmiany logiki**), 1 plik danych (`data/miasto-params.json`) i 4 pliki bramek. Zmierzone `git diff --name-only 3ea531c6..HEAD -- gra/src/map` → **pusto**; `git diff --name-only … -- gra/data` → **wyłącznie `miasto-params.json`**, którego cały diff to trzy linie `easy/normal/hard`. `git status --porcelain` **pusty przed buildem**; po buildzie wyłącznie **9 oczekiwanych plików deployu** pod `gra-robocza/`. HEAD budowania **= czubek `origin/claude/sprawdzenie-funkcjonalnosci-ek4ra0`** (zweryfikowane `git fetch` po buildzie — gałąź nie odjechała w trakcie).
|- ✅ **Twardy dowód, że zmiany realnie weszły do bundla (porównanie dwóch bundli, nie odczyt kodu).** Zmiana danych: w bundlu FALI 286 (zachowana kopia `49594c1e69cf39fcd7983e25ca33e33a`) stoi dosłownie `manpower_uzupelnienie_hp_proc_max_tura:{easy:25,normal:20,hard:15,jednostka:"% maxHP/tura"…}`, w bundlu FALI 287 — `{easy:40,normal:30,hard:20,…}`; ta sama wartość potwierdzona **także w bundlach playtestowych** (`…-PLAYTEST-MIASTO`, `…-PLAYTEST-WALKA`), więc synchronizacja nie zgubiła zmiany. Reskin: `civ-emp-hero` **15→34** trafień, `civ-emp-box` **22→30**, `civ-emp-eyebrow` **18→22**, a marker Spichlerza `civ-emp-sp-city-nm` **0→3** (klasa fizycznie nie istniała w poprzednim bundlu).
|- Bramki (17 uruchomionych, wszystkie zgodne z punktem odniesienia): `tsc --noEmit` **0 błędów** (TypeScript **5.9.3**, wersja zweryfikowana przed zaufaniem wynikowi — worktree bez własnych `node_modules`, `C-029`) · `logic-test` **213/213** (`R-BRAMKA-MINDIST-Q1 = A`) · `tech-tree-test` **19/0** · `research-test` **33/33, ALL GREEN** · `unit-replace-test` **13/13** · `ai-founding-territory-test` **28/0** · `combat-test` **6/6** — oraz bramki tematyczne tej fali: `manpower-test` **62/0** · `empire-panel-split-test` **25/0** · `empire-panel-econ-slider-visibility-test` **60/0** · `empire-miasta-table-test` **89/0** · `cuda-handel-test` **26/0** · `hud-armia-chip-jednostki-test` **58/58** · `surowce-dostep-test` **13/0** · `surowce-katalog-kolejnosc-test` **62/0** (punkt odniesienia po `R-SUROWIEC-CYNA-DO-BRAZU`, nie 61) · `spichlerz-deficyt-scalenie-test` **51/0** (było 50/0 — wzrost o dokładnie tę jedną asercję z rundy 3, pilnującą użycia `fedMark` w wierszu) · `spichlerz-panel-food-parity-test` **11/0** · `hud-obywatele-chip-test` **20/20**. Build `vite` **824 moduły, 24,37 s, 37,13 MB**, exit 0.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły** (kontrola, czy fala nie schowała regresji pod przykrywką znanego długu): `unit-power-test` **4/2** · `prereq-budynkow-test` **51/8** · `mennica-magazyn-test` **38/3** · `empire-food-b5-test` **25/3** · `budynek-civ-bonus-u17-test` **3/3** · `trade-routes-income-test` **52/1** · `growthmult-compound-test` **17/7** · `population-growth-v85-test` **48/2** · `spichlerz-widocznosc-test` **37/8** · `spichlerz-wzrost-test` **2/7** · `food-hodowla-test` **20 OK/4 FAIL** · `ai-test` **285/8** · `empire-panel-moc-scroll-preserve-test` **38/9** — **wszystkie co do jednej dokładnie na punkcie odniesienia z `CLAUDE.md` §BRAMKI, zero wzrostu liczby porażek**.
|- `map-gen-regression-test` — **oba kryteria akceptacji z `CLAUDE.md` ZIELONE, oba zmierzone ŚWIEŻO w tej fali, nie odziedziczone.** ✅ **0 rzek bez ujścia: PASS** — przebieg dokończony w całości, **40/40 map** (5 seedów × 4 typy × 2 gęstości): `Trasy OK (ujscie morze lub rzeka): 2124/2124`, `Glowne rzeki OK (REALNE morze): 1235/1235 (0 przypadkow fail)`, zero linii FAIL, exit 0. ✅ **Determinizm A=B: PASS** — `hash A=fdb5e82c B=fdb5e82c -> IDENTYCZNY`, exit 0; hash **identyczny z falami 284 i 285**, co niezależnie potwierdza brak dryfu generatora. **Metodyka jak w FALI 285, świadoma i nazwana, nie przemilczana luka:** zamiast pełnego `map-gen-regression-test.cjs` (który na tej maszynie wisi na trzecim benchmarku czasowym mapy „duża" 240×168 — znany `P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`) użyto **dwóch celowanych harnessów będących wiernymi wycinkami tego samego pliku testu** (`scratchpad/FALA287/rzeki-harness.cjs`, `…/determinizm-harness.cjs` — kopie harnessów FALI 285 przekierowane na worktree tej fali). **Zweryfikowano, że wycinek jest nadal wierny:** `git diff --name-only 25104155..HEAD -- gra/tools/map-gen-regression-test.cjs` → **pusto**, plik testu nie zmienił się od FALI 285, więc identyczne pozostają `hexHash`, `DENSITY`, `SEEDS`, `TYPES` i wywołania `generujSwiat`; różnica polega WYŁĄCZNIE na pominięciu trzech benchmarków czasowych, które są pomiarem wydajności sandboksa, nie kryterium poprawności. **Ta fala nie dotyka generatora w ogóle** (`gra/src/map/**` = 0 plików), więc regresja mapgenu jest tu strukturalnie niemożliwa.
|- **Nowe znaleziska Evaluatora, zarejestrowane w `PYTANIA-OTWARTE.md`, świadomie NIE naprawione w tej fali** (nie blokują deployu, wszystkie czekają na decyzję właściciela, nie na wykonawcę): `P-PANEL-MIASTO-OBYWATELE-TRESC-NIEPELNA` — nowe zakładki Miasto/Obywatele realizują **około połowy** zatwierdzonych list z `R-DESIGN-11-ZAKLADEK Q2=B` (weszły wpływy do skarbca, surowce, nauka, populacja; **nie weszły** budynki wg `BUILDING_GROUP_ORDER`, kolejka produkcji i obrona miasta — `EmpireDetailSnap` nie niesie tych danych, dociągnięcie wymaga zmian w `main.ts`/`empireDetailTypes.ts`, poza zakresem tamtego zlecenia); `P-SPICHLERZ-SUMA-WZROST-NOMINALNY-VS-EFEKTYWNY` — nowy wiersz SUMA/WZROST% agreguje wzrost **nominalny** (`breakdown.total`), nie **efektywny** (`fed ? total : 0`, jak liczy `cityPanel.ts`), więc **miasto głodujące nadal wnosi swój nominalny wzrost do sumy**; to nie regresja (komórki per-miasto miały tę własność wcześniej), ale nowy wiersz SUMA podnosi ją do liczby nagłówkowej — **pytanie produktowe, celowo nie zgadywane** (`CLAUDE.md` §6); `P-DESIGN-11-ZAKLADEK-ODSTEPSTWA-OD-MAKIETY-RUNDA-2` — uwaga N7 (Kultura/ZASIĘG pokazuje „-" tam, gdzie makieta ma „6 heksów”), kodu świadomie NIE zmieniono, bo uzasadnienie merytoryczne stoi; **wprost nazwane odstępstwo od zatwierdzonej makiety**, czeka na literę.
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **leczenie jednostek jest teraz wyraźnie szybsze** — jednostka po bitwie regeneruje **40 % maxHP na turę na łatwym, 30 % na normalnym, 20 % na trudnym** (było 25/20/15); sprawdź, czy pula Manpower imperium schodzi proporcjonalnie i czy przy jej niedoborze leczenie jest **częściowe**, a nie zerowe; (b) **sześć przereskinowanych zakładek panelu imperium** — Kultura, Surowce, Armia, Handel, Spichlerz Centralny, Miasto/Obywatele: czy wyglądają spójnie z wcześniej zrobionymi Skarbiec/Praca/Nauka/Religia; (c) **chipy HUD „miasta" i „ludność" prowadzą teraz do DWÓCH RÓŻNYCH zakładek** (przedtem do jednej wspólnej) — sprawdź, czy każdy trafia tam, gdzie obiecuje; (d) **Armia** ma pokazywać parę boxów **KOSZT ZŁOTA / ZAOPATRZENIE**, wcześniej nieobecną; (e) **Spichlerz** — wiersz SUMA/WZROST% ma pokazywać **liczbę**, nie „-"; (f) **Surowce** — surowiec ze stanem dodatnim, ale **tempem 0**, ma pisać „bez zmian", nie „rośnie"; (g) **Kultura** — **zerowy** przyrost nie może być zielony.
|- ⚠️ **NIE WESZŁO do tego bundla — commit `6936d4d3` („`P-EPOKA-BRAK-INFO`: napraw toast przejścia epoki połykany przez EOT defer", `gra/src/main.ts` + nowa bramka `era-change-toast-defer-test.cjs`).** Wpadł na gałąź **po** zakończeniu builda tej fali (build szedł z `3e36fcc5`, wtedy czubka gałęzi; commit pojawił się dopiero przy kontroli przed pushem) i **nie przeszedł bramek tej fali**. Świadomie **nie przebudowywano** bundla, żeby md5 `a2afa359` pozostało tym, co faktycznie zmierzono 17 bramkami wyżej — commit deployu został **zrebasowany** na `6936d4d3` (bez force-push, cudza praca nienaruszona), więc kod jest na gałęzi i **wejdzie do FALI 288**. **Skutek dla playtestu: toast przejścia epoki NIE jest jeszcze naprawiony w `a2afa359` — nie zgłaszać ponownie.**
|- **Scalenie do `main`: FALA 286 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 286 `3ea531c6`**, nie w czubek gałęzi. **FALA 287 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 288.

## ROBOCZA 49594c1e - 2026-08-16 11:30 UTC (16.08.2026 13:30 PL) - FALA 286: pięć poprawek panelu akcji jednostki i lewego doku, wszystkie z werdyktem Evaluatora **PASS-WITH-NOTES**. (1) **`P-UNITACTIONBAR-UNFORTIFY-ALL-NIEOSIAGALNE`** — akcja **„Zdejmij fortyfikację ze wszystkich"** wreszcie **osiągalna dla gracza**: istniała w warstwie danych, ale brakowało jej w `COMPACT_ACTION_ORDER`, więc renderer nigdy jej nie pokazywał (ta sama klasa przyczyny co `march-stop` z FALI 285 — akcja w `actions[]`, nigdy w HTML). (2) **`P-UNITACTIONBAR-RENDERER-BRAK-BRAMKI-KOMPLETNOSCI`** — dołożona **bramka kompletności renderera** (`unit-action-bar-completeness-test.cjs`, 31 asercji), która strukturalnie wyłapuje właśnie tę klasę błędu: każde id akcji produkowane przez silnik musi mieć ścieżkę renderu. Bramka od razu znalazła **kolejną martwą akcję — `siege-hold`** (brak ścieżki renderu), naprawioną w tym samym temacie. (3) **`P-SIDEPANEL-CTX-DOCK-SCROLL-MARTWY`** — **przewijanie kółkiem myszy działa w lewym doku** `.civ-side-ctx-dock` (karta armii z dużym stosem jednostek była nieprzewijalna); ten sam wzorzec naprawy co wcześniejsza naprawa `.civ-side-panel` z FALI 284 — scroll na wewnętrznym wrapperze `.sp-ctx-scroll`, BEZ cofania martwej strefy kontenera (`pointer-events:none` rundy 1 nienaruszony, przypięte asercją). (4) **kosmetyka** — usunięta osierocona reguła CSS `.civ-unit-panel .unit-ic-medallion` (zero pasujących elementów w DOM). (5) **jakość samej bramki z pkt (2), 2 rundy poprawek po uwagach Evaluatora** — N1: regex ekstrakcji dawał fałszywe negatywy, N5: przeciek identyfikatora z adnotacji typu do listy realnych akcji; obie naprawione, bramka nie zgłasza już nieistniejących akcji. ⚠️ **`gra/data` NIE ruszone · `gra/src/map/**` NIE ruszone** — regresja generatora mapy strukturalnie niemożliwa. Twardy dowód wejścia do bundla (porównanie dwóch bundli, nie odczyt kodu): `unfortify-all` 4→5 trafień, `siege-hold` 1→3, `sp-ctx-scroll` 0→2, `unit-ic-medallion` 1→0. - **ZASTĄPIONA** (→ `a2afa359`, FALA 287)
|- md5 (pelne): 49594c1e69cf39fcd7983e25ca33e33a · źródłowy HEAD `8cdf6664` · commit deployu `3ea531c6` (uzupełnione wstecznie przy FALI 287 — wpis FALI 286 nie miał linii szczegółowych, pełny opis stał wyłącznie w `KANAL-PRACA.md`)

## ROBOCZA 97b3b12a - 2026-08-16 10:02 UTC (16.08.2026 12:02 PL) - FALA 285: ⛔ **PILNA POPRAWKA REALNEGO BUGA Z FALI 284 — przycisk „Anuluj atak" wreszcie ISTNIEJE w grze.** (1) **`P-BITWA-OBLEZENIE-NIE-ANULUJE-ZAKOLEJKOWANEGO-ATAKU` (decyzja właściciela `Q1 = B`)** — plan ataku nadal jest chroniony przy rozpoczęciu oblężenia (`commitBesiege` BEZ `force=true`, zero zmian w tej ścieżce), a gracz dostaje przycisk **„Anuluj atak" także w trybie oblężenia**, widoczny wyłącznie gdy faktycznie jest co anulować (`hasPlan && marchAttackTargets.has(id)`); reużywa istniejący handler `stopPlannedMarchForSelected()`, zero duplikacji logiki. **Przy okazji wyszedł ukryty, cięższy błąd i to on jest powodem pilności:** przycisk „Anuluj atak"/„Zatrzymaj" **NIGDY nie renderował się w DOM** — ani w nowej gałęzi oblężenia, ani w PRE-ISTNIEJĄCEJ gałęzi `hasPlan` wdrożonej już do gracza w FALI 284 (Przyczyna B). Realny renderer (`gra/src/ui/unitActionBarHtml.ts`) pokazywał tylko id z `COMPACT_ACTION_ORDER` + hardkodowany `disband`, a `march-stop` nie był w żadnej z tych dwóch ścieżek — akcja trafiała do `actions[]` (dane), ale nigdy do HTML. **Skutek dla gracza: przez cały czas życia FALI 284 obietnica z Przyczyny B — możliwość odwołania zakolejkowanego marszu-z-atakiem — była martwa.** Naprawione jednym wspólnym rendererem (osobny blok dla `march-stop`, przycisk tekstowy jak `disband`, bo etykieta jest dynamiczna, + nowa klasa CSS `.uc-act-text`), więc poprawka obejmuje OBA miejsca naraz. (2) **`P-TOOLTIP-CIV-UNIT-PANEL-BRAK-W-SCOPE`** — `.civ-unit-panel` dopisany do `SCOPE_SELECTOR` w `hudTitleTooltip.ts` (ta sama przyczyna co runda 2 tematu tooltipów, ostatnia nienaprawiona instancja). ⚠️ **Bez widocznego efektu dziś:** `createUnitPanelHud()` ma ZERO wywołań w `main.ts`/`hud.ts` — panel [H] jest udokumentowanym martwym kodem (żywe zaznaczenie obsługuje `.civ-army-stack`, już w scope), więc to przygotowanie gruntu na przyszłość, nie naprawa dzisiejszego objawu. Oba tematy z werdyktem Evaluatora **PASS-WITH-NOTES**. - **ZASTĄPIONA** (→ `49594c1e`, FALA 286)
|- md5 (pelne): 97b3b12aabf74f0ab60ab6e803de044c · stempel: `ROBOCZA · b8b7b47c · 2026-08-16 10:02` (UTC) · stamp md5 b8b7b47c (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `597972be`
|- Build z HEAD `597972be` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — 8 commitów od commitu deployu FALI 284 (`8f53c800`), z czego **2 commity realnego kodu** (`46ebea84`, `13ac8d35`); pod `gra/` 7 plików, w `gra/src` **3** (`main.ts`, `ui/hudTitleTooltip.ts`, `ui/unitActionBarHtml.ts`), reszta to 2 testy i 2 stuby testowe. **`gra/data` NIE ruszone · `gra/src/map/**` NIE ruszone** — zmierzone `git diff --name-only 8f53c800..597972be -- gra/src/map gra/data` (pusto) oraz to samo względem `97a61e7b` (HEAD, z którego zbudowano FALĘ 284) — również pusto. `git status --porcelain` pusty przed buildem; po buildzie wyłącznie 9 oczekiwanych plików deployu pod `gra-robocza/`.
|- ✅ **Twardy dowód, że poprawka renderera realnie weszła do bundla (porównanie dwóch bundli, nie odczyt kodu):** w bundlu FALI 284 (`git show 8f53c800:gra-robocza/Gra-ROBOCZA.html`) klasa `uc-act-text` występuje **0 razy**, a `march-stop` **2 razy** (wyłącznie warstwa danych + routing handlera) — markup przycisku fizycznie nie istniał. W bundlu FALI 285: `uc-act-text` **2 razy**, `march-stop` **5 razy**, z widocznym `<button type="button" class="uc-act-btn uc-act-text" data-act="march-stop" …>`. Analogicznie `civ-unit-panel`: **1 → 2** trafienia (dopisanie do `SCOPE_SELECTOR`).
|- Bramki (8 tematycznych/rdzeniowych, wszystkie exit 0): `tsc --noEmit` **0 błędów** (TypeScript **5.9.3**, wersja zweryfikowana przed zaufaniem wynikowi, `C-029`) · `logic-test` **213/213** (punkt odniesienia `R-BRAMKA-MINDIST-Q1 = A`) · `tech-tree-test` **19/0** · `research-test` **33/33, ALL GREEN** · `unit-replace-test` **13/13** · `ai-founding-territory-test` **28/0** · `combat-test` **6/6** · `hud-tooltip-body-mounted-panels-test` **16/0** (było 8/0 w FALI 284 — wzrost z tematu 2, nowy „Scenariusz 2" z dowodem mutacyjnym) · `march-attack-queue-persist-test` **55/0** (było 41/0 w FALI 284 — wzrost z tematu 1, sekcja (i) oblężenia z dowodem mutacyjnym `MUT-SIEGE-GUARD`). Build `vite` **824 moduły, 23,41 s, exit 0**.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły** (kontrola, czy fala nie schowała regresji pod przykrywką znanego długu): `unit-power-test` **4/2** · `prereq-budynkow-test` **51/8** · `mennica-magazyn-test` **38/3** · `empire-food-b5-test` **25/3** · `budynek-civ-bonus-u17-test` **3/3** · `trade-routes-income-test` **52/1** · `growthmult-compound-test` **17/7** · `population-growth-v85-test` **48/2** · `spichlerz-widocznosc-test` **37/8** · `spichlerz-wzrost-test` **2/7** · `food-hodowla-test` **20 OK/4 FAIL** · `ai-test` **285/8** · `empire-panel-moc-scroll-preserve-test` **38/9** — **wszystkie co do jednej dokładnie na punkcie odniesienia z `CLAUDE.md` §BRAMKI, zero wzrostu**. `empire-panel-econ-slider-visibility-test` **60/0, exit 0** (zielona, zgodnie ze sprostowaniem z 2026-08-14).
|- `map-gen-regression-test` — **oba kryteria akceptacji z `CLAUDE.md` ZIELONE, oba zmierzone ŚWIEŻO w tej fali, nie odziedziczone z FALI 284.** ✅ **0 rzek bez ujścia: PASS** — `Trasy OK (ujście morze lub rzeka): 2124/2124`, `Główne rzeki OK (REALNE morze): 1235/1235 (0 przypadków fail)`, pełne 40 map (5 seedów × 4 typy × 2 gęstości rzek), exit 0. ✅ **Determinizm A=B: PASS** — `hash A=fdb5e82c B=fdb5e82c → IDENTYCZNY`, exit 0. **Świadoma decyzja metodyczna, nie przemilczana luka:** pełny przebieg `map-gen-regression-test.cjs` został uruchomiony i po ~13 min utknął na trzecim benchmarku czasowym (mapa „duża" 240×168, znany limit `P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY` — w FALI 280 nie zmieściła się w budżecie 900 s), więc został zatrzymany i **zastąpiony dwoma celowanymi harnessami będącymi wiernymi wycinkami tego samego pliku testu** (`scratchpad/FALA285/rzeki-harness.cjs` = linie 40–93 `countBadRiverPaths` + pętla linii 99–125; `scratchpad/FALA285/determinizm-harness.cjs` = linie 157–164) — identyczny `hexHash`, identyczne `DENSITY`/`SEEDS`/`TYPES`, identyczne wywołania `generujSwiat`; różnica polega WYŁĄCZNIE na pominięciu trzech benchmarków czasowych (`bench maly/standardowy/duzy`), które są pomiarem wydajności sandboksa, a nie kryterium poprawności. Zmierzone czasy tej maszyny dla kontekstu: mała 39,26 s, standardowa 76,25 s (progi AC to <5 s/<15 s — czerwone z powodu maszyny, nie generatora). **Ta fala nie dotyka generatora w ogóle: `gra/src/map/**` = 0 zmienionych plików, `gra/data` = 0** — regresja mapgenu jest tu strukturalnie niemożliwa; hash determinizmu `fdb5e82c` jest zresztą identyczny z tym zmierzonym w FALI 284, co niezależnie to potwierdza.
|- **Nowe znaleziska Evaluatora, zarejestrowane w `PYTANIA-OTWARTE.md`, NIE naprawione w tej fali** (nie blokują, świadomie odłożone): `P-UNITACTIONBAR-RENDERER-BRAK-BRAMKI-KOMPLETNOSCI` — krytyczna połowa naprawy (renderer) nie ma ŻADNEJ bramki; mutacja usuwająca cały nowy blok renderujący nie jest łapana przez żadną z 12 bramek, bo `march-attack-queue-persist-test` sprawdza tylko warstwę danych `main.ts`, nie renderowany HTML (ten sam martwy punkt, przez który Przyczyna B przeszła jako „gotowe"). `P-UNITACTIONBAR-UNFORTIFY-ALL-NIEOSIAGALNE` — akcja „Odfortyfikuj całą armię" ma działający handler (`main.ts:17883`) i gotową ikonę, ale nie jest w `COMPACT_ACTION_ORDER`, więc gracz nie może jej dziś wywołać żadną ścieżką. Do tego `P-UNITINFOGRAPHIC-OSIEROCONY-SELEKTOR-CIV-UNIT-PANEL` (znalezisko przy temacie 2).
|- 🔴 **Playtest — na co patrzeć:** (a) ⭐ **zakolejkowany marsz-z-atakiem da się wreszcie odwołać** — zaznacz jednostkę, kliknij wroga w zasięgu dystansowym (kolejkuje się marsz z atakiem), a potem sprawdź, czy w pasku akcji jednostki **jest przycisk „Anuluj atak"/„Zatrzymaj" i czy klik faktycznie kasuje plan**; w FALI 284 tego przycisku fizycznie nie było w ogóle, mimo że fala go obiecywała; (b) **to samo w trakcie oblężenia** — jednostka oblegająca miasto z zakolejkowanym atakiem ma pokazywać „Anuluj atak", a klik ma skasować TYLKO atak, **nie zdejmować oblężenia**; (c) **kontrola czystości UI** — jednostka w oblężeniu BEZ zakolejkowanego ataku nie może pokazywać tego przycisku w ogóle.
|- **Scalenie do `main`: FALA 284 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 284 `8f53c800`**, nie w czubek gałęzi. **FALA 285 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 286.

## ROBOCZA 68c7f238 - 2026-08-15 12:13 UTC (15.08.2026 14:13 PL) - FALA 284: trzy naprawy UI/rozkazów, wszystkie z werdyktem Evaluatora PASS-WITH-NOTES. (1) **Tooltipy ikon (i) znów mają treść** — w panelu Imperium/Surowce i w Audiencji dyplomatycznej kursor zmieniał się w pytajnik, ale dymek nie pokazywał nic (kontenery montowane na `document.body` były poza `SCOPE_SELECTOR`, więc JS-owy tooltip ich nie obsługiwał i zostawał tylko natywny `title=`); runda 2 tematu `P-NEWGAME-KREATOR-TOOLTIP-INFO-NIE-DZIALA`. (2) **Panel HUD „WYDARZENIA" przestał połykać klik na mapę** — pusta strefa `.civ-side-panel` przechwytywała kliknięcia w canvas mapy świata (Przyczyna A tematu `P-BITWA-MARSZ-POTEM-ATAK-NIE-KOLEJKUJE`); runda 1 to naprawiła, ale zabrała przewijanie listy wydarzeń — złapał Evaluator, runda 2 wróciła scroll na wewnętrzny wrapper BEZ cofania martwej strefy. (3) **Rozkaz „marsz, potem atak" przeżywa odznaczenie jednostki** — kliknięcie na wroga w zasięgu dystansowym poprawnie kolejkowało marsz z atakiem, ale odznaczenie jednostki (Escape i 34 inne miejsca w kodzie) w ciszy kasowało zakolejkowany atak, więc jednostka dochodziła i nie strzelała (Przyczyna B tego samego tematu, decyzja właściciela `P-BITWA-MARSZ-POTEM-ATAK-NIE-KOLEJKUJE-Q1 = A` — naprawa u źródła). - **ZASTĄPIONA** (→ `97b3b12a`, FALA 285)
|- md5 (pelne): 68c7f238d8ef8c3ae41d632c52a17a72 · stempel: `ROBOCZA · 019b283d · 2026-08-15 12:13` (UTC) · stamp md5 019b283d (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · źródłowy HEAD `97a61e7b`
|- Build z HEAD `97a61e7b` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — 11 commitów od commitu deployu FALI 283 (`118728a0`), z czego 4 commity realnego kodu; 7 plików pod `gra/` (`+1414/−24`), w tym `gra/src` **3 pliki** (`main.ts`, `ui/sidePanelHud.ts`, `ui/hudTitleTooltip.ts`) i 4 pliki testów. **`gra/data` NIE ruszone** (zero zmian danych) · **`gra/src/map/**` NIE ruszone** (zero plików) — potwierdzone `git diff --name-only`, oraz `git status --porcelain` pustym zarówno przed buildem, jak i po nim.
|- Bramki (10, wszystkie exit 0): `tsc --noEmit` 0 błędów · `logic-test` 213/213 · `tech-tree` 19/0 · `research` 33/0 · `unit-replace` 13/13 · `ai-founding-territory` 28/0 · `hud-tooltip-body-mounted-panels` 8/0 (nowa bramka tematu 1, z mutacją dowodzącą że test nie jest tautologią) · `sidepanel-hud-deadzone` 32/0 (nowa bramka tematu 2, obejmuje i martwą strefę, i przywrócony scroll) · `march-attack-queue-persist` 41/0 (nowa bramka tematu 3) · `scout-explore-deselect-cycle` 34/0. Build 824 moduły, `vite` 23,69 s.
|- `map-gen-regression-test` — **oba kryteria akceptacji z `CLAUDE.md` ZIELONE, przebieg jako całość nieukończony.** ✅ **0 rzek bez ujścia: PASS** — pełna sekcja odjechała do końca: `Trasy OK (ujście morze lub rzeka): 2124/2124`, `Główne rzeki OK (REALNE morze): 1235/1235 (0 przypadków fail)` (5 seedów × 4 typy × 2 gęstości rzek). ✅ **Determinizm A=B: PASS** — `hash A=fdb5e82c B=fdb5e82c → IDENTYCZNY`; zmierzony **osobnym, celowanym harnessem** (`scratchpad/FALA284/determinizm-harness.cjs`: ten sam `hexHash`, ta sama `DENSITY`, to samo wywołanie `generujSwiat(42,'standardowy','kontynenty')` co w oryginale — różni się wyłącznie pominięciem sekcji benchmarków i rzek), bo główny przebieg został ubity przez harmonogram zadań w tle na 18. z 20 map sekcji „Sieć rzek + dopływy", zanim doszedł do sekcji determinizmu. ⚠️ **Nie zmierzone:** ostatnie 3 mapy sekcji „Sieć rzek + dopływy" (17 z 20 przeszło bez ani jednej linii FAIL). ⚠️ Progi czasowe AC „standard <5 s / duża <15 s" **czerwone** (62,28 s / 121,92 s / 1120,55 s) — to pomiar wydajności sandboksa, **nie regresja** (runbook §6, `P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`); przez nie `allOk` tej bramki nie może wyjść 0 na tej maszynie niezależnie od jakości generatora. **Ta fala nie dotyka generatora w ogóle: `gra/src/map/**` = 0 zmienionych plików, `gra/data` = 0** — regresja mapgenu jest tu strukturalnie niemożliwa.
|- Nowe znaleziska/otwarte pytania bez zmian względem FALI 283 — ta fala nic nowego nie otworzyła.

## ROBOCZA e0eb03a9 - 2026-08-14 21:16 UTC (14.08.2026 23:16 PL) - FALA 283: atak z dystansu na mapie świata NAPRAWIONY W CAŁOŚCI (2 rundy) — przyczyna: klik na własny mieszany stos (zwarcie+dystans) wybierał "reprezentanta" po sile w zwarciu, więc łucznik w stosie z wojownikiem nigdy nie był sprawdzany pod kątem zasięgu ataku (nowa funkcja `isTargetWithinStackAttackRange` sprawdza cały stos, nie tylko reprezentanta); runda 2 zamyka lukę, którą runda 1 przypadkiem poszerzyła — atak przez mgłę wojny bez sprawdzenia widoczności celu. Wszystkie 3 ścieżki inicjacji ataku i test odtworzone i potwierdzone żywym kliknięciem myszy w przeglądarce. Do tego: sprostowane dwa komentarze w kodzie (HUD-konwerter już naprawiony, błędny przykład jednostki zwarcia). Wszystkie tematy z serii "wypchnijmy je" (regres państw-miast, city-panel konwertery runda 2, HUD-konwerter, blackout mapy, atak dystansowy) mają dziś werdykt Evaluatora (PASS-WITH-NOTES lub zamknięte bez regresji) - żaden nie czeka już na ocenę. - **ZASTĄPIONA** (→ `68c7f238`, FALA 284)
|- md5 (pelne): e0eb03a96eeb69b88999edb7b1c728af · stempel: `ROBOCZA · 33e6c792 · 2026-08-14 21:16` (UTC) · stamp md5 33e6c792 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · commit `eb4c022d`
|- Nowe znaleziska przy okazji (zarejestrowane, NIE dispatchowane, nie krytyczne): gracz nigdy nie widzi ekranu podsumowania po bitwie 3D (`P-BITWA-PODSUMOWANIE-NIGDY-NIE-WIDOCZNE`), atak na miasto nie sprawdza mgły (`P-BITWA-ATAK-MIASTO-MGLA-BRAK-SPRAWDZENIA`), AI w ogóle nie zna pojęcia mgły (`P-AI-BRAK-POJECIA-MGLY`), dwa drobne wzmocnienia obronne (rejestracja sceny bitwy przed wyjątkiem konstruktora, stos z dwoma różnymi stackGroupId).
|- Czeka na odpowiedź właściciela: `P-BITWA-ATAK-DYSTANS-TELEPORT-Q1` (czy zwycięzca ataku z dystansu teleportuje się, zostaje na miejscu, czy przesuwa o 1 hex) i `P-NEWGAME-KREATOR-TOOLTIP-INFO-Q1` punkty (b)/(c).

## ROBOCZA d3b43ac6 - 2026-08-14 13:11 UTC (14.08.2026 15:11 PL) - FALA 282: ⛔ KRYTYCZNE — naprawiony blackout mapy świata po wygranej bitwie (kamera mapy dryfowała pod nakładką bitwy, bo `camCtrl.update()` leciał bezwarunkowo nawet gdy scena bitwy przykrywała mapę — panowanie kamerą bitwy/edge-pan przesuwało kamerę mapy pod spodem, po powrocie kadr stał poza mapą). Do tego: pełna produkcja konwerterów w liście budynków miasta (Garncarnia/Cegielnia/Odlewnie — osobna linia wejść/wyjść obok kosztu utrzymania, funkcja domenowa wydzielona do silnika, runda 2 po tautologicznym teście), naprawiony rozjazd HUD-u imperium vs silnik dla Odlewni żelaza/Wielkiej odlewni (HUD gubił te dwa budynki przez złe dopasowanie receptura↔budynek). - **ZASTĄPIONA** (→ `e0eb03a9`, FALA 283)
|- md5 (pelne): d3b43ac61a099ad3e167885a6355dba9 · stempel: `ROBOCZA · d104342d · 2026-08-14 13:11` (UTC) · stamp md5 d104342d (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · commit `222277aa`
|- Wszystkie tematy tej fali mają dziś (FALA 283) werdykt Evaluatora — patrz wpis wyżej.

## ROBOCZA 7c92c364 - 2026-08-14 12:40 UTC (14.08.2026 14:40 PL) - FALA 281: droga zwykła ×3/brukowana ×5 szybszy ruch (+podłoga bruku dedykowana, sprostowanie kosztu Pustyni w komentarzu), druga obwódka granicy naprawiona na styku terytoriów nieconvexnych (2 rundy, w tym enklawy), ustawienia okolicy resetują się poprawnie po zmianie właściciela miasta (+luka save/load), nowa ikona Rudy cyny (odróżnialna od Rudy żelaza), bilans transakcji w "Traktacie handlowym" liczony spójnie z bramką akceptacji (2 rundy — runda 1 tylko handel, runda 2 też traktaty/pakiety/live-podgląd), usunięty zbędny wybór miasta przy handlu żywnością (handluje się ze Spichlerza całej cywilizacji), wzmocniony test remisu oblężenia. ⚠️ NOWA FUNKCJA Z ZASTRZEŻENIEM: atak z dystansu na mapie świata (jednostka→jednostka działa poprawnie) — Evaluator dał **FAIL**: AI nie ma parytetu z graczem przy ataku na miasto z dystansu (nadal wymaga adiacencji), zdobycie pustego miasta z dystansu daje "capture" zamiast zgłoszonego "ataku", zwycięski atakujący TELEPORTUJE SIĘ na heks celu (do 6 heksów dla Katapulty) bez sprawdzania terenu — pytanie do Macieja `P-BITWA-ATAK-DYSTANS-TELEPORT-Q1` czeka w `dyspozycje/ZGLOSZENIA-CZEKAJACE-NA-LIMIT.md`; zdeployowane na wyraźne polecenie właściciela mimo FAIL. Tooltipy kreatora nowej gry zbadane — mechanizm działa poprawnie w headless (nic nie zmieniono), pytanie doprecyzowujące do właściciela otwarte. - **ZASTĄPIONA** (→ `d3b43ac6`, FALA 282)
|- md5 (pelne): 7c92c3647a6cc9b1aae22616016ae2ee · stempel: `ROBOCZA · 05d4cfb0 · 2026-08-14 12:40` (UTC) · stamp md5 05d4cfb0 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`) · commit `a0e34214`

## ROBOCZA ab4ffc15 - 2026-08-14 09:02 UTC (14.08.2026 11:02 PL) - FALA 280: modale końca tury przestają nachodzić na siebie (4 rundy), drugi/trzeci atak barbarzyńców nie kasuje już otwartej bitwy (kolejka FIFO), pełny reskin paneli Miasta/Armie na paletę 3b, NOWA FUNKCJA - druga, wewnętrzna obwódka granicy terytorium koduje relację dyplomatyczną, remis szturmu ZAWSZE kończy oblężenie (63/63 zamiast 30/63), tooltip heksu domknięty o irygację/drogę brukowaną/stadninę/kopalnię cyny, Milet przestaje trzymać pracowników na heksie Aten (przy zmianie właściciela, przy wczytaniu save'a i wizualnie w panelu), znacznik Mocy dostaje wreszcie ikonę SVG zamiast surowego glifu ⚜ - **ZASTĄPIONA** (→ `7c92c364`, FALA 281)
|- md5 (pelne): ab4ffc156f11fb6204355db7757b3dbf · stempel: `ROBOCZA · 92c5101d · 2026-08-14 09:02` (UTC) · stamp md5 92c5101d (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`)
|- **FALA 280.** Build z HEAD `28a26339` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **56 commitów** od commitu deployu FALI 279 (`f9589690`), z czego **21 commitów realnego kodu/testów** pod `gra/`; reszta to wpisy rejestru („Rejestr:"/„Evaluator:"/„Kanal:"). **`gra/data` NIE ruszone w tej fali** (zero zmian danych — cała zawartość to logika, UI, render i testy); potwierdzone `git status --porcelain` **pustym zarówno przed buildem, jak i po nim**. Sesja 2026-08-14, wszystkie tematy przez pętlę AutoBot Operator → Evaluator.
|- ⚠️ **Realna delta bundla jest o JEDEN commit większa niż `git log f9589690..HEAD`** — i to nie jest błąd rachunku. Bundle FALI 279 (`4bef7d10`) powstał z HEAD `5dc45b6e`, a commit deployu `f9589690` został potem **zrebase'owany na czubek gałęzi**, więc zgarnął pod siebie commit `d2dd4b89`, którego w tamtym bundlu fizycznie NIE BYŁO. Zmierzone, nie założone: `git merge-base --is-ancestor d2dd4b89 5dc45b6e` → **NIE**, `... d2dd4b89 f9589690` → **TAK**. Prawdziwa delta względem grywalnego bundla 279 to więc `5dc45b6e..HEAD`: **22 commity pod `gra/`, 61 plików, `+7052/−394`, `gra/data` puste**. Praktyczny skutek dla playtestu: **znacznik Mocy w HUD dostaje dedykowaną ikonę SVG zamiast surowego glifu ⚜ — dokładnie tak, jak wpis FALI 279 zapowiadał („wejdzie dopiero w FALI 280"). Potwierdzone w zbudowanym bundlu: `civ-emblem` 2 trafienia.**
|- ⛔ **NIEZATWIERDZONY PROTOTYP KROKU 4 KREATORA NADAL JEST W GRZE — Twoja decyzja z FALI 279 wciąż nie padła.** Wpis FALI 279 zgłosił, że commit `ea0be32a` przypadkiem zgarnął ze wspólnego drzewa prototyp `P-NEWGAME-CYWILIZACJE-ZASLANIAJA-START` wariant A + `P-NEWGAME-OPISY-DO-TOOLTIP` (Cywilizacje przeciwnika po prawej obok siatki 2×3, opisy kursywą → ikona (i) z tooltipem) i poprosił o werdykt „zostaje / wycofaj". **Werdykt nie padł, rewertu nie było** (`git log f9589690..HEAD -- gra/src/ui/newGameFlow.ts` → **zero commitów**), więc prototyp jedzie dalej: w tym bundlu `sett-layout` **5** trafień, `ai-civ-panel` **7**, `ng-info` **4**. Prośba z FALI 279 zostaje w mocy — obejrzyj krok 4 kreatora i powiedz, czy zostaje.
|- **Dziewięć tematów (nazwane wprost):**
|  1. **Koniec tury — modale przestają nachodzić na siebie, temat ZAMKNIĘTY po 4 rundach** (`P-KONIEC-TURY-ZDARZENIA-NACHODZA-NA-SIEBIE`; `a7de65b0`, `136fefbb`, `cb1ea991`, `4f24bcda`, `993adab4`; `gra/src/main.ts`, `ui/preBattle.ts`). Trzy modale końca tury (audiencja dyplomatyczna, preBattle ataku, panel scalenia jednostki) mogły otworzyć się jednocześnie — `showPreBattle()` jako jedyna z trzech **nie miała żadnego strażnika**. Naprawa nie przebudowuje ~27-tysięcznolinijkowej `triggerPlayerEndTurn`, tylko każe każdemu odbiorcy bronić się samemu wzorcem „sprawdź-i-odłóż". Runda 2 (`136fefbb`) naprawiła wyścig: `healStaleEndTurnBlockers` **myliła bitwę stojącą w kolejce z bitwą osieroconą** i kasowała tę pierwszą. Runda 3 (`4f24bcda`, F1–F4) i runda 4 (`993adab4`, G1 bloker + G2) domknięte po dwóch werdyktach **FAIL** Evaluatora — ten temat nie przeszedł „za pierwszym razem" i to jest w rejestrze widoczne. Bramki: `end-turn-modal-sequencing-test` **40/0**, `heal-stale-blockers-pending-battle-test` **23/0**, `koniec-tury-f1-f4-runda3-test` **39/0**, `koniec-tury-g1-g2-runda4-test` **38/0**, `diplomacy-audience-close-flush-test` **37/0** (`cb1ea991`, N3: flush odroczonej bitwy na WSZYSTKICH ścieżkach zamknięcia audiencji, nie tylko na jednej).
|  2. **Barbarzyńcy — drugi atak w tym samym ticku przestaje kasować pierwszą, otwartą bitwę** (`P-BARBARZYNCY-PODWOJNY-ATAK-PREBATTLE-NADPISANY`; `fe39215a`; `gra/src/ui/preBattle.ts`). ⚠️ **Skutek był cięższy, niż mówiło samo zgłoszenie** — i to sam Operator sprostował po commicie: zgubiona bitwa **nie toczyła się dalej niewidocznie w silniku**, tylko `hidePreBattle()` nie wołał ŻADNEGO callbacku porzuconej bitwy, więc `applyMapBattleOutcome` w ogóle się nie wykonywał — **atak barbarzyńcy przepadał bez żadnego efektu mechanicznego, nie tylko wizualnego**. Fix: strażnik w `showPreBattle()` rozszerzony o `isPreBattleOpen()` (była asymetria — strona „flush" już to sprawdzała, strona „show" nie) + jeden nullable slot → **kolejka FIFO**, ten sam wzorzec co istniejące `deferredMergePrompts`. Bramka: `barbarzyncy-podwojny-atak-prebattle-test` **18/0**.
|  3. **Reskin paneli Miasta i Armie na paletę 3b** (`MIASTA-ARMIE-PANEL-LEWY-2026-08-14`; `cbb85e3a` + `0415f258` N1–N4; `gra/src/ui/cityListHud.ts`, `armyListHud.ts`, nowy `sideListHud.css.ts`, `formatPl.ts`, `main.ts`). Obie listy z lewej krawędzi mapy przechodzą na paletę panelu imperium i **kartę-per-wiersz** zamiast płaskiego tekstu; zdublowane `.cl-*`/`.al-*` arkusze zastąpione jednym wspólnym plikiem stylów. Miasta: pasmo podsumowania (N miast + W BUDOWIE/GARNIZONY), plakietka „stolica", pasek postępu produkcji. Armie: 4 plakietki stanu w 3 kolorach, paski Zdrowie/Ruch. ✅ **Zweryfikowane przez agenta deployu NA ZBUDOWANYM BUNDLU w headless Chromium, nie z kodu:** oba panele otwierają się z paska narzędzi mapy, mają wspólną klasę rdzenia `civ-side-list-hud`, `display:flex`, **wszystkie 5 tokenów palety 3b zgadza się co do znaku** (`--panel:#171e2a`, `--border:#2b3543`, `--muted:#7d8798`, `--gold:#d9a441`, `--tile:#1d2634`) i **żadna ze starych klas `.cl-*`/`.al-*` już nie istnieje w DOM**. Bramka: `side-list-hud-panel-coverage-test` **74/74** (opis commita mówił 72/72 — dziś jest 74, wzrost z rundy naprawczej N1–N4, nie zmiana progu).
|  4. **NOWA FUNKCJA — druga, wewnętrzna obwódka granicy terytorium koduje relację dyplomatyczną** (`R-GRANICE-RELACJA-DYPLOMATYCZNA-Q1`; `bf839f81`; `gra/src/game/civ-visual.ts`, `render/rangeOverlay.ts`, `main.ts` — praca renderowa, **Opus 5** zgodnie z regułą stałą). Obok istniejącej ZEWNĘTRZNEJ obwódki w kolorze tożsamościowym cywilizacji dochodzi DRUGA, wewnętrzna, o połowę cieńsza: **wojna → `#FF5252`**, **sojusz → `#4FC3F7`**, **pokój/neutralni → `#FFD700`**. Kolory dobrane świadomie tak, żeby NIE zlewały się z barwami tożsamościowymi (czysta czerwień odpadła, bo ginie na tle Rzymian `#8B1A1A`/Chińczyków `#C41E3A`/Słowian `#B83232`) — **każdy to jedna stała w `civ-visual.ts`, gotowa do korekty odcienia po Twoich oględzinach**. Odświeżenie tylko przy realnej zmianie STATUSU i tylko dla par z graczem (AI↔AI nie przemalowuje niczego) — nie co turę przy drgnięciu zaufania. ⚠️ **Nota autora do playtestu:** sojuszniczy `#4FC3F7` jest w podobnej rodzinie odcieni co linie rzek — rozróżnia je grubość pasma; jeśli w grze będzie mylić, naturalny następny krok to przesunięcie w fiolet/turkus. Bramka: `granice-relacja-dyplomatyczna-test` **52/0**.
|  5. **Oblężenie — remis szturmu ZAWSZE kończy oblężenie** (`R-OBLEZENIE-REMIS-MASZYNY-Q1=A`, `R-OBLEZENIE-REMIS-MASZYNY-Q2=A`; `51a32fe6`, `a159af84`, `678b31e1`; `gra/src/main.ts` `+11`). PRZED: zdjęcie flagi `city.oblegane` po remisie było **wyłącznie przypadkowym skutkiem ubocznym** `validateActiveSieges()` — działało tylko wtedy, gdy geometria odwrotu zostawiła wszystkich oblegających poza dystansem 1. Zmierzone na wszystkich 63 niepustych podzbiorach 6 heksów przyściennych: **30/63 kończyło oblężenie, 33/63 zostawiało je TRWAJĄCE** mimo komunikatu „oblężenie zniesione" — to był werdykt **FAIL** Evaluatora dla rundy 1. PO: `endMapSiege(city.id)` wołane bezwarunkowo → **63/63**. `678b31e1` to sprostowanie samych liczb w trwałym komentarzu `main.ts` (było błędne 32/63, jest zmierzone 30/63). Bramka: `oblezenie-remis-endsiege-test` **265/0**.
|  6. **Tooltip heksu — domknięcie rund 3–5** (`P-HEX-TOOLTIP-MOZLIWE-ULEPSZENIA-BRAK-FILTRA-ZLOZA`; `ec089e9a`, `92be2b28`, `f874fe04`; `gra/src/ui/hexContextTooltip.ts`, `map/improvement-build.ts`). Trzy osobne wady tej samej listy: (a) `droga_brukowana` pokazywana bez wymaganej Drogi, (b) `irygacja` — `hexHasRiverAccess()` czytała `rzeka.obecna` sąsiadów zamiast `map.riverPaths` jak silnik, więc warunek był **szerszy niż silnik o ~7 % heksów FLAT_IRR** (runda 4 zrobiła go naprawdę lustrzanym), (c) **niedomiar w drugą stronę** — `stadnina` i `kopalnia_cyny` **nie miały case'a w `galleryTerrainEligible`**, wpadały w `default → false` i tooltip NIGDY ich nie pokazywał, mimo że silnik na nie pozwalał. Bramki: `hex-tooltip-mozliwe-ulepszenia-zloze-test` **74/0** (było 36/0 w FALI 279 — wzrost z rund 4/5, w tym asercja równości 4 kopii offsetów sąsiadów), `hex-tooltip-stadnina-kopalnia-cyny-test` **29/0** (nowa).
|  7. **„Milet trzyma pracowników na heksie Aten" — trzy warstwy tej samej dziury** (`P-MILET-ATENY-OKOLICA-RECZNE-PRZY-PRZEJECIU*`; `3f1a2f85`, `47a2e1ff`; `gra/src/main.ts`, `map/territory-work.ts`, `ui/cityPanel.ts`). (1) `seedCityOwnerDefaults()` — wołane po KAŻDEJ zmianie `city.ownerId` (kapitulacja z głodu, aneksja miasta-państwa, zdobycie w bitwie, bunt) — resetowało kilka pól, ale **nie wołało `reconcileAllWorkedTiles`**, więc nielegalne wpisy trybu ręcznego okolicy przeżywały do końca tury; naprawione wewnątrz samej funkcji, zgodnie z jej własnym kontraktem JSDoc, nie przy 4 miejscach z osobna. (2) **Save sprzed naprawy nadal niósł zepsute dane** — `restoreGameFromSave()` dostał sanityzację tuż po odtworzeniu miast. (3) Panel miasta oznacza teraz taki heks **czerwono-pomarańczowym przerywanym pasmem z ostrzeżeniem** zamiast zielonego „legalnie pracujące", reużywając nowo wyeksportowanej `isReczneKeyLegal` — **tej samej, bez-mutacji reguły co reconcile, zero duplikacji logiki**. Bramki: `okolica-ownership-change-reconcile-test` **15/15**, `okolica-load-reconcile-test` **11/11** (obie z własną mutacją kontrolną: po usunięciu naprawy asercja faktycznie czerwienieje — dowód, że nie jest tautologią).
|  8. **ESCAPE runda 2 — test na prawdziwych panelach + 5. panel + 3 toasty** (`P-MENU-ESCAPE-NIEPELNOEKRANOWE`; `c3a5652c`; `gra/src/ui/improvementBuildConfirm.ts` i in.). Domknięcie tematu z FALI 279: doszedł piąty panel (`improvementBuildConfirm`) i 3 toasty, a test przestał być atrapą — wykonuje realne panele. Bramki: `escape-overlay-real-panels-test` **49/0** (nowa, z mutacją kontrolną: cofnięcie `pushOverlay` daje czerwony wynik), `escape-overlay-stack-test` **84/0**.
|  9. **Wyrównanie ramek w kroku 4 kreatora — FAŁSZYWY ALARM, potwierdzony pomiarem** (`P-NEWGAME-RAMKI-WYROWNANIE`; `4eaecba4`; wyłącznie nowy test, **zero kodu produkcyjnego**). Zgłoszenie nie potwierdziło się — ramki są wyrównane; dołożony strażnik regresyjny, żeby to zostało. Bramka: `newgame-sett-grid-layout-test` **70/0** (z dowodem mutacyjnym B2: złamanie obu warstw naraz FAKTYCZNIE rozjeżdża wysokości, więc asercja ma zęby).
|- ➕ **Dziesiąta pozycja, dług testowy spłacony: czerwona bramka wniesiona przez FALĘ 279 jest ZIELONA.** `a8b47623` poprawił zduplikowaną asercję `rekruci → ekonomia`, przeoczoną w FALI 279 w drugim z dwóch testów trzymających tę samą asercję. **`empire-panel-econ-slider-visibility-test` = 60 pass / 0 fail, exit 0** (w FALI 279 było 59/1 exit 1 i było tam zgłoszone jako „NOWA CZERWONA BRAMKA WNIESIONA PRZEZ TĘ FALĘ, do naprawy osobnym zleceniem"). **Zlecenie wykonane — ta bramka nie należy już do listy czerwonych.** Do kompletu `1c86d756` domknął notę N1 z tematu Auto-pracy (komentarz w `auto-improvements.ts`, bez zmiany zachowania).
|- **Bramki na HEAD `28a26339` (uruchomione przez agenta deployu na tym właśnie czubku, przed buildem — nie odczytane z raportów Operatora ani z opisów commitów):** `tsc --noEmit` **0 błędów, exit 0** (TypeScript **5.9.3**, wersja zweryfikowana przed zaufaniem wynikowi, `C-029`) · `logic-test` **213/213, exit 0** (punkt odniesienia `R-BRAMKA-MINDIST-Q1 = A`) · `tech-tree-test` **19/0** · `research-test` **33/33, ALL GREEN** · `unit-replace-test` **13/13** · `ai-founding-territory-test` **28/0** · `combat-test` **6/6** · `end-turn-modal-sequencing-test` **40/0** (nowa) · `heal-stale-blockers-pending-battle-test` **23/0** (nowa) · `koniec-tury-f1-f4-runda3-test` **39/0** (nowa) · `koniec-tury-g1-g2-runda4-test` **38/0** (nowa) · `diplomacy-audience-close-flush-test` **37/0** (nowa) · `barbarzyncy-podwojny-atak-prebattle-test` **18/0** (nowa) · `side-list-hud-panel-coverage-test` **74/74** (nowa) · `oblezenie-remis-endsiege-test` **265/0** (nowa) · `granice-relacja-dyplomatyczna-test` **52/0** (nowa) · `newgame-sett-grid-layout-test` **70/0** (nowa) · `hex-tooltip-stadnina-kopalnia-cyny-test` **29/0** (nowa) · `okolica-load-reconcile-test` **11/11** (nowa) · `okolica-ownership-change-reconcile-test` **15/15** (nowa) · `escape-overlay-real-panels-test` **49/0** (nowa) · `hex-tooltip-mozliwe-ulepszenia-zloze-test` **74/0** · `escape-overlay-stack-test` **84/0** · `empire-panel-econ-slider-visibility-test` **60/0** · `empire-panel-split-test` **25/0** · `auto-improvements-test` **37/0** · `vite build` **822 moduły, exit 0** · `verify-robocza-bundle` **VERIFY OK** (`manifest match: OK`). **Wszystkie 25 bramek testowych exit 0**, w tym **14 bramek nowych, powstałych w tej fali**.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły** (kontrola, czy fala nie schowała regresji pod przykrywką znanego długu): `unit-power-test` **4 pass / 2 fail** · `prereq-budynkow-test` **51 pass / 8 fail** · `mennica-magazyn-test` **38 pass / 3 fail** · `empire-food-b5-test` **25 pass / 3 fail** · `budynek-civ-bonus-u17-test` **3 pass / 3 fail** · `trade-routes-income-test` **52 pass / 1 fail** · `growthmult-compound-test` **17 pass / 7 fail** · `population-growth-v85-test` **48 pass / 2 fail** · `spichlerz-widocznosc-test` **37 pass / 8 fail** · `spichlerz-wzrost-test` **2 pass / 7 fail** · `food-hodowla-test` **20 OK / 4 FAIL** · `ai-test` **285 pass / 8 fail** · `empire-panel-moc-scroll-preserve-test` **38 pass / 9 fail** — **wszystkie co do jednej dokładnie na punkcie odniesienia z `CLAUDE.md` §BRAMKI i z wpisu FALI 279, zero wzrostu**, żadna nowa porażka się w nich nie schowała. (`empire-panel-moc-scroll-preserve-test` **nadal nie figuruje na liście w `CLAUDE.md` §BRAMKI**, mimo że zgłaszano to już czterokrotnie — zmiana kanonu wymaga własnej pętli Operator→Evaluator, deploy jej nie robi.)
|- ⏱️ **`map-gen-regression-test` NIEUKOŃCZONY — nieblokujący, ten sam znany artefakt sandboksa co w FALACH 275–279** (`P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`). Zmierzone w tym przebiegu: mapa mała (108×74) **55,97 s**, standardowa (168×120) **112,23 s** (z czego rzeki `riversMain` 66,7 s + `riversFill` 39,8 s), duża nieukończona w budżecie 900 s. **Zero linii FAIL w wypisanej części** — to limit czasu, nie porażka asercji. ✅ **Ryzyko w TEJ fali sprawdzone, nie założone:** pod `gra/src/map/**` fala rusza dokładnie dwa pliki — `improvement-build.ts` (dopisane dwa `case` w `galleryTerrainEligible`, funkcji czytanej przez tooltip) i `territory-work.ts` (`cityCenterKeys` z prywatnej na eksportowaną + nowa czysta funkcja `isReczneKeyLegal`, zero zmiany istniejącego zachowania). **Żadnego z tych dwóch plików nie importuje ani `generator.ts`, ani `gen-helpers.ts`, ani `villages.ts`, ani `newGameMapDefaults.ts`** — czyli żaden z punktów wejścia bramki (sprawdzone grepem po wszystkich czterech, nie założone). Łańcuch generowania mapy jest przez tę falę nietknięty.
|- 🖥️ **Weryfikacja headless na ZBUDOWANYM BUNDLU (Chromium 1194, viewport 1600×950) — nie na kodzie źródłowym: 28 asercji zaliczonych.** Bundle wstaje, `?playtest=mapa` dochodzi do żywego HUD-u i wygenerowanej mapy (Tura 1 · 4000 p.n.e.), menu główne renderuje się poprawnie. Oba panele reskinu otwarte z paska narzędzi mapy i obejrzane na zrzucie: **Miasta** — pasmo „1 miasto / W BUDOWIE 1 z 1 / GARNIZONY 0 jedn.", karta „Testpolis" z plakietką **stolica** i paskiem produkcji „Stolarnia 8/20"; **Armie** — nagłówek „ARMIE · 3 · 3 jedn.", trzy karty Hastati z paskami Zdrowie 38/38 i Ruch 2/2. Na zrzucie widać też **nową obwódkę relacji dyplomatycznej** — czerwone wewnętrzne pasmo wzdłuż granicy Testpolis/Ateny (status wojny). ⚠️ **Jedyny „błąd konsoli" okazał się artefaktem harnessu, nie bundla, i został obalony pomiarem:** `404 /favicon.ico` pochodził z minimalnego serwera HTTP agenta (bundle jest jednoplikowy, favicona nie ma na dysku); **ten sam bundle wczytany przez `file://`, czyli metodą FALI 279, daje 0 błędów konsoli** — sprawdzone osobnym przebiegiem, nie założone.
...
|- 🔴 **Playtest — na co patrzeć:** (a) **koniec tury** — audiencja dyplomatyczna, okno bitwy i panel scalenia armii mają wychodzić **po kolei, nigdy jeden na drugim**; (b) **barbarzyńcy** — gdy dwóch/trzech atakuje w tej samej turze, **każdy atak ma się rozstrzygnąć**, żaden nie może przepaść bez efektu; (c) **panele Miasta i Armie** (lewa krawędź mapy) — nowa ciemna paleta, karta na wiersz, pasek produkcji i plakietka „stolica" w Miastach, paski Zdrowie/Ruch w Armiach; (d) ⭐ **granice terytoriów — NOWE:** przy granicy ma być **druga, cieńsza obwódka wewnętrzna**: czerwona = wojna, błękitna = sojusz, złota = pokój/neutralni; **powiedz, czy odcienie są czytelne** (zwłaszcza czy błękit sojuszu nie myli się z rzekami — to znana wątpliwość autora, korekta to jedna stała); (e) **oblężenie** — **remis** szturmu ma **zawsze** zdejmować oblężenie, niezależnie od tego, gdzie odskoczyły jednostki; (f) **tooltip heksu** — irygacja tylko przy realnej rzece, droga brukowana tylko na Drodze, a **stadnina i kopalnia cyny mają się wreszcie POJAWIAĆ** tam, gdzie da się je postawić (wcześniej nie pokazywały się nigdy); (g) **przejęcie miasta** — po zdobyciu/aneksji sąsiad nie może dalej trzymać pracownika na heksie centrum drugiego miasta; wpis nielegalny ma być w panelu **czerwono-pomarańczowy z ostrzeżeniem**, a wczytanie starego save'a ma go posprzątać; (h) **znacznik Mocy** w HUD ma mieć **ikonę SVG, nie surowy glif ⚜**; (i) ⛔ **krok 4 kreatora nowej gry — WCIĄŻ CZEKA NA TWÓJ WERDYKT z FALI 279** (prototyp wariantu A, patrz ostrzeżenie wyżej): „zostaje" albo „wycofaj".
|- **Scalenie do `main`: FALA 279 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 279 `f9589690`**, nie w czubek gałęzi. **FALA 280 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 281.

## ROBOCZA 4bef7d10 - 2026-08-14 04:41 UTC (14.08.2026 06:41 PL) - FALA 279: portret władcy w nagłówku panelu imperium zamiast greckiej świątyni, ESCAPE zamyka 4 panele zamiast wyrzucać z pełnego ekranu, flaga oblężenia znika od razu po wygranej bitwie (nie dopiero po ruchu), tooltip heksu przestał obiecywać ulepszenia bez wymaganego złoża/nakładki, chip Rekruci trafia do bloku Armii, polityka Pracy imperium jest TWARDYM pułapem sumy mimo override miasta, wyrównanie ramek w kroku 4 kreatora - **ZASTĄPIONA** (→ `ab4ffc15`, FALA 280)
|- md5 (pelne): 4bef7d10cc0567bb88e7a17ea1be62ca · stempel: `ROBOCZA · 516800d7 · 2026-08-14 04:41` (UTC) · stamp md5 516800d7 (label mismatch, „stamp match: WARN" — stan normalny wg runbooku §6; wiążące md5 to `manifest match: OK`)
|- **FALA 279.** Build z HEAD `5dc45b6e` gałęzi `claude/sprawdzenie-funkcjonalnosci-ek4ra0` — **28 commitów** od commitu deployu FALI 278 (`32e68c9c`), z czego **9 commitów realnego kodu/testów** pod `gra/`; reszta to wpisy rejestru („Rejestr:"/„Zasady:"/„Kanal:"). Pod `gra/` łącznie **19 plików, `+1434/−40`** — **`gra/src`: 12 plików, `+403/−38`**, reszta to `gra/tools` (4 nowe bramki). **`gra/data` NIE ruszone w tej fali** (zero zmian danych — cała zawartość to logika, UI i testy); potwierdzone `git status --porcelain` **pustym zarówno przed buildem, jak i po nim**. Sesja poranna 2026-08-14, kontynuacja sesji nocnej.
|- ⚠️ **CZUBEK GAŁĘZI ≠ ŹRÓDŁO BUILDA — jeden commit kodu jest na gałęzi, ale NIE MA GO w tym bundlu.** W trakcie pracy agenta deployu (bramki → build → weryfikacja headless, kilkadziesiąt minut) inne sesje dopchnęły na tę samą gałąź **4 kolejne commity** ponad `5dc45b6e`: `b437cb1d`, `c7f6760a`, **`d2dd4b89`**, `1626fdcb`. Trzy z nich to czysty rejestr (`gra/src` nietknięte — sprawdzone `git show --stat` per commit, nie założone), ale **`d2dd4b89` — „Znacznik power: zamiana surowego glifu ⚜ na dedykowaną ikonę SVG" (`P-ZNACZNIK-POWER-HUD-CIV-EMBLEM`), `gra/src/ui/hud.ts` + `powerOverlayHud.ts` + `power-labels.ts` + `empireDetailPanel.ts`, `+42/−8` — jest realnym kodem UI i NIE WSZEDŁ do bundla `4bef7d10`** (`git merge-base --is-ancestor d2dd4b89 5dc45b6e` → NIE). **Skutek dla playtestu: znacznik Mocy w tej fali nadal pokazuje stary glif ⚜ — to NIE jest błąd naprawy, tylko naprawa, która przyszła po buildzie; wejdzie dopiero w FALI 280.** Nie zgłaszaj jej jako niedziałającej. Bundle NIE został przebudowany pod ten commit świadomie: (a) wszystkie bramki i weryfikacja headless zostały wykonane na `5dc45b6e` i przebudowa unieważniłaby te dowody, (b) `d2dd4b89` **nie ma jeszcze werdyktu Evaluatora** w `PYTANIA-OTWARTE.md` (`grep` po `P-ZNACZNIK-POWER-HUD-CIV-EMBLEM` → 0 trafień w rejestrze), więc dołożenie go po cichu do publikowanego bundla obchodziłoby pętlę AutoBot. Commit deployu został **zrebase'owany** na czubek gałęzi (`466cbdc9`), zero force-push.
|- ⛔ **NAJWAŻNIEJSZE OSTRZEŻENIE TEJ FALI — do bundla wjechał PROTOTYP, którego NIE zatwierdziłeś, a commit który go przyniósł NIC o nim nie mówi.** Commit `ea0be32a`, opisany w wiadomości wyłącznie jako „Auto-praca override: naprawa 3 not Evaluatora (N1-N3), bez zmiany zachowania", zgarnął ze współdzielonego drzewa **dwie obce paczki w toku** (klasyczny wypadek z `CLAUDE.md` §4a — praca poza worktree): (a) **fix tooltipa heksu** (`hexContextTooltip.ts` `+44/−5` + eksport w `map/improvement-build.ts`), którego test przyszedł osobno commitem `94977a20` — to jest w porządku, temat był zatwierdzony; (b) **PROTOTYP kroku 4 kreatora nowej gry** (`newGameFlow.ts` `+129/−20` + `hudTitleTooltip.ts` `+6`) realizujący `P-NEWGAME-CYWILIZACJE-ZASLANIAJA-START` **wariant A** oraz `P-NEWGAME-OPISY-DO-TOOLTIP` — **oba tematy stoją w `PYTANIA-OTWARTE.md` ze statusem „OTWARTE — prototyp do przeglądu przed scaleniem", a dyspozycja brzmiała dosłownie „worktree, NIE scalać do głównego drzewa bez zatwierdzenia"**. **Kod jest ŻYWY, nie martwy** — zweryfikowane w źródle, nie założone: `newGameFlow.ts:1454` tworzy `.sett-layout`, `:1396` tworzy `.ai-civ-panel`, `:1782` woła `syncSettLayoutHeight()` przy `curStep === 4`; w zbudowanym bundlu `sett-layout` **3 trafienia**, `ai-civ-panel` **6**, `ng-info` **3**. **Skutek dla Ciebie:** krok 4 kreatora będzie w tym bundlu WYGLĄDAŁ INACZEJ — Cywilizacje przeciwnika po prawej stronie obok siatki 2×3, opisy kursywą pod wartościami zamienione na ikonę (i) z tooltipem. **To NIE jest podjęta decyzja** — Twoje słowa brzmiały „Nasza opcja byłaby lepsza, ale musiałbym widzieć to w screenshocie". Traktuj tę falę jako ten screenshot: **obejrzyj i powiedz, czy zostaje, czy wycofujemy** (wycofanie = rewert części `newGameFlow.ts`/`hudTitleTooltip.ts` z `ea0be32a`, bez ruszania naprawy Auto-pracy z tego samego commitu). ⚠️ **Nie jest to zgłaszane jako defekt bundla — bundle jest zdrowy;** zgłaszane jest jako **brak Twojej zgody na zmianę, która i tak trafiła do gry**.
|- **Siedem tematów (nazwane wprost):**
|  1. **Panel imperium — portret władcy w nagłówku zamiast emoji cywilizacji** (`P-PANEL-IMPERIUM-PORTRET-WLADCA`; `33eeec11`; `gra/src/ui/empireDetailPanel.ts`, `ui/empireDetailTypes.ts`, `main.ts`). `civEmoji` w `buildEmpireDetailSnap()` było zahardkodowane na **grecką świątynię 🏛️ niezależnie od wybranej cywilizacji**. Reużyte istniejące `leaderPortraitUrl()` (już wpięte w HUD/dyplomację/preBattle) jako główna ikona nagłówka, z fallbackiem do `civEmoji` gdy portret nie istnieje. Jeden wspólny nagłówek `.civ-emp-hdr` obsługuje **wszystkie 11 zakładek** panelu, więc jedna naprawa działa naraz na wszystkich. ✅ **Zweryfikowane przez agenta deployu NA ZBUDOWANYM BUNDLU w headless Chromium, nie z kodu:** po starcie gry i otwarciu panelu `.civ-emp-hdr-portrait` istnieje, `src` to realny **JPEG data-URI o `naturalWidth = 256 px`**, tekst ikony jest **pusty** (żadnego emoji), nazwa cywilizacji „Grecy". Bramki: `empire-panel-split-test` **25/0**, `empire-nauka/praca/religia-coverage` 15/15 ×3, `empire-skarbiec-coverage` 12/12.
|  2. **ESCAPE — 4 panele dopięte do istniejącego stosu overlayów** (`P-MENU-ESCAPE-NIEPELNOEKRANOWE`, zgłoszenie z 2026-07-26 nigdy niezarejestrowane; `4fc770ee`; `gra/src/ui/empireDetailPanel.ts`, `ui/empireOverlayHud.ts`, `ui/gamePauseMenu.ts`, `ui/powerOverlayHud.ts`). Mechanizm priorytetu ESCAPE (zamknij zakładkę → dopiero wyjdź z pełnego ekranu) **już istniał** (`escapeOverlayStack.ts`, `R-ESC-PELNY-EKRAN-Q1=A`, Keyboard Lock API) i działał dla ~20 innych overlayów — bug to nie zła kolejność priorytetów, tylko **brak wpięcia w stos dla 4 paneli**, z których panel imperium miał **ZERO obsługi ESCAPE**. Świadomie NIE dotknięte (blokery decyzyjne, nie modale): `diplomacyPendingHud` i powiadomienia toast. ✅ **Zweryfikowane na bundlu w headless Chromium:** panel imperium otwarty → ESCAPE → klasa `.open` schodzi na `false`, dwiema niezależnymi drogami (syntetyczny `keydown` na `document` w fazie capture ORAZ prawdziwy klawisz przy fokusie w panelu). ℹ️ **Nota metodyczna dla przyszłych weryfikacji:** panel chowa się **transformem CSS, nie `display:none`** — `getBoundingClientRect()` zostaje niezerowy po zamknięciu, więc mierzenie „widoczności" prostokątem daje FAŁSZYWY negatyw; wiążąca jest klasa `.open`. Bramki: `escape-overlay-stack-test` **84/0** (było 72/0, +4 nowe ID), nowy `escape-fullscreen-priority-test` **10/0** (mock Keyboard Lock API, odtwarza scenariusz ze zgłoszenia).
|  3. **Oblężenie — flaga znika od razu po wygranej bitwie, nie dopiero po ruchu jednostki** (`P-OBLEZENIE-IKONA-NIE-ZNIKA-PO-WYGRANEJ`; `7b57eb45`; `gra/src/main.ts` `+21`, nowy `oblezenie-siege-lifted-po-bitwie-test.cjs` 316 linii). Twoje zgłoszenie: „po wygranej bitwie nadal widnieje symbol oblężenia, znika dopiero jak się wejdzie z powrotem jednostką do miasta". Przyczyna zdiagnozowana dokładnie: `city.oblegane` jest **stanowe, nie liczone na żywo**, a `validateActiveSieges()` było wołane **wyłącznie z 3 miejsc** — dwa to zakończenie ruchu jednostki, trzecie to skan po turze AI (i to warunkowo, pomijany gdy trwa bitwa oczekująca). **Żadne z nich nie było wołane z rozstrzygnięcia BITWY**, więc flaga wisiała aż do najbliższego RUCHU dowolnej jednostki — obserwacja „znika po wejściu do miasta" to nie wejście do miasta naprawiało stan, tylko fakt, że to był ruch. Bramka: `oblezenie-siege-lifted-po-bitwie-test` **16/0**.
|  4. **Tooltip heksu — filtr nakładki/złoża dla ulepszeń terenu** (`P-HEX-TOOLTIP-MOZLIWE-ULEPSZENIA-BRAK-FILTRA-ZLOZA`; fix w `ea0be32a` (`hexContextTooltip.ts` `+44/−5`, `map/improvement-build.ts` `+3/−1`), test osobno w `94977a20`). Tooltip pokazywał ulepszenia na podstawie **samego terenu bazowego** (`galleryTerrainEligible`), ignorując że wiele z nich wymaga też konkretnej nakładki/złoża NA TYM heksie — stąd Tartak/Glinianka/kopalnie obiecywane na gołej Łące „bez złoża". Warunki są lustrzanym odbiciem (wyłącznie części hex-property, bez terytorium/tech) `createQualifier()`/`qualifies()` z `map/improvement-build.ts`, przez **reużycie eksportowanych funkcji**, nie ręczne przepisanie. Zmiana w `gra/src/map/**` to **wyłącznie `function` → `export function` + komentarz, zero zmiany zachowania** (sprawdzone diffem, nie założone). Bramka: nowy `hex-tooltip-mozliwe-ulepszenia-zloze-test` **36/0**, w tym **własna mutacja kontrolna w teście** (sekcja [3]: cofnięcie warunku odtwarza stary bug — dowód, że asercja ma zęby, a nie jest martwym kodem).
|  5. **Auto-praca — polityka imperium TWARDYM pułapem sumy mimo override per-miasto + naprawa 3 not Evaluatora** (`R-AUTO-PRACA-BUDZET-PROCENT-Q3=B`; `73ba2cd2`, `ea0be32a`; `gra/src/game/auto-improvements.ts`, `main.ts`). Domyka follow-up ABC zapowiedziany w FALI 278: przed naprawą miasto z override 80% przy polityce imperium 20% zabierało **80% CAŁEJ puli imperium**, zostawiając pozostałym miastom ZERO. **Evaluator (Opus 5) potwierdził szczelność niezależnym fuzzem 200 losowych układów: HEAD 0 naruszeń pułapu, commit-rodzic 123/200 naruszeń** — dowód przed-po, nie deklaracja. Edge case wielu override'ów naraz zmierzony wprost (S2≡S3 co do bajta: kolejność ustala wyłącznie `id`). 6 mutacji kontrolnych: 3 złapane, 2 równoważne, 1 (M6, znany dług testowy) nieprzechwycona. Runda naprawcza `ea0be32a` zamknęła 3 noty: **N1** — komentarze w kodzie KŁAMAŁY („override decyduje o pierwszeństwie/udziale" — nieprawda dla override POWYŻEJ polityki, obalone przez S2≡S3); **N2** — jedyne miejsce wiążące politykę gracza z silnikiem (`pracaBudgetPercent: empirePol.pracaAutoPercent` w `main.ts`) **nie miało ŻADNEGO testu, cichy revert przechodził tsc + wszystkie 5 bramek** → nowy strażnik tekstowy, mutacja kontrolna Operatora potwierdziła że go łapie; **N3** — ujednolicone ID. Bramka: `auto-improvements-test` **37/0** (było 29/0 w FALI 278 → 35/0 po `73ba2cd2` → 37/0 po N2). ⚠️ **Nota N4 do Twojej wiedzy, nie blokuje:** panel nadal pokazuje per-miasto „X% Pracy", które po tej naprawie bywa **nieosiągalne** (miasto z override 80% przy polityce 20% realnie dostaje 20%, panel dalej obiecuje 80%) — do Twojej decyzji przy playteście, czy UI ma to inaczej komunikować.
|  6. **Panel imperium — chip „Rekruci" trafia do bloku Armii** (`P-ZASOBY-IMPERIUM-REKRUCI-STARY-WIDOK`; `f536b792`; `gra/src/ui/empirePanelSectionMap.ts` `+11`). Recon rozdzielił zgłoszenie na dwie rzeczy: część to backlog Fazy 3+ (oczekiwane, nie bug), ale **routing chipa `econ-rekruci` był realnym bugiem** — nie mapował się na przerobiony blok Armii. Bramka: `empire-panel-split-test` **25/0** (było 22/0, +3 asercje routingu).
|  7. **Kreator nowej gry krok 4 — wyrównanie ramek sekcji „Ustawienia Rozgrywki"** (`P-NEWGAME-RAMKI-WYROWNANIE`; `091f833b`; `gra/src/ui/newGameFlow.ts`, **dokładnie `+3/−3`**). Regresja wprowadzona wcześniej commitem `ebb14136`. ⚠️ **Uwaga:** ten sam plik dostał w tej fali osobno `+129/−20` niezatwierdzonego prototypu (patrz ostrzeżenie wyżej) — jeśli będziemy wycofywać prototyp, **ta poprawka ma zostać**.
|- ➕ **Ósma pozycja, bez kodu produkcyjnego: strażnik regresji opóźnienia startu muzyki menu** (`e0b20c56`, nowy `menu-music-delay-test.cjs`, **9/0**) wraz ze **sprostowaniem w rejestrze** (`57046c7f`): zgłoszenie `P-MENU-START-MUZYKI-OPOZNIENIE` okazało się **już naprawione** wcześniej (07-26/07-27/08-05), a nie otwartym bugiem — dołożono wyłącznie brakujący strażnik, żeby naprawa nie zniknęła po cichu.
|- **Bramki na HEAD `5dc45b6e` (uruchomione przez agenta deployu na tym właśnie czubku, przed buildem — nie odczytane z raportów Operatora ani z pamięci):** `tsc --noEmit` **0 błędów, exit 0** (TypeScript **5.9.3**, wersja zweryfikowana przed zaufaniem wynikowi, `C-029`) · `logic-test` **213/213, exit 0** (punkt odniesienia `R-BRAMKA-MINDIST-Q1 = A`) · `tech-tree-test` **19/0** · `research-test` **33/33, ALL GREEN** · `unit-replace-test` **13/13** · `ai-founding-territory-test` **28/0** · `combat-test` **6/6** · `escape-overlay-stack-test` **84/0** · `escape-fullscreen-priority-test` **10/0** (nowa) · `hex-tooltip-mozliwe-ulepszenia-zloze-test` **36/0** (nowa) · `menu-music-delay-test` **9/0** (nowa) · `oblezenie-siege-lifted-po-bitwie-test` **16/0** (nowa) · `auto-improvements-test` **37/0** · `empire-panel-split-test` **25/0** · `vite build` **821 modułów, exit 0** · `verify-robocza-bundle` **VERIFY OK** (`manifest match: OK`). **Wszystkie 13 bramek testowych exit 0**; wszystkie trzy wzrosty względem FALI 278 rozliczone przy tematach (`auto-improvements` 29→37, `empire-panel-split` 22→25, `escape-overlay-stack` 72→84 — nowe asercje, nie zmiana progu). Dodatkowo zweryfikowane **własnym uruchomieniem, nie przepisane z opisów commitów** (bo jeden z tych opisów okazał się nieprawdziwy — patrz punkt niżej): `empire-nauka-panel-coverage-test` **15/15**, `empire-praca-panel-coverage-test` **15/15**, `empire-religia-panel-coverage-test` **15/15**, `empire-skarbiec-panel-coverage-test` **12/12**, `empire-panel-sliders-always-visible-test` **8/0**.
|- 🔴 **NOWA CZERWONA BRAMKA WNIESIONA PRZEZ TĘ FALĘ — `empire-panel-econ-slider-visibility-test` 59 pass / 1 fail, exit 1** (było **60/0, exit 0**). **To NIE jest dług pre-istniejący i NIE wolno go tak liczyć.** Przyczyna ustalona **bisekcją przyczynową, nie z opisu commita**: na commicie-rodzicu `4f208a0f` (osobny worktree, `node_modules` przez symlink) bramka jest **60/0 exit 0 — ZIELONA**, na HEAD jest **59/1 exit 1**; czerwoność wnosi **`f536b792`** (routing chipa Rekruci do bloku Armii). Padająca asercja to dosłownie `FAIL chip "rekruci" -> blok ekonomia` — czyli test **asercjonuje STARE zachowanie**, które ten commit celowo zmienił na życzenie właściciela. **Dług testowy, nie regresja silnika: gra działa tak, jak chciałeś** (chip Rekruci otwiera blok Armii), po prostu jedna z DWÓCH bramek trzymających tę asercję nie została zaktualizowana. Operator poprawił bliźniaczą asercję w `empire-panel-split-test.cjs` (22→25) i **przeoczył jej duplikat** w `empire-panel-econ-slider-visibility-test.cjs`; ani opis commita, ani Evaluator tego tematu (werdykt PASS, `abb97a90`) tego nie złapały. ⛔ **Powiązane sprostowanie liczby: `empire-panel-econ-slider-visibility-test` = 60/0 jest FAŁSZEM w trzech miejscach** — w opisach commitów `33eeec11` i `4fc770ee` oraz **we wpisie FALI 278 wyżej**; niezależnie stwierdził to Evaluator ESCAPE (`7d9f34f0`, „zmierzone dwukrotnie"), a agent deployu potwierdził własnym uruchomieniem. **Do naprawy osobnym zleceniem w pętli AutoBot** (aktualizacja asercji w drugim teście) — deploy jej nie robi, bo to zmiana kodu, nie publikacja.
|- **Znane pre-istniejące czerwone bramki — sprawdzone, że liczby się NIE pogorszyły** (kontrola, czy fala nie wniosła regresji pod przykrywką znanego długu): `unit-power-test` **4 pass / 2 fail** · `prereq-budynkow-test` **51 pass / 8 fail** · `mennica-magazyn-test` **38 pass / 3 fail** · `empire-food-b5-test` **25 pass / 3 fail** · `budynek-civ-bonus-u17-test` **3 pass / 3 fail** · `trade-routes-income-test` **52 pass / 1 fail** · `growthmult-compound-test` **17 pass / 7 fail** · `population-growth-v85-test` **48 pass / 2 fail** · `spichlerz-widocznosc-test` **37 pass / 8 fail** · `spichlerz-wzrost-test` **2 pass / 7 fail** · `food-hodowla-test` **20 OK / 4 FAIL** · `ai-test` **285 pass / 8 fail** · `empire-panel-moc-scroll-preserve-test` **38 pass / 9 fail** — **wszystkie co do jednej dokładnie na punkcie odniesienia z `CLAUDE.md` §BRAMKI i z wpisu FALI 278, zero wzrostu**, żadna nowa porażka się w nich nie schowała. (`empire-panel-moc-scroll-preserve-test` **nadal nie figuruje na liście w `CLAUDE.md` §BRAMKI**, mimo że zgłaszano to już trzykrotnie — zmiana kanonu wymaga własnej pętli Operator→Evaluator, deploy jej nie robi.)
|- ⏱️ **`map-gen-regression-test` NIEUKOŃCZONY — nieblokujący, ten sam znany artefakt sandboksa co w FALI 278** (`P-SANDBOX-MAPGEN-WYDAJNOSC-LIMITY`). Zmierzone w tym przebiegu: mapa mała (108×74) **58,23 s**, standardowa (168×120) **144,34 s** (z czego rzeki `riversMain` 72,1 s + `riversFill` 65,3 s), duża nieukończona w budżecie — generator jest w tym środowisku wielokrotnie wolniejszy niż limity czasowe zaszyte w teście. ✅ **Ryzyko w TEJ fali znikome i SPRAWDZONE, nie założone:** jedyny plik pod `gra/src/map/**` w całej fali to **`improvement-build.ts`**, a jego cały diff to **`function hasAnimalDeposit` → `export function hasAnimalDeposit` plus komentarz PL/EN — zero zmiany zachowania**; pełna lista konsumentów tego modułu (`grep` po całym `gra/src`) to `game/ai.ts`, `game/auto-improvements.ts`, `game/inca-llama-seed.ts`, `game/resource-access.ts`, `game/terrain-improvements.ts`, `main.ts`, `ui/hexContextTooltip.ts`, `ui/improvementBuildConfirm.ts` — **żaden plik generatora mapy na tej liście nie występuje**. Łańcuch generowania mapy jest przez tę falę nietknięty.
|- 🖥️ **Weryfikacja headless na ZBUDOWANYM BUNDLU (Chromium 1194, `file://`, viewport 1600×950) — nie na kodzie źródłowym:** bundle wczytuje się w całości, menu główne renderuje się poprawnie („THE GAME · WERSJA 0.9 · ROZPOCZNIJ GRĘ / KONTYNUUJ / KAMPANIA / MULTIPLAYER / USTAWIENIA"), start gry dochodzi do żywego HUD-u (Tura 1 · 4000 p.n.e., chipy Skarbiec/Praca/Nauka/Armia/Miasta/Religia, mapa na canvasie). **Zero błędów konsoli (`console.error` + `pageerror`) w KAŻDYM z czterech niezależnych przebiegów** — jedyne ostrzeżenia to `GPU stall due to ReadPixels` (SwiftShader, brak GPU w sandboksie) i `[EndTurn] blocked: awaitingFirstPlayerCity` (poprawne zachowanie: nie da się skończyć tury przed założeniem pierwszego miasta). Dodatkowo smoke-test dwóch napraw tej fali potwierdzony w działającej grze — patrz tematy 1 i 2 wyżej.
|- **Deploy z sesji CHMUROWEJ** (Linux, stamp przez port node'owy `inject-build-stamp.cjs ROBOCZA` — brak PowerShell), bezpośrednio na drzewie głównym (deploy = publikacja stanu już scalonego, nie nowa praca deweloperska). Build wyłącznie przez `vite.js`, **bez `npm run build`**. 6 bundli playtestowych zsynchronizowanych, `START.html` + manifest (10 bundli) przegenerowane.
|- 🔴 **Playtest — na co patrzeć:** (a) **panel imperium** — nagłówek ma pokazywać **portret Twojego władcy**, nie grecką świątynię 🏛️, i to na **wszystkich 11 zakładkach**; (b) **ESCAPE** — w **panelu imperium**, **menu pauzy**, **Kulturze/Religii** i **Mocy** klawisz ESC ma **zamknąć panel**, a NIE wyrzucić Cię z pełnego ekranu; drugi ESC (przy pustym stosie) dopiero wychodzi z pełnego ekranu; (c) **oblężenie** — po wygranej bitwie z armią oblegającą ikona oblężenia ma zniknąć **natychmiast**, bez ruszania jakąkolwiek jednostką; (d) **tooltip heksu** — kliknięty heks ma wymieniać tylko te ulepszenia, które **naprawdę da się tam postawić**: gołą Łąkę bez lasu/złoża przestaje kusić Tartakiem/Glinianką/kopalniami; (e) **auto-praca** — przy **override % dla pojedynczego miasta** polityka imperium ma być **twardym pułapem SUMY**: miasto z override 80% przy polityce 20% NIE zabiera już całej puli, pozostałym miastom coś zostaje (⚠️ panel per-miasto może nadal pokazywać nieosiągalne „80% Pracy" — nota N4, znane); (f) **chip Rekruci** w panelu imperium ma otwierać **blok Armii**, nie stary widok; (g) ⛔ **krok 4 kreatora nowej gry — OBEJRZYJ I ZAWYROKUJ:** Cywilizacje przeciwnika po prawej obok siatki 2×3, opisy kursywą zamienione na ikonę (i) z tooltipem — **to jest niezatwierdzony prototyp wariantu A, który wjechał do bundla przypadkiem** (patrz ostrzeżenie na górze wpisu); powiedz „zostaje" albo „wycofaj", to nie jest jeszcze Twoja decyzja podjęta.
|- **Scalenie do `main`: FALA 278 scalona osobno zaraz po tym deployu** (rytm „jedna fala do tyłu", `R-MERGE-MAIN-RYTM-Q1`) — scalenie celuje w **commit deployu FALI 278 `32e68c9c`**, nie w czubek gałęzi. **FALA 279 zostaje na gałęzi roboczej wyłącznie do testów** i kwalifikuje się do scalenia dopiero wtedy, gdy powstanie FALA 280.

## ROBOCZA 8455b385 - 2026-08-14 01:10 UTC (14.08.2026 03:10 PL) - FALA 278: barbarzyńcy z własną skalą niezależną od poziomu trudności, fort/strażnica ZAMKNIĘTY po 4 rundach, panel imperium 11 zakładek Faza 1+2 (Skarbiec/Praca/Nauka/Religia) wraz z naprawą CSS suwaków, nowy system dróg 6-ramiennych łączących się z każdym sąsiadem, suwak 0-100% budżetu Pracy zamiast przycisków 1/2/3, MP/AI faktycznie zbierają chatki ze skarbami (koniec oscylacji z FALI 276/277), domknięcie martwych stref pokrycia dyplomacji - **ZASTĄPIONA** (→ `4bef7d10`, FALA 279)
