...
|- ⚠️ **`map-gen-regression-test.cjs` uruchomiony bezpośrednio w drzewie głównym po deployu, jeszcze NIE dokończony w chwili tego wpisu** (długi przebieg, >95 min CPU na tej maszynie, historyczny punkt odniesienia ok. 35 min) — świadoma decyzja właściciela nie czekać na jego zakończenie przed deployem, biorąc pod uwagę silne pokrycie innymi bramkami (zwłaszcza `fair-play-grid-test` i `deposit-coast-test`, które faktycznie testują rozmieszczenie złóż) oraz niskie ryzyko zmiany (pojedyncza liczba w JSON, bez zmiany logiki generatora). Wynik zostanie dopisany osobnym wpisem w kanale, jeśli ujawni cokolwiek nieoczekiwanego.
