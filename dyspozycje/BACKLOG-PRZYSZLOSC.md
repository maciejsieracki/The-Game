...
| **Redukcja korupcji do zera w późniejszych epokach** | Maciej 2026-07-25: „W kolejnych epokach pojawiają się kolejne budynki, które będą zmniejszać korupcję **praktycznie do zera**." Dziś korupcję redukują trzy budynki po 30 punktów procentowych każdy (Sąd, Pretorium, Pałac), addytywnie. **UWAGA DLA PRZYSZŁEJ SESJI:** w kodzie stoi jawny `Math.min(0.60, suma)` — realne maksimum to 60 punktów procentowych, bo Pałac stoi tylko w stolicy, a Pretorium tylko w regionie, więc żadne miasto nie ma obu. Dokładanie kolejnych budynków redukujących **wymaga podniesienia albo zdjęcia tego sufitu** — inaczej nowe budynki będą martwe. To nie jest błąd, to świadome zabezpieczenie z dzisiejszej sesji. | Kolejne epoki |
| **Systemy polityczne** | Maciej 2026-07-25: „Czeka nas jeszcze wprowadzenie **różnych systemów politycznych**, które będą mieć też wpływ na wszystko. Ale tym zajmiemy się **na samym końcu, jak gra będzie grywalna**." Wpływ ma być szeroki („na wszystko"), więc przy projektowaniu spodziewać się mnożników na Daninę/Podatek, korupcję, Prawo, Szczęście, Naukę i prawdopodobnie wojsko. **Nie projektujemy tego na zapas i nie zostawiamy pod to haków** — zgodnie z zasadą projektu. | Sygnał Macieja PO osiągnięciu grywalności |
| **Młyn** | Maciej 2026-07-25: „Młyn wprowadzimy w antyku, czyli w epoce klasycznej." Budynku **nie ma dziś w `buildings.json`**, ale silnik ma już gotowe, nieużywane wpięcie: flaga `ctx.maMlyn` i parametr `budynek_mlyn_bonus_pracy` (2) w `econ-params.json` — flaga nigdy nie jest ustawiana na `true`, bo nie ma budynku, który by ją zapalił. Przy tworzeniu epoki klasycznej wystarczy dodać wpis budynku (id `mlyn`, grupa „Produkcja surowców", koszt wg zasady epok) i sprawdzić, czy istniejące wpięcie procentu do Pracy nadal działa zgodnie z zamysłem. | Epoka klasyczna |
...
| 10 | **Żywność w mieście po osiągnięciu limitu populacji (np. 8) z `autoWyzywienie=true` dającym +12% wzrostu — dokąd idzie nadwyżka?** Maciej (2026-09-25): skoro miasto ma limit i nie może dalej rosnąć, a autowyżywienie nadal generuje wysoki wzrost procentowy, trzeba sprawdzić, czy nadwyżka żywności/wzrostu jest tracona w próżnię, czy poprawnie wraca do spichlerza. Oczekiwane zachowanie: albo odkładanie nadwyżki z powrotem do spichlerza, albo — po osiągnięciu limitu — autowyżywienie powinno utrzymywać zerowy wzrost (nie marnować puli), dopóki nowy budynek (np. Akwedukt) nie odblokuje wyższego limitu, wtedy wznowić maksymalizację wzrostu. Punkt startowy do zbadania: `gra/src/game/economy.ts` (`buildingCap`/`resolvedCap`, linia ~1222-1230, już zweryfikowany w tej sesji jako konsument `lud_limit_populacji`) + `gra/src/game/population-growth-v85.ts` (ścieżka wzrostu/klamrowania przy capie) + `gra/src/game/empire-city-defaults.ts:508-517` (`autoWyzywienie` flaga). Nie zbadane głębiej w tej sesji — tylko zlokalizowane pliki startowe. | Sygnał Macieja |
| 11 | **Autonomiczna kolejka budowy: budynki standardowe → ulepszenia nie działa dla ulepszeń.** Ustalona zasada: najpierw buduj standardowe budynki z puli; gdy pula się wyczerpie, przechodź do ulepszeń; nowy odblokowany budynek (nie ulepszenie) ma pierwszeństwo i wraca na start kolejki, ulepszenia wchodzą tylko gdy kolejka budynków jest pusta. Maciej (2026-09-25): w praktyce tryb autonomiczny NIE buduje ulepszeń wcale. Punkt startowy do zbadania: `gra/src/game/auto-manage.ts::buildUpgradeCandidates()` (linia ~290, wywoływana z ~374) — sprawdzić, czy kandydaci na ulepszenia w ogóle trafiają do faktycznej kolejki auto-budowy, czy funkcja tylko liczy kandydatów bez ich kolejkowania, oraz czy istnieje warunek blokujący (np. błędne sprawdzenie „czy pula budynków pusta") które nigdy nie jest spełnione. Nie zbadane głębiej w tej sesji — tylko zlokalizowana funkcja startowa. | Sygnał Macieja |
| 12 | **AI wciąż zakłada miasta daleko poza własnym terytorium (10-15 pól od granicy) — „teleportacja", mimo wcześniejszej naprawy.** Maciej (2026-09-25): ustalona zasada — AI może zakładać miasta wyłącznie na tych samych zasadach co gracz, nowe miasto musi przylegać do/leżeć w granicach już posiadanego terytorium tej cywilizacji, zero teleportacji na odległe pola. **WAŻNE:** dokładnie ten problem był już raz zgłoszony i rzekomo naprawiony — `gra/src/game/ai.ts::planCityFounding()` (linia ~2835-2904) ma jawny komentarz odwołujący się do karty `P-AI-ZAKLADANIE-MIAST-BEZ-ZASADY-ODLEGLOSCI` (decyzja Macieja, 2026-08-09): „AI dostaje ten sam twardy wymóg withinTerritory co gracz... nowe miasto musi leżeć w zasięgu terytorium JEDNEGO z już posiadanych miast TEJ SAMEJ cywilizacji" — realizowane przez `findCityFoundingHex()` z parametrem `myCities`/`myFortNodes`. Skoro problem nadal występuje, albo (a) `findCityFoundingHex()` nie egzekwuje tego poprawnie mimo otrzymanych parametrów, albo (b) istnieje INNA, odrębna ścieżka zakładania miast AI, która całkowicie omija `planCityFounding()` — najbardziej prawdopodobny kandydat: `gra/src/game/cluster-start.ts` (rozstawienie startowego klastra cywilizacji na początku gry, osobny mechanizm od kolonizacji w trakcie rozgrywki, nie przechodzi przez `planCityFounding`). Nie zbadane głębiej w tej sesji — wymaga ustalenia, czy zgłoszone „odległe miasta" powstają na starcie gry (cluster-start) czy w trakcie rozgrywki (planCityFounding regresja). | Sygnał Macieja |
