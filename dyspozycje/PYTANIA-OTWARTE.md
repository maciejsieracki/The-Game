...
- **R-DYPLOMACJA-HANDEL-BRAMKA-PRIORYTET-Q1, notatka Evaluatora rundy 4 (N3)**: mnożnik chęci handlu (`handelWillingnessMultiplier`) działa też dla par AI↔AI (nie tylko gracz↔AI), bo `responderIsPlayer` jest fałszywe dla obu proponentów-AI. Zweryfikowany sweep (1728 przypadków): 31 różnic względem starego kodu, wszystkie w stronę ZAOSTRZENIA (odrzucenie zamiast akceptacji) przy niskiej chęci respondenta dokładnie na parytecie. Kierunek bezpieczny (nie otwiera exploita), spójny z Q1=B+C, ale nieopisany w oryginalnym zleceniu i niepokryty żadnym testem.
