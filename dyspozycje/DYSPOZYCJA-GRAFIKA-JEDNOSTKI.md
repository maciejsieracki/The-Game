...
KOREKTA MACIEJA 2026-07-10 (wykonana w modelach): **Thorakites = WŁÓCZNIK** (dory 0.70·HEX_R nadręcznie zza thureos — model poprawiony w z2); **Evocati = MIECZNIK** (zweryfikowany: gladius, bez zmian). **ZMIANA DANYCH (decyzja Macieja, wykonuje Code):** `gra/data/units.json` Thorakites `Typ: Swordsman → Spearman` + korekta pola `Uwagi` („tarcza + włócznia") — wpływa na countery! Po zmianie: sync Panel-C (reguła §8, bramka round-trip diff=0).
