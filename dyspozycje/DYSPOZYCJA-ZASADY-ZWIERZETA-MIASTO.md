...
1b. **TRZODA (decyzja Macieja 2026-07-09):** zasób „bydło" staje się **„Trzoda"** (livestock; symbol = krowa+świnia). (a) Rename etykiet: `render/improvements.ts` label 'Bydło'→'Trzoda', UI preview (`cityPanel.ts:1988`), teksty w danych; klucz techniczny `bydlo` może zostać. (b) Grafika: registry `bydlo` → `buildTrzoda()` z `_sandbox/MASTER/render-zwierzeta/swinia-trzoda.ts` (krowa+świnia N-NE, jakość: WYSOKA=2 szt., NISKA/NORM=krowa; instrukcja w nagłówku pliku). (c) **WSZYSTKIE reguły warunkowane bydłem/wołem przechodzą na „dostęp do trzody"** — grep po kodzie: `bydlo`, `BydloWol`/`bydlo_wol`, `wol` w wymaganiach jednostek/budynków (np. RYDWAN oparty o woła → wymaga dostępu do trzody); wypisz w meldunku pełną listę podmienionych warunków. UWAGA: `BydloWol` (wół pociągowy) jako BYT zostaje — zmienia się tylko WARUNEK dostępu.
