...
### PKT 2 — SPEC JEDNOSTEK Celtowie/Germanie (INPUT dla Civ-UNITS; ja NIE buduję jednostek)
Pełne staty (Atak/Obrona/Health/Morale/Zasięg) + modele = lane Civ-UNITS (Jednostki.xlsx). Poniżej propozycja ról/nazw.
...
To dane jednostek → **Jednostki.xlsx = lane Civ-UNITS** (nie civs.json). Ja podaję propozycję jako INPUT.
Dotyczy WSZYSTKICH 9 nacji — wg §6a super-jednostki 6 z 7 typów są wciąż „do zaproponowania" (tylko Inkowie = Królewska Gwardia) → robota Civ-UNITS.

### Propozycja PER EPOKA — Celtowie i Germanie (INPUT dla Civ-UNITS; staty/modele/epoka = UNITS)
...
7. Per-epokowe zamienniki dla 7 istniejących typów?
   A) Robi Civ-UNITS (Jednostki.xlsx) — ja tylko relay zasady.
...
**7B:** rozpisuję propozycje per-epoka dla 7 typów (INPUT dla Civ-UNITS).
...
## [2026-06-22 21:52] DANE — korekta zakresu + paczka dla Civ-UNITS + mapa sesji
**Korekta zakresu (Maciej):** DANE = dane cywilizacji + mechanizmy + **historyczny kierunek**; NIE projektuję
konkretnych jednostek (to robi Civ-UNITS). Paczkę dla UNITS przerobiłem na „kierunek", nie detale.
**Przygotowałem plik:** `PACZKA-DLA-UNITS-od-DANE.md` (w folderze Civ) — gotowy do wklejenia do tasku Civ-UNITS:
...
Re-pointy do innych sesji i zakładanie Civ-LOGIKA/Civ-UI = akcje Mastera (nie tworzę sesji).
...
## [2026-06-22 23:15] DANE — braki jednostek skonsolidowane → przekazane Civ-UNITS
...
Paczka też odzwierciedla 8C (sekcja 5). Plik gotowy do wklejenia do tasku Civ-UNITS.
...
(opc. 2, gdzie zasługuje). To PROPOZYCJA do dyskusji; po akceptacji spakuję do Civ-UNITS (oni robią staty atak/obrona).
...
Po Twoich uwagach → aktualizuję `PACZKA-DLA-UNITS-od-DANE.md` (sekcja Żelazo) i dopiero wtedy idzie do Civ-UNITS.
...
- Kolory: żółty = PROPOZYCJA, szary = opc. 2., zielony = istniejąca super. Staty (Atak/Obrona…) = Civ-UNITS po akceptacji.
- Plik = do analizy/dyskusji; po Twoich uwagach spinam finalną listę Żelaza do Civ-UNITS.
...
**Moja działka (DANE):**
1. Jednostki Żelaza — czekają na analizę `Jednostki-specjalne-przeglad.xlsx`; po decyzji → finalizacja + przekazanie Civ-UNITS.
2. **Religie Celtów i Germanów — BRAK bonusów mechanicznych** w „Religie cywilizacji" (Spoleczenstwo-parametry: tylko 7 typów). civs.json ma same nazwy. Do dopisania 2 (uzgodnić z Civ-SILNIK). ← najkonkretniejszy następny krok.
3. (opc.) Kwantyfikacja bonusów/minusów cyw. (teraz opisowe) — jeśli logika ma je liczyć.

**Do zgłoszenia innym lane'om:**
4. Civ-UNITS: zbudować jednostki Celtów+Germanów (Brąz+super, Żelazo) + jednostki Żelaza po decyzji — paczka gotowa.
5. Civ-SILNIK: dodać do `CivDef` pola `Religia` i `Typ główny` przy wpinaniu civs.json.
...
1. **Jednostki Żelaza** — założenie: 1 wyjątkowa/cyw.; 2. PRZYJĘTA dla Chiny/Egipt/Sumer; Inkowie = elita brąz/miedź (champi, bez kucia żelaza). → **ZROBIONE:** finał w `PACZKA-DLA-UNITS-od-DANE.md` (sekcja B) + `Jednostki-specjalne-przeglad.xlsx`. Build jednostek = Civ-UNITS.
...
4. **Civ-UNITS (budowa jednostek)** — nie moja działka; paczka gotowa, czeka na wklejenie.
5. **Civ-SILNIK (CivDef: `Religia`, `Typ główny`)** — nie moja działka; zgłoszone.
...
8. **export-data.py (zaszyta ścieżka)** — nie moja działka (kod); zostawiam Civ-SILNIK.
...
- **FLAGA do mastera/MAPA:** `DESIGN-cywilizacje-spawn.md` — wspólny (roster+spawn; spawn=MAPA), CZĘŚCIOWO NIEAKTUALNY (mówi „7 typów, rozważane +2"; finalnie **9**). Część „roster" jest pełniej i aktualnie w mojej DOKUMENTACJI. Decyzja czy zostaje (spawn) czy do archiwum — master/Civ-MAPA (nie ruszam cudzego pliku).
