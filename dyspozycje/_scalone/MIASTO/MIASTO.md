...
ZASADY: tylko swoje pliki. Tylko Grupa F rusza main.ts + publikuje kanon. Build do testu: `npx vite build --outDir /tmp/civ-dist` (zwykly pada na blokadzie OneDrive dist/) -> cp do celu. NIGDY npm run build / export-data.py.
...
(Wpiecie w petle tury + panel UI: Grupa F + Grupa A. Styk z Grupa B: produkcja uzywa ekonomii -> handoff.)
