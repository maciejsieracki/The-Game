...
| **3 — twarde progi traktatów** | **WDROŻONE (2026-08-03, D3).** Stałe `wiarygodnoscProgSojuszMin: 0` i `wiarygodnoscProgNapMin: -40` w `DIPLOMACY_PARAMS` (`game/diplomacy.ts`). `ProposalEvalContext` ma pola `proposerWiarygodnosc`/`responderWiarygodnosc`; case `'nap'` i `'sojusz_defensywny'`/`'sojusz_pelny'` w `evaluateProposal` (`game/diplomacy-proposals.ts`) sprawdzają W proponenta **przed** progami Zaufania/Relacji/Respektu. `main.ts` `buildProposalEvalContext` podaje `getWiarygodnosc(proposerId/responderId)`. Testy: `wiarygodnosc-test.cjs` §8d. | Tak — twarda bramka binarna przy propozycji |
