'use strict';
/**
 * zrzut-mapy-runda-2.cjs — R-MAPA-ETYKIETA-STOLICY-NAZWA-MIASTA-Q1, runda 2, ZYWY dowod w Chromium.
 *
 * ROZNICA WOBEC RUNDY 1: ten sam scenariusz, ale dowodzi stanu po R2-1 (plakietka = sama
 * nazwa miasta) i po R2-2 (stolica AI bierze nazwe z miasta_cywilizacji[0], wiec Chinczycy
 * maja "Xi'an", nie "Qin"). Zrzut rundy 1 pokazywal "QIN · CHINCZ…" — porownanie jest sensem
 * tego dowodu. Skrypt wypisuje tez nazwy WSZYSTKICH obcych stolic w partii.
 *
 * PO CO: plakietka miasta na mapie to sprite THREE.js z tekstura z canvasu, nie DOM —
 * zadna asercja tekstowa nie pokaze, czy dwuczlonowa etykieta („Uruk · Sumerowie")
 * rozjezdza plakietke albo zachodzi na sasiednie heksy. Jedynym dowodem jest zrzut.
 *
...
(async () => {
  ...
  await page.screenshot({ path: path.join(HERE, 'mapa-obca-stolica-runda2-1600x1000.png') });
  await page.screenshot({
    path: path.join(HERE, 'mapa-obca-stolica-runda2-zblizenie.png'),
    clip: { x: 500, y: 250, width: 700, height: 500 },
  });
  console.log('Zrzuty zapisane w ' + HERE);
