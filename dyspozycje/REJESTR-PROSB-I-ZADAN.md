...
| `R-HANDEL-SZLAKI-PRZEBUDOWA-Q1` | `ABC-OCZEKUJE (5 pytań)` | Zlecenie właściciela (2026-08-21): przebudowa mechaniki szlaków handlowych — odwrócić zależność od dystansu (dalej=więcej), ×2 za połączenie morskie, ×5 stawki dystansowe, przypisać 5%-bonus do konkretnej trasy, limit „jedna umowa handlowa na cywilizację", dwuetapowa aktywacja (umowa+łączność=dochód od razu bez budynków; budynki=dodatkowo 5%), UI pokazujący kwoty per-droga. Pełny recon (dwa niezależne przebiegi) rozbił to na 7 punktów: (3) stawki×5 i (7) UI ROZSTRZYGNIĘTE bez ABC (techniczna konsekwencja pozostałych); (1)(2)(4)(5)(6) wymagają ABC — pełne warianty w `docs/decyzje/R-HANDEL-SZLAKI-PRZEBUDOWA-Q1.md`. Sprostowanie bez ABC: nazwa „Handel"/„Handel ze szlaków" zostaje — właściciel pomylił to z osobnym, już dawno przemianowanym strumieniem Handel→Podatek. Podział na T1-T6 zaproponowany w decyzji. Nie rozpoczęte — czeka na ECHO. |
