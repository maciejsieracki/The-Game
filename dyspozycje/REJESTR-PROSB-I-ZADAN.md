...
| `P-HANDEL-SZLAKI-WZOR-DUPLIKAT-Q1` | (duplikat wpisu wyżej — patrz `ROZWIĄZANE`, linia ok. 63) | — |
...
| `R-PODBOJ-ELIMINACJA-PULA-PRACY-TRANSFER-Q1` | `ZINTEGROWANE` | Commit `bcb2ac4b` (ręczne scalenie z `R-PODBOJ-SUROWCE-BILANS-BRAK-WIERSZA-Q1`, ten sam interfejs). Pula pracy całej cywilizacji → do zdobywcy przy eliminacji ostatniego miasta (dodane, nie zastąpione); bez eliminacji — bez zmian. Operator→Evaluator→Final Control PASS. Szczegóły wyżej (linia ok. 6849). Nic do dispatchu. |
| `R-PODBOJ-SUROWCE-BILANS-BRAK-WIERSZA-Q1` | `ZINTEGROWANE` | Commit `0283fa60`. Bilans zdobycia miasta pokazuje teraz wiersz "Surowce zdobyte" (transfer już działał po cichu, tylko brakowało UI). Operator→Evaluator→Final Control PASS. Szczegóły wyżej (linia ok. 6837). Nic do dispatchu. |
...
| `R-HOTSEAT-ETAP4-NOOP-HARNESS-Q1` | `ZINTEGROWANE` | Commit `2a99ec69`. Nowa bramka dowodu no-op z recon §6.2 (`gra/tools/hotseat-etap4-noop-test.cjs`, 30-turowa headless symulacja + hash SHA-256 stanu, deterministyczny `Math.random`) — zero zmian w `main.ts`, wyłącznie infrastruktura testowa pod przyszłą, faktyczną rundę rozcięcia `triggerPlayerEndTurn()`. Operator PASS; Evaluator FAIL — 2 zarzuty realne (Run B crashował w 3/3 prób różnych turach, plus zawis zamiast szybkiego BLOCK przy crashu przeglądarki); Obrona PRZYJĘŁA oba, naprawiła (`runOnceWithRetry` + `closeBrowserSafely` z race+SIGKILL fallback). Final Control PASS: 2 niezależne pełne przebiegi 30/30, w tym ręczne przejście przez wszystkie 3 historyczne punkty crasha bez powtórki. Bramka 30/30 (weryfikowana też na zintegrowanym `main`). 5 bramek referencyjnych zielone. Nic do dispatchu z tego tematu — pełne rozcięcie Etapu 4 (kolejny krok planu hot-seat) wciąż niedispatchowane. |
| `R-HOTSEAT-ETAP4A-WORLD-CODE-REALOKACJA-Q1` | `ZINTEGROWANE` | Commit `3fe8c232`. Pierwszy, najbezpieczniejszy pod-etap rozcięcia `triggerPlayerEndTurn()` (Krok 0+1 z recon §6.1) — wydzielona `runWorldEndTurn()` (fazy 7-14), wołana z tego samego miejsca co dawniej te fazy. Trzy bloki "światowe" osadzone tekstowo w bloku "gracza" (czyszczenie `st.bunt`, `evictForeignUnitsFromCityHexes()`, reset ruchu wszystkich jednostek) przeniesione na początek nowej funkcji — no-op w single-player, zapobiega podwójnemu wykonaniu per-fotel w przyszłym hot-seat (Etap 8). `nextTurnNum` przeliczane lokalnie (nie martwe odwołanie do domknięcia). Operator→Evaluator→Final Control PASS, zero zarzutów blokujących (2 notatki informacyjne ODDALONE przez Final Control). Bramka `hotseat-etap4-noop-test.cjs`: wszystkie 30 hashy bajt-w-bajt identyczne PRZED/PO, zweryfikowane niezależnie 4x (Operator, Evaluator, Final Control pełne 4×30, orkiestrator na zintegrowanym `main`). Krok 2-5 (endActiveHumanTurn/advanceSeat/call-site/bitwy) POZA ZAKRESEM — osobny temat `R-HOTSEAT-ETAP4B-SPLIT-Q1` (zintegrowany, patrz niżej). |
| `R-HOTSEAT-ETAP4B-SPLIT-Q1` | `ZINTEGROWANE — ETAP 4 CAŁEGO PLANU HOT-SEAT KOMPLETNY` | Commit `27c96c23`. Ostatni pod-etap rozcięcia `triggerPlayerEndTurn()` (Krok 2-4 recon) — rename na `endActiveHumanTurn(humanOwnerId)` z ciałem w 100% niezmienionym (mechanicznie zweryfikowane), nowa `advanceSeat()` (dziś: woła `endActiveHumanTurn(HUMAN_OWNER_PRIMARY)`), trzy zewnętrzne call-site (HUD/`__eraTestDebug`/skrót „N") przepięte na `advanceSeat()`. Świadomy wybór architektury (a) zamiast rekomendowanego w dyspozycji (b) — jawnie uzasadniony (rozbicie jednego `try/catch/finally` między dwie funkcje groziło zmianą zachowania na ścieżce błędu, niewykrywalną przez bramkę happy-path) i zaakceptowany przez Final Control po własnej, niezależnej ocenie. Krok 5 (bitwy/zwycięstwo) POZA ZAKRESEM, jawny dług. Operator→Evaluator→Final Control PASS, zero zarzutów. Bramka no-op: 30/30 identycznych hashy, zweryfikowane niezależnie 4× (Operator, Evaluator, Final Control — każdy z osobnym tymczasowym worktree bazowym, plus orkiestrator na zintegrowanym `main`). `end-turn-modal-sequencing-test` 39/40 (jeden pre-istniejący FAIL potwierdzony identyczny na czystej bazie). 5 bramek referencyjnych zielone. Nic do dispatchu z tego tematu. |
| `R-HOTSEAT-ETAP5-SWITCH-HUMAN-Q1` | `ZINTEGROWANE` | Commit `93fd6ffb`. Implementacja Etapu 5 na podstawie zamkniętego recon: `switchActiveHuman(newActiveOwnerId)` (7 kroków: seed trzech map `XByHuman`, zamknięcie 17 paneli + hint toast + build-mode, zaznaczenie/marsze, wyczyszczenie logów zdarzeń, przełączenie fotela, zerowanie cache HUD, kamera, fog/HUD) + `ui/hotSeatHandoff.ts` (zasłona przekazania). Nowy, IZOLOWANY kod (wzorzec Etapu 0) — zero call-site produkcyjnego, będzie wołany dopiero przez `advanceSeat()` w Etapie 8. Runda 1 FAIL (bramka BLOCK + realny wyciek mgły znaleziony przez Evaluatora — fallback `playerStartHex` globalny, nie per-owner), naprawione strażnikiem `ME()===HUMAN_OWNER_PRIMARY` (dowiedzione logicznie jako no-op). Runda 2 przerwana restartem kontenera, praca odzyskana z filesystemu i dokończona w rundzie 2b (5 kolejno odkrytych problemów konfiguracji bramki testowej, nie regresji silnika). **Decyzja autonomiczna orkiestratora, DO POTWIERDZENIA ABC**: KROK 1c odstępuje od invariantu `R-PIERWSZE-MIASTO` w kontekście handoff między fotelami. Operator→Evaluator→Final Control PASS. Bramka `hotseat-etap5-no-leak-test.cjs` oba scenariusze A+B zielone. 5 bramek referencyjnych + `river-fog-visibility-test` zielone. |
| `R-HOTSEAT-ETAP5-RECON-SWITCH-HUMAN-Q1` | `ZAMKNIĘTE (dokument)` | Commit `6a8dd9a6`. Recon-only (zero kodu) dla Etapu 5 planu hot-seat: `switchActiveHuman()` + `ui/hotSeatHandoff.ts`. Kompletna inwentaryzacja źródeł stanu (cache'e `_last*`, logi zdarzeń, `selectedId`/`plannedMarches`/kamera, overlaye mapy, mgła/minimapa), projekt funkcji (7 kroków) + zasłony przekazania + plan dowodu "no leak". Evaluator znalazł 2 realne luki: globalny toast `hintToast` (info-leak) i globalny build-mode/found-city-mode (ryzyko WYŻSZE — możliwe przejęcie niedokończonej akcji budowy przez fotel B). Obie przyjęte i domknięte przez 2 rundy Obrony/poprawek (runda 3 uzupełniła częściową poprawkę build-mode o brakujące `clearBuildModeVisuals()`/`popOverlay`). PASS, zero zarzutów po rundzie 4. Final Control nie dotyczy (dokument). Następny krok: dispatch implementacji `R-HOTSEAT-ETAP5-SWITCH-HUMAN-Q1` na podstawie tego recon. |
| `R-HOTSEAT-ETAP6A-RECON-INPUT-Q1` | `ZAMKNIĘTE (dokument)` | Commit `941676d5`. Recon-only (zero kodu) dla pierwszego pod-etapu Etapu 6 planu hot-seat — kategoria „input" z tabeli B2 (klik/zaznaczenie/ruch/atak/marsz/cykl jednostek, **BEZ** końca tury — już pokryty Etapem 4). Świeży grep `main.ts` skorygował liczbę z planowanych „~25" na **42 miejsca** (nie 41 — Evaluator znalazł 2 pominięte call-site'y: klawisz B `disbandPlayerUnit`-sąsiedztwo i Spacja `cycleToAdjacentPlayerUnit`, main.ts:33273/33215). Każde miejsce ma cytat linii przed/po i proponowaną podmianę `ownerId===0`→`isMe(id)`. Jawne rozliczenie nakładania z Etapami 4/5 (13/42 miejsc już fizycznie wewnątrz kodu przepisanego przez Etap 4b lub zamykanego przy handoff przez Etap 5 — nie dublować). Plan dowodu no-op: Chromium (input dotyka DOM/kliku), analogiczny do harnessów Etap 4/5. 2 rundy Evaluatora: runda 1 — 2 zarzuty materialne (brakujące call-site'y) + 1 kosmetyczny (arytmetyka „10 z 13" powinno być „8"), wszystkie przyjęte przez Obronę w tej samej rundzie (korekta 41→42, dodanie `disbandPlayerUnit` do tabeli wykluczeń z uzasadnieniem: efekt terminalny, bliżej pod-etapu (c) ekonomia niż (a) input); runda 2 — 1 zarzut kosmetyczny (literówka numeru linii `6017`→`6019` dla `refundManpowerToEmpire`), poprawiony bezpośrednio przez orkiestratora (mechaniczna korekta, bez dalszej rundy — Evaluator sam stwierdził że dokument jest gotów do PASS bez dalszych rund merytorycznych). Final Control nie dotyczy (dokument). **Uwaga dla implementacji**: recon pisany gdy Etap 5 NIE był jeszcze zintegrowany — dziś JEST zintegrowany, zweryfikować przy implementacji czy to zmienia klasyfikację któregoś z 13 nakładających się miejsc. Następny krok: dispatch implementacji `R-HOTSEAT-ETAP6A-INPUT-Q1` na podstawie tego recon. |
| `R-HOTSEAT-ETAP6B-RECON-UI-Q1` | `ZAMKNIĘTE (dokument)` | Commit `192b9170`. Recon-only (zero kodu) dla drugiego pod-etapu Etapu 6 planu hot-seat — kategoria „UI/HUD/panele/listy" z tabeli B2 (NAJWIĘKSZA kategoria wg planu, najmniej ryzykowna per-miejsce). Świeży grep skorygował liczbę z planowanych „~75" na **78 miejsc rdzeniowych** (klastry U1-U14 main.ts + V1-V11 ui/*.ts) — znacznie mniejszy rozjazd niż +68% Etapu 6a. Kluczowe znalezisko (Evaluator, zarzut ISTOTNY rundy 1, przyjęty przez Obronę): cache HUD `_last*` (20 zmiennych, zerowane przy handoff przez Etap 5 KROK 5) ma TRZECI, niezależny write-site w `runWorldEndTurn()` z własnymi literałami `ownerId===0`/`.get(0)` — migracja samych funkcji zasilających (`updateHud`/`refreshLiveEmpireRatesUnsafe`) NIE WYSTARCZY, `runWorldEndTurn()` musi zostać przełączony na hak `humanOwnerId` RÓWNOLEGLE — oznaczone jako TWARDA ZALEŻNOŚĆ dla rundy implementacji, ten sam wzorzec haka co klaster D+F Etapu 6a. Drugi zarzut (przyjęty): 3 klastry komunikatów `showHintMessage` bramkowanych `ownerId===0` w tickach ekonomii/głodu/buntu dopisane jako pozycja graniczna (b)/(c) — suma graniczna 17→20. 2 rundy Evaluatora + 1 runda Obrony; drobna niespójność „Podsumowania" z poprawionymi §4/§5 (Evaluator runda 2) naprawiona bezpośrednio przez orkiestratora (synchronizacja tekstu, bez nowej rundy agenta — mechaniczna, zweryfikowana). Final Control nie dotyczy (dokument). Następny krok: dispatch implementacji `R-HOTSEAT-ETAP6B-UI-Q1` na podstawie tego recon, gdy zwolni się lania (po Etapie 6a). |
| `R-HOTSEAT-ETAP6C-RECON-ECONOMY-Q1` | `ZAMKNIĘTE (dokument)` | Commit `14960b8e`. Recon-only (zero kodu) dla trzeciego pod-etapu Etapu 6 planu hot-seat — kategoria „ekonomia" z tabeli B2 (docelowy alias **`isHuman(id)`, NIE `isMe(id)`** — ekonomia liczy WSZYSTKICH ludzi jednocześnie w tej samej fazie EOT, w odróżnieniu od input/UI gdzie liczy się wyłącznie aktywny fotel). Świeży grep częściowo OBALIŁ twierdzenie planu §A5 „rdzeń już per-owner" — `empire-food.ts`/`society-inputs.ts` mają realne literały `ownerId===0`, nie tylko fallbacki. Liczba skorygowana z „~50" na **32** (-36%, mniejsza niż plan zakładał). Największe znalezisko: blok bankowania Skarbiec/Nauka/utrzymanie wewnątrz `runWorldEndTurn()` to realny ZAPIS ekonomii gracza (nie odczyt HUD) — zahardkodowana ścieżka równoległa do generycznej pętli AI, wysokie ryzyko implementacyjne (przepisanie logiki, nie mechaniczna podmiana). **Znalezisko dla przyszłej integracji Etapu 6b (UWAGA dla implementatora)**: CZWARTY, nieznaleziony przez recon 6b write-site cache HUD — `refreshPlayerCityEcon()` (`main.ts:6717-6731`), zasila `_lastPlayerCityEcon` — do uwzględnienia przy dispatchu `R-HOTSEAT-ETAP6B-UI-Q1`. Evaluator runda 1 FAIL (2 zarzuty: pominięte nakładanie ze zintegrowanym `R-HOTSEAT-ETAP4PREP-DEFERRED-OWNER-GUARDS-Q1` dotykającym tej samej pętli auto-racji main.ts:29237; niekompletna inwentaryzacja — 3 pominięte funkcje ery/technologii main.ts:3466/1988/2170, w tym `unlockedTechSetForOwner` faktycznie aktywnie przekazywana do `advanceCityEconomy`), Obrona przyjęła oba (suma 19→22 main.ts, 29→32 całość), Evaluator runda 2 PASS zero zarzutów. 1 pozycja jawnie odłożona jako niejednoznaczna (rebelia, `main.ts:30169`) — jawnie zgłoszona, nie przemilczana. Final Control nie dotyczy (dokument). Następny krok: dispatch implementacji `R-HOTSEAT-ETAP6C-ECONOMY-Q1` na podstawie tego recon, gdy zwolni się lania. |
| `R-HOTSEAT-ETAP6D-RECON-DIPLOMACY-Q1` | `ZAMKNIĘTE (dokument) — ABC dla właściciela` | Commit `c6015522`. Recon-only (zero kodu) dla czwartego pod-etapu Etapu 6 planu hot-seat — kategoria „dyplomacja" z tabeli B2. Potwierdzona symetria `getDiploRelation(a,b)` (klucz `a<b?a_b:b_a`, para `0_1` legalna już dziś). **Kategoria strukturalnie różni się od 6a/6b/6c**: hardkody `0` siedzą WEWNĄTRZ ciał funkcji silnika (np. `applyDiploEventTracked`, `buildDiplomacyTickCtxForPair`), nie tylko na granicy wywołania — prosty grep z planu je pomija. Alias docelowy NIE jest jeden dla całej kategorii: rdzeń silnika liczący dowolną parę ownerów → `isHuman`; HUD/komunikaty/akcje z perspektywy aktywnego fotela → `isMe`/`ME()`. Liczba „~55" z planu ZANIŻONA — potwierdzony floor **≥136** miejsc, z pełną tabelą funkcja→linie dla 19 z 89 sprawdzonych nazwanych funkcji (metoda brace-matched, zweryfikowana niezależnie 3 razy). **DECYZJA WŁAŚCICIELA WYMAGANA przed dispatchem implementacji `R-HOTSEAT-ETAP6D-DIPLOMACY-Q1`**: (a) czy kontynuować pełną inwentaryzację pozostałej części kategorii (HUD-call-site/dev-harness/`diplomacy-border-march.ts`, poza klastrem silnika) w kolejnej rundzie recon, czy (b) podzielić temat na pod-kategorie i dispatchować implementację klastra silnika już teraz z jawnie ograniczonym zakresem. Potwierdzone: równoległa implementacja Etapu 6a (w toku w chwili tego recon) dotknęła TYLKO call-site `openPlayerMapUnitAttack` (`ownerId===0`→`isMe`), ciało `playerIsAtWarWith` nietknięte — rekomendacja: scalać 6d PO 6a na tym call-site (oba kierunki zgodne, konflikt tekstowy nie logiczny). 3 rundy Evaluatora (nakładanie z 6a niepełnie sprawdzone w rundzie 1; niekompletna tabela §4 nawet w jej własnym zawężonym zakresie w rundzie 2 — 10 zamiast 2 linii dla `buildPlayerDiploSummary`, błędny numer linii dla `getRelationBreakdown`; PASS zero zarzutów runda 3) + 2 rundy Obrony, wszystkie zarzuty przyjęte i naprawione z dowodem. Final Control nie dotyczy (dokument). |
| `R-HOTSEAT-ETAP6E-RECON-RENDER-Q1` | `ZAMKNIĘTE (dokument)` | Commit `42e919d7`. Recon-only (zero kodu) dla piątego pod-etapu Etapu 6 planu hot-seat — kategoria „render/kamera" z tabeli B2. Potwierdzone twierdzenie planu §A8: `render/camera.ts` jest owner-agnostyczna, zero pracy. Liczba skorygowana z „~15" na **27** — 3 rundy Evaluatora (5 pominiętych hardkodów w `render/*.ts` — grep pierwszej rundy był ślepy na formy `= 0`/`?? 0`, ten sam kształt błędu co C-031; pominięty guard `main.ts:5372` `syncOkolicaOverlay`; fałszywa przesłanka wykluczenia martwej `computePotegaComponents`; błąd arytmetyczny sumowania 21 zamiast 27 w rundzie 2) + 2 rundy Obrony, wszystkie naprawione z dowodem. Docelowy alias: `isMe`/`ME()` (perspektywa aktywnego fotela) — reużycie `isMe()` z równoległej implementacji Etapu 6a zamiast tworzenia duplikatu. Zero nakładania z Etapami 6a (potwierdzone też w stanie roboczym równoległego worktree, nie tylko zintegrowanym main)/6d. Final Control nie dotyczy (dokument). Następny krok: dispatch implementacji `R-HOTSEAT-ETAP6E-RENDER-Q1` na podstawie tego recon, gdy zwolni się lania. |
| `R-HOTSEAT-ETAP6F-RECON-START-Q1` | `ZAMKNIĘTE (dokument) — OSTATNI z 6/6 recon Etapu 6` | Commit `8a541a57`. Recon-only (zero kodu) dla szóstego i OSTATNIEGO pod-etapu Etapu 6 planu hot-seat — kategoria „start gry/wybór cywilizacji". Liczba skorygowana z „~15" na **63** (4.2×, największy rozjazd z całej serii 6a-6f) — 2 rundy Evaluatora znalazły pominiętą funkcję `restoreAiRosterFromSave` (trzecie wystąpienie `assignAiCivTypes`, realne nakładanie z Etapem 1 przez `isAiOwner(humanSeats,...)` na `main.ts:7570`) i błąd arytmetyczny w bucketach literalnych wejść New Game — oba naprawione z dowodem w jednej rundzie Obrony. **Jawny podział, inny niż w Etapach 6a-6e**: tylko 3 z 63 miejsc to prosta migracja literału (`fillAiOwnerCivMap`+`repairAiRosterFromMap`+`restoreAiRosterFromSave`, wzorzec „wyklucz cywilizację OBU ludzi" zamiast jednej), pozostałe **52 miejsca wymagają NOWEJ funkcjonalności** (drugi heks startowy w `game/cluster-start.ts`, drugi wybór cywilizacji w menu — dziś `NewGameParams.civId`/`civName` to pojedyncze pola, nie listy) — opisanej co-dodać-i-gdzie bez pisania kodu implementacji, zgodnie z zakresem recon. **Z zamknięciem tego dokumentu wszystkie 6 podetapów Etapu 6 (a-f) mają gotowy, adwersaryjnie zweryfikowany recon** — kolejny krok to dispatch implementacji pozostałych podetapów (6b-6f; 6a patrz osobny wpis). 1 pozycja ABC odłożona do rundy implementacji (czy 5 ścieżek dev/quick-start dostaje drugi sztywny civ czy zostaje single-seat-only). Final Control nie dotyczy (dokument). |
| `R-HOTSEAT-ETAP6A-INPUT-Q1` | `ZINTEGROWANE` | Commit `4e7a5491`. Implementacja Etapu 6a (podetap "input" Etapu 6) na podstawie zamkniętego recon: 42 miejsca main.ts (klastry A-H: zaznaczenie, klik mapy, marsz, inicjacja ataku, dokończenie animacji ruchu, podgląd trasy, scal/rozdziel armii) zmigrowane z `ownerId===0`/`!==0` na nowy alias `isMe(ownerId)`/`!isMe(ownerId)`/`ME()`. Klaster D+F (13 miejsc kontynuacji marszu/animacji na koniec tury) podłączony do realnego parametru `humanOwnerId` funkcji `endActiveHumanTurn` (hak świadomie zostawiony przez Etap 4b) zamiast globalnego `ME()` — architektura gotowa pod przyszły hot-seat. `game/army-cycle.ts`: wstrzykiwany parametr `isMe` w `cyclablePlayerArmyLeadsBase` (moduł czysty), wzorcem istniejącego `canMove`. Nowa bramka `hotseat-etap6a-input-noop-test.cjs` (Chromium, 60 tur/3 warianty): 20/20 identycznych hashy PRZED/PO, nietautologiczność potwierdzona. Operator→Evaluator→Final Control PASS, zero zarzutów blokujących. Jedno świadome odchylenie od litery dispatchu (F6-F10 w `renderLoop()` używa `isMe`/`ME()` zamiast `humanOwnerId` — `renderLoop()` nie ma strukturalnego dostępu do tego parametru, zgodne z oryginalną tabelą recon), zaakceptowane niezależnie przez Evaluatora i Final Control. tsc czysty, 5 bramek referencyjnych zielone. **2 follow-upy do rejestru (nie blokują)**: `mgla-odkrycie-wzdluz-sciezki-test.cjs` ma 2 nowe FAIL — hardkodowany literał `ownerId===0` WE WŁASNYM regexie testu (poza allowlistą tematu), wymaga osobnej rundy naprawy testu; `plannedMarchesSize=0` przez cały bieg bramki no-op — klaster D zweryfikowany tylko strukturalnie (Read sygnatur), nie zaobserwowany żywo w działaniu (mapa sandboxa nie dała pewnego wieloturowego marszu po lądzie) — kandydat do dociągnięcia przy kolejnej rundzie tej bramki. Następny krok: implementacja Etapu 6b (UI) na podstawie zamkniętego recon, sekwencyjnie (dotyka tego samego main.ts). |
| `R-HOTSEAT-ETAP7-RECON-SAVELOAD-Q1` | `ZAMKNIĘTE (dokument) — OSTATNI etap całego planu` | Commit `b29c6972`. Recon-only (zero kodu) dla Etapu 7 — OSTATNIEGO etapu całego planu hot-seat (save/load v3), dispatchowany równolegle z podetapami Etapu 6 (dotyka innych plików — `game/save.ts`, `buildSaveGameSnapshot`/restore). Zweryfikowana zgodność z **ABC-4** (decyzja właściciela z 2026-09-05: BEZ migracji v2→v3, stare zapisy przestają działać z czytelnym komunikatem zamiast cichej awarii) — dispatch orkiestratora jawnie poprawił sprzeczność między tą decyzją a przestarzałym wierszem tabeli §C planu ("stary sejw v2 wczytuje się bez zmian"), zanim Operator zaczął pracę. Potwierdzone: bug `aiSkarbiecByOwner`/`aiNaukaPoolByOwner` opisany w planie (Audyt #44) **jest już naprawiony** w dzisiejszym kodzie — Etap 7 nie ma tu nic do zrobienia (plan opisywał stan sprzed naprawy). Zaprojektowany kształt `GraczSaveV3` (`gracze[]`, `exploredByHuman`, `humanOwnerIds`, `activeHumanOwnerId`) + mechanizm detekcji starego formatu z czytelnym komunikatem (`IncompatibleSaveFormatError`). 2 rundy Evaluatora znalazły i naprawiły: odwróconą kolejność wywołań w projekcie komunikatu niekompatybilności (dokładnie ta sama klasa buga co wcześniej naprawiony N-ZINDEX-TOAST — `openStartupMainMenu()` musi poprzedzać `showHintMessage()`) i błędne typy pól tempa w kontrakcie (`number` zamiast realnych string-unionów z `game/playerState.ts`). PASS, zero zarzutów po rundzie 2. Final Control nie dotyczy (dokument). Następny krok: dispatch implementacji Etapu 7, prawdopodobnie pod koniec — po zamknięciu podetapów Etapu 6, bo format zapisu opiera się na docelowych strukturach z 6b-6f. |
| `R-HOTSEAT-ETAP6A-FOLLOWUP-MGLA-TEST-Q1` | `ZINTEGROWANE` | Commit `27c8a31b`. Follow-up po integracji `R-HOTSEAT-ETAP6A-INPUT-Q1` — `gra/tools/mgla-odkrycie-wzdluz-sciezki-test.cjs` miał hardkodowany literał `ownerId===0` we własnym regexie/parserze, dostał 3 nowe FAIL (nie 2, jak sądzono w dispatchu) po tym jak main.ts zaczął w tych miejscach używać `isMe(ownerId)`/`ownerId===humanOwnerId`/`ownPlayerVisibleHexes()+ME()`. NIE regres logiki gry (main.ts poprawny, potwierdzone trzykrotnie przy integracji 6a) — czysta naprawa testu. Rozszerzono 3 asercje o zamknięty zbiór realnie istniejących wariantów strażnika tożsamości, kontrola nie-maskowania potwierdza że test nadal odróżnia stary/zły stan od nowego (stary test na dzisiejszym main.ts nadal daje identyczne 3 FAIL). Zero zmian w `gra/src/**`. Operator→Evaluator→Final Control PASS, zero zarzutów. Test 17/17, 5 bramek referencyjnych zielone, `tsc` czysty. |
| `R-HOTSEAT-ETAP6B-UI-Q1` | `ZINTEGROWANE` | Commit `e9e6a325`. Implementacja Etapu 6b (podetap "UI/HUD/panele" Etapu 6) na podstawie zamkniętego recon: 78 miejsc rdzeniowych (67 main.ts klastry U1-U14 + 11 ui/*.ts klastry V1-V11) zmigrowane na `isMe(ownerId)`/`!isMe(ownerId)`/`ME()`, reużywając alias z Etapu 6a. **Krytyczne znalezisko recon zaadresowane**: write-site cache `_last*` wewnątrz `runWorldEndTurn()` podłączony do realnego parametru `humanOwnerId` (11 zmiennych) zamiast globalnego `ME()` — ten sam wzorzec haka co klaster D+F Etapu 6a. `refreshPlayerCityEcon()` (znalezisko Etapu 6c) również sparametryzowana. `ui/*.ts` (cityPanel/siegeMapPanel/preBattle/powerOverlayHud): wstrzykiwany parametr `isMe` wzorcem `game/army-cycle.ts`. Nowa bramka `hotseat-etap6b-ui-noop-test.cjs` (Chromium, 20 tur): 20/20 identycznych hashy, nietautologiczność potwierdzona. Operator→Evaluator→Obrona→Final Control PASS. 2 pozycje jawnie rozliczone jako poza zakresem (`cityPanel.ts:7476/7575`→`difficulty-cost.ts:44-46 isPlayerOwner`, `turn-economy.ts:1219`) — przypisane Etapowi 6c/6d (migracja kosmetyczna bez migracji docelowej funkcji poza allowlistą). tsc czysty, 5 bramek referencyjnych zielone. Potwierdzony brak konfliktu z równoległą implementacją Etapu 6e. Następny krok: implementacja Etapu 6c (ekonomia) sekwencyjnie na tym samym `main.ts`/`runWorldEndTurn()`. |
...
| `R-HOTSEAT-ETAP6F-PART2-RECON-Q1` | `ZINTEGROWANE (dokument, recon projektowy)` | Commit `7cd680e6`. Recon (docs-only) części (ii) pod-etapu 6f planu hot-seat — drugi heks startowy + druga cywilizacja w menu startowym. Zawiera: dokładny opis dzisiejszego stanu (`aiStartHexes`/`ClusterStartPlan`/menu startowe) z cytatami kodu, listę brakujących elementów danych/UI dla drugiego fotela, 2-3 warianty projektowe (sekwencyjny/naprzemienny/inny) z realnymi za/przeciw, jawną listę pytań ABC do właściciela. Zero propozycji gotowego rozstrzygnięcia — to jest z definicji temat projektowy/UX. Operator→Evaluator, 4 rundy (2 drobne korekty off-by-one w cytowanych numerach linii `cluster-start.ts`, wychwycone przez Evaluatora), zero zarzutów w rundzie końcowej. Final Control nie dotyczy (dokument). **Implementacja tej części wymaga osobnej decyzji właściciela** co do wybranego wariantu — poza zakresem automatycznego dispatchu. |
...
| P-PRACA-MARTWE-PROCENT-PULI-IMPERIUM-FOR-OWNER-Q1 | 2026-08-26 | `main.ts:4808` `procentPuliImperiumForOwner()` martwe po usunięciu `getEmpirePracaSplit`. | **ZAMKNIĘTE 2026-09-08 przez `P-MARTWY-KOD-PROCENT-PULI-IMPERIUM-Q1`** | Funkcja usunięta (zero konsumentów, potwierdzone grepem); jedyna zależna asercja `ai-praca-split-parity-test.cjs` przepisana pod realny mechanizm parytetu gracz/AI (`pracaAutoPercent`, `R-PRACA-JEDEN-PODZIAL-Q1` runda 2/F1) zamiast nieistniejącego dopełnienia do 100%. Zero zmian zachowania gry. |
...
| P-SCIENCEHUB-TOOLTIP-EMOJI-ZAMIAST-IKON-Q1 | 2026-09-02 | Druga, NIEZALEZNA instancja tej samej klasy buga: `sciencePicker.ts:~905-925` (funkcja tooltipa hover, INNA powierzchnia UI niz lista huba badan) rowniez emituje surowe emoji 🏛/🌾 zamiast ikon marki. | **ZINTEGROWANE** (`1fbe1551`, 2026-09-07) | Recon rundy 1 skorygował lokalizację: bug żył w `buildTooltipHTML()` (sekcja „Warunek badania:"), nie w liniach z pierwotnego reconu. Naprawiony tym samym wzorcem co lista huba (`a2c887e8`): nowy producent danych `techRequirementItems()` renderowany przez wspólny resolver `unlockIconSvg` z `brandAssets.ts` (reużyty z `scienceHubHud.ts` przez `export`). Obrona naprawiła 2 zarzuty Evaluatora (Opus 5): (1) fixture bramki nie stosował produkcyjnego filtra placeholderów — dodano drugi bundle na podmienionej `tech.json` z realnym `mouseover`, dający żywe pokrycie; (2) etykieta i ikona mogły pochodzić z różnych członów pola wielowartościowego — usunięto `firstLabel()`, zastąpiono pętlą per człon. Nowa bramka `science-picker-tooltip-icons-real-render-test.cjs` 141/0, 9 żywych zrzutów Chromium PRZED/PO + porównanie z panelem miasta i listą huba. Operator+Evaluator Opus 5, Final Control Sonnet 5, wszystkie PASS. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `R-DYPLO-CLAMP-PLATNOSC-PROPORCJONALNA-Q1` | (bug znaleziony podczas rozstrzygania "Oferują: —" ze zrzutu właściciela, nie osobne zgłoszenie) | **ZINTEGROWANE** (`c1d5d9f6`, 2026-09-07) | Domain GAME, ekonomia/uczciwość ofert. Dispatch zakładał, że `clampBasketItemsToAffordable` usuwa całą pozycję zamiast redukować proporcjonalnie — recon PIERWOTNY był NIEAKTUALNY (mechanizm zdążył się zmienić między reconem a dispatchem, ostatnia zmiana `546f6a51` z 2026-08-17). Dzisiejszy kod JUŻ liczy `Math.min(qty, floor(zasob/(perUnit×mult)))` dla złoto/praca/żywność/surowiec_ilość i zwraca zredukowaną ilość — zero zmiany produkcyjnej potrzebne, potwierdzone żywym wywołaniem przez Operatora, Evaluatora I Final Control niezależnie (3 różne scenariusze liczbowe każdy). Zamiast naprawy: dołożono 5 testów jednostkowych domykających lukę pokrycia (złoto/praca/żywność nie miały dotąd testu tej funkcji). Zidentyfikowany nieblokujący róg granularności (skarbiec>0 ale <1 jednostka×mnożnik tur → floor nadal usuwa) jest spójny z ustaloną konwencją `diplomacyNormalizeSurowiecIlosc` — kandydat na osobne pytanie ABC, nie regresja. Bramka: 36/36. Operator→Evaluator→Final Control wszystkie PASS. |
...
**DRUGI POTWIERDZONY PRZYPADEK (2026-09-05, potknął się o niego orkiestrator, nie agent).**
`gra/tools/ai-buduje-budynki-test.cjs:81` — `TMP_ROOT = os.tmpdir()/civ-ai-buduje-budynki`,
stała ścieżka bez sufiksu, a `runVariantBuild` (`:189`, `:219`) buduje do
`TMP_ROOT/dist-<wariant>` z `--emptyOutDir`. Uruchomiłem tę bramkę dwa razy równolegle
(stare zadanie w tle nadal biegło, gdy odpaliłem drugie) i dostałem **`exit=1`, czyli
fałszywy CZERWONY** — jeden przebieg wyczyścił `dist` drugiemu w locie. Po ubiciu obu
i przebiegu POJEDYNCZYM wynik był inny. **To jest dokładnie ta klasa błędu, tylko odwrócona
znakiem:** temat wyżej opisuje fałszywy ZIELONY (parytet), tutaj wyszedł fałszywy CZERWONY.
Obie strony monety kosztują tak samo — jedna przepuszcza regres, druga każe naprawiać
sprawną pracę. Wzorzec `os.tmpdir()/<stała-nazwa>` ma więc już **dwa** potwierdzone
...
**właściciel zamówił trwały weryfikator** („żeby to już nigdy nie wróciło"), a nowa bramka
`gra/tools/ai-buduje-budynki-test.cjs` **nie figuruje w `docs/decyzje/R-PROC-AUTOBOT.md` §6**
(sprawdzone: `grep -n "ai-buduje-budynki" docs/decyzje/R-PROC-AUTOBOT.md` → zero trafień).

**Dlaczego to nie jest formalność.** Bramka niewpisana do §6 nie trafia do żadnej listy
„uruchom przed integracją". Żyje wyłącznie w allowliście jednego, zamkniętego już tematu —
czyli przy następnej zmianie w `main.ts` albo `empire-city-defaults.ts` nikt jej nie odpali
i defekt (`budowaTryb` resetowany na `reczny`) może wrócić dokładnie tą samą drogą, którą
przyszedł. To jest ta sama luka, która pozwoliła mu żyć niezauważonym za pierwszym razem
(recon (E): „ZERO bramek sprawdzających, że AI stawia budynki").

**Zakres:** wpis do `R-PROC-AUTOBOT.md` §6 z jawną notą, że bramka jest **wolna** (4 buildy
vite + 4 przebiegi Chromium, rząd minut) i **nie nadaje się do puli referencyjnej odpalanej
przy każdym temacie** — należy do bramek warunkowych, wyzwalanych zmianą w `main.ts` /
`empire-city-defaults.ts` / `auto-manage.ts` / `ai.ts`. Przy okazji rozstrzygnąć, czy §6
w ogóle ma dziś kategorię „bramka warunkowa" — jeśli nie, to jest jej pierwszy przypadek.
**STATUS: ZAREJESTROWANE, NIE DISPATCHOWANE.** DOMAIN: PROCESS.

**Uwaga druga z tego samego Final Control (nie wymaga osobnego tematu):** klauzula o ownerze
ujemnym w asercji `A4d-b` jest w turze 0 **spełniona pusto** (nie ma jeszcze ownerów ujemnych
do sprawdzenia) — asercja nie jest błędna, ale też niczego w tym momencie nie dowodzi.
Realną pracę wykonuje `A7` (pokrycie per-miasto). **`A7` nie wolno osłabiać** — pod mutacją
`A1`/`A2` (sumy imperium) zostają ZIELONE, bo AI ma drugą drogę do kolejki; tylko `A7`
łapie defekt, wskazując siedem miast dwóch cywilizacji trzymanych 16–45 tur z zerem budynków.
...
## ZAMKNIETE 2026-09-05 — dwa tematy procesowe domkniete reka orkiestratora (nie Operatora)

| ID | Status | Co zrobiono i dlaczego orkiestrator, a nie Operator |
|---|---|---|
| `P-BRAMKA-AI-BUDYNKI-NIEZAREJESTROWANA-W-PROC-Q1` | **ZAMKNIETE** (`docs/decyzje/R-PROC-AUTOBOT.md` §6) | Do tabeli bramek §6 dopisane TRZY bramki, ktore powstaly w ostatnich falach i nigdzie nie figurowaly: `ai-buduje-budynki-test.cjs` 42/0 (wraz z ustaleniem Final Control, ze `A1`/`A2` zostaja zielone pod mutacja i defekt lapie wylacznie `A7` — **`A7` nie wolno oslabiac**), `map-field-battle-test.cjs` 20/20 i `entity-card-contract-test.cjs` 75/0. Dopisana tez regula ogolna: **nowa bramka istnieje dopiero wtedy, gdy jest w tabeli §6** — bramka spoza tabeli nie trafia do zadnej listy uruchamianej przez kolejne tematy, wiec weryfikator zamowiony jako trwaly cicho umiera. Wpis do tabeli jest czescia integracji tematu, ktory bramke stworzyl. **Dlaczego orkiestrator:** `docs/decyzje/**` jest dla agentow zakazane bezwzglednie (§9), wiec kanon procesu strukturalnie nie moze byc zmieniony przez Operatora. |
| `P-IZOLACJA-DWOCH-PISARZY-JEDEN-WORKTREE` | **ZAMKNIETE** (`R-PROC-AUTOBOT.md` §2b + `.claude/skills/civ-autobot-workflow/SKILL.md`) | Regula **jeden pisarz na worktree, w kazdej chwili, bez wyjatku** — z jawnym rozroznieniem od istniejacej reguly §2b (tamta o dwoch ROZNYCH tematach, ta o dwoch przebiegach TEGO SAMEGO). Sedno: skrypt Workflow ma trzy fazy, nie dwie, a Obrona pracuje w tym samym worktree — **runda N jest zamknieta dopiero, gdy skrypt zwroci wynik**, nie po Evaluatorze. Kazdy prompt dostaje guard `git log -1` + `git status --short` z instrukcja `BLOCK` przy rozbieznosci. Druga czesc reguly: **mutacje weryfikacyjne cofa sie przez kopie pliku, nigdy przez `git checkout`** — praca w incydencie zrodlowym ocalala wylacznie dlatego, ze wszyscy trzej agenci sami z siebie tak robili. Do checklisty skilla dopisane dwie pozycje. **Przyczyna incydentu byla moja, nie agentow** — dispatchowalem runde 2 przed zwrotem skryptu rundy 1, potem runde 3 w trakcie rundy 2; trzy wystapienia w jednym temacie. |

## ECHO WLASCICIELA 2026-09-05 (noc) — cztery decyzje ABC naraz
...
## `R-PODBOJ-SUROWCE-BILANS-BRAK-WIERSZA-Q1` — GAME — **ZINTEGROWANE 2026-09-07** (commit `0283fa60`)

Żywe zgłoszenie właściciela (popup "Bilans zdobycia" bez informacji o zdobytych
surowcach). Recon ustalił: mechanizm transferu surowców JUŻ działał po cichu
(`city.surowce` nigdy nie było zerowane przy zmianie `ownerId`), brakowało wyłącznie
wiersza w UI. Dodano wiersz "Surowce zdobyte" w `buildCityCaptureReportRows()`
(`main.ts`) — zero zmiany mechaniki transferu, zero zmiany wiersza "Pula pracy" (osobny,
zamknięty temat). Operator→Evaluator→Final Control PASS; Final Control niezależnie
oddalił jeden niebilokujący zarzut Evaluatora (odczyt `city.surowce` następuje
technicznie PO zmianie `ownerId`, nie przed — ale nic nie modyfikuje tego pola na
żadnej z 3 ścieżek podboju, więc funkcjonalnie równoważne). Nic do dispatchu.

## `R-PODBOJ-ELIMINACJA-PULA-PRACY-TRANSFER-Q1` — GAME — **ZINTEGROWANE 2026-09-07** (commit `bcb2ac4b`)

ECHO właściciela (żywa rozmowa, częściowo odwraca `P-PODBOJ-PRZEJECIE-SUROWCOW-PANSTWA-MIASTA`
2026-08-09): przy ELIMINACJI ostatniego miasta/stolicy cywilizacji pula pracy CAŁEJ
ofiary (`ownerPracaPool`) trafia (DODAJE SIĘ) do puli zdobywcy zamiast być zerowana.
Przy zdobyciu stolicy BEZ eliminacji (cywilizacja przeżywa z innymi miastami) — BEZ
ZMIAN, pula ofiary nadal przepada (kanon 2026-08-09 niezmieniony dla tego
podprzypadku). Budynek w budowie w zwykłym mieście zachowuje postęp po zdobyciu —
zweryfikowane BEZ zmiany kodu (sanityzacja kolejki forfeituje wyłącznie legacy
jednostki i cuda zablokowane bramką, nie zwykłe budynki). Operator→Evaluator→Final
Control, wszystkie PASS, zero zarzutów; konkretne liczby: eliminacja 30+120→150
(zdobywca, DODANE)/120→0 (ofiara); bez eliminacji 15→15 (zdobywca nietknięty)/90→0
(ofiara).

**Integracja wymagała ręcznego scalenia** — ten temat i równolegle zintegrowany
`R-PODBOJ-SUROWCE-BILANS-BRAK-WIERSZA-Q1` oba dodawały nowe pole do tego samego
interfejsu (`CityCaptureReportInput`) i tych samych 4 wywołań `buildCityCaptureReportRows()`
w `main.ts` — `git apply --3way` oznaczył 5 konfliktów (zawsze ten sam wzorzec: oba
pola dodane w sąsiednich liniach), rozwiązane ręcznie zachowując OBA pola
(`surowce` + `pracaPoolPrzejeta`) w każdym miejscu. Potwierdzone pełną baterią testów
po scaleniu (`capital-capture-test` 107/107, `miasto-zdobycie-raport-test` 105/105,
`eliminacja-lup-kwoty-test` 38/38, `podboj-kolejka-budynek-niemozliwy-test` 79/79,
5 bramek referencyjnych). Nic do dispatchu.

## `R-HANDEL-WYMIANA-DAR-DEADLOCK-Q1` — GAME — **ZINTEGROWANE 2026-09-07** (commit `ed0b518b`)

Żywe zgłoszenie właściciela: technologia "Wymiana" (jedyna odblokowująca CAŁY handel
z partnerem) nie dawała się podarować/przehandlować cywilizacji Harappa, mimo jej
posiadania. Recon ujawnił realny deadlock kompozycyjny między dwiema OSOBNO poprawnymi,
wcześniej zamkniętymi decyzjami: `R-HANDEL-WYMIANA-TECH-GATE-Q1` (cały handel wymaga
zbadanej "Wymiany" przez partnera) + `P-HANDEL-TECH-BRAK-PREREQ-PO-FILTRZE` (dar
technologii wymaga zbadanych jej prereqów przez odbiorcę) — razem: partnera bez
Garncarstwa/Rolnictwa/Oswojenia zwierząt nie dało się nigdy podciągnąć przez dar.
Naprawa: punktowy wyjątek WYŁĄCZNIE dla `TRADE_TECH`="Wymiana" w
`techIdsWithPrereqsMetForRecipient` i `grantTechToOwner` — reguła ogólna (prereq dla
wszystkich innych technologii) nietknięta, potwierdzone kontrolą regresu (Koło nadal
blokowane). **Decyzja projektowa podjęta autonomicznie przez orkiestratora w nocy**
(właściciel spał) — odwracalna, do ABC jeśli właściciel się nie zgodzi. Operator→
Evaluator→Final Control PASS, zero zarzutów.

## `R-HUD-ZETONY-EKONOMIA-BRUTTO-Q1` — GAME — **ZINTEGROWANE 2026-09-07** (commit `7147ba87`)

Żywe zgłoszenie właściciela (po ECHO na 3 opcje, wybrał "Wszystkie trzy na brutto"):
żetony HUD Praca/Skarbiec/Nauka pokazywały netto (po odjęciu kosztów) zamiast łącznego
przyrostu. To odwraca wcześniejszy, identyczny wzorzec (netto na żetonie, brutto tylko
w tooltipie) obowiązujący od dawna dla wszystkich trzech surowców — po przedstawieniu
że to ta sama, świadoma wcześniejsza decyzja, właściciel zdecydował się ją odwrócić
teraz dla spójności zamiast robić wyjątek tylko dla Pracy. Dociągnięto brakujące pole
...
## `P-KARTA-PRZEBUDOWA-UKLAD-Q1` — GAME — **ZINTEGROWANE 2026-09-08** (commit `4a9011c6`)

Wdrożenie zaakceptowanego przez właściciela układu sekcji karty encji (`entityCards/`):
Wymagania → Opis → Rys historyczny (WŁASNA nazwana sekcja, pozycja 4, zaraz po Opisie) → Top 3
→ Charakterystyka (bez poziomu, ten w nagłówku) → Efekty/Statystyki bojowe → Koszty
(rozdzielone budowa/utrzymanie lub rekrutacja/utrzymanie) → Poziomy. Formalnie uchyla starą
decyzję `P-KARTA-OPIS-PRZED-STATYSTYKAMI-Q1` (rys historyczny był dotąd zawsze zaraz po
nagłówku dla wszystkich typów kart) — `renderer.ts` ma teraz JEDEN wspólny punkt
pozycjonowania historii, zweryfikowany żywo dla wszystkich 4 typów encji (building/unit/
technology/improvement), nie tylko budynku/jednostki. Temat WYŁĄCZNIE infrastrukturalny —
masowe autorstwo nowej treści opisowej per encja to osobna, przyszła fala (wzorem
`R-KARTY-HISTORIA-Q1`). Evaluator znalazł jeden realny defekt: sekcja historii nie miała
dosłownej etykiety "Rys historyczny" w DOM (tylko akapit kursywy) — naprawione nagłówkiem h3.
Final Control (własny, niezależny render Chromium/Playwright) potwierdził etykietę, kolejność
sekcji na 3 typach kart, zerowy diff pozostałych adapterów, brak regresji `unitInfoCard.ts`/
`statuses`. Testy: `tsc --noEmit` czysty, 5 bramek referencyjnych zielone,
`entity-card-contract-test` 75/0, `entity-card-historia-section-test` 36/36. Migration testy
