...
| R-KARTA-JEDNOSTKI-3D-EKSPOZYCJA-UX-Q1 | 2026-09-02 | Lepsza kompozycja/ekspozycja podgladu 3D jednostki na kartach encji — dzis maly plaski box, wlasciciel sugeruje ze moze wymagac nowego UX/designera. | **WARIANT A ZAAKCEPTOWANY, dispatchowane, Workflow w toku** | Recon (Explore): podglad 3D renderuje sie w `.entity-card-medallion` — twardy 34x34px (24x24px compact) slot w naglowku, `unitMiniPreview.ts` uzywa cache'owanego, jednorazowego snapshotu WebGL. Zbudowana i opublikowana makieta (artefakt claude.ai, 3 warianty: A-diorama pelnej szerokosci/rekomendacja, B-powiekszony medalion, C-dwie kolumny desktop) w skali 1:1 wzgledem `ENTITY_CARD_CSS`. **Wlasciciel zaakceptowal Wariant A** ("wariant a zrob i potem deploy"). Drugi recon (blast radius): zero testow w `gra/tools/*.cjs` sprawdza rozmiar/piksele medalionu (zmiana rozmiaru bezpieczna); `unit-card-3d-preview-coverage-test.cjs`/`unit-info-card-entitycard-migration-test.cjs` sprawdzaja wylacznie TRESC (zamontowany 3D/fallback), nie pozycje; `entity-card-historia-section-test.cjs` sprawdza wylacznie kolejnosc historia-przed-body, agnostyczny na ksztalt headera. `.entity-card--compact` (uzywane przez `technologyAdapter.ts` dla zagniezdzonej listy jednostek w karcie technologii) JEST aktywnie uzywane — dispatch jawnie wymaga zachowania trybu compact BEZ ZMIAN (diorama dotyczy wylacznie normalnego naglowka). Dispatch (`00-dispatch.md`, commit `3d9dd86c`): diorama ~180-200px zastepuje maly header, ten sam zamontowany podglad powiekszony+wysrodkowany, tytul/podtytul jako overlay w lewym dolnym rogu, kolejnosc DOM diorama->historia->body zachowana. Sciezka A (Workflow), Operator+Evaluator Opus 5 (temat graficzny, R-PROC-AUTOBOT.md par.5a), Final Control Sonnet 5, obowiazkowe zywe zrzuty Playwright. Workflow `wf_941a64bc-334`. |
