...
- **U1 (13 jednostek): Operator PASS, Evaluator 1 zarzut proceduralny (status powinien byc PASS-WITH-NOTES, ta sama klasa fixture-driftu co B1/T1/I1) - Obrona PRZYJELA i skorygowala etykiete (commit `a04ff877`) — Final Control w toku.**
...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| P-KARTY-HISTORIA-TEST-FIXTURE-REALNE-DANE-Q1 | 2026-09-01 | Naprawa `entity-card-historia-section-test.cjs` (temat INFRA), ktorego fixture "jeszcze pustych" encji (stolarnia/Lowiectwo/farma) uzywal REALNYCH danych produkcyjnych zamiast syntetycznych - kazdy kolejny batch tresci ktory wypelnia jedna z tych 3 encji powoduje falszywy FAIL. Dodatkowo sekcja [5] ma pokrewny blad: fixture "zla wielkosc liter" dziedziczy z prawdziwego wiersza, wiec po wypelnieniu poprawnego pola test przypadkiem przechodzi/nie przechodzi z innego powodu niz zamierzony. | **OTWARTE — dispatch zapisany, Workflow uruchomiony** | Dispatch: `dyspozycje/autobot/runs/P-KARTY-HISTORIA-TEST-FIXTURE-REALNE-DANE-Q1/00-dispatch.md`. Fix: sekcja [4] asercja WARUNKOWA na realnym stanie pola zamiast twardego "nie istnieje"; sekcja [5] usuniecie poprawnego pola z kopii wiersza przed wstrzykniecem zlej wielkosci liter. |
...
| R-SKARBIEC-HANDEL-PODGLAD-ZERO-Q1 | 2026-09-01 | Wlasciciel: panel Handel pokazuje +157 zlota/ture z 23 tras, ale panel Skarbiec Imperium (ta sama tura) pokazuje "Handel ze szlakow: 0" i Netto skarbiec liczone bez tego dochodu - podejrzenie ze pieniadze z handlu "sa sztucznie wyswietlane". Recon: SILNIK jest poprawny (na koncu tury `computeTradeRouteIncomeByCity` realnie dolicza do `player.skarbiec`) - bug jest WYLACZNIE w podgladzie HUD: `refreshLiveEmpireRatesUnsafe()` (main.ts:15917-15941) woli `previewCityEconomy` z DWOMA literalnymi `undefined` (pozycje 10-11: tradeRouteBuildingBonusByCity/tradeIncomeByCity), wiec podglad ZAWSZE liczy handel jako 0, mimo ze gotowa aktualna zmienna `tradeRouteBuildingBonusByCie` juz istnieje w tym samym domknieciu. Luka testowa potwierdzona: istniejace testy skarbca maja fixture bez zadnych tras, wiec "handel=0" wychodzi trywialnie z konstrukcji testu. | **RUNDA 2 FAIL — fix w main.ts nadal poprawny, ale Final Control zlapal TRZECIA z rzedu luke w guardzie testu — runda 3 w toku** | Dispatch: `dyspozycje/autobot/runs/R-SKARBIEC-HANDEL-PODGLAD-ZERO-Q1/00-dispatch.md`. Fix main.ts (podmiana dwoch `undefined` na realnie policzone mapy) POTWIERDZONY empirycznie 3x przez Final Control - nietkniety od rundy 1. Runda 2 utwardzila `checkRealFixSiteInMainTs()` (usuniecie komentarzy + scisle porownanie), ale Final Control znalazl NOWY atak: `extractCallArgs()` lokalizuje wywolanie surowym `indexOf` PRZED oczyszczeniem z komentarzy - martwy komentarz z fikcyjnym, poprawnie wygladajacym wywolaniem UMIESZCZONY PRZED prawdziwym (zregresowanym) kodem myli lokalizacje (znajduje falszywe pierwsze). Runda 3: lokalizacja wywolania ma sie odbywac na tekscie JUZ oczyszczonym z komentarzy (nie surowym), z zachowaniem mapowania offsetow. |
