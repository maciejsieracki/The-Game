...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-JEDNOSTKA-NIEWIDOCZNA-PO-WYBUDOWANIU-Q1` | Dwa zrzuty ekranu tej samej okolicy: "po wybudowaniu nowej jednostki nie pojawia się ona, tak jakby była niewidoczna... jakby coś później renderuje, a nagle się pojawia... może to wprowadzać graczy w błąd." Zrzut 1: malutki uproszczony znacznik zamiast jednostki; zrzut 2 (później): pełny model z paskiem zdrowia. | **RUNDA 2 DISPATCHOWANA** | Domain GAME, wizualny — Operator+Evaluator Opus 5 (`gra/src/render/**`, stała zgoda właściciela), Final Control Sonnet 5. Runda 1: recon potwierdził że filtrowanie `deferredPlayerUnitRevealIds` z `syncUnitsRender()` (nie medalion) jest bezsporną przyczyną — token w ogóle nie istnieje w scenie w oknie odroczenia. Zakończona DECISION_REQUIRED (3 opcje). Właściciel wybrał **opcję A: pokazuj token w budowie** (przyciemniony/półprzezroczysty zamiast całkowicie niewidoczny). Runda 2 (dispatch `00-dispatch-runda2.md`, worktree kontynuowany z rundy 1 po synchronizacji z origin/main): jednostki w oknie odroczenia przechodzą do `unitRenderer.sync()` oznaczone jako "w budowie" zamiast być filtrowane, render nadaje im opacity ok. 0.4-0.5, flush przywraca pełną widoczność bez migotania.
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-MGLA-ODKRYCIE-WZDLUZ-SCIEZKI-Q1` | "Jeżeli jednostka szybko się porusza, np. Scout, na duże tereny, to pomimo tego, że przeszła przez jakiś teren, nie odkrywa go... Tu chodzi chyba o [...] opóźnienie [klatek]." | **DISPATCHOWANE** | Domain GAME — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Recon (subagent Explore) OBALIŁ hipotezę o klatkach/wydajności — przyczyna jest DETERMINISTYCZNA: `refreshFog()`/`currentVisible()` liczą widoczność WYŁĄCZNIE z pozycji KOŃCOWEJ ruchu (`main.ts` zakończenie animacji ~31363-31424), nigdy z heksów pośrednich przebytej ścieżki — cała animacja wieloheksowa (`renderLoop`, 31474-31494) przesuwa wyłącznie token wizualnie, bez odświeżania fog. Efekt: Zwiadowca (ruch≫sight) w marszu liniowym nigdy nie odkrywa heksów pośrednich poza zasięgiem widzenia z punktu docelowego — 100% powtarzalne, niezależne od FPS. Istniejący wzorzec do naśladowania: `checkVillageRewardsAlongPath`/`checkBarbCampDestructionAlongPath` już iterują po ścieżce ruchu. Dispatch `00-dispatch.md`. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-USTAWIENIA-MIASTA-PANSTWA-WYLACZONE-Q1` | "W ustawieniach państw-miast, kiedy robimy generator, powinna być opcja nie tylko wyboru trudności [...] ale też 'wyłączone', czyli całkowicie miasta i państwa się nie generują. Są tylko same cywilizacje." | **DISPATCHOWANE** | Domain GAME — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Recon: UI (`newGameFlow.ts:1221-1237`) ma dziś 3 opcje, gotowy wzorzec 4. opcji "Brak" istnieje dla `barbariansLevel` (analogiczny mechanizm). `clampMiastaPanstwaCount` (`newGameMapDefaults.ts:520-528`) wymusza min. 1 — do poluzowania do 0 WYŁĄCZNIE dla nowej opcji. Rdzeń world-genu (`clusters.ts`) już toleruje 0 miast-państw bez awarii (potwierdzone presetem testowym). Dispatch `00-dispatch.md`. |
| `R-MIASTA-PANSTWA-STARTOWE-JEDNOSTKI-Q1` | "Na najtrudniejszym poziomie [...] każde państwo-miasto powinno zaczynać od razu z dwiema jednostkami wojskowymi. Na najłatwiejszym zero, na normalnym jedna jednostka." | **ZAREJESTROWANE, DISPATCH WSTRZYMANY (kolejka main.ts)** | Domain GAME. Recon: dziś miasta-państwa dostają 0 jednostek startowych NIEZALEŻNIE od trudności — `qualifiesForMajorAiDifficultyBonus` (`ai-difficulty-bonus.ts:17-22`) jawnie WYKLUCZA `isCityState` z bonusu startowych jednostek. Wymaga nowej funkcji analogicznej do `spawnDifficultyBonusUnit`, wołanej przy foundowaniu miasta-państwa (`main.ts` ok. 8205-8306) — dotyka `main.ts`, który mają dziś w allowliście DWA już uruchomione tematy (`R-DYPLO-WSPOLNA-WALKA-BARB-KARENCJA-Q1` runda 2, `P-DYPLO-KARTA-DECYZJI-BILANS-SKROT-Q1`) — WSTRZYMANE do zwolnienia main.ts, dispatch napiszę i uruchomię gdy oba się zintegrują. |
...
| `R-MIASTA-PANSTWA-PRODUKCJA-OBRONNA-Q1` | "Państwa-miasta powinny się na początku skupić na budowie jednostek wojskowych, żeby się obronić. W tej chwili nie są w ogóle wyzwaniem. Ewentualnie powinny budować palisadę jako budynki, rekrutować jednostki i na początku skupić się na tym w dużej mierze." | **DISPATCHOWANE** | Domain GAME, balans AI — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Recon: `cityStateMilitaryProductionCap` (`city-state-difficulty.ts:38-47`) dziś maleje z trudnością (hard=4 < easy=bez limitu) — ODWROTNIE niż intencja. `ai.ts:1400-1454` wręcz KARZE (`-90`/`-25` score) budowę murów u miast-państw zamiast preferować. "Palisada" nigdy nie trafia do kandydatów AI (zero wystąpień w `ai.ts`). Dispatch `00-dispatch.md`. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `R-PRODUKCJA-POSTEP-PAMIEC-PO-USUNIECIU-Q1` | "System nie zapamiętuje, ile już postępu zostało wykonane przy budowie budynku... jeżeli usuniemy go z kolejki albo w połowie zrobiony, gdy przywrócimy go znowu do kolejki, powinien być znowu w połowie zrobiony, a nie resetować się od zera... budynki powinny pamiętać swój stan, ile zostało już przeznaczonych środków na ich wybudowanie." | **ABC ZADANE I ODPOWIEDZIANE — DISPATCHOWANE** | Domain GAME. Recon (subagent Explore) potwierdził: to NIE przeoczenie, tylko świadoma, udokumentowana decyzja `P-PROMOCJA-FRONT-RESET-POSTEPU-Q1=B` (2026-08-13, `production.ts:1240-1262`, komentarz wprost: "cancelled item loses ITS OWN progress"). `dequeue()` (production.ts:1264-1289) gubi postęp frontu bezpowrotnie — nie zwraca go nawet do puli Pracy (w przeciwieństwie do `filterQueue()`, które ma `forfeitedPostep` właśnie do tego celu, ale `cancelQueueItem` w `cityPanel.ts:5891-5901` używa `dequeue`, nie `filterQueue`). Brak jakiejkolwiek pamięci per-typ-budynku/miasto niezależnej od samej kolejki. ABC zadane właścicielowi (3 opcje: pełna pamięć per budynek/miasto / tylko front kolejki / zostawić bez zmian) — wybrana odpowiedź: **pełna pamięć per typ budynku w danym mieście, przetrwa usunięcie z kolejki, wymaga migracji zapisu gry**. Wymaga zmian w `production.ts` (`dequeue`/`enqueue`), `cityPanel.ts` (`cancelQueueItem`), `types/city.ts` (nowe pole), `save.ts` (serializacja/migracja) — dispatch do napisania i uruchomienia. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `R-DYPLO-WSPOLNA-WALKA-BARB-KARENCJA-Q1` | "Wspólna walka z barbarzyńcami miała być oddzielną umową, którą można podpisać na 5, 10, 15 tur lub na czas nieokreślony, aż do momentu, w którym jedna ze stron ją usunie. Czas trzech tur miał być wtedy, kiedy wygaśnie umowa... czas bez kary, dający możliwość powrotu jednostkom wojskowym na własne terytorium lub wyprowadzenia ich z terytorium innej cywilizacji, aby nie płacić kar." | **DISPATCHOWANE** (`P-DYPLO-PRZEMARSZ-CHECKBOX-PRZYCISK-Q1` zintegrowane `78151c38`, blokada zdjęta) | Domain GAME. Bieżąca implementacja (`BARBARIAN_COOPERATION_TURNS=3` w `diplomacy-barbarian-cooperation.ts`, użyte jako `wygasaTura` w `diplomacy-proposals.ts:1492,1880`) traktuje 3 tury jako CAŁKOWITY czas trwania samej umowy — niezgodne z zamierzeniem właściciela. Właściwy kształt: (1) czas trwania umowy wybierany przez strony (5/10/15 tur lub bezterminowo do jednostronnego usunięcia — wzorzec identyczny jak `turnChips`/case '2' w `diplomacyTradeBasket.ts`, tylko inne wartości); (2) po wygaśnięciu LUB jednostronnym usunięciu umowy, 3-turowy okres karencji BEZ kary za nieautoryzowany przemarsz (`diplomacy-border-march.ts::applyUnauthorizedBorderPenalties`), dający czas na wycofanie jednostek wojskowych. Wymaga: nowego stanu karencji (kiedy zaczyna się i kończy, per para stron), zmiany w `diplomacy-proposals.ts` (budowa deala z wybieralnym czasem zamiast stałego), UI wyboru czasu zamiast prostego checkboxa/przycisku. WSTRZYMANE do zakończenia już uruchomionego `P-DYPLO-PRZEMARSZ-CHECKBOX-PRZYCISK-Q1` (ten sam plik `diplomacyTradeBasket.ts`, case '4') — dispatch napiszę i uruchomię sekwencyjnie po jego integracji, żeby uniknąć konfliktu dwóch tematów na tym samym pliku w tej samej fali. |
