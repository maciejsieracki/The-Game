...
| R-MP-MODELE-KAMIEN | 2026-07-26 | „wydaje mi się, że państwa miasta używają starych grafik dla jednostek typu kamienia." | **W DIAGNOZIE** (subagent Opus 5) | Najpierw rozstrzygnięcie, czy dispatch modelu (`render/units.ts`) rozjeżdża się dla miast-państw (hipotezy: brak `civId`, inna epoka właściciela, inny `typeId`), potem naprawa przyczyny — bez rozgałęzień po `ownerId` (parytet AI). |
