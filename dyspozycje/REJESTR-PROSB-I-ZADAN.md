...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| P-UI-ZOOM-PRZEGLADARKI-PANELE-UCIETE-Q1 | 2026-09-02 | Panele (min. dyplomacja) uciete gora/dol przy zoomie przegladarki 125%, strona sie nie doskalowuje. | **RECON W TOKU** | Explore dispatchowany — szuka mechanizmu layoutu paneli (fixed vh/px vs relative), viewport meta, czy jest gdziekolwiek `100vh`/sztywna wysokosc bez uwzglednienia zoomu. |
| R-KARTA-JEDNOSTKI-3D-EKSPOZYCJA-UX-Q1 | 2026-09-02 | Lepsza kompozycja/ekspozycja podgladu 3D jednostki na kartach encji — dzis maly plaski box, wlasciciel sugeruje ze moze wymagac nowego UX/designera. | **RECON W TOKU, prawdopodobnie wymaga etapu mockupu designera (wzorem R-SUROWCE-MOCKUP) przed implementacja** | Explore dispatchowany — inwentaryzacja obecnego ukladu (`unitMiniPreview.ts`, kontrakt karty encji renderer.ts) i ograniczen technicznych (rozmiar canvas WebGL, gdzie w gridzie karty jest miejsce). |
| P-KARTA-OPIS-PRZED-STATYSTYKAMI-Q1 | 2026-09-02 | Przeniesc sekcje opisu/rysu historycznego NAD blok statystyk we wspolnym kontrakcie karty encji (jednostki/budynki/cuda/technologie). | **RECON W TOKU** | Explore dispatchowany — lokalizacja wspolnego renderera `entityCards/renderer.ts` i miejsca doklejania sekcji "Rys historyczny" vs blok statystyk, sprawdzenie czy to jeden wspolny punkt (latwa zmiana) czy per-adapter (wieksze rozproszenie). |
