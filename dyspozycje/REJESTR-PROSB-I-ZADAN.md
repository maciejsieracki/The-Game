...
| R-UI-PRZYCISK-ZAKONCZ-TURE-DUPLIKAT-Q1 | 2026-08-21 | Stary i nowy przycisk „Zakończ turę"/„Wykonaj" widoczne razem na ekranie (zgłoszone głosowo). Recon: `hud.ts::renderBarLegacy()` ma własny, drugi przycisk „Zakończ turę", ale `useD1BLayout()` (bramka legacy/nowy) powinno dziś zawsze zwracać `true` (jedyne wywołanie `showHud()` w `main.ts` zawsze przekazuje `onExecutePending`+`mapToolbar`) — więc legacy TEORETYCZNIE nie powinien się renderować. Dokładny mechanizm duplikatu NIE jest jeszcze ustalony. | **OTWARTE — recon w toku, nie rozpoczęta implementacja** | Nie wymaga ABC (bug UI, nie decyzja). Branch: `autobot/R-UI-PRZYCISK-ZAKONCZ-TURE-DUPLIKAT-Q1`. Dispatch: `dyspozycje/autobot/runs/R-UI-PRZYCISK-ZAKONCZ-TURE-DUPLIKAT-Q1/00-dispatch.md`. |
