...
|---|---|---|---|
| `P-DYPLO-BILANS-GATE-NIESPOJNY-N-E1-REPRODUKCJA` | Panel „Wymiana" pokazuje bilans dodatni mimo blokady akceptacji oferty wielo-umowowej (N-E1 potwierdzone ponownie ze zrzutami Macieja) + nowy wątek: możliwy brak odświeżania panelu po edycji koszyka bez usunięcia/ponownego dodania. | **RUNDA 1 ZINTEGROWANA** (`3ca7c600`) — **RUNDA 2 W PRZYGOTOWANIU** | Domain GAME, Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Runda 1: obie pierwotne hipotezy (formuła N-E1/staleness) NIE reprodukowały się na HEAD, ale Evaluator/Obrona znalazły i naprawili trzeci, realny defekt — etykieta „Bilans (Oni)"/„(netto)" zależna od KOLEJNOŚCI dodania wierszy do koszyka, nie od treści oferty (`renderPnBalancePanelHtml` czytał `mode` wyłącznie z wiersza `primary`). Final Control PASS, zintegrowane. **RUNDA 2 (nowe dowody 2026-09-03, DWA zrzuty w przeciwnych kierunkach):** (a) „Stół negocjacji" z Chińczykami — bilans −51, a mimo to zielone „Możesz przyjąć" z aktywnym Przyjmij; (b) trzy umowy z kontrofertą — bilans **+123** (dodatni!), a mimo to czerwone „Nie można przyjąć — warunki niespełnione". Oba kierunki błędu na tym samym mechanizmie dowodzą, że `canAccept` i wyświetlany bilans liczą się NAPRAWDĘ różnymi ścieżkami dla pakietów wielopozycyjnych (zwłaszcza 3+ pozycje z kontrofertą — ten przypadek nie był objęty testem rundy 1). **WIĄŻĄCA REGUŁA właściciela (do wpisania jako GOAL rundy 2):** (1) Przyjmij MUSI być zablokowane dokładnie wtedy, gdy poprawnie policzony bilans jest ujemny dla gracza — `canAccept` sam musi respektować regułę bilans≥0, nie tylko wyświetlanie; (2) AI-autorskie propozycje (`diplomacy-proposals.ts`) też NIE MOGĄ proponować deali o ujemnym dla gracza bilansie. Właściciel zażądał "porządnego audytu" całego mechanizmu, nie punktowej łatki. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-JEDNOSTKA-NIEWIDOCZNA-PO-WYBUDOWANIU-Q1` | Dwa zrzuty ekranu tej samej okolicy: "po wybudowaniu nowej jednostki nie pojawia się ona, tak jakby była niewidoczna... jakby coś później renderuje, a nagle się pojawia... może to wprowadzać graczy w błąd." Zrzut 1: malutki uproszczony znacznik zamiast jednostki; zrzut 2 (później): pełny model z paskiem zdrowia. | **DISPATCHOWANE** | Domain GAME, wizualny — Operator+Evaluator Opus 5 (`gra/src/render/**`, stała zgoda właściciela), Final Control Sonnet 5. Recon (subagent Explore): silnik to Three.js, cały model jednostki buduje się w pełni SYNCHRONICZNIE (`units.ts:1053`, `UnitRenderer.sync()`) — wyklucza hipotezę "model 3D ładuje się z opóźnieniem". Najbardziej prawdopodobny, ale NIE potwierdzony 1:1 mechanizm: medalion właściciela na tabliczce jednostki (`unitOwnerEmblem.ts:328-414`) rysuje tarczę+glif natychmiast, portret/sygnet dociąga asynchronicznie — analogiczny wzorzec do już naprawionego `BUG-IKONA-KULTURY-PLACEHOLDER` w `cityMapStatChip.ts` (`ce69cf45`), ale cache w tym pliku działa inaczej (całу obiekt assetu od razu, nie sam string `'loading'`) — wymaga potwierdzenia żywym testem, nie kopiowania naprawy mechanicznie. Dispatch wymaga od Operatora NAJPIERW żywej reprodukcji z serią zrzutów w headless Chromium, dopiero potem naprawy. Dispatch `00-dispatch.md`.
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-DYPLO-PRZEMARSZ-DUPLIKAT-AKTYWNY-Q1` | Zrzut panelu audiencji: mimo aktywnego "Traktatu przemarszu (wojskowy)" w "Aktywne traktaty", ten sam traktat nadal widnieje jako klikalna, niezablokowana pozycja w "Możliwe umowy". "Powinien być tylko jeden... Tutaj jest błąd. Trzeba było też sprawdzić, jak jest z innymi traktatami. Większość powinna być jednorazowa, poza umową wymiany surowców." | **DISPATCHOWANE** | Domain GAME — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Przyczyna POTWIERDZONA w recon orkiestratora: `resolveDiplomacyActionLock` (`diplomacy-locks.ts:191-196`, case '4') jako JEDYNY z traktatów ('2'/'3'/'5'/'14' mają) nie sprawdza w ogóle, czy umowa już jest zawarta — brak pola `hasGranice` w `DiplomacyActionLockContext` i w budowie kontekstu w `main.ts`. Dispatch obejmuje też lekki audyt informacyjny case '12' (Wasalizacja — ten sam wzorzec braku sprawdzenia, do ustalenia czy zamierzone) i '14' (czy poprawnie wygasa). Dispatch `00-dispatch.md`. |
| `P-DYPLO-PRZEMARSZ-CHECKBOX-PRZYCISK-Q1` | Ten sam zrzut modala traktatu przemarszu: "Poza tym powinny być zamienione na Przyciski, bo wygląda to bardziej profesjonalnie." (checkboxy "Wariant wojskowy (+ opłata)" i "Wspólna walka z barbarzyńcami (3 tury)"). | **DISPATCHOWANE** | Domain GAME, UI — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Wzorzec przycisku-toggle już istnieje w tym samym pliku (`diplomacyTradeBasket.ts`, `cdb-chip`/case '2' czas trwania paktu) — do reużycia zamiast nowego systemu. Dispatch `00-dispatch.md`. |
