...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-DYPLO-KARTA-DECYZJI-DISMISS-Q1` | "Na propozycjach dyplomatycznych powinna być też możliwość wyłączenia tej propozycji... ewentualnie do niej wrócić, jeżeli się da." + "część wydarzeń da się usunąć, to są informacyjne, a propozycji się nie da. Propozycje handlowe też powinny się usuwać jak wszystkie inne." | **DISPATCHOWANE** | Domain GAME, UI — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Recon (subagent Explore): backend JUŻ obsługuje ten przypadek — `handleSidePanelEventDismiss` (`main.ts:19544-19617`) ma gałąź dla `kind==='diplo'` identyczną z mechanizmem "Zignoruj" buntu (`dismissedSidePanelEventIds`, czyszczone co turę — karta wraca jeśli propozycja nadal aktualna, sama propozycja pozostaje osiągalna w panelu audiencji). Brakuje WYŁĄCZNIE przycisku w `sidePanelHud.ts` — dziś tylko bunt ma widoczny "Zignoruj", karty diplo nie mają żadnego. Dispatch `00-dispatch.md`. |
