...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| P-UI-ZOOM-PRZEGLADARKI-PANELE-UCIETE-Q1 | 2026-09-02 | Panele (min. dyplomacja) uciete gora/dol przy zoomie 125%, strona sie nie doskalowuje. | **RUNDA 1 PASS (po Obronie), Final Control w toku** | Recon (Explore): brak jednego wspolnego mechanizmu layoutu — kazdy panel ma WLASNA regule `vh`/scroll. Kluczowe znalezisko: `main.ts:1490-1493` swiadomie ustawia `body.style.overflow='hidden'` (canvas 3D pelnoekranowy), zero fallbacku scrolla strony. Dispatch (`00-dispatch.md`, commit `fc9a60c3`) zawezony do JEDNEGO panelu (audiencja dyplomatyczna), wymagal zywej reprodukcji PRZED fixem. Workflow `wf_dd4ab811-a65`: Operator zywo zreprodukowal uciecie DWOMA mechanizmami (zoom przegladarki `documentElement.style.zoom` ORAZ wewnetrzny zoom UI gry z `hud.ts`) — zoom systemowy/DPI (CDP `setDeviceMetricsOverride`) NIE reprodukowal (vh skaluje sie spojnie pod DPI). Przyczyna: `.civ-diplo-aud{align-items:center}` ("unsafe" centrowanie flex) obcina symetrycznie nadmiar bez sciezki scrolla gdy `.civ-diplo-aud-box{max-height:94vh}` przekracza realnie dostepna wysokosc. Fix (commit `0c7634d1`): `align-items:center→flex-start`+`overflow-y:auto` na `.civ-diplo-aud`, `margin:auto 0` na `.civ-diplo-aud-box` ("bezpieczne" centrowanie — identyczny wyglad przy 100%, box przykleja sie do gory i nadmiar jest scrollowalny gdy brakuje miejsca). Evaluator zlapal 1 zarzut: nowy test regresyjny nie mial `page.screenshot()` mimo ze R-PROC-AUTOBOT.md par.9 pkt 6a wymaga zywego zrzutu dla tematow wizualnych/UX (bariera bez ECHO na oslabienie). Obrona (commit `50857b70`) PRZYJELA, dodala 6 wywolan `page.screenshot()` (dowody PNG w `dyspozycje/autobot/runs/.../dowody/`) i po drodze naprawila ze sekcja "PO" testu nie uzywala `openAudienceWithFiller()` (zrzuty bylyby identyczne piksel-w-piksel bez tej poprawki). Nowy test `diplomacy-audience-zoom-cutoff-real-render-test.cjs` 11/11 PASS, tsc 0 bledow, 5 bramek referencyjnych bez regresu. main.ts i hud.ts NIETKNIETE (nie byly potrzebne — zakres pozostal w oryginalnej allowlisze). Final Control dispatchowany osobnym Workflow `wf_91a8ff71-11f`. Pozostale panele (empireDetailPanel/cityPanel/entityCards/diplomacyPanel) swiadomie POZA zakresem — przyszle, osobne tematy po walidacji podejscia. |
| R-KARTA-JEDNOSTKI-3D-EKSPOZYCJA-UX-Q1 | 2026-09-02 | Lepsza kompozycja/ekspozycja podgladu 3D jednostki na kartach encji — dzis maly plaski box, wlasciciel sugeruje ze moze wymagac nowego UX/designera. | **MAKIETA OPUBLIKOWANA, czeka na akceptacje wlasciciela** | Recon (Explore): podglad 3D renderuje sie w `.entity-card-medallion` — twardy 34x34px (24x24px compact) slot w naglowku, `unitMiniPreview.ts` uzywa cache'owanego, jednorazowego snapshotu WebGL (tani, nie zywy render — powiekszenie nie kosztuje wydajnosciowo). Wczesniejsza migracja T1/T4 do wspolnego kontraktu `EntityCardData` SWIADOMIE zmniejszyla wczesniejszy wiekszy podglad (150px) do dzisiejszego malego medalionu — to nie regres, tylko swiadomy ksztalt kontraktu w tamtym momencie. Potwierdzony silny precedens projektu: tematy "graficzne/wizualne" ida przez R-PROC-AUTOBOT.md par.5a (Operator+Evaluator na Opus 5, obowiazkowy zywy zrzut Playwright) i czesto przez etap mockupu (wzorem R-SUROWCE-MOCKUP: artefakt claude.ai, akceptacja wlasciciela, potem dopiero dispatch implementacji). Zbudowana i opublikowana makieta (artefakt claude.ai, 3 warianty: A-diorama pelnej szerokosci/rekomendacja, B-powiekszony medalion, C-dwie kolumny desktop) w skali 1:1 wzgledem prawdziwego `ENTITY_CARD_CSS`. Czeka na wybor/akceptacje wlasciciela przed dispatchem implementacji (Opus 5). |
