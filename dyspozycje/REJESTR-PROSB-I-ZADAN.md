...
| R-AI-KUP-JEDN | 2026-07-24 | AI NIE ma ścieżki „kup jednostkę za złoto" (`purchaseRecruitmentUnit` main.ts:2054 zablokowane do `ownerId===0`). Maciej 2026-07-24: „działać" = **naprawić parytet**. | **W TOKU** (po integracji Zadania 1 — te same pliki) | Plan: uogólnić `purchaseRecruitmentUnit` na dowolnego ownera (skarb z `aiSkarbiecByOwner`) + konserwatywna decyzja AI: kupuje za złoto TYLKO gdy (a) w stanie wojny, (b) front kolejki miasta to już jednostka (przyspiesza, nie zmienia planu), (c) złoto ≥ próg rezerwy, (d) miasto ma Manpower, (e) max 1 zakup/turę/civ. Próg = PLACEHOLDER do strojenia. Parytet: mechanizm ownerId-agnostic. |
