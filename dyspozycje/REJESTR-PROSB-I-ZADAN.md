...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| R-ZELAZO-MODELE-BRAKUJACE-Q1 | 2026-08-24 | Właściciel poprosił o listę jednostek epoki Żelazo, które nie przeszły procesu naprawy modeli 3D „Opus 5" (precedens: `R-BRAZ-SUPER-DISPATCH-Q1`, zamknięty dla epoki Brąz). Audyt (subagent Explore) znalazł 4 jednostki bez dedykowanego modelu (Falanga, Jeździec z oszczepami, Konnica lancowa asyryjska, Konnica łucznicza asyryjska — ta ostatnia dodatkowo dzierży kopię zamiast łuku) + 2 jednostki dzielące identyczny model (Soldurii/Gaesatae, Celtowie). ECHO właściciela: wszystkie 6 dostają nowe, dedykowane, historycznie uzasadnione modele w stylu serii Opus 5. Jawna zgoda na pełną autonomię (workflow, pętla, deploy+push bez check-inów). | **W TRAKCIE — dispatch T1/4 wystartowany** | Pełny opis, tabele audytu i podział na 4 sekwencyjne tematy (T1 Asyria-konnica ×2, T2 Celtowie Soldurii/Gaesatae, T3 Falanga, T4 Jeździec z oszczepami/Słowianie): `docs/decyzje/R-ZELAZO-MODELE-BRAKUJACE-Q1.md`. Ślad: `dyspozycje/autobot/runs/R-ZELAZO-MODELE-BRAKUJACE-Q1-T{1..4}/`. |
