...
| `R-MIASTA-PANSTWA-STARTOWE-JEDNOSTKI-Q1` | "Na najtrudniejszym poziomie [...] każde państwo-miasto powinno zaczynać od razu z dwiema jednostkami wojskowymi. Na najłatwiejszym zero, na normalnym jedna jednostka." | **DISPATCHOWANE** | Domain GAME. Odblokowane — oba tematy blokujące main.ts (`R-DYPLO-WSPOLNA-WALKA-BARB-KARENCJA-Q1`, `P-DYPLO-KARTA-DECYZJI-BILANS-SKROT-Q1`) zintegrowane. Recon: dziś miasta-państwa dostają 0 jednostek startowych NIEZALEŻNIE od trudności — `qualifiesForMajorAiDifficultyBonus` (`ai-difficulty-bonus.ts:17-22`) jawnie WYKLUCZA `isCityState` z bonusu startowych jednostek. Wymaga nowej funkcji analogicznej do `spawnDifficultyBonusUnit`, wołanej przy foundowaniu miasta-państwa (`main.ts` ok. 8205-8306) — dotyka `main.ts`, punkt inny/nienakładający się z pozostałymi aktywnymi tematami. Dispatch `00-dispatch.md`. |
