...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| P-UI-ZOOM-PRZEGLADARKI-PANELE-UCIETE-Q1 | 2026-09-02 | Panele (min. dyplomacja) uciete gora/dol przy zoomie 125%, strona sie nie doskalowuje. | **DISPATCHOWANE, Workflow w toku** | Recon (Explore): brak jednego wspolnego mechanizmu layoutu — kazdy panel (diplomacyAudience/diplomacyPanel/empireDetailPanel/cityPanel/entityCards) ma WLASNA regule `vh`/scroll. Kluczowe architektoniczne znalezisko: `main.ts:1490-1493` swiadomie ustawia `body.style.overflow='hidden'` (canvas 3D pelnoekranowy) — zero fallbacku scrolla strony gdy panel jest wyzszy niz viewport. Mechanizm zoomu (przegladarka/DPI systemu/wewnetrzny zoom UI gry `hud.ts`) NIE zostal jednoznacznie potwierdzony przez recon. Dispatch (`00-dispatch.md`, commit `fc9a60c3`) zawezony do JEDNEGO panelu ze zgloszenia (audiencja dyplomatyczna), wymaga zywej reprodukcji WSZYSTKICH trzech mechanizmow PRZED fixem, z jawnym DECISION_REQUIRED jesli zaden nie reprodukuje. Workflow `wf_dd4ab811-a65`. Pozostale panele swiadomie POZA zakresem tej rundy (przyszle, osobne tematy po walidacji podejscia). |
...
| P-KARTA-OPIS-PRZED-STATYSTYKAMI-Q1 | 2026-09-02 | Przeniesc sekcje opisu/rysu historycznego NAD blok statystyk we wspolnym kontrakcie karty encji (jednostki/budynki/cuda/technologie). | **DISPATCHOWANE, Workflow w toku** | Recon (Explore): JEDEN wspolny punkt wstawienia — `renderer.ts::renderEntityCard` (linie 313-337) doklejal `historia` PO sekcji `body` przez zwykla sekwencje `card.appendChild()`, nie pozycje w tablicy `sections`. Wszystkie 4 adaptery + wonderAdapter.ts (cuda uzywaja TEGO SAMEGO renderera, nie osobnego systemu) tylko wypelniaja dane, zaden nie steruje kolejnoscia DOM — jeden reorder naprawia wszystkie 5 rodzajow encji. Test asercji kolejnosci `entity-card-historia-section-test.cjs:220-236` (do odwrocenia w tym samym dispatchu). `wikiHubHud.ts` (CivPedia) to NIEZALEZNA sciezka markdown, swiadomie POZA zakresem. Dispatch (`00-dispatch.md`, commit `26f24f6b`), Workflow `wf_2aa675af-e68`. |
