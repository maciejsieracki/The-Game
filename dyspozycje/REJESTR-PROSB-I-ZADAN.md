...
| R-MIASTA-LIMIT-PODBOJ-SILA-LICZY-SIE-Q1 | 2026-09-01 | Wlasciciel: ustawil limit miast epoki Kamien na 15, ma 21 miast. Po recon (dyplomatyczne wchloniecie omija limit) wlasciciel doprecyzowal DWUKROTNIE: to NIE bylo wchloniecie, tylko podboj SILA (bitwa/kapitulacja) - te maja SIE LICZYC do limitu, tak jak zalozone. Swiadome ODWROCENIE wczesniejszej decyzji `R-MIASTA-LIMIT-PODBOJ-Q1=A` (ktora wylaczala podbite miasta z limitu) na wyrazne, potwierdzone zyczenie wlasciciela. Wchloniecie dyplomatyczne (annexCityStateToOwner) JUZ DZIS poprawnie liczy sie do limitu (nie dotyka flagi foundedByOwner) - zero zmian tam potrzebne, wczesniejsza hipoteza orkiestratora o „brakujacym foundedByOwner=false" w tej funkcji byla bledna i zostala odrzucona. | **RUNDA 1/5 — dispatch zapisany, Workflow uruchomiony** | Dispatch: `dyspozycje/autobot/runs/R-MIASTA-LIMIT-PODBOJ-SILA-LICZY-SIE-Q1/00-dispatch.md`. Fix: usuniecie `city.foundedByOwner = false` w DWOCH miejscach (`post-battle-map.ts:488`, `main.ts:12629`). Swiadomie POZA zakresem: blokowanie samego podboju przy przekroczonym limicie (osobna, wieksza decyzja produktowa, niezadana przez wlasciciela). `city-limit-conquered-test.cjs` do przepisania (odwrocone asercje). |
