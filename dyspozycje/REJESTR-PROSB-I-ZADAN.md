...
| P-FARMA-COFNIJ-ZWRACA-PRACE-NIEAKTUALNY-WPIS-Q1 | 2026-08-27 (dispatch 2026-09-02) | Znalezisko Final Control z tematu `R-ULEPSZENIA-FARMA-LESIE-USUN-ISTNIEJACE-Q1` (`03-final-control.md` par.6 "ZNALEZISKO 2"), swiadomie zostawione poza GOAL tamtego tematu, zarejestrowane osobno, nigdy nie zdispatchowane az do dzis. Kontynuacja petli AutoBot po zamknieciu CivPedii Fazy 2/3. Recon (Explore): `undoPendingBuildRequest` (`gra/src/main.ts:11711-11743`) bezwarunkowo zwraca `pending.kosztPraca` bez sprawdzenia czy `req.key` faktycznie nadal istnieje w `placedImprovements.get(req.hexKey)` - desynchronizacja mozliwa gdy `sweepLegacyFarmsOnForest` (main.ts:12199-12217) usunie farme z lasu niezaleznie od kolejki `pendingImprovementsTurn`, ktora o tym nie wie (osobna struktura, zero synchronizacji). Exploit: gracz cofa wpis dla juz nieistniejacego ulepszenia i dostaje Prace za nic. | **RUNDA 1 PASS, Final Control w toku** | Dispatch zapisany (`dyspozycje/autobot/runs/P-FARMA-COFNIJ-ZWRACA-PRACE-NIEAKTUALNY-WPIS-Q1/00-dispatch.md`, commit `21f699fc`). Sciezka A (Workflow), Operator Sonnet 5 effort=medium -> Evaluator Sonnet 5 effort=high. Workflow `wf_683bec3d-634`: Operator PASS (commit `ebe493b4` na galezi `autobot/P-FARMA-COFNIJ-ZWRACA-PRACE-NIEAKTUALNY-WPIS-Q1`, diff wylacznie `undoPendingBuildRequest` w `main.ts` +14/-2 + nowy `gra/tools/farma-cofnij-nieaktualny-wpis-test.cjs` 21/21 PASS na naprawionym kodzie, kontrola negatywna na kodzie sprzed poprawki: 19/21, dokladny dowod starego buga "Cofnięto — Praca zwrócona (0)" mimo nieistniejacego ulepszenia). Evaluator: ZERO zarzutow po niezaleznej weryfikacji (merge-base diff, tsc, 5 bramek, wlasne uruchomienie testu na naprawionym i kontrola negatywna na kodzie sprzed poprawki - wlasna, nie tylko odczyt raportu Operatora). Obrona pominieta (lista zarzutow pusta). tsc 0 bledow, 5 bramek referencyjnych bez regresu obustronnie potwierdzone. Final Control dispatchowany osobnym Workflow `wf_9bdee3f8-a90` (wlasny izolowany worktree, swieza niezalezna weryfikacja + wlasna kontrola negatywna). |
