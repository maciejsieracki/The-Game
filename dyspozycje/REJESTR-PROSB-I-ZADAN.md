...
| R-UNIT-KOSZT-ŁUCZ | 2026-07-24 | Łucznicy brązowi = 1 Brąz czy 0? | **ZDEPLOYOWANE** (verify main `28819c7`) | `gra/data/units.json`: Łucznik L171-178, Łucznik akadyjski L2193-2194, Łucznik asyryjski L2973-2974 — `Surowiec: "-"` · `Surowiec (ilość): 0` (wszystkie `Rola: Dystans`). AutoBot VERIFY 2026-08-05. |
...
| R-PALAC-KOSZT | 2026-07-24 | Pałac (budynek startowy) nie może mieć kosztu surowcowego — na starcie pula = 0, więc pierwszego Pałacu nie da się postawić. Zostawić koszt PRODUKCJI (Praca). | **DO WDROŻENIA** | **GAP verify 2026-08-05:** `gra/data/buildings.json` palac L1037-1039 nadal `koszt_surowce: {drewno:8}` — oczekiwane C-PALAC-Q1=A: usunąć surowce, zostawić `kosztBudowy:40`. `docs/decyzje/VERIFY-WDROZONE-KOD-REST.md` |
...
| R-PIERWSZE-MIASTO | 2026-07-27 | Decyzja **B** (Maciej): pełna blokada — tylko „Załóż miasto", bez ruchu/innych akcji osadnika. Tylko gracz (AI: nie). | **ZAMKNIĘTE ABC** — kod CZĘŚCIOWO | `docs/decyzje/R-PIERWSZE-MIASTO.md` · brakuje: `planMarchTo`, `onSelectType`, panel osadnika. |
...
| R-SUROWCE-DOSTEP | 2026-07-26 | „sprawdź, czy w surowcach jest miejsce na surowce, które tylko trzeba mieć dostęp… powinno być chociaż zasugerowane miejsce na surowce, które są dostępem" + „trzeba dodać złoto". | **DO WDROŻENIA** | **GAP verify 2026-08-05:** `main.ts` `buildEmpireResourceRows` L2335-2371 — logika `dostep`/`zrodlo` OK, ale wszystkie wiersze dostają `cap: empireCap` → `empireDetailPanel.ts` L736 `access = rows.filter(r => r.cap == null)` zawsze puste; podsekcja „Dostęp — nie magazynowane" (L763) nie renderuje kafelków. Brak `cap` undefined dla ceramika/sól/koń. |
| R-TURA-JEDN-AKTYWNA | 2026-07-26 | „po zakończeniu tury, okazuje się zwiadowca czy armia, ale nie w formie takiej, że jest zaznaczona i można gdzieś ruszać, tylko po prostu jakoś bezimiennie. Więc wydaje mi się, że po zakończeniu tury powinna się pojawiać możliwość od razu ruchu jednostką z możliwością wybrania kierunku. Jeżeli już coś albo w ogóle nie powinna się ta armia pokazywać." | **ZDEPLOYOWANE** (verify main `28819c7`) | `main.ts` L2666+L20845 `ruchLeft: ruch` przy spawnie · L8169-8185 `afterPlayerUnitSpawned` select+kamera · C-TURA-Q1=A · AutoBot VERIFY 2026-08-05 |
...
| R-DYP-NEGOCJACJE-NA-ZYWO | 2026-07-26 | negocjacje w audiencji na bieżąco, nie po turze | **ZDEPLOYOWANE** (verify main `28819c7`) | `main.ts` L11429 `resolveNegotiationEntryAt` — C-DYP-Q1=B natychmiast po propozycji gracza · `docs/decyzje/R-DYP-NEGOCJACJE-NA-ZYWO.md` · FALA 18 |
...
| R-RZEKI-UJSCIE-FALA138 | 2026-08-01 | regres: rzeki kończą się w środku lądu | **GOTOWE (kod)** | Fix `9c4320b`: ensureRiverOutlets po topUp + po finalizeCoastAndInlandWater; scrubStrayRiverHexMarks. Smoke 12 map: 0 bez ujścia, 0 sierot. tsc PASS. Czeka deploy. |
...
| R-SCENA-PERF-FALA138 | 2026-08-01 | Budowanie sceny: **bardzo długo** | **🔵 W TOKU** · Q1=A · F12 wstrzymane (fokus R-AUTO) · instr. w FALA 226 | `docs/decyzje/R-SCENA-PERF.md` · handoff sesji 2026-08-05 |
| R-SPAWN-CLUSTER-KULTURA | 2026-08-01 | Jakość klastrów: cywilizacje jednego typu czasem lądują w kręgu innego typu zamiast razem wokół siebie (stolica + mp) | **ZDEPLOYOWANE** (verify main `28819c7`) | `gra/src/map/clusters.ts` L2294 `assignTypesToClusterCenters()` + wywołania L3431/L3491 · AutoBot VERIFY 2026-08-05 |
...
| R-MP-NAZWA-CIV-MISMATCH | 2026-08-01 | MP: nazwa miasta ≠ kultura (Jin + Argos·Grecy); Chińczycy oznaczeni jako Grecy | **ZDEPLOYOWANE** (verify main `28819c7`) | `cluster-spawn.ts` L362-366 `pendingSameTypeRivalOwnerIds` · `main.ts` L5498+L6285 `spawnPendingSameTypeRivals` · AutoBot VERIFY 2026-08-05 |
...
| R-TEREN-DOPIAC | 2026-07-26 | **Decyzja C-TEREN-Q1 = A** — trzy etapy terenu bitwy. | **KOD GOTOWY** (`gra/src/`, test 26/26). **C-TEREN-IMPL-1=A:** jeden deploy (nie 3×). Czeka na build Integratora. | `teren-walki-etapy-test.cjs` |
...
| ZNALEZISKO-86 | 2026-07-27 | Decyzja **A**: % HP + pasek jak `postBattleSummary` w „Szczegóły bitwy". | **ZAMKNIĘTE ABC** — kod CZĘŚCIOWO | `docs/decyzje/ZNALEZISKO-86.md` · maxHp jest; brak % i paska. |
...
| R-BUD-LOKALIZACJA-UI | 2026-07-26 | „jeżeli budynek jest tylko dla regionalnych terenów, to nie powinien się wyświetlać w stolicy, a jeżeli jest dla stolicy, nie powinien się wyświetlać w miastach regionalnych." | **ZDEPLOYOWANE** (verify main `28819c7`) | `cityPanel.ts` L7161 `if (e.locationBlocked) return false` w filtrze listy budynków · AutoBot VERIFY 2026-08-05 |
| R-SKARBIEC-PRZECINEK | 2026-07-26 | „po ostatnich zmianach skarbca … miało nie być błędu po przecinku danych. Ale jednocześnie trzeba pamiętać, żeby dane się nie traciły przy zaokrągleniu." (zrzut: „Skarbiec +6.600000000000005") | **ZDEPLOYOWANE** (verify main `28819c7`) | `ui/formatPl.ts` L29-40 `formatLiczbaPl`/`signedPl` · importy `hud.ts` L27, `cityPanel.ts` L221, `empireDetailPanel.ts` L17 · prezentacja only |
