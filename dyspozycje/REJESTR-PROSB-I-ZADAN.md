...
| R-HUD-SUROWCE | 2026-07-24 | Surowce niewidoczne w górnym HUD (są tylko w panelu imperium → „SUROWCE STRATEGICZNE"). Dodać **chip „Surowce"** obok Skarbca/Pracy + **osobna zakładka magazynu surowców**. Relayout: **Naukę** przenieść na PRAWO (obok Zaopatrzenia/Ludności/Kultury), **Surowce** na LEWO (obok Skarbca/Pracy). | **W TOKU — implementacja (mockup zaakceptowany przez Macieja)** | HUD dziś: lewo Skarbiec·Praca·Nauka / prawo Zaopatrzenie·Ludność·Kultura (hud.ts). Licznik surowców istnieje (R-LICZNIK) ale w panelu imperium, nie w HUD. Najpierw mockup zakładki → potem designer → potem wdrożenie. |
...
| R-PARYTET-SUROWCE-MP | 2026-07-24 | Zweryfikować: czy inne cywilizacje AI ORAZ miasta-państwa też mają surowce i płacą nimi (parytet ekonomii surowcowej z graczem). | **W WERYFIKACJI (recon)** | Wstępny recon: produkcja (`computeTerritoryResourceYieldByCity`) i cap (`reconcileOwnerResourceCaps`, turn-economy.ts:1704) ownerId-agnostic; AI civ płacą z puli (main.ts ~14420). Miasto-państwo = owner z `city.startCityState`. Otwarte: czy MP przechodzą ekonomię i czy w ogóle budują/handlują. Agent Explore sprawdza 4 punkty. |
