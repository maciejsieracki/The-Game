...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `P-DYPLO-PRZEMARSZ-DUPLIKAT-AKTYWNY-Q1` | Zrzut panelu audiencji: mimo aktywnego "Traktatu przemarszu (wojskowy)" w "Aktywne traktaty", ten sam traktat nadal widnieje jako klikalna, niezablokowana pozycja w "Możliwe umowy". "Powinien być tylko jeden... Tutaj jest błąd. Trzeba było też sprawdzić, jak jest z innymi traktatami. Większość powinna być jednorazowa, poza umową wymiany surowców." | **RUNDA 1 FAIL — RUNDA 2 W PRZYGOTOWANIU** | Domain GAME — Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Sam fix logiki (`diplomacy-locks.ts`/`main.ts`, pole `hasGranice`) w pełni poprawny i zweryfikowany żywo przez Evaluatora i Final Control. Final Control złapał JEDEN defekt, którego Evaluator nie złapał: nowy plik testu (`diplomacy-przemarsz-duplikat-real-render-test.cjs`) uruchomiony BEZ zmiennej env (domyślnie, tak jak uruchomi go każda przyszła bramka) asercjuje że bug MA WYSTĄPIĆ — na poprawionym kodzie to `exit 1`. Odwraca konwencję repo (`*_SRC_DIR`: domyślnie = poprawiony kod = zielono). Runda 2 (ten sam ID/gałąź, worktree `/home/user/wt-dyplo-przemarsz-duplikat`, commit `70b5667a`): Operator zmienia domyślny tryb testu na PO (zielono na dzisiejszym kodzie), PRZED jako jawny opt-in (`DPD_MODE=PRZED`). |
