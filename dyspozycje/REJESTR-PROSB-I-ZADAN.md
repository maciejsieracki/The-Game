...
| R-DYPLOMACJA-HANDEL-BRAMKA-PRIORYTET-Q1 | 2026-08-06 | Priorytet bramek uczciwość vs chęć do handlu | **FAIL Evaluatora runda 1** — mechanizm modyfikatora działa, ale komunikat odrzucenia podaje fałszywą przyczynę (winą obarcza niechęć AI, gdy realną przyczyną jest zwykła nieuczciwość oferty) + martwa strefa mnożnika przy Relacji>~111; runda 2 w toku | `docs/decyzje/R-DYPLOMACJA-HANDEL-BRAMKA-PRIORYTET-Q1.md` · B+C połączone |
...
| UNIT-REPLACE-EVOCATI-Q1 | 2026-08-06 | unit-replace-test.cjs 2/10: Evocati znika z listy "Zastąp" | **FAIL Evaluatora runda 1** — Operator naprawił tylko test (dodał empireResourceStock do kontekstu testowego), ale Evaluator udowodnił że to REALNY BUG PRODUKCYJNY: `main.ts` (`replaceAvailabilityCtxForCity`/`replaceAvailabilityCtxEmpireWide`) nigdy nie przekazuje `empireResourceStock` do `availableReplacementsFor` — od FALI 96 (`daacd43`, DOSTEP-SUROWCE-Q1) mechanizm "Zastąp" w realnej grze gubi WSZYSTKIE jednostki brązowe/żelazne; runda 2 (naprawa silnika, nie testu) w toku | `dyspozycje/PYTANIA-OTWARTE.md` do dopisania jeśli runda 2 się przeciągnie |
