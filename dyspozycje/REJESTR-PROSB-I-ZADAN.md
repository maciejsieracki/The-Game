...
| `P-PRACA-PANEL-IKONY-NIESPOJNE-Q1` | `NOWE — recon zamknięty, WYMAGA ABC przed dispatchem` | Zgłoszenie regresu (2026-08-22): panel miasta „PODZIAŁ PRACY" ma DWIE różne ikony dla „Ulepszenia" (`tb-build` młotek vs `chip-crate` skrzynka, ta druga dodana commitem `bd03ed3e`/Wątek F 2026-08-21). Brak jednoznacznego wzorca w kodzie (panel globalny z tego samego dnia nie ma ikon w ogóle) — wymaga ABC właściciela: która ikona, czy w ogóle ikony w panelu miasta. Pełny opis w `PYTANIA-OTWARTE.md`. |
