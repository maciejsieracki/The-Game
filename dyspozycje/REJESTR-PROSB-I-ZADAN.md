...
| `R-HANDEL-SZLAKI-PRZEBUDOWA-Q1` | `ECHO ZAPISANE (5/5) — gotowe do dispatchu` | Przebudowa mechaniki szlaków handlowych. ECHO (2026-08-21, pełna treść `docs/decyzje/R-HANDEL-SZLAKI-PRZEBUDOWA-Q1.md`): odwrócenie zależności od dystansu osobno per medium (ląd max=12/morze max=20, identyczny szczyt); bonus morski ×2 sumuje się z istniejącym `PORT_SEA_TRADE_BONUS_PIENIADZ`; 5% przypisane per trasa jako stały procent JEJ WŁASNEGO dochodu, zastępuje stary globalny mnożnik w `economy.ts` (realny transfer budżetu); limit „jedna umowa" bez zmian (już istnieje per-para, temat T5 wypada z zakresu); Port zostaje wymogiem istnienia trasy morskiej + NOWA zasada: morze rozważane tylko gdy ląd fizycznie niemożliwy (inny kontynent/wyspa) — ląd ma bezwarunkowe pierwszeństwo w `detectBestConnection`, nie wybór po dochodzie. Stawki ×5 tylko na stałych dystansowych. Podział T1(wzór)→T2(morze×2+priorytet lądu)→T3(rozdzielenie gatingu budynkami)→T4(atrybucja 5%)→T6(UI). Nie rozpoczęte — gotowe do dispatchu Operatora T1. |
