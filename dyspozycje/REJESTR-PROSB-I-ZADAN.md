...
| `R-FEATURE-KARTY-ENCYKLOPEDIA-CIVPEDIA-Q1` | `DEPLOY-ROBOCZA (T4/T8, FALA 309/311); T5 ZINTEGROWANE` | T1+T1b+T3 ZDEPLOYOWANE FALA 307, T4 FALA 309, T8 (kategoria Technologie, 32/32) FALA 311. **T5 (migracja karty budynku w panelu miasta) ZINTEGROWANE do `main`** — `buildingAdapter.ts` w pełni wypełniony, nowy addytywny tryb `hover` w `renderer.ts`, stara implementacja jako `_legacyBuildBuildingDetailCard` fallback. Operator+Evaluator+Final Control PASS niezależnie (real-Chromium hover-layout test złapał i naprawił 2px overflow). Czeka na deploy ROBOCZA. Następny krok: T6 (zależny od T5) i T9 (zależny od T8) dispatchowane. |
