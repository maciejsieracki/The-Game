...
| R-KARTA-JEDNOSTKI-3D-EKSPOZYCJA-UX-Q1 | 2026-09-02 | Lepsza kompozycja/ekspozycja podgladu 3D jednostki na kartach encji — dzis maly plaski box, wlasciciel sugeruje ze moze wymagac nowego UX/designera. | **RUNDA 1 PASS (po Obronie), Final Control w toku** | Recon (Explore): podglad 3D renderuje sie w `.entity-card-medallion` — twardy 34x34px (24x24px compact) slot w naglowku. Makieta (artefakt claude.ai, 3 warianty) — **wlasciciel zaakceptowal Wariant A**. Dispatch (`00-dispatch.md`, commit `3d9dd86c`): diorama ~190px zastepuje maly header, ten sam zamontowany podglad powiekszony+wysrodkowany, tytul/podtytul jako overlay, kolejnosc DOM diorama->historia->body, tryb compact bez zmian. Sciezka A (Workflow), Operator+Evaluator Opus 5 (temat graficzny), Final Control Sonnet 5. Workflow `wf_941a64bc-334`: Operator PASS (commit `2ce99aae`, nowy `entity-card-diorama-real-render-test.cjs` 33/33). Evaluator — 5 PONUMEROWANYCH zarzutow, wszystkie realne, zmierzone w zywym Chromium: (1) regres trybu compact — nowa regula CSS specyficznoscia bila pozycjonowanie przycisku zamkniecia w `showTechDiscoveryNotice`, przycisk przeskakiwal o 34px; (2) fallback "Render 3D niedostepny" nieczytelny na obu sciezkach (przegrywal specyficznoscia LUB byl przyciety przez nowy `overflow:hidden`); (3) elipsa "gruntu" praktycznie niewidoczna (roznica pikselowa 8/255), a asercja testu byla tautologiczna (sprawdzala tylko obecnosc wezla); (4) tryb compact jednak nieznacznie zmienial rozmiar SVG (28->24px), wbrew GOAL "calkowicie bez zmian"; (5) brak `01-operator.md` w katalogu runu (naruszenie INDEX-PROCESU.md). Obrona (commit `0d4e67e4`) PRZYJELA wszystkie 5 z dowodami: usunieta kolidujaca regula CSS (przycisk wraca do absolute/10px), fallback dostal wlasna regule ze zwiekszona specyficznoscia (11px, pelne zdanie czytelne, `scrollH===clientH`), elipsa przerobiona na jasny radial-gradient kontakt-swiatla (pomiar pikselowy: 3473/82080 zmienionych pikseli, bylo tautologiczne), SVG w compact przywrocone do rozmiaru wlasnego (28px), zapisany `01-operator.md`. Test rozszerzony do 46/46. Final Control dispatchowany osobnym Workflow `wf_cae47f92-6dc` (wlasny detached worktree, wlasny niezalezny pomiar wszystkich 5 napraw + otwarcie zrzutow PNG). |
