...
| ID | Data | Prośba | Status | Uwagi |
|---|---|---|---|---|
| P-WYDARZENIA-DEDUP-KONIEC-TURY-Q1 | 2026-08-20 | Panel boczny wydarzeń (`sidePanelHud.ts`) stackuje wiele identycznych kart informacyjnych „Koniec tury" (ta sama treść, np. „Wyrąb: +25 Drewna (pozostało 0 tury)" powtórzona per miasto) zamiast łączyć je w jeden wpis. Poza zakresem `DYSPOZYCJA-WDROZENIE.md` Karty 3 — brief pokrywał wyłącznie rozróżnienie blokująca/informacyjna i kolejkę dla blokujących, nie deduplikację treści informacyjnych. | **READY_FOR_DEPLOY (Final Control, izolowany branch) — czeka na integrację** | Operator PASS (19/19 nowych testów, 0 regresji) → Evaluator PASS (adwersaryjnie, bez zmian kodu) → Final Control PASS (osobny subagent, READY_FOR_DEPLOY). Branch: `autobot/P-WYDARZENIA-DEDUP-KONIEC-TURY-Q1`, poza `main`, bez push. Ślad: `dyspozycje/autobot/runs/P-WYDARZENIA-DEDUP-KONIEC-TURY-Q1/`. |
| P-TECHNOLOGIA-POPUP-KARTA-ODKRYCIA-Q1 | 2026-08-17 | Ogólny wzorzec karty odkrycia technologii. | **ECHO=A ZAPISANE, RECON ZAMKNIĘTY (2026-08-21)** | Recon wykazał 2 realne bugi w żywym kodzie FALI 300 (widmowe/nieaktualne nazwy ulepszeń terenu + systemowy zły dobór ikony) — naprawa wydzielona jako osobny temat `R-TECH-ULEPSZENIA-TERENU-SYNC-Q1`. Branch: `autobot/P-TECHNOLOGIA-POPUP-KARTA-ODKRYCIA-Q1`. Ślad: `dyspozycje/autobot/runs/P-TECHNOLOGIA-POPUP-KARTA-ODKRYCIA-Q1/01-operator-recon.md`. |
| R-UI-PRZYCISK-ZAKONCZ-TURE-DUPLIKAT-Q1 | 2026-08-21 | Stary i nowy przycisk „Zakończ turę"/„Wykonaj" widoczne razem (zgłoszone głosowo). | **RECON ZAMKNIĘTY — brak bugu w kodzie** | 3 hipotezy (podwójny montaż `bottomBarHud.ts`, osobny przycisk `preBattle.ts`, race condition `cfg`) wykluczone niezależnie zweryfikowanym recon. Najbardziej prawdopodobna przyczyna: stary zbuforowany build przeglądarki (ten sam mechanizm co karta technologii). Rekomendacja: ABC do właściciela — czy duplikat utrzymuje się po twardym odświeżeniu. Branch: `autobot/R-UI-PRZYCISK-ZAKONCZ-TURE-DUPLIKAT-Q1`. |
| R-UI-OBRAMOWKA-PASEK-OSTRZEGAWCZY-Q1 | 2026-08-21 | Diagonalny pasek `.sp-blk-stripe` nad kartą blokującą wyglądał jak taśma ostrzegawcza (zgłoszone głosowo). | **OPERATOR PASS — w toku pętli (Evaluator dalej)** | Pasek usunięty, zastąpiony istniejącymi sygnałami karty (rant+poświata+plakietka), zgodnie z jedynym wzorcem panelu imperium (`.civ-emp-thr.now`, tło+border na całym elemencie). Testy: toolbar 19/19, deadzone 43/43, tsc czysty — zweryfikowane niezależnie przez orkiestratora. Branch: `autobot/R-UI-OBRAMOWKA-PASEK-OSTRZEGAWCZY-Q1`. |
| R-TECH-ULEPSZENIA-TERENU-SYNC-Q1 | 2026-08-21 | Naprawa dwóch bugów znalezionych w recon `P-TECHNOLOGIA-POPUP-KARTA-ODKRYCIA-Q1`: widmowe/nieaktualne nazwy ulepszeń terenu w `tech.json` (Brązownictwo, Murarstwo, Oswojenie zwierząt, Wojskowość) + systemowy zły dobór ikony w `techDiscoveryNotice.ts` dla wszystkich technologii z tą sekcją. | **OTWARTE — dispatch Operatora w toku** | Nie wymaga ABC (bug danych/kodu, kierunek jasny z recon). Allowlista ograniczona do `tech.json` (4 pola) + `techDiscoveryNotice.ts`; `terrain-improvements.json` NIETKNIĘTY (kanon B1-tech-MACIEJ-2026-06-29.md). Branch: `autobot/R-TECH-ULEPSZENIA-TERENU-SYNC-Q1`. Dispatch: `dyspozycje/autobot/runs/R-TECH-ULEPSZENIA-TERENU-SYNC-Q1/00-dispatch.md`. |
