...
|---|---|---|---|
| `P-DYPLO-BILANS-GATE-NIESPOJNY-N-E1-REPRODUKCJA` | Panel „Wymiana" pokazuje bilans dodatni mimo blokady akceptacji oferty wielo-umowowej (N-E1 potwierdzone ponownie ze zrzutami Macieja) + nowy wątek: możliwy brak odświeżania panelu po edycji koszyka bez usunięcia/ponownego dodania. | **DISPATCHOWANE, RUNDA 1 W TOKU — nowy dowód doszedł 2026-09-03** | Domain GAME, Operator+Evaluator+Final Control Sonnet 5, effort HIGH. Recon-first: dwie konkurencyjne hipotezy (formuła N-E1 vs staleness UI) do rozstrzygnięcia żywym testem w headless Chromium, nie zgadywaniem. Dispatch `00-dispatch.md`. **NOWY DOWÓD (zrzut ekranu, panel „Stół negocjacji" z Chińczykami, pakiet Traktat+Umowa surowców):** BILANS (NETTO) = −51, jednocześnie zielone „Możesz przyjąć — przewaga u Ciebie o 51 PW" z aktywnym przyciskiem Przyjmij, JEDNOCZEŚNIE notatka „WPŁYW RELACJI NA DEAL −61,2% · Relacja 38,8 · musisz dać więcej (×2,6 PW), by oferta była uczciwa" — trzy wzajemnie sprzeczne sygnały naraz. Zlokalizowane w kodzie (orkiestrator, `diplomacyAcceptanceBalance.ts` `verdictHtml` ~403-457): `data.canAccept` (właściwa brama) i `net`/`balancePn` (wyświetlana liczba) to DWA NIEZALEŻNE pola — tu się rozjechały, mimo że wcześniejsza poprawka `R-DYPLOMACJA-BILANS-UNIFIKACJA-Q1` (komentarz linia 380-391) miała temu zapobiegać dla pakietów wielopozycyjnych. **WIĄŻĄCA REGUŁA właściciela do wpisania jako kryterium końca tej rundy (lub rundy 2, zależnie na jakim etapie jest Operator gdy ten wpis trafi do kontekstu):** (1) przycisk Przyjmij MUSI być zablokowany, gdy poprawnie policzony bilans jest ujemny dla gracza — nie wystarczy poprawić WYŚWIETLANIE, `canAccept` sam musi respektować regułę bilans≥0; (2) AI-autorskie propozycje (`diplomacy-proposals.ts`, strona generująca oferty AI) też NIE MOGĄ proponować deali o ujemnym dla gracza bilansie — to dotyczy generowania ofert, nie tylko bramki UI. Właściciel zażądał "porządnego audytu" całego mechanizmu bilans/akceptacja, nie tylko punktowej łatki. Zgodnie z R-PROC-AUTOBOT.md §2b NIE dispatchowano równoległego tematu na te same pliki — czekamy na wynik tej rundy, wciągamy nowy dowód+regułę do niej. |
...
| ID | Prośba | Status | Uwagi |
|---|---|---|---|
| `R-DYPLO-PROG-NAP-90-DO-110-Q1` | "Zmieńmy jeszcze pakt o nieagresji z 90 na 110." | **DISPATCHOWANE** | Domain GAME, balans — Operator+Evaluator+Final Control Sonnet 5, effort MEDIUM. Ten sam wzorzec co wcześniejsza zmiana `progNapRelacja` 50→90 w tej samej sesji (FALA 340, `R-BALANS-PAKT-NIEAGRESJI-I-GLINA-Q1`) — czysta decyzja balansu, bez ABC. Dwa źródła prawdy: `gra/src/game/diplomacy.ts:204` i `gra/data/diplomacy.json:46`. `ai.ts` czyta pole dynamicznie (bez własnej kopii liczby), podchwyci zmianę automatycznie. Dispatch `00-dispatch.md`.
