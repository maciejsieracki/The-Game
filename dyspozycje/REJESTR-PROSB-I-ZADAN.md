...
| `R-PRACA-SUWAKI-DUPLIKAT-I-CAP-MIASTO-Q1` | `FAIL (runda 2, blokada proceduralna — do rundy 3)` | Watki A-F: logika POPRAWNA i pokryta testami (`tsc` 0; `praca-limit-50-test` 23/23, `praca-miasto-limit-50-cap-test` 46/46, `praca-miasto-limit-50-test` 33/33, `praca-split-ui-test` 14/14, `praca-pula-rate-parity-test` 3/3 + regresja zielona). Final Control r2: **FAIL wyłącznie proceduralny** — branch (worktree lokalny, commit `e21bc639`, NIE na origin) odgałęził się przed 3 commitami już wdrożonymi na `main` (runda 3 pasek-ostrzegawczy + scrollbar popup-odkrycia, FALA 304); diff `main..HEAD` cofałby te dwie zamknięte paczki w `bottomBarHud.ts`/`sidePanelHud.ts`/`techDiscoveryNotice.ts`. Raport: `.../runs/R-PRACA-SUWAKI-DUPLIKAT-I-CAP-MIASTO-Q1/05-final-control-r2.md` (na razie tylko w worktree). Następny krok: Operator runda 3 — rebase na aktualny `main`, ograniczyć diff do allowlisty Watków A-F, ponowna weryfikacja, Evaluator→Final Control. **NIE zdeployowane.** |
| `R-FEATURE-KARTY-ENCYKLOPEDIA-CIVPEDIA-Q1` | `DEPLOY-ROBOCZA (T4, FALA 309)` | T1+T1b+T3 ZDEPLOYOWANE FALA 307. **T4 (migracja karty jednostki na mapie, `unitInfoCard.ts`, 3D-podgląd) ZDEPLOYOWANE FALA 309** — runda 1 (migracja, publiczna sygnatura bez zmian, stara implementacja jako fallback) + runda 2 (fix CSS `.entity-card-badges`/`.entity-card-badge`, znaleziony przez Evaluatora w żywej przeglądarce). Evaluator+Final Control PASS niezależnie na scalonym `main` (197/197 testów na 6 bramkach, mutation-test potwierdzony, `git merge-tree` bez konfliktów). Merge `54c3bb1c` → `main`. Następny krok: T5 (karta budynku w panelu miasta). |
