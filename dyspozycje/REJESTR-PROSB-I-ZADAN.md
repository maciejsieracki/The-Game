...
| `P-HOTSEAT-ETAP7-FIXTURE-BUMP-Q1` | `NOWE — follow-up (znalezisko Etapu 7, nie blokuje niczego dzisiejszego)` | `gra/tools/barb-camp-blacklist-test.cjs` i `gra/tools/fsa-autosave-test.cjs` mają fixture'y zapisów w starym formacie (`wersja:1`/`wersja:2`) — po integracji formatu v3 (`R-HOTSEAT-ETAP7-SAVELOAD-Q1`) te bramki są/będą czerwone (`barb-camp-blacklist-test.cjs` crashuje, `fsa-autosave-test.cjs` 2 asercje failują), potwierdzone jako fixture, nie regres logiki zapisu. Wymaga osobnej, małej rundy bumpującej fixture'y do `wersja:3`. |
