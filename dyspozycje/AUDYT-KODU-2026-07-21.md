# AUDYT KODU — Civ „The Game” (2026-07-21)

> Audyt wieloagentowy READ-ONLY (nikt nie zmienial kodu). 12 audytorow x obszary + adwersaryjna weryfikacja (sceptycy probowali obalic kazde znalezisko).
> Przerwany na etapie weryfikacji rundy 2 (awaria API 522 + limit sesji). Znaleziska z rundy 1 oznaczone POTWIERDZONE przeszly 2 niezaleznych sceptykow — 0 odrzuconych z 20. Znaleziska rundy 2 i zawieszone: wykryte, ale weryfikacja przerwana — do potwierdzenia przy naprawie (56 werdyktow ktore zdazyly sie policzyc = wszystkie realny=true).

**Lacznie 73 znalezisk.** Rozklad wag: 2 krytyczna · 31 wysoka · 25 srednia · 15 niska

---

## 1. [KRYTYCZNA] Koszyk PN: oddanie 'jednostka' nic nie kosztuje dawcy — darmowy zakup zasobow AI

- **Plik:** `gra/src/main.ts` (ok. linii 3437)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/manpower.ts` (ok. linii 279)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/turn-economy.ts` (ok. linii 1176)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/main.ts` (ok. linii 11545)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/victory.ts` (ok. linii 39)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/map/gen-helpers.ts` (ok. linii 1601)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/audio/filePlayer.ts` (ok. linii 347)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/audio/muzyka-antyczna.ts` (ok. linii 1601)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/data/units.json` (ok. linii 2287)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/data/units.json` (ok. linii 619)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 12082)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 8081)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 12759)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 13670)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 6667)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/ui/cityPanel.ts` (ok. linii 706)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 6972)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/game/diplomacy-proposals.ts` (ok. linii 455)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/diplomacy-value-catalog.ts` (ok. linii 301)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/diplomacy-proposals.ts` (ok. linii 287)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
## 22. [WYSOKA] Rozjazd kierunku Respektu: silnik trzyma udzial AI, UI liczy udzial gracza

- **Plik:** `gra/src/main.ts` (ok. linii 11586)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
## 23. [WYSOKA] Atrycja garnizonu przy oblężeniu zmienia licznik pochodny — bez realnego efektu

- **Plik:** `gra/src/main.ts` (ok. linii 11008)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/ai.ts` (ok. linii 1364)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
## 25. [WYSOKA] Odbicie miasta rebeliantow = falszywa ELIMINACJA frakcji -99 + powtarzalne Power-zdobycze (exploit)

- **Plik:** `gra/src/main.ts` (ok. linii 9797)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 10562)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
## 27. [WYSOKA] cityFogVisible: pelne currentVisible() liczone osobno dla kazdego obcego miasta przy kazdym sync

- **Plik:** `gra/src/main.ts` (ok. linii 3610)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/units/setup.ts` (ok. linii 696)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/ai.ts` (ok. linii 1475)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
## 30. [WYSOKA] mousemove: pelne currentVisible() przy kazdym zdarzeniu ruchu myszy z zaznaczona jednostka

- **Plik:** `gra/src/main.ts` (ok. linii 8321)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 11822)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/production.ts` (ok. linii 711)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 2293)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/empire-food.ts` (ok. linii 61)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/economy.ts` (ok. linii 767)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/turn-economy.ts` (ok. linii 1306)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/ai.ts` (ok. linii 404)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/map/wonder-placement.ts` (ok. linii 33)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/audio/muzyka-antyczna.ts` (ok. linii 1782)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
## 40. [SREDNIA] Wyjscie do menu w trakcie bitwy nie resetuje ambBattleMuted — nowa gra z niema natura

- **Plik:** `gra/src/main.ts` (ok. linii 7069)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/data/buildings.json` (ok. linii 933)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 13576)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 1303)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 13741)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/ui/diplomacyTradeBasket.ts` (ok. linii 176)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/game/diplomacy-proposals.ts` (ok. linii 423)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 3377)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 12012)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 11178)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 10090)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 10041)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 5527)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/battle/mapFieldBattle.ts` (ok. linii 219)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/combat.ts` (ok. linii 539)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/post-battle-map.ts` (ok. linii 341)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/ai.ts` (ok. linii 1521)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 1227)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 1532)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/economy.ts` (ok. linii 697)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/main.ts` (ok. linii 8102)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/game/playerState.ts` (ok. linii 159)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/map/generator.ts` (ok. linii 283)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/map/startScoring.ts` (ok. linii 80)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/data/map-gen-params.json` (ok. linii 99)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/audio/filePlayer.ts` (ok. linii 280)
- **Status:** POTWIERDZONE (2x sceptyk, 0 odrzucen)
...
- **Plik:** `gra/src/main.ts` (ok. linii 3417)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/data/units.json` (ok. linii 1179)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 12699)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/ui/gamePauseMenu.ts` (ok. linii 109)
- **Status:** wykryte (runda 1), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 2661)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/game/civ-roster.ts` (ok. linii 102)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/main.ts` (ok. linii 10897)
- **Status:** wykryte (runda 2), weryfikacja przerwana
...
- **Plik:** `gra/src/render/cities.ts` (ok. linii 585)
- **Status:** wykryte (runda 2), weryfikacja przerwana
