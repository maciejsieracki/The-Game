# Karawanseraj

## Metadane

| Pole | Wartość |
|------|---------|
| **id** | `karawanseraj` |
| **tytuł** | Karawanseraj |
| **kategoria** | Miasto — budowa |
| **poradnik_ref** | `docs/PORADNIK-GRACZA/45-katalog-budynkow.md` |
| **json_ref** | `buildings.json` |
| **status_v1** | ✅ |

---

## Wiki‑S

**Karawanseraj** — budynek (Pieniadz), epoka Brąz. Koszt od **25** pracy, utrzymanie **2** ¤/t. Technologia **Handel**.

---

## Wiki‑M

### Co robi
Karawanseraj wzmacnia miasto w kategorii **Pieniadz**. Poziom 1: **+6 złota** (+3 na poziom), **+8 % mnożnika handlu** (+3 na poziom). Maksymalnie **10** poziomów — każdy kosztuje więcej pracy (+**10** od poprzedniego), ale daje większy przyrost.
### Koszty
- **Budowa poz. 1:** 25 pracy
- **Każdy kolejny poziom:** +10 pracy
- **Utrzymanie:** 2 ¤/turę (+1 ¤/poziom)
- **Surowiec do budowy:** 4× ceramika (poz. 1; wymaga Garncarstwa + Cegielni/Garncarni w mieście)
- Technologia **Handel**.
- **Uwaga:** Mnoznik % dotyczy handlu ladowego (szlaki miedzy miastami)
### Strategia gracza
Rozwijaj, gdy masz nadwyżkę pracy w imperium — nie blokuj kolejki wojska w mieście granicznym.
### Typowe błędy
- Budowa bez technologii (szara na liście) — sprawdź drzewko nauki.
- Ignorowanie utrzymania: 2 ¤/t × 10 poziomów × kilka miast = wyczerpanie skarbca.
- Rush za złoto „na siłę" przy pustym skarbcu — najpierw Targowisko / podatki.
**Powiązane:** Produkcja miejska · Utrzymanie · Pieniadz

---

## Przykład liczbowy

**Scenariusz:** miasto ma **10 pracy/t** na budynki (suwak pracy 70%, miasto produkuje ~14 pracy/t).

| Etap | Koszt pracy | Czas budowy (~) | Co daje (poz. 1) | Utrzymanie |
|------|-------------|-----------------|------------------|------------|
| Poziom 1 | 25 | **3 tur** | +6 złota, +8 % mnożnika handlu | 2 ¤/t |
| Poziom 2 | 35 | **4 tur** | więcej (patrz niżej) | 2 ¤/t |
| Poziom 3 | 45 | — | **+12 złota, +14 % mnożnika handlu** | 2 ¤/t |

Przy +6 złota/t, utrzymanie 2 ¤/t → netto **+4 ¤/t**. Koszt 25 pracy przy 10/t ≈ **3 tur** pracy — złotem „zwraca się" po ok. **7 tur** (uproszczenie, bez inflacji).

**Przyspieszenie za złoto:** jeśli brakuje **5** pracy po 2 turach, możesz dokupić rush — koszt rośnie z pozostałą pracą (szczegóły w Części VII poradnika).

---

## Poradnik‑L

→ `docs/PORADNIK-GRACZA/45-katalog-budynkow.md`

---

## Historia / decyzje

Wygenerowano z danych gry · rev. E 2026-07-03 (pogłębienie + przykłady).
