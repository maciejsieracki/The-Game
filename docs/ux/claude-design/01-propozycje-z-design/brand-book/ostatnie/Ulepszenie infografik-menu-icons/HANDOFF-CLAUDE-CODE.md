...
## HANDOFF v2 — mapa ekran → komponent (integracja gra/src)
Tokeny **FROZEN v1.0** (`eksport/tokens.css` / `.json`). Poniżej sugerowane mapowanie makiet `.dc.html` → komponenty TS w `gra/src` (Cursor decyduje o strukturze docelowej).

| Ekran (`brand-book/`) | Proponowany komponent | Dane / źródło |
|---|---|---|
| Przegląd (1E) | `gra/src/ui/DevOverview.ts` (tylko dev) | — |
| Ekran Menu (1E) | `gra/src/ui/MainMenu.ts` | routing: NewGame/Settings |
| Ekran Kreator + Kreator Kroki | `gra/src/ui/NewGameWizard.ts` (kroki 1–5) | preset gry |
| HUD Kit (1E) | `gra/src/ui/Hud/` (ResourceBar, Toolbar, LeaderBanners, Minimap, EndTurnCompass) | stan gry |
| Ekran Miasto (1E) | `gra/src/ui/CityPanel.ts` | miasto, produkcja, rail |
| Ekran Dyplomacja (1E) | `gra/src/ui/DiplomacyScreen.ts` | lista cywilizacji, oferty |
| Ekran Walka + Walka Warianty | `gra/src/ui/Combat/` (PreBattle, BattleHud, Siege) | jednostki, prognoza |
| Ekran Badania (1E) | `gra/src/ui/TechTree.ts` | **tech.json** (repo) |
| Ekran Wojsko (1E) | `gra/src/ui/ArmyList.ts` + `gra/src/ui/UnitCard.ts` | jednostki, armie |
| Ekran Koniec Gry / Porażka | `gra/src/ui/GameOver.ts` (variant: win/lose) | wynik |
| Komponenty (1E) | `gra/src/ui/` (Toast, Tooltip, Slider, Card, ProgressBar, TechNode) | — |
| Motion (1E) | `gra/src/styles/motion.css` (@keyframes tg-*) | — |

**Zasady integracji:**
- Wszystkie kolory/geometria z `--tg-*` (tokens.css) — nie hardkodować hex w komponentach.
- Ikony: `eksport/icons/` — Tier 1 `tier1/`, Tier 2 `tier2/` (warianty `-24`/`-40`). `currentColor` → kolor dziedziczony z rantu medalionu.
- Semantyka ikon zamknięta (§6). Dyplomacja = uścisk dłoni (`tb-diplomacy.svg`) wszędzie.
- TechTree i ArmyList czytają realne dane z repo (`tech.json`, definicje jednostek) — makieta 1E to warstwa wizualna.
- Klasy pomocnicze w `tokens.css`: `.tg-medallion(.--active)`, `.tg-btn-primary`, `.tg-btn-default`, `.tg-panel`.
