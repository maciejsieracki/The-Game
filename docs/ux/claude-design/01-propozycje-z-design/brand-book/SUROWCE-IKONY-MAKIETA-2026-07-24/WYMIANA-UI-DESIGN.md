...
- 2026-07-24 · **KOŃ — łeb konia wg referencji Macieja** ✅ — res-horse (profil w prawo: kark, dwoje uszu, wcięcie pyska, żuchwa) + res-horses (drugi łeb zza karku); stara ikona była nieczytelnym zawijasem. W paczce SUROWCE v4.
- 2026-07-24 · **SUROWCE — ikony v2 (decyzja Macieja: kształty + WYJĄTEK kolorystyczny)** ✅ — przy złotej obwódce medalionu kreski ikon w kolorach: drewno = PIENIEK podłużny (złoto) · kamień = 2 KOSTKI jedna na drugiej, BIAŁE · cegła = kostka 3D CZERWONA · ruda miedzi = 2 GRUDKI POMARAŃCZOWE · ruda żelaza = 3 GRUDKI SREBRNE · brąz = sztabka ZIELONA (patyna) · żelazo = TA SAMA sztabka SREBRNO-SZARA · stal = PODŁUŻNA sztabka SZARA. resource-icon-map.json + resources-preview.html zaktualizowane. Tooltip makiety przeniesiony poza panel (fix nakładania z weryfikacji).
- 2026-07-24 · **SUROWCE — ikony + makieta Magazynu** ✅ — (a) 6 NOWYCH ikon `eksport/icons/resources-map/`: res-copper-ore (żyłki) · res-iron-ore (kryształ) · res-brick (blok 3D) · res-bronze (sztabka) · res-steel (sztabka+iskra) · res-ceramics (amfora) — koniec interimowego kolorowania medalionów; do kompletu `resource-icon-map.json` (id gry → ikona, aliasy) + `resources-preview.html` (9 starych + 6 nowych). (b) NOWA makieta kanonu `KANON/mockupy/The Game - Surowce magazyn i formy v1 (1E).dc.html`: klatka A = pełna zakładka „Magazyn Państwa / Surowce imperium" (paski stanów zielony/złoty/czerwony, PEŁNY, dostępowe chipy Ceramika/Sól/Koń, nota Żywność, legenda, tooltip wiersza: typ · źródło · konsument · reguła magazynu); klatka B = 3 formy uproszczone (HUD chips · miasto-budowa · rekrutacja wg epoki BRĄZ/ŻELAZO). Paczka: `_dist/SUROWCE-IKONY-MAKIETA-2026-07-24`.
- 2026-07-23 · **PORTRETY — medaliony z obwódką 1E** ✅ — nowy wariant `eksport/portraits/medaliony/portrait-{civ}-{epoka}-medalion.png` (45×: kamien/braz/zelazo × 15 civ): koło na przezroczystym tle, winieta przy krawędzi, złota obwódka kanonu (#6a5212 rant · #e8d88a pierścień · akcent #f4e6a8). Kwadraty = surowiec pod obwódki kontekstowe UI; medaliony = gotowe do wstawienia. Dołożone do paczki PORTRETY-ZELAZO FINAL.
- 2026-07-23 · **PORTRETY — korekta Macieja: swap Brąz↔Żelazo dla Greków i Chińczyków** ✅ — zamienione medaliony i wersje full (nowe full odtworzone 1:1 z arkuszy źródłowych); mapa bez zmian (te same ścieżki). Paczka nakładkowa: `_dist/PORTRETY-SWAP-2026-07-23` (8 PNG — nadpisać po wgraniu PORTRETY-ZELAZO).
- 2026-07-23 · **PORTRETY ŻELAZA** (Zlecenie 6, część 1/2) ✅ — arkusz Gemini od Macieja pocięty 1:1 jak Kamień/Brąz (poz. 2 = Rzymianie, potwierdzone): 15× medalion 418×418 (boki wtopione, nakrycia głów całe) + 15× full 251×418; `portrait-map.json`: klucz `zelazo` wypełniony, epoka `antyk` dopisana (null — czeka na arkusz, część 2/2); `portraits-preview.html`: trzeci rząd. Paczka: `_dist/PORTRETY-ZELAZO-2026-07-23`.
- 2026-07-23 · **IKONY 32 TECHNOLOGII** (Zlecenie 7) ✅ — komplet: `eksport/icons/tech/tech-<slug>.svg` (32 szt., line-art 1E, stroke currentColor, viewBox 24, jak icons/units) + `eksport/tech-icon-map.json` (klucz = id z tech.json, pole name = nazwa PL) + `eksport/tech-icons-preview.html`. Użycie: węzły drzewka + panel badań. Paczka: `_dist/TECH-IKONY-32-2026-07-23`. · Do paczki CUDA-SWIATA-v1 dołożony STANDALONE (wymóg proxy).
- 2026-07-23 · **CUDA ŚWIATA — galeria Antyku v1** (Zlecenie 4, TURA 2) — nowy ekran: 19 cudów 1:1 z `gra/data/wonders.json` w pasmach epoki wejścia (Kamień 2 · Brąz 9 · Żelazo 8), dostęp E/R, wszystkie stany (dostępny · zablokowany-brak-techa · w budowie z postępem · nasz ✓ [not. ukοńczenia] · przepadł · ekskluzywny-cudzy · zbudowany-cudzy); klatka B: karta cudu ×3 stany (koszt Pracy, tury, wymogi ✓/✗, efekty ×każde-miasto, reguła atrakcji po Średniowieczu); klatka C: powiadomienia nasz-vs-cudzy (racing przepada). Bez emoji, tekstowe efekty. Plik: `KANON/mockupy/The Game - Cuda swiata v1 (1E).dc.html`. Parkowane ep.4+ poza galerią.
