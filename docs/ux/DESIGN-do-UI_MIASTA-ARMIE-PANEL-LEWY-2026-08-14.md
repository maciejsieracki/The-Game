...
## 5. Zgłoszenia — do decyzji integratora
...
2. **Rozbicie `productionLine` — jedyna zmiana danych w całym zleceniu.** Pasek postępu i wyrównana
   ...
   Jeśli to zbyt duża zmiana — wypada pasek, liczba wraca w linii tekstu, karta zostaje.
3. **Plakietki „stolica" i „nowe" nie mają dziś źródła danych.** „kolejka pusta" wynika z braku
   `productionLine`, więc działa od razu; dwie pozostałe wymagałyby flag w `CityListEntry` (stolica
   jest znana grze, „nowe" to arbitralny próg wieku miasta). Pokazane jako propozycja, nie jako
   gotowe do portu — plakietka jest opcjonalna w każdym wierszu.
