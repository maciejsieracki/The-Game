...
### [2026-06-28] MACIEJ → EKONOMIA (via SILNIK): zlecenie wykonania

**Od:** Maciej · **Przekazał:** Grupa F  
**Czat:** **Civ-EKONOMIA** · komenda: **`start`**  
**Zadanie:** EKO-P2-01 tick B5 · B1 tech po ABC Macieja  
**Manifest:** `dyspozycje/MACIEJ-DELEGACJA-LANE-2026-06-28.md`

---

### [2026-06-28] Grupa F (SILNIK) → EKONOMIA: przekazanie delegacji

**Od:** Grupa F · routing MASTER Work  
**Twoje zadanie:** `dyspozycje/EKONOMIA.md` § DO ZROBIENIA TERAZ — **EKO-P2-01** (pełny tick B5)  
**Handoff:** `_handoff/EKONOMIA-do-SILNIK_B5-empire-food.md` · HUD B5 już w silniku  
**B1 tech drzewko:** `docs/decyzje/B1-tech-ABC-OTWARTE.md` — **CZEKA litery Macieja** (Q1–Q5) · wspólnie z CYW  
**Komenda w czacie Civ-EKONOMIA:** `start`  
**Po GOTOWE:** meldunek tutaj + `→ SILNIK: GOTOWE` jeśli dotyczy kontraktu silnika  
**NIE ruszaj `main.ts`.**
...
## 2026-06-26 — SESJA lane (Composer): audyt + RDY-01 civBonusy

**Audyt stanu:** Żelazo/Stal już w `resources.json` (16 wpisów). RDY-11 (mnoznikHandelPieniadz per-cyw + Waluta×2) już w `economy.ts`/`turn-economy.ts`. RDY-12 compound `buildingValue` już zaimplementowany.

**ZROBIONE (kod lane, bez main.ts):**
- `economy.ts`: `civBonusyForCivKey`, `civEconomyYieldMultipliers`; ctx `civHandelMult`/`civNaukaMult` → Grecy +15% handel, Inkowie +15% nauka.
- `turn-economy.ts`: lookup bonusów z `data.civs` przez `ownerCivByOwnerId` (bez zmiany sygnatury — działa gdy SILNIK podaje mapę).
- `production.ts`: `civRecruitmentDiscount` + ulga Zulusi Impi -10% w `availableProduction` i `unitPurchaseCost`.
- `terrain-improvements.json` `_meta.klucze_surowcow_ASCII`: dodano `zelazo`, `stal`.
- `dyspozycje/EKONOMIA-STAN.md` — pierwszy STAN file (Faza A S1.6).

**NIE RUSZANE (świadomie):** `main.ts`, Wealth wpiecie, plaster EKONOMIA+UI, Lazaret (baseline-red oczekiwany).

**BLOKERY ABC dla Macieja:** patrz EKONOMIA-STAN.md + sekcja poniżej.

---

## 2026-06-26 — SESJA lane EKONOMIA (Composer subagent)

**RDY-11 DONE (kod):** `mnoznikHandelPieniadzForCiv` + `ctx.walutaMnoznikOverride` w economy.ts; `advanceCityEconomy(..., ownerCivByOwnerId)` w turn-economy.ts. Handoff: `_handoff/EKONOMIA-do-MASTER_mnoznik-per-cyw.md`. MASTER: wpina mapę civ w main.ts (1 linia).
**1A surowce:** resources.json + Żelazo (surowy) + Stal (przetworzony).
**NIE ruszano:** lazaret/koszary-gate (baseline-red wg Macieja), main.ts, Wealth (D3).
**Testy do uruchomienia:** currency-test (+5), logic-test, wire-ekonomia-test.
**STAN:** utworzono `dyspozycje/EKONOMIA-STAN.md`.

---

## 2026-06-26 — Sprint 1 lane EKONOMIA (Composer subagent)

**ZROBIONE:**
- `gra/tools/civ-bonusy-test.cjs` — RDY-01: Grecy +15% handel, Inkowie +15% nauka, Zulusi -10% rekrutacja (`civEconomyYieldMultipliers`, `cityYieldPerTurn` ctx, `civRecruitmentDiscount`, `unitPurchaseCost`).
- Handoff D3=C: `_handoff/EKONOMIA-do-MASTER_wealth-minimal-D3C.md` (Wealth minimal — WIRE 3 w turn-economy gotowy, MASTER: typ City + wealthZadowolenie→MIASTO + HUD).
- Handoff D2=A: `_handoff/EKONOMIA-do-MASTER_plaster-D2A.md` (splitPraca WIRE 2 gotowy; gate terytorialny API w cities.ts — predykat od MAPA).
- `EKONOMIA-STAN.md` zaktualizowany (~74%).

**Testy (do uruchomienia lokalnie z `gra/`):**
```bash
...
```
Oczekiwane: civ-bonusy ~22 PASS, currency zielony, wealth 25/25, wire-ekonomia zielony.

**NIE RUSZANO:** `main.ts`, Lazaret/koszary-gate (baseline-red).

**MASTER kolejka:** BLK-02 plaster (D2=A) → D3=C Wealth typ/HUD → ownerCivMap (RDY-11).

---

## 2026-06-26 — D4=B Excel ulepszeń (Master Work)

**ZROBIONE:** `MIASTO/Ulepszenia-terenu.xlsx` wygenerowany z `terrain-improvements.json` (`gen-ulepszenia-xlsx.py`, 15 wierszy). Kolumna **Komentarz Naster** (żółta) — Maciej wpisuje uwagi. Opis tekstowy: `EKONOMIA/EKONOMIA-ulepszenia-terenu-v01.md`. Podgląd HTML: `Civ-MAPA/Gra-podglad-ULEPSZENIA.html`.

**CZEKA:** akceptacja Macieja (D4=A) lub poprawki w Excelu → `export-ulepszenia.py` → JSON → dopiero wtedy SILNIK wpiecie (wątek #9).

---

## 2026-06-27 — podziałHandlu / podziałPracy per City (1A, 3A)

**ZROBIONE (Composer lane EKONOMIA):**
- `gra/src/game/cities.ts` — `CityPodzialHandlu`, `CityPodzialPracy`, pola `podzialHandlu?` / `podzialPracy?`; init w `foundCity` / `foundCityAt` (default 20/70/10 Handel, 70% Praca→budynki).
- `gra/src/game/turn-economy.ts` — `toEconomyCity()` czyta podział z miasta; `splitPraca` używa `city.podzialPracy` gdy jest.
- `gra/tools/wire-ekonomia-test.cjs` — scenariusz per-city luksus 30% (luksus=3 vs default 10% → luksus=1).
- Handoff: `_handoff/EKONOMIA-do-MASTER_podzial-per-city.md`.

**Backupy:** `cities.ts.bak-EKONOMIA-2026-06-27`, `turn-economy.ts.bak-EKONOMIA-2026-06-27`, `wire-ekonomia-test.cjs.bak-EKONOMIA-2026-06-27`.

**Test:** `node tools/wire-ekonomia-test.cjs` — oczekiwane wszystkie OK (w tym sekcja „Per-city podzialHandlu: luksus 30%”).

**NIE RUSZANO:** `main.ts`. UI suwak Społeczeństwo w panelu miasta = osobny task (lane UI).

**MASTER:** brak wymaganego wpiecia silnika — pola już konsumowane w `advanceCityEconomy`; opcjonalnie playtest dwóch miast z różnym luksusem.

---

## 2026-06-26 — SPEC: żywność hybrydowa (miasto + zapasy państwa + głód wojska)

**Decyzja:** Maciej Q1 HUD mapa (custom hybryda). **Handoff źródło:** `_handoff/MACIEJ-do-EKONOMIA_zywnosc-hybrid.md`. **Status:** SPEC + stub typów; tick **NIE** zaimplementowany (następny batch lane). **main.ts:** NIE ruszany.

### Model (1 strona)

**Dwie warstwy żywności:**

| Warstwa | Stan | Konsumpcja | UI |
|---------|------|------------|-----|
| **Miasto** | `city.magazynZywnosci` | populacja × `zywnosc_zuzytka_populacja` | panel miasta (bez zmian intencji) |
| **Imperium** | `EmpireFoodState.zapasyPanstwa` per `ownerId` | armia × `militaryFoodConsumption` | HUD mapy + suwak splitu |

**Suwak splitu (global per owner):** `% rozwój miast` / `% zapasy państwa` (suma = 100). **Default: 70 / 30** (`suwak_zywnosc_rozwoj_domyslnie`).

**Przepływ tury (docelowy):**

```
advanceCityEconomy (istniejący)
  per miasto: zywnoscBrutto z pól+budynków
  USUNĄĆ wojsko z netto miasta (ctx.wojskoZuzycieZywnosci → 0)
  netto miasto = brutto − populacja (tylko część „rozwój” ze splitu)
  populationGrowth / magazyn lokalny — bez zmian
  obleżenie (WIRE 4) — bez zmian (garnizon z magazynu miasta)

advanceEmpireFood (NOWY — empire-food.ts)
  per owner:
    S = Σ zywnoscBrutto miast ownera
    doRozwoju = floor(S × procentRozwoj / 100)      // już w pipeline miasta*
    doPanstwa = S − doRozwoju                       // lub floor(S × (100−R)/100)
    zapasy += doPanstwa
    kosztArmii = militaryFoodConsumption(units, upkeepParams)
    zapasy −= kosztArmii
    glodWojska = zapasy < 0

UNITS: applyArmyStarvationAttrition (glodWojska → −8% maxHP/tura)
```

\* **Implementacja splitu (rekomendacja techniczna):** przed `populationGrowth` per miasto: `zywnoscMiasto = floor(zywnoscBrutto_miasto × procentRozwoj/100) − pop×zużycie`; reszta brutto miasta idzie do puli `doPanstwa` agregowanej per owner. Alternatywa: split na sumie imperium, potem proporcjonalny rozdźiał „rozwój” na miasta wg udziału brutto — **v1.0 = proporcjonalnie do brutto miasta** (sprawiedliwe multi-city).

**Cap zapasów państwa:** brak (v1.0). **Głód:** `zapasyPanstwa < 0` po koszcie armii → `glodWojska=true`.

### Parametry econ-params.json (normal)

| Klucz | easy | normal | hard | Uwagi |
|-------|------|--------|------|-------|
| `suwak_zywnosc_rozwoj_domyslnie` | 70 | **70** | 60 | % brutto → miasta |
| `zywnosc_jednostka_ruch` | 1 | **1** | 2 | reuse — koszt armii (marsz/garnizon) |
| `zywnosc_jednostka_oboz` | 0.38 | **0.5** | 0.62 | reuse — obóz |
| `glod_wojska_hp_frac` | 0.06 | **0.08** | 0.10 | atrycja UNITS |

Zapasy państwa = `100 − suwak_zywnosc_rozwoj_domyslnie` (nie osobny klucz — unikamy rozjazdu sumy).

### Pliki lane (stan po SPEC)

| Plik | Stan |
|------|------|
| `gra/src/game/empire-food.ts` | **STUB** typy + API read; `advanceEmpireFood` throws |
| `gra/data/econ-params.json` | +2 klucze (`suwak_zywnosc_rozwoj_domyslnie`, `glod_wojska_hp_frac`) |
| `turn-economy.ts` / `cities.ts` | bez zmian ticku (następny batch) |

### Handoffy cross-lane

- `_handoff/EKONOMIA-do-UNITS_glod-8hp.md` — atrycja −8% maxHP, `RuntimeUnit.hp/maxHp`, DoD testu
- `_handoff/EKONOMIA-do-UI_zywnosc-hud.md` — `HudState.zapasyPanstwa`, `glodWojska`, suwak 70/30

### MASTER — wpiecie (kolejność)

1. `Map<ownerId, EmpireFoodState>` w save/load + `bindEmpireFoodRuntime`.
2. Po `advanceCityEconomy`: `advanceEmpireFood(...)` (gdy zaimplementowane).
3. UNITS: `applyArmyStarvationAttrition` na `units[]`.
4. HUD: `getEmpireFoodReserve` / `isArmyStarving` → `HudState`.

### DoD EKONOMIA (kod — następny batch)

- [ ] `advanceEmpireFood` + test `tools/food-army-test.cjs`
- [ ] Wyłączenie `wojskoZuzycieZywnosci` z netto miasta (armia tylko z zapasów państwa)
- [ ] `buildEmpireFoodParams()` z econ-params (mirror `buildEconParams`)

**NIE RUSZANO:** `main.ts`, pełny tick, kanon.

---

## 2026-06-27 — RAPORT: B2-Q6 kary Porządku + migracja buntu (lane EKONOMIA)

**Decyzje Macieja (zamknięte):** B2-Q6=C — kary produkcyjne/ekonomiczne + migracja ~5%/turę w imperium; nie utrata miasta.

**Zrobione:**
- `order.ts` — `pieniadzMult`, `naukaMult`, `kulturaMult`; `orderEffectsToYieldMults()`; `pickRevoltMigrationTarget()`
- `society-params.json` — `porzadek_kara_pieniadz/nauka/kultura_t1`; ryzyko 0.05 normal
- `turn-economy.ts` — `orderMultByCity` (7. opcjonalny param po `ownerCivByOwnerId`)
- `logic-test.cjs` — nowe assercje order B2

**Handoff SILNIK:** `dyspozycje/_handoff/EKONOMIA+UI-do-SILNIK_B2-porzadek-komplet.md` — **GOTOWE**

**NIE ruszano:** `main.ts` (migracja + orderMultMap = batch SILNIK).

→ SILNIK: wpięcie + bramka ROBOCZA  
→ MASTER: kanon po Opus

---

## 2026-06-27 ~18:00 — RAPORT: paczka ABC 5–14 + handoff zbiorczy → SILNIK

**Decyzje Macieja (zamknięte):** B1-Q2/3/11, B4-Q1/Q2, B5-Q1/Q2, B-Power-Q1/Q2/Q3 (+ wcześniej B2 society).

**Zrobione (lane EKONOMIA+UI, bez `main.ts`):**
- `terrain-improvements.ts` — bonusy 15 typów z JSON
- `power.ts` — `computePowerContributionsCityEconomy`, `buildPowerSnapshots`
- `empire-food.ts` — pełny tick `advanceEmpireFood` + UI hooks
- `economy.ts` / `turn-economy.ts` — `tileYield` × ulepszenia, ranking pól
- `cityPanel.ts` — auto-zarządca ⚙ ON/OFF, okolica profile/👤, kultura+religia, split imperium
- `tools/grupa-b-lane-test.cjs` — testy power + empire-food + tileYield

**Handoff zbiorczy:** `dyspozycje/_handoff/EKONOMIA+UI-do-SILNIK_GRUPA-B-batch-2026-06-27.md`  
**Kolejność batchy SILNIK:** F-B2-society → F-B5-empire-food → F-B-power → F-B4-kultura → F-B1-okolica → F-B1-improvements

**Testy (do uruchomienia przez F):** `node tools/grupa-b-lane-test.cjs` + `society-breakdown-test.cjs` + `logic-test.cjs`

**NIE ruszano:** `main.ts`, `Gra-podglad.html`

→ **SILNIK: GOTOWE** (6 batchy · Grupa F wpina + ROBOCZA)  
→ **MASTER:** kanon po Opus

---

## 2026-06-27 ~19:00 — HANDOFF: zasięg miasta + mgła → Grupa A + Grupa E

**Decyzja Macieja (Spec):** zasięg okolicy start **5**, rośnie 1:1 z pop (cap 15); widok miasta bez jednostek = okolica + kultura (+0…+3).

**Handoffy:**
- `EKONOMIA-do-GRUPA-A_zasieg-miasta-fog.md` → **Grupa A (MAPA)**
- `EKONOMIA-do-GRUPA-E_start-zasieg-fog.md` → **Grupa E (start meta)**

**Zapis:** `docs/decyzje/B-zasieg-miasta-fog.md`

**Fix lane EKONOMIA:** ✅ `cityRangeForPopulation` → `max(5, min(pop, 15))` · `citySightRadius()` · handoff `EKONOMIA-do-SILNIK_city-sight-zasieg-batch.md`

→ **SILNIK: GOTOWE** (7 batchy) · F wpięcie `currentVisible()`

---

## 2026-06-27 ~20:00 — RAPORT: fix zasięg + citySight + handoff SILNIK batch 7

**Zrobione:**
- `okolica.ts`: `max(5, min(pop,15))`, `citySightRadius`, `CITY_RANGE_MIN/CAP`
- `territory.ts`: re-export z okolica (jedna formuła)
- `okolica-test.cjs`, `grupa-b-lane-test.cjs`, `civ-bonusy-test.cjs` (Grecy WorkedTile)
- `miasto-params.json`: opisy zasieg_okolicy_baza/max
- Handoff: `EKONOMIA-do-SILNIK_city-sight-zasieg-batch.md`
- Power handoff A1 → **GOTOWE**

**Testy:** do uruchomienia lokalnie (node niedostępny w shell agenta):
`node tools/grupa-b-lane-test.cjs` · `okolica-test.cjs` · `civ-bonusy-test.cjs`

**NIE ruszano:** `main.ts`

→ **SILNIK: GOTOWE** batch 7 · kolejka 1–7 w `EKONOMIA+UI-do-SILNIK_GRUPA-B-batch-2026-06-27.md`

---

## 2026-06-27 — RAPORT: playtest miasta (sandbox Macieja)

**Cel:** jedna mapa / jedno miasto do testu panelu (budowa, rekrutacja, podatki, okolica 👤).

**Pliki:**
- `gra/src/game/playtestMiastoEkonomia.ts` (seed 271828, Testpolis pop 9)
- `main.ts`: `doStartPlaytestMiastoEkonomia()`, URL `?playtest=miasto`
- `mainMenu.ts`: przycisk „Playtest miasta (ekonomia)”
- `Gra-podglad-PLAYTEST-MIASTO.html` (build MD5: bf99e18b…)
- Instrukcja: `docs/grupa-b/PLAYTEST-MIASTO-MACIEJ.md`

**Wyjątek workflow:** wpięcie playtestu w `main.ts` (wzorzec jak playtest walki) — na prośbę Macieja do playtestu UI.

→ **Maciej: GOTOWE** — dwuklik `Gra-podglad-PLAYTEST-MIASTO.html`

---

## [2026-06-27] PILNE — luki częściowe → SILNIK (F-B-PILNE)

**Od Macieja:** wszystkie pozycje 🟠 przenieść do wykonania natychmiast.

| Moduł lane (NOWY) | Plik |
|-------------------|------|
| Surowce v0.1 | `gra/src/game/resource-access.ts` |
| Wejścia 2A (kultura/religia) | `gra/src/game/society-inputs.ts` |
| Głód −8% HP | `gra/src/game/army-starvation.ts` |
| Nagłówek panelu (Porządek %) | `gra/src/ui/cityPanel.ts` |

**Handoff SILNIK:** `dyspozycje/_handoff/EKONOMIA+UI-do-SILNIK_PILNE-luki-2026-06-27.md`  
**UNITS:** `dyspozycje/_handoff/UNITS-do-SILNIK_army-starvation-hp.md`  
**Test:** `node tools/grupa-b-lane-test.cjs` — PASS po uruchomieniu

**→ SILNIK: GOTOWE — PILNE** · Grupa F: wpięcie main.ts + ROBOCZA

---

## [2026-06-27] Wyrąb / Tartak / tech gate → SILNIK

**Decyzja Maciej:** wyrąb darmowy (wycinka lasu); tartak osobno (płatny, tech Obróbka drewna); panel szary bez tech.

| Lane | Pliki |
|------|-------|
| JSON + logika | `terrain-improvements.json`, `improvement-tech.ts` |
| MAPA | `improvement-build.ts` (tartak, action wycinka) |
| UI | `buildModeHud.ts` (FREE, 🔒 tech) |
| Render | `improvements.ts` (model tartak) |

**Handoff:** `dyspozycje/_handoff/MAPA+EKONOMIA-do-SILNIK_wyrab-tartak-tech.md`  
**Decyzja:** `docs/decyzje/B1-wyrab-tartak-tech.md`

**→ SILNIK: GOTOWE** batch `F-B-WYRAB-TARTAK`

## [2026-06-27] Korekta Maciej: Tartak na lesie

Tartak = ląd w terytorium **w tym las**; **nie** usuwa `nakladka Las` (tylko wyrąb). Zaktualizowano JSON, `improvement-build.ts`, decyzję + handoff.

**→ SILNIK: GOTOWE — wykonaj TERAZ** · Kolejka: `F-KOLEJKA-P0.md` · Flaga: `docs/czaty/DO-MASTERA.md` § F-B-WYRAB-TARTAK

## [2026-06-27] Bonus tartaku — Excel + JSON

**Ustalone (Maciej 2026-06-27):** tartak **+3 Pracy** / turę · drewno = **dostęp** (`surowiecOdblokowany`), bez liczenia ilości · Excel + JSON + `resource-access.ts`.

**→ SILNIK:** `getResourceAccessForCity(..., placedImprovements)` — 3. argument w hooku panelu.

## [2026-06-28] Domknięcie lane → SILNIK micro-batch

**F-B-TARTAK-DREWNO** — jedna luka po wpięciu F: panel Surowce bez „Drewno” po tartaku.

**Handoff:** `dyspozycje/_handoff/EKONOMIA-do-SILNIK_tartak-drewno-access.md`  
**→ SILNIK: GOTOWE — wykonaj TERAZ** · Kolejka: `F-KOLEJKA-P0.md`

**Blokada (nie lane):** drzewko tech Q1–Q5 → `docs/decyzje/B1-tech-ABC-OTWARTE.md`

---

### [2026-06-29] ROZPOZNANIE — P0 B1 domknięte · EKO-P2-01 ROBIA

**Lane→Silnik P0:** F-B-PILNE + WYRAB-TARTAK + TARTAK-DREWNO — **wszystko przekazane i wpięte** (potwierdzone `main.ts`).

**Silnik:** kolejka P0 od Grupy B **pusta** — meldunek `_handoff/GRUPA-B-do-SILNIK_rozpoznanie-2026-06-29.md`

**Lane dalej:** EKO-P2-01 tick B5 (`EKONOMIA.md` § DO ZROBIENIA) — **nie** `main.ts` dopóki moduł nie GOTOWE.

**Czeka Maciej:** B1 tech Q1–Q5.
