...
### [2026-06-28] MACIEJ → MAPA (via SILNIK): zlecenie wykonania

**Od:** Maciej · **Przekazał:** Grupa F  
**Czat:** **Civ-MAPA** · komenda: **`start`**  
**Zadanie P0:** OBL-S6 obóz 3D · potem E-P0-04/05 złoża (`MAPA.md` § MACIEJ)  
**Manifest:** `dyspozycje/MACIEJ-DELEGACJA-LANE-2026-06-28.md`

---

### [2026-06-28] Grupa F (SILNIK) → MAPA: przekazanie delegacji

**Od:** Grupa F · routing MASTER Work  
**Twoje zadanie:** `dyspozycje/MAPA.md` § DO ZROBIENIA TERAZ — **OBL-S6 TERAZ** (obóz 3D)  
**Handoff:** `_handoff/MASTER-do-MAPA_oboz-3D-OBL-S6.md` · potem E-P0-04 złoża  
**Komenda w czacie Civ-MAPA:** `start`  
**Po GOTOWE:** meldunek tutaj + `→ SILNIK: GOTOWE` jeśli kontrakt cross-lane  
**NIE ruszaj `main.ts`.**

---

### [2026-06-28] MASTER → MAPA: dyspozycja OBL-S6 PILNE

**Od:** MASTER (sesja pilna Maciej)  
**Do:** czat **Civ-MAPA** — napisz `start`

**Zadanie:** Obóz 3D Q10=C — handoff ten plik. OBL-S5 machiny ✅ w silniku.  
**Po GOTOWE:** `MAPA-DO-MASTERA.md` + flaga `→ SILNIK: GOTOWE`  
**Mapa delegacji:** `dyspozycje/MASTER-DELEGACJA-LANE-2026-06-28.md`
...
---
[26.06 ~12:00] MAPA — SPRINT 1 ZAMKNIĘTY (D4=A, D12=A, D15=B).

**D15=B getMinimapData** — `gra/src/map/minimap.ts`:
- `getMinimapData(map, camera, cities?, units?, options?)` → `MinimapData` (zgodne z hud.ts)
- HANDOFF UI: `_handoff/MAPA-do-UI_minimap-data.md`

**D4=A ulepszenia z mapy** — `gra/src/map/improvement-build.ts`:
- `createImprovementBuildApi(state, mode)` → kwalifikacja + koszt Pracy (terrain-improvements.json)
- `ImprovementBuildRequest` dla kolejki tury; prototyp UX: placementpreview/
- HANDOFF MASTER: `_handoff/MAPA-do-MASTER_ulepszenia-D4A.md`

**D12=A miasta BRAZU** — bronzepreview rozszerzony:
- URL: `bronzepreview/index.html?pack=d12` (Sumer/Egipt/Inkowie/Zulusi, poz. 1→10, z/bez murów)
- Eksport `D12_BRONZE_CIVS`; modele bez zmian w bronzeCity.ts
- HANDOFF MASTER: `_handoff/MAPA-do-MASTER_miasta-brazu-D12A.md` — kanon po akceptacji Macieja

STAN: `dyspozycje/MAPA-STAN.md`. main.ts NIE dotykany.

---

## [2026-06-26] E1 — typ Ziemia + skala rywali menu

Od: Grupa E · Lane MAPA

ZROBIONE:
- `gen-helpers.ts` — `TypSwiata` + `ziemia`, `landMaskZiemia`, preset `ZIEMIA_LAND_CENTERS`
- `generator.ts` — obsługa `ziemia` w `generateMap`
- `newGameMapDefaults.ts` — skala rywali per mapa, mapowanie typu świata, default civ id
- `mappreview/main.ts` — typ Ziemia w podglądzie

HANDOFF MASTER: `_handoff/UI-MAPA-do-MASTER_E1-defaulty-startu.md`

→ SILNIK: **GOTOWE DO WPIĘCIA** (`generateMap(..., typSwiata)`, seed z params)

---

## [2026-06-27] A4-D4 — kwalifikacja ulepszeń (Grupa A implementacja)

**ZROBIONE:**
- `improvement-build.ts` — pastwisko/plantacja/tarasy/warzelnia wg decyzji Macieja
- `gen-helpers.ts` — złoża owce, bydło, lama, luksus, sól na mapie
- Handoff: `_handoff/UI-MAPA-do-SILNIK_D1B-A4-batch.md`

→ **SILNIK:** `playerCivArchetype` + raycaster + koszt Pracy

---

## [2026-06-27] F → MAPA — fog of war nie przykrywa rzek (playtest Macieja)

**Zgłoszenie:** Grupa F (playtest ROBOCZA). Mgła na terenie OK, **rzeki widoczne** na heksach unknown — „prześwitują”.

**Owner:** lane MAPA (`gra/src/render/scene.ts` + ewent. `riverPaths` w generatorze). **F nie naprawia** — tylko przekazuje.

**Handoff:** `_handoff/F-do-MAPA_fog-rzeki.md` (objaw, hipoteza `setFog` linie ~1153–1158, DoD, AC).

**PROŚBA MAPA:** odtwórz w ROBOCZA / `Gra-podglad-MAPA.html`, zdiagnozuj (segment vs hexKeys vs krawędzie), popraw i melduj.

---

## [2026-06-27] EKONOMIA → MAPA (Grupa A) — zasięg miasta + mgła

**Od:** Grupa B · **Decyzja Macieja (Spec):** okolica start **r=5**, rośnie 1:1 z pop (pop 9 → r9, cap 15); widok miasta = okolica + kultura (+0…+3).

**Handoff:** `dyspozycje/_handoff/EKONOMIA-do-GRUPA-A_zasieg-miasta-fog.md`  
**Decyzja:** `docs/decyzje/B-zasieg-miasta-fog.md`

**MAPA:** minimapa fog, kontrakt sight; **F** wpina `currentVisible()` po fix EKONOMIA `max(5,pop)`.

→ **Grupa A: CZEKA**

---

## [2026-06-27] Grupa A → SILNIK: A-FOG-Q1=B (mgła per jednostka)

**Decyzja Macieja:** Widok pola = Ruch (mapa strategiczna); **Zwiadowca** min. 5 heksów.  
**Miasto:** Grupa B — poza tym batchem (tymczasowo DEFAULT_SIGHT=3).

**Zrobione (lane A):**
- `gra/data/units.json` — 39 wierszy „Widok pola" zaktualiz.
- `gra/src/game/visibility.ts` — `buildUnitSightResolver`, overload `computeVisible`
- `Civ-UNITS/widok-pola-A-FOG-Q1B.csv` — do Excel `Jednostki.xlsx`
- `docs/grupa-a/A-FOG-Q1-widok-jednostki.md`

**Handoff → SILNIK:** `dyspozycje/_handoff/MAPA-do-SILNIK_fog-widok-pola-A-FOG-Q1B.md`  
**Status:** → SILNIK: WPIĘTE (~21:10)

---

## [2026-06-27] A-START-03 — rzeki pod fog (playtest Macieja)

**Problem:** rzeki prześwitywały na heksach unknown.  
**Fix (`scene.ts`):**
- segment rzeki ukryty, gdy choć jeden hex segmentu jest unknown (`isHidden`);
- węższe `hexKeys` — tylko hexy współdzielone przez wierzchołki krawędzi (nie union całego wierzchołka).

**Handoff:** `_handoff/F-do-MAPA_fog-rzeki.md` — **→ SILNIK: GOTOWE**  
**CZEKA:** playtest wizualny Maciej (ROBOCZA)

---

## [2026-06-27] B-zasieg-miasta-fog — kontrakt + minimapa (A-START-04)

**Decyzja:** `docs/decyzje/B-zasieg-miasta-fog.md`  
**Lane:**
- `visibility.ts` — `computePlayerVisibility()` + re-export `citySightRadius`
- `minimap.ts` — kontrakt fog (te same zbiory co `setFog`)
- testy logic-test (pop 9 → radius 9, start fallback r=5)

**SILNIK:** `main.ts` już używa `citySightRadius` w `currentVisible()` — opcjonalny refactor na `computePlayerVisibility`.  
**Handoff:** `_handoff/MAPA-do-SILNIK_mgla-miasto-minimapa.md` — **→ SILNIK: GOTOWE**

---

## [2026-06-27] MAP-P1-01 — pełny klaster obcych typów (D-START)

**Decyzja:** `docs/decyzje/D-START-miasta-kopie-typu.md`  
**Handoff źródłowy:** `_handoff/CYWILIZACJE-do-MAPA_spawn-obcy-klaster.md`

**Zrobione (lane MAPA):**
- `cluster-spawn.ts` — obcy typ spawnuje **wszystkie** `klaster.miasta[]` (stolica + rywale), nie tylko stolica
- API `ForeignTypeClusterGroup`: `{ typ, ownerIds[], positions[] }` + helper `groupForeignTypeClusters()`
- `cluster-start.ts` — pole `foreignTypeClusters` w `ClusterStartPlan`
- Test: `gra/tools/cluster-start-test.cjs` (MAP-P1-01)

**Bez zmian:** `clusters.ts` (computeClusters już daje pełne pozycje), `generator.ts`, `territory.ts`, `main.ts`

**Handoff:** `_handoff/MAPA-do-SILNIK_spawn-obcy-klaster.md` — **→ SILNIK: GOTOWE**  
**CZEKA:** Grupa F — weryfikacja `applyClusterStartPlan` + CYW-P1-01 (AI defensywne)
