...
**Data propozycji:** 2026-08-06
**Data wdrożenia:** 2026-08-07
**Status:** 🟢 **WDROŻONE.** Wszystkie 41 parametrów Wiarygodności wyeksportowane z
`DIPLOMACY_PARAMS` (TypeScript) do `gra/data/diplomacy.json` (`params`, 1:1, wartości
identyczne poza jedną zatwierdzoną korektą) + 2 zatwierdzone korekty wartości
(`wiarygodnoscProgNapMin`, rozbicie S3/S4 na trudność) + 3 korekty opakowania
(`wiarygodnoscZaufanieDzielnikPerTura` komentarz, `wiarygodnoscZaufanieDryfNa100`,
`wiarygodnoscTempoAmplituda`) wdrożone w kodzie. Zobacz sekcję **10. Wdrożenie
2026-08-07** na końcu dokumentu.
...
| `wiarygodnoscS3HandelPerTure` | +0,3 | pkt Wiarygodności / turę — umowa handlowa / handel cykliczny ZE 100% zrealizowanych dostaw tej tury | 0,3 (bez zmian) | Trzecia w hierarchii wagi zobowiązań (Sojusz > NAP > Handel > Przemarsz), zgodna z kolejnością kar N5 (traktat −6 > handel −4). Warunek „100% dostaw" (bez zmian numerycznych, to reguła atomowości C-HANDEL-3) chroni przed farmieniem tej nagrody bez realnego ryzyka. | POTWIERDZENIE — **ZASTĄPIONE w §10 wdrożenia, patrz sekcja 10** |
| `wiarygodnoscS4PrzemarszPerTure` | +0,2 | pkt Wiarygodności / turę — prawo przemarszu / otwarte granice aktywne | 0,2 (bez zmian) | Najsłabsze zobowiązanie w tabeli — otwarcie granic nie wymaga wzajemnego zaufania w takim stopniu jak handel czy sojusz. Domyka spójną drabinę 1,0 / 0,5 / 0,3 / 0,2. | POTWIERDZENIE — **ZASTĄPIONE w §10 wdrożenia, patrz sekcja 10** |
...
## 9. Podsumowanie (stan propozycji, przed wdrożeniem)
...
| **KOREKTA wartości liczbowej** | **0** (w propozycji pierwotnej; patrz §10 — Maciej zatwierdził 2 korekty wartości PO propozycji) |
...
Po propozycji (sekcje 0–9 wyżej) Maciej zatwierdził **OK dla wszystkich 39 wartości
POTWIERDZENIE bez zmian**, plus **2 korekty wartości** zgłoszone w tej samej turze:

1. **`wiarygodnoscProgNapMin`: −40 → 0.** Cytat Macieja: „tutaj też powinna być wiarygodność
   zero tak samo jak przy sojuszu" — wyrównanie z `wiarygodnoscProgSojuszMin=0`.
2. **S3/S4 strumienia rozbite na trudność, S4 podniesione trzykrotnie.** Cytat (streszczenie):
   „S4 za słabe, powinno rosnąć trzykrotnie szybciej, z rozbiciem na trudny dwukrotnie, łatwy
   czterokrotnie; S3 podniesione proporcjonalnie żeby zachować stosunek S3/S4=1,5 na każdym
   poziomie trudności." Płaskie `wiarygodnoscS3HandelPerTure=0,3` i
   `wiarygodnoscS4PrzemarszPerTure=0,2` zostały **USUNIĘTE** z `DIPLOMACY_PARAMS` i zastąpione
   sześcioma nowymi kluczami:

   | Parametr | Wartość | S3/S4 |
   |---|---|---|
   | `wiarygodnoscS3HandelPerTureLatwy` | 1,2 | |
   | `wiarygodnoscS3HandelPerTureNormalny` | 0,9 | |
   | `wiarygodnoscS3HandelPerTureTrudny` | 0,6 | |
   | `wiarygodnoscS4PrzemarszPerTureLatwy` | 0,8 | 1,5 |
   | `wiarygodnoscS4PrzemarszPerTureNormalny` | 0,6 | 1,5 |
   | `wiarygodnoscS4PrzemarszPerTureTrudny` | 0,4 | 1,5 |

Plus **3 korekty opakowania** z §8/§7 (bez zmiany żadnej wartości liczbowej):

3. Komentarz przy `wiarygodnoscZaufanieDzielnikPerTura` poprawiony — opisuje dziś prawdziwe
   jedyne użycie (Dźwignia 4, `modyfikatorZaufaniaD4OdWiarygodnosci`), nie nieaktualny opis
   „strumień W→Zaufanie/turę" (ANULOWANY, WIAR-Q3=C).
4. `WIARYGODNOSC_ZAUFANIE_DRYF_NA_100` (moduł-const w `diplomacy-credibility.ts`) przeniesiona
   do `DIPLOMACY_PARAMS.wiarygodnoscZaufanieDryfNa100 = 0,03`.
5. Literał `0,5` w `wiarygodnoscWzrostMult`/`wiarygodnoscSpadekMult` nazwany jako
   `DIPLOMACY_PARAMS.wiarygodnoscTempoAmplituda = 0,5`.

**Eksport do JSON:** wszystkie 47 kluczy `wiarygodnosc*` finalnego `DIPLOMACY_PARAMS`
(41 oryginalnych − 2 płaskie S3/S4 + 6 nowych S3/S4-per-trudność + 2 z korekt opakowania =
47) dopisane do `gra/data/diplomacy.json` → `params`, wartości identyczne z domyślnymi w TS
(zweryfikowane testem — patrz niżej).

### Zmienione pliki

- `gra/src/game/diplomacy.ts` — `DIPLOMACY_PARAMS`: `wiarygodnoscProgNapMin` 0 (było −40);
  `wiarygodnoscS3HandelPerTure`/`wiarygodnoscS4PrzemarszPerTure` usunięte, zastąpione 6 kluczami
  per trudność; dodane `wiarygodnoscTempoAmplituda`, `wiarygodnoscZaufanieDryfNa100`; komentarz
  `wiarygodnoscZaufanieDzielnikPerTura` poprawiony.
- `gra/src/game/diplomacy-credibility.ts` — `credibilityStreamWeight(typ, poziomTrudnosci)` i
  `freshCredibilityStreamEntry(typ, poziomTrudnosci)`: nowy WYMAGANY drugi parametr, switch po
  trudności dla `strumien_handel`/`strumien_przemarsz` (S1/S2 bez zmian); usunięta stała
  `WIARYGODNOSC_ZAUFANIE_DRYF_NA_100`, `zaufanieDryfOdWiarygodnosci` czyta
  `DIPLOMACY_PARAMS.wiarygodnoscZaufanieDryfNa100`; `wiarygodnoscWzrostMult`/`wiarygodnoscSpadekMult`
  czytają `DIPLOMACY_PARAMS.wiarygodnoscTempoAmplituda` zamiast literału `0,5`.
- `gra/src/main.ts` — `tickWiarygodnoscStreamFor`: `freshCredibilityStreamEntry(typ,
  _menuDifficulty)` (drugi argument dodany, jedyne realne wywołanie w repo).
- `gra/data/diplomacy.json` — `params`: dopisanych 47 kluczy `wiarygodnosc*` (było 0), wartości
  identyczne z `DIPLOMACY_PARAMS` (TS).
- `gra/tools/wiarygodnosc-test.cjs` — sygnatury `credibilityStreamWeight`/`freshCredibilityStreamEntry`
  w entry-eksporcie zaktualizowane (usunięty eksport `WIARYGODNOSC_ZAUFANIE_DRYF_NA_100`, dodany
  `loadDiplomacyParams`); S3/S4 asercje rozbite na 3 poziomy trudności × 2 typy + asercja
  rozróżnienia trudności + asercja stosunku S3/S4=1,5; próg NAP w testach Dźwigni 3 zaktualizowany
  do nowej wartości 0; nowa sekcja 10 — spójność JSON↔TS dla wszystkich kluczy `wiarygodnosc*`.

### Wynik bramek (2026-08-07)

- `npx tsc --noEmit` — **0 błędów** (zmiana sygnatury `credibilityStreamWeight`/
  `freshCredibilityStreamEntry` na wymagany drugi parametr złapałaby przez błąd kompilacji
  KAŻDE pominięte wywołanie — bramka przeszła czysto, co jest dowodem, że wszystkie wywołania
  w repo zostały zaktualizowane).
- `node tools/wiarygodnosc-test.cjs` — **212 pass, 0 fail** (152 stare + nowe asercje z tego
  wdrożenia).
- `node tools/diplomacy-test.cjs` — **148 pass, 0 fail**.
- `node tools/diplomacy-proposal-test.cjs` — **87/90 PASS** (3 niepowodzenia PRZEDISTNIEJĄCE,
  niezwiązane z Wiarygodnością — bramki handlu/willingness w `diplomacy-proposals.ts`, plik
  nietknięty przez to wdrożenie).
- `node tools/diplomacy-acceptance-points-test.cjs` — **225 pass, 0 fail**.
