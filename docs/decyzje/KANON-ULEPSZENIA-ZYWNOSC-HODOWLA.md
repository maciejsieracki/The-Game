...
| 2026-09-07 | **R-ULEPSZENIA-FARMA-IRYGACJA-BYDLO-STACK-Q1 — ECHO właściciela (żywa rozmowa, wiążąca decyzja) ODWRACA częściowo §2.7/§3/§6 poniżej.** Cytat właściciela: „Chciałbym jeszcze jedną rzecz zmienić, mianowicie możliwość robienia irygacji razem z farmą. W tej chwili grafika na to pozwala i myślę, że mechanika też powinna pozwalać, czyli teoretycznie moglibyśmy w jednym miejscu, tam gdzie jest rzeka, postawić farmę, irygację oraz trzodę. To byłaby ogromna przewaga miejsc z dostępem do wody." Właściciel sam nazwał to „ogromną przewagą" — świadoma decyzja balansu, nie nieporozumienie. **Zmiana reguły (§2.7, §3, §6 poniżej dotyczą TERAZ tego wpisu, nie oryginalnego zapisu z 2026-06-29):** na heksie **płaskim, przy rzece**, spełniającym warunki każdej z trzech warstw z osobna (Irygacja: sąsiedztwo rzeki; Bydło: złoże na pierwsze postawienie, potem odblokowane dla całego imperium jak dotąd), **Farma + Irygacja + Bydło mogą stać RAZEM** — zamiast dawnego „Farma łączy się wyłącznie z jednym dodatkiem: irygacją XOR bydłem" (ten zapis, oryginalnie w §3/§3-tekst poniżej, jest od tej daty NIEAKTUALNY dla kombinacji z farmą; pozostałe reguły nakładania w §3 — tarasy solo, owce solo, lama solo, hodowla vs farma na złożu itd. — bez zmian). Podprzypadek **irygacja+bydło BEZ farmy pozostaje NIEDOZWOLONY** (właściciel o to nie prosił; obie warstwy nadal wymagają w kodzie obecności farmy w istniejących warstwach pola — `canAddFoodLayer`). Suma bonusów na heksie z trójką (wg `terrain-improvements.json`, żywe wartości — różnią się od starej tabeli §4/§6 poniżej, która nie była zsynchronizowana z kodem nawet przed tą zmianą): **+10 żywności / +9 pracy / +8 handlu** ponad puste pole. Zweryfikowane: (a) żywym zrzutem Playwright/Chromium — realny `applyBuildRequest`/`qualifies()`/`canAddFoodLayer()` pozwala postawić wszystkie trzy warstwy na jednym heksie bez błędu (zrzut ekranu: `dyspozycje/autobot/runs/R-ULEPSZENIA-FARMA-IRYGACJA-BYDLO-STACK-Q1/dowody/01-triple-build-toast.png`, toast „Postawiono: bydlo" po trzeciej warstwie); (b) numerycznie przez prawdziwą funkcję silnika `tileYield` (economy.ts, ta sama, której używa `cityYieldPerTurn`) — delta triple vs puste pole = dokładnie +10/+9/+8; (c) renderer (`gra/src/render/improvements.ts`, NIEZMIENIONY w tym temacie) pokazuje wizualnie wszystkie trzy warstwy na heksie — model `pole_irygowane` (farma+irygacja) plus dorysowane osobno ikony bydła, zrzut zbliżenia: `dyspozycje/autobot/runs/R-ULEPSZENIA-FARMA-IRYGACJA-BYDLO-STACK-Q1/dowody/02-triple-render-closeup.png`. Implementacja: **wyłącznie** `canAddFoodLayer()` w `gra/src/map/improvement-build.ts` (usunięte warunki `&& !hasB` / `&& !hasI` blokujące trzecią warstwę gdy pozostałe dwie już stoją) — `terrain-improvements.json` (bonusy) i renderer bez zmian. Pełny raport: `dyspozycje/autobot/runs/R-ULEPSZENIA-FARMA-IRYGACJA-BYDLO-STACK-Q1/01-operator-runda1.md`. |

---

*Przy rozbieżności z kodem — obowiązuje **ten dokument** do momentu wdrożenia i review Opus. Od 2026-09-07: dla kombinacji Farma+Irygacja+Bydło obowiązuje wpis z 2026-09-07 w §11 powyżej, NIE oryginalny zapis „Farma łączy się wyłącznie z jednym dodatkiem" w §3/§6 poniżej — ten pozostawiony jest jako historia, nie usunięty.*
