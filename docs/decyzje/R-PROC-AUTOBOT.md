...
| **Bitwa na polu mapy** | `node tools/map-field-battle-test.cjs` | 22/22 po `P-ROSTER-ZWIADOWCA-DRIFT-DWOCH-FUNKCJI-Q1` rundzie 2 (naprawa fixture'u, nie kodu gry — asercja `adjacent scout excluded` przepisana na zbiór ID, dopisana asercja parytetu `collectBattleRoster == collectAtkRosterNearCity`); wcześniej 20/20, wcześniej 19/1 — jedna asercja czerwieniła się od zawsze, ukryta pod `TypeError` sprzed naprawy INFRA |
| **Kontrakt karty encji** | `node tools/entity-card-contract-test.cjs` | 75/0 |
| **Karta technologii — sekcje rozwinięte i pasek przewijania** | `xvfb-run -a node tools/entitycard-rozwiniete-scrollbar-real-render-test.cjs` | **29/0** — żywy render Chromium/Playwright; K1-K2 stan startowy sekcji i akordeon ręczny, K3 rezerwacja miejsca paska (tautologiczne samo w sobie), K4 dowód przyczynowości na kodzie sprzed tematu, K5/K6 analiza pikseli uchwytu (namalowany, przesuwa się, znika na starym kodzie) — dopiero K5/K6 realnie odróżniają obie wersje. |
| **Krążenie barbarzyńców (NOWA, `P-BARBARZYNCY-KRAZENIE-NIEBRONIONE-Q1`)** | `node tools/barbarzyncy-krazenie-test.cjs` | **249/0** — barbarzyńca stojący przed wyborem celu podejmuje decyzję i ją realizuje, bez oscylacji między niebronionymi miastami; jedna reguła na wszystkich poziomach trudności (dowiedzione: brak `difficulty`/`easy`/`normal`/`hard` w ciele `decideBarbarianMoves` poza sygnaturą). Siostrzana `barb-city-behavior-test.cjs` **178→177 asercji** (jeden żywy dowód mutacyjny, `expectSelfCheckFails` 8→7, stał się zachowaniowo martwy przez tę naprawę) — **ubytek ratyfikowany przez właściciela 2026-09-06**, nie osłabiać dalej bez nowego ECHO. |
| **AI zdobycie miasta przez adiacencję (NOWA, `P-AI-BRAK-SCIEZKI-ZDOBYCIA-MIASTA-ADIACENCJA-Q1`)** | `node tools/ai-zdobycie-miasta-adiacencja-test.cjs` | **88/88** — duże AI ma teraz działającą ścieżkę zdobycia sąsiedniego, niebronionego obcego miasta (parytet z graczem/barbarzyńcami: `tryAutoCaptureEmptyCityAt`). Wpięte `unitIsCivilian` do wywołania egzekutora w `main.ts:31532` — cywil nie przejmuje miasta, tak jak gracza. Final Control ustalił trzy luki pokrycia poza zakresem tej rundy (`targetVisible` niepilnowane nigdzie w rodzinie, `K4-DYSTANS` tautologiczna lokalnie ale łapana przez bramki sąsiednie, `A5f-A5h` przepuszczają kopię formuły zamiast prawdziwego użycia `isCivilianUnit`) — zarejestrowane osobno w `REJESTR-PROSB-I-ZADAN.md`, nie naprawiać przy okazji innych tematów bez ECHO. |
...
| 11 | **Parytet gracz ↔ AI w mechanizmach balansu i wyliczeń.** Każda reguła, formuła, bramka epoki/technologii, skala i próg obowiązują tak samo gracza, każdą cywilizację AI i każde państwo-miasto. **Odstępstwo jest dopuszczalne wyłącznie wtedy, gdy właściciel podjął je jawnie**, jest zapisane w rejestrze z uzasadnieniem, a w bramce istnieje asercja pilnująca, że odstępstwo jest **zamierzone** — nie przypadkowe. Odstępstwo bez tych trzech elementów jest defektem, nawet gdy wszystkie bramki są zielone. **Uwaga na fallbacki i parametry opcjonalne** — to jest ich najczęstsza droga do kodu: funkcja przyjmuje `resolveOwnerX?`, żywa gra go nie podaje, a fallback po cichu traktuje AI inaczej niż gracza. **Bramka wołająca taką funkcję Z rezolwerem testuje wtedy ścieżkę, której gra nigdy nie wykonuje, i jest zielona** | ECHO właściciela 2026-09-06 („zasady, które obowiązują dla gracza, zawsze powinny tak samo obowiązywać dla AI"); pięć niezamówionych odstępstw zmierzonych tego dnia — `R-AI-PARYTET-AUDYT-GLEBOKI-Q1` |
