...
0a. **AUTOBOT Evaluator–Operator (Maciej 2026-08-05).** Operator (playbook) → Evaluator (metryki + postmortem + update playbook). Guardrails: bez merge/`deploy` bez człowieka; win_rate&lt;30%→deprecate; feature pruning. Kanon: `dyspozycje/autobot/` · `.cursor/rules/autobot-evaluator-operator.mdc` · `docs/decyzje/R-PROC-AUTOBOT.md`.
