/**
 * bronzeCity.ts (Grupa A / MAPA — render) — miasta EPOKI BRAZU per cywilizacja.
...
export type BronzeCiv = 'grecja' | 'rzym' | 'sumer' | 'egipt' | 'inka' | 'aztek' | 'chiny' | 'zulu' | 'celtowie' | 'germanie' | 'hetyci';
export const BRONZE_CIVS: BronzeCiv[] = ['grecja','rzym','sumer','egipt','inka','aztek','chiny','zulu','celtowie','germanie','hetyci'];

/** ikonaId z civs.json / menu → klucz buildBronzeCity (np. rzymianie → rzym). */
const IKONA_TO_BRONZE: Record<string, BronzeCiv> = {
  grecy: 'grecja', grecja: 'grecja',
  rzymianie: 'rzym', rzym: 'rzym',
  chinczycy: 'chiny', chiny: 'chiny',
  inkowie: 'inka', inka: 'inka',
  zulusi: 'zulu', zulu: 'zulu',
  egipt: 'egipt',
  babilon: 'sumer', babilonia: 'sumer', sumerowie: 'sumer', sumer: 'sumer',
  harappa: 'sumer', asyria: 'sumer',
  celtowie: 'celtowie',
  germanie: 'germanie', slowianie: 'germanie',
  hetyci: 'hetyci', fenicjanie: 'hetyci',
  aztek: 'aztek',
};

export function ikonaIdToBronzeCiv(ikonaId: string): BronzeCiv {
  return IKONA_TO_BRONZE[ikonaId] ?? 'grecja';
}
...
/** Hetyci / Fenicjanie — kamienna brama fortecy (szary blok, łuk). */
function hittiteGate(scale: number, m: BM): THREE.Group {
  const g = new THREE.Group();
  const tw = 0.11 * scale, th = 0.28 * scale, td = 0.14 * scale, gap = 0.1 * scale;
  const tower = (x: number) => {
    const t = new THREE.Mesh(new THREE.BoxGeometry(tw, th, td), m.stone);
    t.position.set(x, th / 2, 0); t.castShadow = true; g.add(t);
    const cap = new THREE.Mesh(new THREE.BoxGeometry(tw * 1.08, 0.04 * scale, td * 1.08), m.stoneDk);
    cap.position.set(x, th + 0.02 * scale, 0); g.add(cap);
    for (let i = 0; i < 3; i++) {
      const b = new THREE.Mesh(new THREE.BoxGeometry(tw * 0.92, 0.05 * scale, td * 0.35), m.stoneDk);
      b.position.set(x, 0.06 * scale + i * th * 0.28, td * 0.22); g.add(b);
    }
  };
  tower(-(gap / 2 + tw / 2)); tower(gap / 2 + tw / 2);
  const lintel = new THREE.Mesh(new THREE.BoxGeometry(gap + tw * 1.4, 0.06 * scale, td * 0.55), m.stoneDk);
  lintel.position.set(0, th * 0.78, 0); g.add(lintel);
  const arch = new THREE.Mesh(new THREE.BoxGeometry(gap * 0.72, th * 0.42, td * 0.42), m.stone);
  arch.position.set(0, th * 0.36, 0); g.add(arch);
  const court = new THREE.Mesh(new THREE.BoxGeometry(gap + tw * 2, 0.025 * scale, 0.22 * scale), m.stoneDk);
  court.position.set(0, 0.012 * scale, -0.15 * scale); court.receiveShadow = true; g.add(court);
  return g;
}
...
function palette(civ: BronzeCiv, ownerCol: number): BM {
  ...
  switch (civ) {
    ...
    case 'hetyci':   return { ...base, wall: mk(0x9e9e9e), roof: mk(0x757575), roofDk: mk(0x616161), col: mk(0x8a8a8a), stone: mk(0x9e9e9e), stoneDk: mk(0x616161), accent: mk(0x546e7a), wood: mk(0x6d4c41) };
    default:         return { ...base, wall: mk(0x9a8c70), roof: mk(0x77694f), roofDk: mk(0x6e5f4f), col: mk(0x9a8c70) };
  }
}
...
export function buildBronzeCity(civ: BronzeCiv, level: number, ownerCol: number, withWalls = false): THREE.Group {
  ...
  const temple: Record<BronzeCiv, () => THREE.Group> = {
    ...
    hetyci: () => hittiteGate(tScale, m),
  };
  group.add((temple[civ] ?? temple.grecja)!());
  ...
  const house = (idx: number, r: number): THREE.Group => {
    switch (civ) {
      ...
      case 'hetyci': return boxHouse(r, idx, m.stone, m.stoneDk, 0.05, 0.88);
      default: return boxHouse(r, idx, m.wall, m.roof, 0.1, 1.0);
    }
  };
