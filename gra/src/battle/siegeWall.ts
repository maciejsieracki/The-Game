...
const mk = (c: number, side?: boolean) =>
  new THREE.MeshLambertMaterial({ color: c, ...(side ? { side: THREE.DoubleSide } : {}) });
...
function buildWallBodySegmented(
  ...
  for (let sx = startX; sx < endX - T * 0.01; sx += T) {
    ...
    const seg = new THREE.Mesh(new THREE.BoxGeometry(T, H, wallD), mat);
...
function buildGrecja(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  const stone = mk(0xd4c5a9);
  const stoneDk = mk(0xc8b596);

  const leftW = (L - gateW) / 2;

  // Mur jako segmenty per-tile (umozliwia wyłom per-rząd)
  buildWallBodySegmented(g, L, gateW, H, wallD, T, stone);

  // Blanki (merlons) — TYLKO na zewnetrznej krawedzi (+Z)
  ...
  for (let side = 0; side < 2; side++) {
    ...
    for (let i = 0; i < blankCount; i++) {
      const bx = startX + (i + 0.5) * (leftW / blankCount);
      const blank = new THREE.Mesh(new THREE.BoxGeometry(blankW, blankH, blankD), stoneDk);
      blank.position.set(bx, H + blankH / 2, extZ);
...
function buildRzym(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  // Blanki — tylko zewnetrzna krawedz (+Z strona atakujacego = -wallD/2)
...
function buildSumer(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  // Blanki — tylko zewnetrzna krawedz
  const extZ = -wallD / 2 + T * 0.1;
...
function buildInka(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  // Blanki Inka — male kwadratowe, tylko zewnetrzna krawedz
...
function buildChiny(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  // Mur jako segmenty per-tile (umozliwia wyłom per-rząd)
  buildWallBodySegmented(g, L, gateW, H, wallD, T, brick);
  ...
  for (let side = 0; side < 2; side++) {
    ...
    for (let i = 0; i < blankCount; i++) {
      const bx = startX + (i + 0.5) * (leftW / blankCount);
      const blank = new THREE.Mesh(new THREE.BoxGeometry(T * 0.22, H * 0.2, T * 0.15), brickDk);
      blank.position.set(bx, H + H * 0.1, extZ);
      blank.castShadow = true;
      g.add(blank);
    }
  }
...
function buildZulu(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  const wallD = T;
  const wood = mk(0x5c3d1e);
  const woodDk = mk(0x3d2510);
  const thorn = mk(0x2d3a18);

  const leftW = (L - gateW) / 2;

  // Cierniowy wal u podstawy (segmentowany dla wyłomów)
  const thornH = H * 0.2;
  buildWallBodySegmented(g, L, gateW, thornH, wallD, T, thorn);
...
function buildCeltowie(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  const gateW = gateWidthTiles * T;
  const wallD = Math.max(T, T * 1.15);
  const stone = mk(0x888877);
  const wood = mk(0x6b4226);
  const woodDk = mk(0x4a2d18);
...
function buildGermanie(opts: Required<SiegeWallOpts>): THREE.Group {
  ...
  const wood = mk(0x4a3728);
  const woodDk = mk(0x32241a);
  const dirt = mk(0x5a4530);

  const leftW = (L - gateW) / 2;

  // Wal ziemny — segmentowany (umozliwia wyłom per-rząd)
  buildWallBodySegmented(g, L, gateW, H, wallD, T, dirt);
