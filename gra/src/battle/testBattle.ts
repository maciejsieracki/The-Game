...
export type PresetName = 'maly' | 'duzy' | 'rzym_grecja' | 'konnica' | 'oblezenie';
...
/** Health for a units.json row (the "Health" field), min 1. */
function rowHealth(row: any): number {
  return Math.max(1, Math.round(num(row?.['Health'], 10)));
}
...
export function buildTestArmies(
  ...
  return { attacker, defender, teren: def.teren, label: def.label };
}
