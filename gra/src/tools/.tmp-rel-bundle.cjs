...
// .tmp-rel-test.ts
var tmp_rel_test_exports = {};
__export(tmp_rel_test_exports, {
...
module.exports = __toCommonJS(tmp_rel_test_exports);
...
var RELATION_DEAL_TOOLTIP = "Relacja = Zaufanie + Respekt. Powy\u017Cej 100 obni\u017Ca wymagane PW traktatu (max \u221290%), poni\u017Cej podnosi (max +90%).";
...
function renderRelationDealModRowHtml(relTotal, context = "treaty") {
  ...
  return '<div class="da-pn-rel-mod ' + tone + '"' + tip + '><span class="da-pn-rel-mod-label">Wp\u0142yw Relacji na deal</span><span class="da-pn-rel-mod-text"><strong>' + esc(relDisplay) + '</strong> \xB7 <span class="da-pn-rel-mod-deal">' + esc(dealText) + "</span>" + balanceNote + "</span></div>";
}
...
function renderPnBalancePanelHtml(data) {
  ...
  const balCls = incomingTrade ? data.canAccept !== false ? "ok" : "no" : their.accepted ? "ok" : "no";
  const delta = incomingTrade ? netPw > 0 ? `+${netPw}` : String(netPw) : formatBalanceDelta(their.balancePn, their.accepted);
  const deltaCls = netPw >= 0 ? "pos" : "neg";
  const centerLabel = incomingTrade ? "Bilans (netto)" : "Bilans (Oni)";
  const hint = incomingTrade ? incomingTradeBalanceHint(netPw) : balanceHint(their);
  ...
  const treatyNote = their.treatyEffectivePn != null && their.treatyEffectivePn > 0 ? '<div class="da-pn-bal-meta">Traktat: wym. ' + their.treatyEffectivePn + " PW</div>" : "";
...
function renderPnBalancePanelForTreaty(treatyEffectivePw, basketGivePn, basketReceivePn, relTotal, actionLabel, relRequired, treatyMetaLabel = "Traktat") {
  ...
  const hint = !relOk ? `Relacja ${relTotal} \u2014 wym. ${relRequired}` : balancePn > 0 ? `Nadwy\u017Cka ${balancePn} PW` : "R\xF3wno \u2014 spe\u0142nia";
  const verdict = !relOk ? `Relacja ${relTotal} \u2014 wymagane \u2265 ${relRequired} (nie progu fair-min handlu)` : balancePn > 0 ? "Partner prawdopodobnie przyjmie \u2014 nadwy\u017Cka " + balancePn + " PW" : treatyMetaLabel + ": " + treatyEffectivePw + " PW @ Rel " + relTotal + " \u2014 spe\u0142nione";
  const relNote = !relOk ? '<div class="da-pn-bal-meta warn">Relacja ' + relTotal + " \u2014 wym. " + relRequired + "</div>" : "";
  return '<div class="da-pn-balance-bar ' + balCls + ' da-pn-balance-bar--basket da-pn-balance-bar--treaty"><div class="da-pn-bal-head">' + pwTitleHeadHtml() + '<span class="da-pn-bal-deal">' + esc(actionLabel) + '</span></div><div class="da-pn-bal-cols"><div class="da-pn-bal-cell my"><span class="da-pn-bal-lbl">My oddajemy</span>' + pwAmountHtml(myDisplay) + '</div><div class="da-pn-bal-cell center ' + balCls + '"><span class="da-pn-bal-lbl">Bilans (Oni)</span><span class="da-pn-bal-num ' + deltaCls + '"' + pwTipAttr() + ">" + esc(delta) + '</span><span class="da-pn-bal-hint">' + esc(hint) + '</span></div><div class="da-pn-bal-cell they"><span class="da-pn-bal-lbl">Oni oddaj\u0105</span>' + pwAmountHtml(theirDisplay) + "</div></div>" + renderRelationDealModRowHtml(relTotal, "treaty") + '<div class="da-pn-bal-meta">' + esc(treatyMetaLabel) + ": " + treatyEffectivePw + " PW @ Rel " + relTotal + (basketGivePn > 0 || basketReceivePn > 0 ? " \xB7 koszyk netto " + (basketNet > 0 ? "+" + basketNet : "0") + " PW" : "") + "</div>" + relNote + '<div class="da-pn-bal-verdict ' + balCls + '">' + esc(verdict) + "</div></div>";
}
