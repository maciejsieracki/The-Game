...
function computeLayout(nodes: TechNode[]): { svgW: number; svgH: number; dividers: number[]; zoneInfos: ZoneInfo[]; bottomLaneYStart: number } {
  ...
  function computeDepth(id: string): number {
    ...
    const maxD = Math.max(...sameEpochPrereqs.map(pid => computeDepth(pid)));
    const d = maxD + 1;
    depthMap.set(id, d);
    return d;
  }
  ...
  for (let zi = 0; zi < activeEpochs.length; zi++) {
    ...
    const numCols = Math.max(...epNodes.map(n => n.zoneCol)) + 1;
...
function epochStyle(epoka: string): EpochStyle {
  if (epoka === 'Kamień') return {
    bg: 'rgba(18,30,10,0.5)',
    border: 'rgba(80,140,40,0.18)',
    label: '#6aaa38',
    nodeFill: '#243a14',
    nodeStroke: '#6aaa38',
    glowColor: 'rgba(100,200,60,0.16)',
  };
  if (epoka === 'Brąz') return {
    ...
    nodeFill: '#3e2008',
    nodeStroke: '#d08030',
    glowColor: 'rgba(210,130,40,0.16)',
  };
  return {
    bg: 'rgba(10,18,36,0.5)',
    border: 'rgba(40,80,180,0.18)',
    label: '#60a0e0',
    nodeFill: '#0e1e38',
    nodeStroke: '#4080c0',
    glowColor: 'rgba(60,120,220,0.16)',
  };
}
...
function buildSVG(
  ...
  for (let zi = 0; zi < zoneInfos.length; zi++) {
    const z = zoneInfos[zi];
    if (z === undefined) continue;
    const st = epochStyle(z.epoch);
    const zoneW = z.numCols * COL_STEP + (zi < zoneInfos.length - 1 ? Math.round(DIVIDER_GAP * 0.42) : NW + 16);
    const zoneX = z.startX - 16;
    lines.push(`<rect x="${r(zoneX)}" y="${r(PAD_T - 46)}" width="${r(zoneW)}" height="${r(svgH - (PAD_T - 46) - 4)}" rx="8" fill="${st.bg}" stroke="${st.border}" stroke-width="1"/>`);
  }
...
.civ-sci .cs-tree-scroll{overflow-x:auto;flex:1;padding:18px 24px 48px;}
/* Tooltip */
.civ-sci-tooltip{
...
function renderProgress(state: {
  ...
  if (state === null || state.targetId === null || state.kosztCelu <= 0) {
    html += '<div class="cs-prog-row"><span>&#x1F52C; Wybierz technologi&#x0119;, klikaj&#x0105;c w&#x0119;ze&#x0142; na drzewku</span></div>';
  ...
  html += '<div class="cs-prog-row">'
    + '<span>&#x1F52C; Cel: <strong class="cs-val">' + esc(targetName) + '</strong></span>'
...
function render(): void {
  ...
  const nodes = Array.from(TECH_MAP.values());
  const layout = computeLayout(nodes);
  const routedEdges = routeEdges(nodes, layout.zoneInfos, layout.bottomLaneYStart);
  const svgStr = buildSVG(nodes, layout, routedEdges, targetId, researchedIds, availableIds);
...
function onKeyDown(e: KeyboardEvent): void {
  if (e.key === 'Escape') hideSciencePicker();
}
...
export function showSciencePicker(ownerId: number = 0): void {
  ...
  if (overlayEl === null) {
    ...
    overlayEl.addEventListener('click', (e) => {
      if (e.target === overlayEl) hideSciencePicker();
    });
