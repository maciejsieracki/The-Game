...
export const ARCHETYPE_AGGRESSION: Record<TypCywilizacji, number> = {
  ...
  [TypCywilizacji.Celtowie]:          0.60, // Wysoka (brawura szarzy, agresywna piechota)
  [TypCywilizacji.Germanie]:          0.65, // Wysoka (piechota lesna, zasadzki, furia bojna)
...
const ARCHETYPE_TRADE: Record<TypCywilizacji, number> = {
  ...
  [TypCywilizacji.Celtowie]:          0.35, // Niska (kultury wojownikow, niski handel)
  [TypCywilizacji.Germanie]:          0.30, // Niska (piechota lesna, handel wtorny)
 ...
 *   3 = Przyjazny  (score < 120  = DIPLOMACY_PARAMS.progSojuszRelacja)
 *   4 = Sojusz     (status === 'sojusz' overrides score; OR score >= 120)
 *
...
export function relationTier(rel: Relation): 0 | 1 | 2 | 3 | 4 {
  ...
  if (s < DIPLOMACY_PARAMS.progSojuszRelacja)    return 3; // Przyjazny (< 120)
  return 4;                                                // Sojusz (>= 120)
}
...
export interface PotegaKomponenty {
  ...
  wygraneBitwy:  number;
  /** Sumaryczna ludność (mieszkancy w miastach), znorm. [0,1]. */
  ludnosc:       number;
...
export interface PotegaWagi {
  wielkoscArmii: number; // 28
  wygraneBitwy:  number; // 20
  ludnosc:       number; // 18
  miasta:        number; // 14
  gospodarka:    number; // 12
  epoka:         number; // 8
}

/**
 * Domyslne wagi Potegi zgodne z panel_sterowania A w data/diplomacy.json.
 * Suma = 100. Militaria (armia+bitwy) = 48% — celowy sygnał: Respekt = strach/hard power.
 */
...
export function tickDiplomacy(rdip: RelacjaDyplomatyczna, ctx: TickCtx): RelacjaDyplomatyczna {
  ...
  let noweUrazy = rdip.urazyHistoryczne;
  ...
  // --- wygasanie traktatów ---
  const aktywne = rdip.traktaty.filter(
