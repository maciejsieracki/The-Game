...
import type { TechDef, CivBonus } from '../data/loader';
 ...
 */
export function availableTechs(techs: TechDef[], researched: Set<string>): TechDef[] {
 ...
 */
export function cheapestAvailable(techs: TechDef[], researched: Set<string>): string | null {
 ...
 */
export function researchStep(state: PlayerState, techs: TechDef[]): ResearchStepResult {
  ...
  if (
    ...
    byId.has(state.playerResearchTargetId) &&
    !state.zbadane.has(state.playerResearchTargetId)
  ) {
    // Player explicitly chose this target; override badana.
    state.badana = state.playerResearchTargetId;
  } else if (state.badana === null || !byId.has(state.badana) || state.zbadane.has(state.badana)) {
    // No explicit target and no valid current target: default to first available.
  ...
  while (state.badana !== null && iters < maxIters) {
    ...
    if (!def) { state.badana = cheapestAvailable(techs, state.zbadane); continue; }
