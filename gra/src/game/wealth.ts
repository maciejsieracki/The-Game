...
/** Opcje ticka Wealth (D16-A: immunitet startowy). */
export interface WealthTickOptions {
  /** Minimalny poziom W — brak spadku poniżej (np. 1 w pierwszych turach). */
  minPoziom?: number;
}
...
export function advanceWealth(
  ...
  if (pula < 0) {
    pula = 0;
    if (poziom > minPoz) { poziom -= 1; spadek = 1; }   // bufor wyczerpany -> -1 poziom (9A)
  }
