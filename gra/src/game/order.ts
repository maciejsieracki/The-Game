...
export interface OrderParams {
  ...
  /** Unrest Pieniadz penalty (B2-Q6, normal −0.15). */
  karaPieniadzT1: number;
  /** Unrest Nauka penalty (B2-Q6, normal −0.10). */
  karaNaukaT1: number;
  /** Unrest Kultura penalty (B2-Q6, normal −0.10). */
  karaKulturaT1: number;
  /** Unrest growth penalty as a fraction (negative, e.g. -0.25). */
  karaWzrostT1: number;
  /** Unrest revolt/migration risk per turn [0,1] (B2-Q6=C: normal 0.05). */
...
export interface OrderEffects {
  ...
  /** Multiplier on Pieniadz yield (niepokój). 1 = no effect. */
  pieniadzMult: number;
  /** Multiplier on Nauka yield (niepokój). 1 = no effect. */
  naukaMult: number;
  /** Multiplier on Kultura yield (niepokój). 1 = no effect. */
  kulturaMult: number;
  /** Multiplier on population growth speed. 1 = no effect. */
  growthMult: number;
  /** Multiplier on trade / Pieniadz (ład). 1 = no effect. */
  tradeMult: number;
  /** Per-turn migration probability in [0,1]. 0 = no risk. */
  revoltRisk: number;
}

/** Mnożniki plonów per miasto — do advanceCityEconomy (następna tura, jak growthMult). */
export interface OrderYieldMults {
  productionMult: number;
  pieniadzMult: number;
  naukaMult: number;
  kulturaMult: number;
...
interface RawPorzadekBlock {
  ...
  porzadek_kara_pieniadz_t1?: RawSocietyParam;
  porzadek_kara_nauka_t1?: RawSocietyParam;
  porzadek_kara_kultura_t1?: RawSocietyParam;
...
/**
 * Mnożniki plonów do advanceCityEconomy (następna tura, jak growthMult).
 * Przy tierze 'order' bonus handlu idzie na pieniadzMult.
 */
export function orderEffectsToYieldMults(
  tier: OrderTier,
  effects: OrderEffects,
): OrderYieldMults {
  if (tier === 'order') {
    return {
      productionMult: effects.productionMult,
      pieniadzMult:   effects.tradeMult,
      naukaMult:      1,
      kulturaMult:    1,
    };
  }
  return {
    productionMult: effects.productionMult,
    pieniadzMult:   effects.pieniadzMult,
    naukaMult:      effects.naukaMult,
    kulturaMult:    effects.kulturaMult,
  };
}

/** Minimalny kształt miasta do migracji buntu (B2-Q6=C). */
export interface RevoltMigrationCity {
  id: string;
  ownerId: number;
  population: number;
}
...
/**
 * Cel migracji: inne miasto tego samego właściciela z najwyższym Porządkiem.
 * Brak kandydatów → null (bunt bez skutku demograficznego tej tury).
 */
export function pickRevoltMigrationTarget(
  sourceId: string,
  ownerId: number,
  cities: ReadonlyArray<RevoltMigrationCity>,
  orderByCityId: ReadonlyMap<string, number>,
): string | null {
  let bestId: string | null = null;
  let bestOrder = -Infinity;
  for (const c of cities) {
    if (c.id === sourceId || c.ownerId !== ownerId || c.population < 1) continue;
    const o = orderByCityId.get(c.id);
    const orderVal = Number.isFinite(o) ? (o as number) : 0;
