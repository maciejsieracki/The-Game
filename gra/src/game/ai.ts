...
export function decideAIDiplomacy(
  ...
  const p: DiplomacjaParams = {
    progWojnaSila:    params?.progWojnaSila    ?? PROG_WOJNA_SILA,
    progWojnaAgresja: params?.progWojnaAgresja ?? PROG_WOJNA_AGRESJA,
    progTrybut:       params?.progTrybut       ?? PROG_TRYBUT,
    progPokojSlabosc: params?.progPokojSlabosc ?? PROG_POKOJ_SLABOSC,
    progSojusz:       params?.progSojusz       ?? PROG_SOJUSZ,
    progHandel:       params?.progHandel       ?? PROG_HANDEL,
  };
  ...
  for (const rel of inp.relacje) {
    ...
    const { progMinimalnyRelacja } = DIPLOMACY_PARAMS;
    ...
    // Warunek: !stanWojny, duza przewaga (>= PROG_TRYBUT=0.7), agresja srednia (>= 0.25, < 0.75).
    // Przy b. duzej przewadze + umiarkowanej agresji AI woli inkasowac trybut niz ryzykowac wojne.
    // Agresja >= 0.75 (bardzo agresywna cyw) -> blok P4 moze wybronic wypowiedzenie wojny.
    if (!rel.stanWojny && rw >= p.progTrybut && effAgresja >= p.progWojnaAgresja * 0.5 && effAgresja < 0.75) {
      komendy.push({
        type:     'zadaj_trybut',
        targetId: rel.partnerId,
        powod:    `duza przewaga (respektWzgledny=${rw.toFixed(2)} >= prog=${p.progTrybut}), srednia agresja (effAgresja=${effAgresja.toFixed(2)} < 0.75): zadamy trybut zamiast wojny`,
      });
      continue;
    }
    ...
    // Warunki: !stanWojny, willingnessAlly >= PROG_SOJUSZ(0.6),
    //          respektWzgledny w [0.4..0.7] (równy partner, nie ofiara ani nie dominacja).
    // Cel: AI proponuje sojusz gdy relacja jest dobra i siły są zbliżone.
    if (
      !rel.stanWojny &&
      stance.willingnessAlly >= p.progSojusz &&
      rw >= 0.4 && rw <= 0.7
    ) {
      komendy.push({
        type:     'zaproponuj_sojusz',
        targetId: rel.partnerId,
        powod:    `willingnessAlly=${stance.willingnessAlly.toFixed(2)} >= prog=${p.progSojusz}, rowny partner (rw=${rw.toFixed(2)} in [0.4,0.7]): proponujemy sojusz`,
      });
      continue;
    }
    ...
    const handlowosc = inp.handlowosc ?? 0.4;
    if (
      !rel.stanWojny &&
