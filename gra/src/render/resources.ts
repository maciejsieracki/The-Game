/**
 * resources.ts (lane Civ-MAPA / render)
...
function horse(): THREE.Group {
  const g = new THREE.Group(); const brown = mat(0x6b4a2b), dark = mat(0x3f2c1a);
  legsAt(g, dark, 0.05, 0.045, 0.018);
  const body = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.05, 0.055), brown); body.position.y = 0.075; body.castShadow = true; g.add(body);
  const neck = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.06, 0.04), brown); neck.position.set(0.06, 0.11, 0); neck.rotation.z = -0.5; g.add(neck);
  const head = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.026, 0.032), brown); head.position.set(0.088, 0.125, 0); g.add(head);
  const tail = new THREE.Mesh(new THREE.BoxGeometry(0.012, 0.045, 0.012), dark); tail.position.set(-0.066, 0.07, 0); tail.rotation.z = 0.5; g.add(tail);
  return g;
}
