...
// tools/.production-overflow-entry.ts
var production_overflow_entry_exports = {};
__export(production_overflow_entry_exports, {
  advanceProduction: () => advanceProduction,
  cityPracaInteger: () => cityPracaInteger,
  pracaImperialPoolGain: () => pracaImperialPoolGain,
  previewPracaPoolBrutto: () => previewPracaPoolBrutto,
  splitPraca: () => splitPraca
});
module.exports = __toCommonJS(production_overflow_entry_exports);
...
function frontItem(prod) {
  return prod.kolejka.length > 0 ? prod.kolejka[0] : null;
}
function advanceProduction(prod, pracaPerTurn) {
  ...
  if (front === null) {
    return {
      prod: {
        ...
        rekrutacja: prod.rekrutacja ? [...prod.rekrutacja] : void 0
      },
      completed: null,
      overflowToPool: praca > 0 ? praca : void 0
    };
  }
  const accumulated = prod.postep + praca;
  const rqCopy = prod.rekrutacja ? [...prod.rekrutacja] : void 0;
  ...
  return {
    ...
    completed: front,
    overflowToPool: remainder > 0 ? remainder : void 0
  };
}
...
function previewPracaPoolBrutto(ticks, options) {
  let sum = 0;
