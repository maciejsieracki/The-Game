...
var FALLBACK_SIGHT = 3;
function mapGenDefaultSight() {
  const v = map_gen_params_default.mgla?.default_sight_jednostki?.wartosc;
  return typeof v === "number" && v > 0 ? v : FALLBACK_SIGHT;
}

// src/game/visibility.ts
var DEFAULT_SIGHT = mapGenDefaultSight();
function computeVisibleAt(q, r, map, sight) {
...
function addExplored(explored, visible) {
...
function occupiedExcept(units, unitId) {
...
function deductStepCost(unit, cost) {
...
function advanceScoutAutoExplore(unit, map, explored, allUnits, sight, rng = Math.random) {
...
function runScoutsAutoExplore(units, map, explored, playerOwnerId, sightResolver, rng = Math.random) {
  const movedUnitIds = [];
  let totalSteps = 0;
  for (const u of units) {
