...
// tools/.post-capture-law-entry.ts
var post_capture_law_entry_exports = {};
__export(post_capture_law_entry_exports, {
  POST_CAPTURE_FRESH_TURNS: () => POST_CAPTURE_FRESH_TURNS,
  POST_CAPTURE_LAW_PCT: () => POST_CAPTURE_LAW_PCT,
  POST_CAPTURE_REBELLION_RECONQUEST_TURNS: () => POST_CAPTURE_REBELLION_RECONQUEST_TURNS,
  REBEL_FACTION_OWNER_ID: () => REBEL_FACTION_OWNER_ID,
  applyPostCaptureLawOnCapture: () => applyPostCaptureLawOnCapture,
  applyPostCaptureLawOverride: () => applyPostCaptureLawOverride,
  evaluateOrderFromBreakdown: () => evaluateOrderFromBreakdown,
  isPostCaptureLawActive: () => isPostCaptureLawActive,
  isRebellionReconquest: () => isRebellionReconquest,
  markCityRebellionStarted: () => markCityRebellionStarted,
  tickPostCaptureLawEndOfTurn: () => tickPostCaptureLawEndOfTurn
});
module.exports = __toCommonJS(post_capture_law_entry_exports);
...
var POST_CAPTURE_LAW_PCT = 100;
function markCityRebellionStarted(city) {
...
function isPostCaptureLawActive(city) {
  const n = city.postCaptureLawTurnsRemaining;
  return typeof n === "number" && n > 0;
}
function tickPostCaptureLawEndOfTurn(city) {
  const n = city.postCaptureLawTurnsRemaining;
...
function postCaptureLawBannerLabel(city) {
  const n = city.postCaptureLawTurnsRemaining;
...
function applyPostCaptureLawOverride(ordPct, city, society, difficulty = "normal") {
  ...
  const banner = postCaptureLawBannerLabel(city) ?? "Podb\xF3j \u2014 wymuszone Prawo";
  const overrideLine = {
    id: "post_capture_law",
    label: banner,
    value: prawPct
