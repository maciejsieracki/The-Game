...
function ensureRiverGridAndProximity(hexes, massSet, cellSize, seaDist, gridCtx, maxProximityDist = RIVER_PROXIMITY_MAX_DIST) {
  const maxRounds = Math.max(8, Math.min(32, 8 + Math.floor(Math.sqrt(massSet.size / 200))));
  const forceCtx = {
    ...gridCtx,
    acceptLen: 3,
    sourceSep: 0,
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (roundPlaced === 0 && unfilled.length === 0 && proxGap <= maxProximityDist) break;
    ...
    placed += roundPlaced;
    if (roundPlaced === 0 && unfilled.length === 0) break;
  }
  return placed;
}
