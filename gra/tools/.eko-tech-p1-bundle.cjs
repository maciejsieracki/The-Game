...
// tools/.eko-tech-p1-entry.ts
var eko_tech_p1_entry_exports = {};
__export(eko_tech_p1_entry_exports, {
  applyCompletedBuildingIds: () => applyCompletedBuildingIds,
  availableProduction: () => availableProduction,
  availableTechs: () => availableTechs,
  buildingGateMet: () => buildingGateMet,
  canResearch: () => canResearch,
  createPlayerState: () => createPlayerState,
  createResearchState: () => createResearchState,
  isBuildingSupersededByUpgrade: () => isBuildingSupersededByUpgrade,
  playerAvailableTechs: () => availableTechs2,
  resolveRequiredBuildingId: () => resolveRequiredBuildingId,
  setPlayerResearchTarget: () => setPlayerResearchTarget
});
module.exports = __toCommonJS(eko_tech_p1_entry_exports);
...
function prerequisitesOf(tech) {
  return parsePrerequisites(tech["Wymaga (prereq)"]);
}
function findTech(techData, techId2) {
  return techData.find((t) => t.Technologia === techId2);
}
function availableTechs(state, techData, gate) {
...
function canResearch(state, techData, techId2, gate) {
  if (state.ukonczone.includes(techId2)) return false;
  const tech = findTech(techData, techId2);
...
function applyCompletedBuildingIds(builtIds, completedBuildingId, buildings) {
...
function setPlayerResearchTarget(state, techId2, techs, gate) {
