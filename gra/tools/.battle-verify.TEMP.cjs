...
__installTimerShim(window);
global.setTimeout = window.setTimeout;
global.clearTimeout = window.clearTimeout;

// Captured errors list (reset between phases)
const capturedErrors = [];
global.__CLOCK__ = 1000;
// Non-recursing setTimeout shim: avoids jsdom timerInitializationSteps stack
// overflow under massive synchronous frame draining. Timers fire from the mock
// clock, driven by __pumpTimers() inside drainRAF.
const __timers = [];
let __timerId = 1;
function __installTimerShim(win) {
  win.setTimeout = function (fn, ms) {
    const id = __timerId++;
    __timers.push({ id, fn, due: (global.__CLOCK__||0) + (ms||0) });
    return id;
  };
...
function __pumpTimers(errs) {
  let guard = 0, ran = true;
  while (ran && guard < 5000) {
    ran = false; guard++;
    for (let i = 0; i < __timers.length; i++) {
      if (__timers[i].due <= (global.__CLOCK__||0)) {
        const t = __timers.splice(i, 1)[0]; i--;
        try { t.fn(); } catch (e) { if (errs) errs.push(e); }
        ran = true;
      }
    }
  }
...
installGlobal('performance', { now: () => (global.__CLOCK__||0), timeOrigin: _nodePerf.timeOrigin || Date.now() });
...
try {
  ...
  fn(
    ...
    { now: () => (global.__CLOCK__||0), timeOrigin: _nodePerf.timeOrigin || Date.now() }, 1, 800, 600, window.screen,
...
function drainRAF(maxFrames, errs) {
  for (let i = 0; i < maxFrames && rafCallbacks.length > 0; i++) {
    global.__CLOCK__ = (global.__CLOCK__||0) + 20;
    const cb = rafCallbacks.shift();
    try { cb(global.__CLOCK__); }
    catch (err) {
      const msg = err && err.message ? err.message : String(err);
      console.error('[battle-smoke] Error in rAF frame:', msg);
      errs.push(err);
    }
    if (typeof __pumpTimers === 'function') __pumpTimers(errs);
  }
}
...
// === EXTENDED RUNTIME DRIVE (verification harness, not committed) ===
console.log('[verify] Driving full battle with mock clock...');
{
  let frames = 0;
  const maxDrive = 40000;
  let concluded = false;
  while (rafCallbacks.length > 0 && frames < maxDrive) {
    frames++;
    drainRAF(1, capturedErrors);
    if (capturedErrors.length > 0) break;
    if (/Zwyciestwo|Zwycięstwo/.test(document.body.textContent || '')) { concluded = true; break; }
  }
  // Let the post-conclusion onFinish timers settle (a few more pumps).
  for (let k = 0; k < 200 && capturedErrors.length === 0; k++) { global.__CLOCK__ += 50; __pumpTimers(capturedErrors); }
  console.log('[verify] frames driven:', frames, '| sim ms:', global.__CLOCK__, '| concluded:', concluded);
  if (capturedErrors.length > 0) {
    console.error('[verify] RUNTIME ERROR during battle drive:');
    for (const e of capturedErrors) console.error('   x', (e&&e.stack)?e.stack.split('\n').slice(0,10).join('\n'):String(e));
    console.error('VERIFY FAIL'); process.exit(2);
  }
  if (!concluded) { console.error('VERIFY FAIL: no conclusion banner within budget'); process.exit(3); }
  console.log('VERIFY OK: battle ran to a clean conclusion with no runtime errors.');
  process.exit(0);
}
