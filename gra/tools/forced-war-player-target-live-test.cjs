'use strict';
/**
 * forced-war-player-target-live-test.cjs — P-WOJNA-WYMUSZONA-TRZY-NAPRAWY-Q1,
 * kryteria końca 4 i 6 (Operator Sonnet 5, effort=medium, 2026-08-30, worktree izolowany).
 *
 * Dispatch ZAKAZUJE zamknięcia kryteriów 4 i 6 samym testem jednostkowym — wymaga
 * realnej weryfikacji w headless Chromium (Playwright), wzorem
 * `era-change-toast-live-test.cjs`. TA bramka: realny `vite build`, realny headless
 * Chromium (`?playtest=mapa`), REALNE funkcje silnika (`triggerPlayerEndTurn` →
 * `runAiPhase` → `decideAIDiplomacy` → `pickBronzeForcedWarTargetId` → komenda
 * `wypowiedz_wojne` → `applyDiploEventTracked`/`showHintMessage`/`recordWarDeclarationEvent`)
 * — ZERO reimplementacji formuły wyboru celu ani warstwy dyplomacji.
 *
 * Fast-forward: hak `window.__eraTestDebug.forceBronzeForcedWarOnPlayer()` (main.ts,
 * obok `prepareOneTechFromBronze()`) wybiera realnego AI ownera z bieżącego świata,
 * czyści jego realne wojny (poza barbarzyńcami — C-BARB-Q1, struktura NIE dotknięta),
 * ustawia mu `bronzeForceWarPendingOwners` (dokładnie to, co normalnie ustawia awans
 * do Brązu) i przesuwa jego miasto referencyjne na tę samą pozycję co pierwsze miasto
 ...
 * REALNYMI `endTurn()` do tury 24 (krok A2) PRZED wywołaniem `forceBronzeForcedWarOnPlayer()`,
 ...
 *  B. `forceBronzeForcedWarOnPlayer()` faktycznie wybrał realnego AI ownera i wyzerował
 ...
 *     dobiega końca.
 *  D. SEDNO kryterium 4: relacja attacker↔gracz faktycznie zmienia status na 'wojna'
 *     — gracz ZOSTAŁ WYBRANY jako cel wymuszonej wojny (nie jest już strukturalnie
 *     wykluczony), a nie tylko "AI mogłaby, gdyby chciała".
 *  E. SEDNO kryterium 6: wypowiedzenie wojny jest WIDOCZNE w UI (toast `#civ-hint-toast`)
 *     i w dzienniku wydarzeń (`warEventLog`) — dokładnie ta ścieżka, którą właściciel
 *     zgłosił jako "nic nie widzę" (patrz WYZWALACZ dispatchu).
 *  F. Zero console.error / pageerror w trakcie scenariusza.
 *
 * Bramka (z katalogu gra/): node tools/forced-war-player-target-live-test.cjs
 */
...
const OUT_DIR = path.join(GRA_DIR, 'dist-forced-war-player-target-live-test');
...
function buildBundle() {
  console.log('[forced-war-player-target-live-test] budowanie bundla (vite build, wyłącznie dozwolona komenda z CLAUDE.md)...');
  execSync(
  ...
  console.log('[forced-war-player-target-live-test] build OK.');
}
...
async function launchBrowser(chromium) {
  try {
    ...
    console.log('[forced-war-player-target-live-test] domyślny Chromium niedostępny, fallback na', FALLBACK_CHROME);
...
async function main() {
  ...
  try {
    ...
    console.error('[forced-war-player-target-live-test] playwright nie znaleziony. Uruchom z gra/ (npm i już zrobione).');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('\n-- B. forceBronzeForcedWarOnPlayer(): realny AI owner wybrany, wojny wyzerowane --');
    ...
    console.log('\n-- D. SEDNO kryterium 4: gracz ZOSTAŁ WYBRANY jako cel — relacja attacker<->gracz to teraz "wojna" --');
    ...
    console.log('\n-- E. SEDNO kryterium 6 (i WYZWALACZA dispatchu "nic nie widzę"): DOW widoczny w UI i w dzienniku --');
...
main().catch((e) => {
  console.error('[forced-war-player-target-live-test] błąd:', e);
