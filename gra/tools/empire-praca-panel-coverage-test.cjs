'use strict';
/**
 * empire-praca-panel-coverage-test.cjs — Panel 11 zakladek Faza 2 (Praca).
 ...
 * `praca`: żadna INNA bramka nie dowodzi, że `if (block === 'praca') body += praca;` w
 ...
 * `tsc` zielono, panel renderowałby się CAŁKOWICIE PUSTY dla klika "Praca".
 *
 ...
 *      `if (block === 'praca') body += praca;` istnieje w bloku składania `body`.
 ...
 *      `praca` ustawionym na marker zawierający dokładnie ten sam tekst, jaki
 *      `renderPracaSection()` faktycznie emituje (`<div class="civ-emp-eyebrow">PRACA
 *      IMPERIUM</div>`, zweryfikowane osobną asercją source-text) — dla `block='praca'` body
 *      MUSI zawierać ten marker i być niepuste; dla innych bloków (`'moc'`, `'all'`, `'nauka'`,
 *      `'religia'`, `'skarbiec'`) NIE MOŻE.
 *   C. Weryfikacja negatywna (mutacyjna) — ta sama linia usunięta, uruchomiona na tych samych
 *      parametrach (block='praca') — dowodzi, że test B faktycznie łapie regresję (panel
 ...
 *
 * Run from gra/: node tools/empire-praca-panel-coverage-test.cjs
 */
...
console.log('========================================================================');
console.log('Faza 2 (Praca) -- render(): blok "praca" faktycznie dolewa tresc do body (nie tylko routing)');
console.log('========================================================================\n');

// ---------------------------------------------------------------------------
// A. Weryfikacja strukturalna na prawdziwym pliku (source-text).
// ---------------------------------------------------------------------------
console.log('A. render(): linia "if (block === \'praca\') body += praca;" istnieje w bloku skladania body (source-text)');

const marker = '<div class="civ-emp-eyebrow">PRACA IMPERIUM</div>';
assert(
  'renderPracaSection() faktycznie emituje marker "PRACA IMPERIUM" (kotwica markera dla dowodu B nie ucieka od prawdziwego kodu)',
...
const pracaLine = "if (block === 'praca') body += praca;";
assert(
  'wycinek zawiera "' + pracaLine + '"',
  assemblySnippet.includes(pracaLine),
);
console.log('');

// ---------------------------------------------------------------------------
// B. Dowod wykonywalny -- dosłowny wycinek skladania body URUCHOMIONY (new Function) z
//    zaslepkami tresci per blok; `praca` niesie realny marker sekcji.
// ---------------------------------------------------------------------------
console.log('B. Dowod wykonywalny -- wyciety wycinek skladania body uruchomiony z realnym markerem "PRACA IMPERIUM"');
...
const pracaMock = marker + '<div class="civ-emp-hero pos">48 Pracy / turę</div>';
const mockArgsFor = (block) => [
  block, '<div>PARAMS</div>', '<div>MOC</div>', '<div>ZASOBY</div>', '<div>SKARBIEC</div>', pracaMock,
  '<div>NAUKA</div>', '<div>RELIGIA</div>', '<div>SPICHLERZ</div>', '<div>ARMIA</div>',
  '<div>KULT</div>', '<div>SUR</div>', '<div>HANDEL</div>',
];

const bodyPraca = safeRun('B1', runReal, mockArgsFor('praca'));
assert(
  'RDZEN: block="praca" -> body NIEPUSTE i zawiera marker "PRACA IMPERIUM" (dokladnie luka opisana przez wzorzec M2/Evaluatora)',
  typeof bodyPraca === 'string' && bodyPraca.length > 0 && bodyPraca.includes(marker),
  { bodyPraca },
);

for (const otherBlock of ['moc', 'all', 'nauka', 'religia', 'skarbiec']) {
  const bodyOther = safeRun('B-' + otherBlock, runReal, mockArgsFor(otherBlock));
  assert(
    `block="${otherBlock}" -> body NIE zawiera marker Pracy (bloki sie nie mieszaja)`,
...
// ---------------------------------------------------------------------------
// C. Weryfikacja negatywna (mutacyjna) -- linia "if (block === 'praca') body += praca;"
//    usunieta -> panel calkowicie pusty. Trwaly odpowiednik zleconej recznej mutacji
//    kontrolnej (usun -> czerwono -> przywroc).
// ---------------------------------------------------------------------------
console.log('C. Weryfikacja negatywna -- mutacja usuwajaca "body += praca;", lapana przez B1');

const mutatedSnippet = assemblySnippet.replace(
  pracaLine,
  "// MUTANT: linia 'body += praca;' usunieta -- regresja analogiczna do M2 (Skarbiec, Faza 1)",
);
...
const bodyMutant = safeRun('C1', runMutant, mockArgsFor('praca'));
assert(
  'MUTANT (linia usunieta) lapany CZERWONO: block="praca" -> body PUSTE (""), NIE zawiera marker',
  bodyMutant === '',
  { bodyMutant },
);
console.log('');
