...
function parseDefenseBonusPct(bonus) {
...
var FORTIFY_OBRONA_PROC_FIELD = OBL.fortify_obrona_proc;
...
function createPlaylist(trackUrls, repeatsPerTrack, kolejnosc = "losowa") {
  ...
  function clearStartFade() {
    if (startFadeTimer !== null) {
      window.clearInterval(startFadeTimer);
      startFadeTimer = null;
    }
    startFadeActive = false;
  }
  function startFadeStep() {
    if (!playing || !startFadeActive) return;
    const u = Math.min(1, (performance.now() - startFadeT0) / 1e3 / startFadeDurSec);
  ...
  function startWithFadeIn(fadeMs) {
    if (!hasTracks() || playing) return;
    clearStopFade();
    clearStartFade();
    playing = true;
    ...
    clearMonitor();
    monitorTimer = window.setInterval(monitorTick, MONITOR_STEP_MS);
    const durSec = Math.max(0, fadeMs) / 1e3;
    if (url && durSec > 0) {
...
var dyplomacjaCivModules = import_meta.glob("./utwory/dyplomacja/*/*.mp3", {
  eager: true,
  import: "default"
});
...
function rosterFieldPowerM(roster, powerScaledDefFor) {
  return sumRosterFieldM(roster.map((u) => ({ typeId: u.typeId, def: powerScaledDefFor(u) })));
}
function effectiveDefenderM(defRoster, terrain, structBonusPct, atkLeadDef, fortifyScaledDefFor, terrainCombatData) {
...
function preBattleSzanseAtkPct(atkRoster, defRoster, terrain, structBonusPct, unitDefFor, fortifyScaledDefFor, terrainCombatData) {
