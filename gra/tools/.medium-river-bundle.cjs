...
// tools/.medium-river-entry.ts
var medium_river_entry_exports = {};
__export(medium_river_entry_exports, {
  MEDIUM_TRIBUTARY_MIN_NET_LEN: () => MEDIUM_TRIBUTARY_MIN_NET_LEN,
  MEDIUM_TRIBUTARY_SPACING_HEX: () => MEDIUM_TRIBUTARY_SPACING_HEX,
  buildOceanReachableRiverHexKeys: () => buildOceanReachableRiverHexKeys,
  collectPathHexKeysForKinds: () => collectPathHexKeysForKinds,
  collectRiverPathHexKeys: () => collectRiverPathHexKeys,
  countMediumInlandLandHexes: () => countMediumInlandLandHexes,
  generateMap: () => generateMap,
  hexKey: () => hexKey,
  mediumPathStartsOnMain: () => mediumPathStartsOnMain,
  mediumRiverRenderPath: () => mediumRiverRenderPath,
  mediumTributaryMinNetLen: () => mediumTributaryMinNetLen,
  mediumTributaryNetHexCount: () => mediumTributaryNetHexCount,
  oceanConnectedWaterKeys: () => oceanConnectedWaterKeys,
  pathEndsAtSea: () => pathEndsAtSea,
  pruneInvalidMediumRiverPaths: () => pruneInvalidMediumRiverPaths,
  riverPathHasBranchHexWrap: () => riverPathHasBranchHexWrap,
  riverPathHasJoinHexWrap: () => riverPathHasJoinHexWrap,
  riverPathViolatesTurnWindow: () => riverPathViolatesTurnWindow,
  trimMediumBranchHexWrap: () => trimMediumBranchHexWrap,
  trimMediumJoinHexWrap: () => trimMediumJoinHexWrap,
  trimMediumRenderPathAtMain: () => trimMediumRenderPathAtMain
});
module.exports = __toCommonJS(medium_river_entry_exports);
...
function findInlandSeaHexes(hexes, width, height) {
...
function mediumRiverRenderPath(hexes, path) {
  const land = [];
  for (const p of path) {
    const h = hexes[hexKey(p.q, p.r)];
...
function riverPathHasJoinHexWrap(path, mainKeys, networkKeys) {
  if (path.length < 3) return false;
...
function riverPathHasBranchHexWrap(path, mainKeys) {
  if (path.length < 3) return false;
  const a = path[0];
  const b = path[1];
  const c = path[2];
...
function trimMediumRenderPathAtMain(path, mainPaths) {
  const mainKeys = /* @__PURE__ */ new Set();
