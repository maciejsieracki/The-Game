...
__export(picker_test_entry_exports, {
  HEX_R: () => HEX_R,
  ROBLOX_TERRAIN_VIS: () => ROBLOX_TERRAIN_VIS,
  THREE: () => THREE4,
  axialToWorld: () => axialToWorld,
  clientRectToNdc: () => clientRectToNdc,
  pixelToHex: () => pixelToHex,
  refreshInstancedPickBounds: () => refreshInstancedPickBounds,
...
function worldToAxial(x, z, R2) {
  const rf = z / (R2 * 1.5);
  const qf = x / (R2 * SQRT3) - rf * 0.5;
  const xc = qf;
  const zc = rf;
  const yc = -xc - zc;
...
function axialToWorld(q, r, R2 = HEX_R) {
  const x = R2 * SQRT32 * (q + r * 0.5);
  const z = R2 * 1.5 * r;
  return { x, z };
}
...
var TEREN_MATERIAL = new THREE2.MeshLambertMaterial({
  vertexColors: true,
  flatShading: true
});
var promien = new THREE2.Raycaster();
var promienStart = new THREE2.Vector3();
var promienDol = new THREE2.Vector3(0, -1, 0);
...
// tools/.picker-test-entry.ts
var THREE_NS = __toESM(require("three"), 1);
var THREE4 = THREE_NS;
