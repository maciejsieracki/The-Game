...
// tools/_tmp-mapgen-audit-entry.ts
...
function run(label, w, h, typ) {
  const t0 = import_node_perf_hooks.performance.now();
  const map = generateMap(w, h, SEED, typ, { worldDensity: DENSITY_MEDIUM, mapSizeMenuLabel: label });
  const ms = import_node_perf_hooks.performance.now() - t0;
  const kinds = map.riverPathKinds ?? [];
  const main = kinds.filter((k) => k === "main").length;
  const trib = kinds.filter((k) => k === "tributary").length;
  const medium = kinds.filter((k) => k === "medium").length;
  const short = kinds.filter((k) => k === "short").length;
  const params = resolveRiverMapParams("medium", w, h);
  return { ms, rivers: map.riverPaths?.length ?? 0, main, trib, medium, short, params };
}
function runLow(label, w, h, typ) {
  const t0 = import_node_perf_hooks.performance.now();
  generateMap(w, h, SEED, typ, { worldDensity: DENSITY_LOW, mapSizeMenuLabel: label });
  return import_node_perf_hooks.performance.now() - t0;
}
var cases = [
  { label: "Standardowy", rozmiar: "standardowy", typ: "kontynenty" },
  { label: "Du\u017Cy", rozmiar: "duzy", typ: "pangea" }
];
console.log("=== MAPGEN AUDIT seed=42 tier=medium ===\n");
for (const c of cases) {
  const { w, h } = rozmiarToDims(c.rozmiar);
  const full = run(c.label, w, h, c.typ);
  const lowMs = runLow(c.label, w, h, c.typ);
  const riverDelta = full.ms - lowMs;
  console.log(`--- ${c.label} ${w}\xD7${h} ${c.typ} ---`);
  console.log(`  pe\u0142ny:        ${fmtSec(full.ms)}`);
  console.log(`  rivers=low:   ${fmtSec(lowMs)}`);
  console.log(`  delta rzek:   ${fmtSec(riverDelta)} (${(riverDelta / full.ms * 100).toFixed(0)}%)`);
  console.log(`  reszta:       ${fmtSec(lowMs)} (${(lowMs / full.ms * 100).toFixed(0)}%)`);
  console.log(`  rzeki: main=${full.main} trib=${full.trib} med=${full.medium} short=${full.short} total=${full.rivers}`);
  console.log(`  params: mainCell=${full.params.mainCell} topUp=${full.params.topUpPasses} feeder=${full.params.feederPasses} maxLen=${full.params.maxLen}`);
  console.log("");
}
