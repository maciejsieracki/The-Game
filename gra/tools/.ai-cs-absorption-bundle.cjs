...
// tools/.ai-cs-absorption-entry.ts
var ai_cs_absorption_entry_exports = {};
__export(ai_cs_absorption_entry_exports, {
  aiCsAbsorptionParams: () => aiCsAbsorptionParams,
  applySameCivClusterAbsorptionBoost: () => applySameCivClusterAbsorptionBoost,
  decideAiCsClusterAction: () => decideAiCsClusterAction,
  isAiMajorToSameCivCityStatePair: () => isAiMajorToSameCivCityStatePair,
  isSisterAllianceThreatOwner: () => isSisterAllianceThreatOwner,
  maintainAiMajorSameCivRelation: () => maintainAiMajorSameCivRelation,
  rollAiCsAccept: () => rollAiCsAccept,
  startRelationForAiMajorSameCivCityState: () => startRelationForAiMajorSameCivCityState,
  unitTriggersSisterAllianceThreat: () => unitTriggersSisterAllianceThreat
});
module.exports = __toCommonJS(ai_cs_absorption_entry_exports);
...
function clampRate(n, lo = 0, hi = 0.99) {
  return Math.max(lo, Math.min(hi, Math.round(n * 100) / 100));
}
function applySameCivClusterAbsorptionBoost(params, diff) {
  switch (diff) {
    case "easy":
      return {
        ...params,
    ...
    case "hard":
      return {
        ...
        militaryRatioMin: Math.min(params.militaryRatioMin, 1),
        trybutAccept: 0.99,
        wasalAccept: 0.99,
...
function isSisterAllianceThreatOwner(ownerId) {
  return ownerId === 0;
}
function unitTriggersSisterAllianceThreat(unitOwnerId, sisterOwnerSet) {
  return isSisterAllianceThreatOwner(unitOwnerId) && !sisterOwnerSet.has(unitOwnerId);
}
function decideAiCsClusterAction(input) {
  const base = aiCsAbsorptionParams(input.difficulty);
  const params = input.sameCivCluster ? applySameCivClusterAbsorptionBoost(base, input.difficulty) : base;
  ...
  if (params.instantAnnexIfRatio != null && params.instantAnnexMinTurn != null && input.turn >= params.instantAnnexMinTurn && input.militaryRatio >= params.instantAnnexIfRatio) {
    return { action: "instant_annex", reason: "hard_ratio_instant" };
  }
  if (input.hasWasalDeal && input.wasalSinceTurn != null && input.turn - input.wasalSinceTurn >= params.annexAfterVassalTurns) {
  ...
  const canForceWar = input.failCount >= params.vassalFailBeforeWar && input.turn >= params.clusterWarMinTurn;
  if (input.hasTrybutDeal && input.trybutSinceTurn != null && input.turn - input.trybutSinceTurn >= params.wasalAfterTrybutTurns) {
    if (canForceWar) {
...
function startRelationForAiMajorSameCivCityState() {
...
function maintainAiMajorSameCivRelation(rel) {
  ...
  const status = rel.status === "sojusz" || zaufanie >= floor.zaufanie ? "sojusz" : rel.status;
  return { zaufanie, respekt, status };
}
function isAiMajorToSameCivCityStatePair(ownerA, ownerB, opts) {
