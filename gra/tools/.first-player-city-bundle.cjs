...
// tools/.first-player-city-entry.ts
var first_player_city_entry_exports = {};
__export(first_player_city_entry_exports, {
  FIRST_CITY_START_REVEAL_DENIED: () => FIRST_CITY_START_REVEAL_DENIED,
  hexDistance: () => hexDistance,
  isAwaitingFirstPlayerCity: () => isAwaitingFirstPlayerCity,
  isHexInStartReveal: () => isHexInStartReveal,
  validateFirstPlayerCityPlacement: () => validateFirstPlayerCityPlacement
});
module.exports = __toCommonJS(first_player_city_entry_exports);

// src/game/first-player-city.ts
var FIRST_CITY_START_REVEAL_DENIED = "poza o\u015Bwietlonym obszarem startu";
function isAwaitingFirstPlayerCity(playerEverOwnedCity, cities, playerOwnerId = 0) {
  return !playerEverOwnedCity && !cities.some((c) => c.ownerId === playerOwnerId);
}
function isHexInStartReveal(q, r, awaitingFirstCity, playerStartHex, startRevealRadius, hexDistance2) {
  if (!awaitingFirstCity || playerStartHex === null) return true;
  return hexDistance2(q, r, playerStartHex.q, playerStartHex.r) <= startRevealRadius;
}
function validateFirstPlayerCityPlacement(q, r, awaitingFirstCity, inStartReveal, baseOk, baseReason = "") {
