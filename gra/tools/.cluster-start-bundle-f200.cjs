...
// tools/.cluster-start-entry-f200.ts
var cluster_start_entry_f200_exports = {};
__export(cluster_start_entry_f200_exports, {
  CLUSTER_CITY_STATE_MAX_HEX: () => CLUSTER_CITY_STATE_MAX_HEX,
  CLUSTER_CITY_STATE_MIN_HEX: () => CLUSTER_CITY_STATE_MIN_HEX,
  ISLAND_FALLBACK_MASS_FRAC: () => ISLAND_FALLBACK_MASS_FRAC,
  LOCAL_LAND_DOMINANCE_FRAC: () => LOCAL_LAND_DOMINANCE_FRAC,
  LOCAL_LAND_DOMINANCE_RADIUS: () => LOCAL_LAND_DOMINANCE_RADIUS,
  MIN_DEVELOPMENT_HEX_PER_CIV: () => MIN_DEVELOPMENT_HEX_PER_CIV,
  ...
  MIN_MASS_HEXES_FOR_SPAWN: () => MIN_MASS_HEXES_FOR_SPAWN,
  PLAYER_START_MASS_MIN_ABSOLUTE: () => PLAYER_START_MASS_MIN_ABSOLUTE,
  PLAYER_START_MIN_MASS_HEXES: () => PLAYER_START_MIN_MASS_HEXES,
  REGION_MASS_DOMINANCE_FRAC: () => REGION_MASS_DOMINANCE_FRAC,
  SMALL_MASS_CAP_THRESHOLD: () => SMALL_MASS_CAP_THRESHOLD,
  allocateTypyToMasses: () => allocateTypyToMasses,
  ...
  capitalMaxSeaDist: () => capitalMaxSeaDist,
  capitalMinSeaDist: () => capitalMinSeaDist,
  capitalMinSeparation: () => capitalMinSeparation,
  capitalMinSeparationForMap: () => capitalMinSeparationForMap,
  civIdsAvailableAtGameEpoch: () => civIdsAvailableAtGameEpoch,
  clusterCityStateRadius: () => clusterCityStateRadius,
  clusterCohesionMaxHex: () => clusterCohesionMaxHex,
  clusterPackRadius: () => clusterPackRadius,
  computeClusters: () => computeClusters,
  computeSameTypeRivalHalfPlaneAxis: () => computeSameTypeRivalHalfPlaneAxis,
  developmentSpaceScore: () => developmentSpaceScore,
  ...
  isInSameTypeRivalHalfPlane: () => isInSameTypeRivalHalfPlane,
  localLandFraction: () => localLandFraction,
  massTypeCap: () => massTypeCap,
  packCityStatesHubChain: () => packCityStatesHubChain,
  packRivalCitiesAroundCore: () => packRivalCitiesAroundCore,
  passesCapitalSeaBandGate: () => passesCapitalSeaBandGate,
  passesLocalLandGate: () => passesLocalLandGate,
  passesMinCapitalSeparationGate: () => passesMinCapitalSeparationGate,
  passesMinSeaDistGate: () => passesMinSeaDistGate,
  passesPlayerStartMassGate: () => passesPlayerStartMassGate,
  pickPlayerClusterCenter: () => pickPlayerClusterCenter,
  qualifyingMassIndicesForSpawn: () => qualifyingMassIndicesForSpawn,
  regionMassDominance: () => regionMassDominance,
  rosterKluczeForStartEpoch: () => rosterKluczeForStartEpoch,
  seaDistAt: () => seaDistAt,
  setRiverGenPhaseOverride: () => setRiverGenPhaseOverride
});
module.exports = __toCommonJS(cluster_start_entry_f200_exports);
...
function setRiverGenPhaseOverride(phase) {
  ...
  while (path.length < growthTarget && path.length < stepCap) {
    ...
    if (candidates.length === 0) {
      if (hardMeander && path.length < hardMeanderLen) {
        ...
        const pick2 = softCandidates[0];
        path.push({ q: pick2.q, r: pick2.r });
        visited.add(hexKey(pick2.q, pick2.r));
        continue;
      }
      break;
    }
...
var MIN_MASS_HEXES_FOR_SPAWN = 60;
...
var FOREIGN_MASS_MIN_ABSOLUTE = 90;
var FOREIGN_MASS_FRAC_OF_LARGEST = 0.09;
var LOCAL_LAND_DOMINANCE_FRAC = 0.7;
var REGION_MASS_DOMINANCE_FRAC = LOCAL_LAND_DOMINANCE_FRAC;
var LOCAL_LAND_DOMINANCE_RADIUS = 3;
var PLAYER_START_MIN_MASS_HEXES = 25;
var PLAYER_START_MASS_MIN_ABSOLUTE = 30;
function foreignSpawnMassThreshold(largestMassSize) {
  return Math.max(
    FOREIGN_MASS_MIN_ABSOLUTE,
    Math.floor(largestMassSize * FOREIGN_MASS_FRAC_OF_LARGEST)
  );
}
function massQualifiesForForeignSpawn(massSize, masses) {
...
function createMassLandCache(ladowe) {
...
function allocateTypyToMasses(nForeignTypy, masses) {
  ...
  remainders.sort(
    (a, b) => b.remainder - a.remainder || b.massSize - a.massSize || b.exact - a.exact || a.massIdx - b.massIdx
  );
  ...
  if (leftover > 0 && typeof console !== "undefined") {
    console.warn(
      `[clusters] allocateTypyToMasses: ${leftover} obcych typ\xF3w bez miejsca po capach (n=${nForeignTypy}, largest=${largestSize}, threshold=${foreignSpawnMassThreshold(largestSize)})`
    );
  }
  return slots;
}
function qualifyingMassIndicesForSpawn(masses) {
...
function sortSpawnLandCandidates(candidates, mapCenter, seaFirst = false, minSeaDist = 0, maxSeaDist = 0) {
  candidates.sort((a, b) => {
...
function sortSpawnCandidatesWithSpreadBias(candidates, mapCenter, priorCapitals, landBBox, spreadN, seaFirst = false, minSeaDist = 0, maxSeaDist = 0) {
  ...
  const enriched = candidates.map((c) => ({
    ...
    minExist: priorCapitals.length === 0 ? 999 : Math.min(...priorCapitals.map((p) => hexDistanceAxial(c.h.q, c.h.r, p.q, p.r)))
  }));
  let pool = enriched;
...
function pickTiedSpawnLandCandidate(candidates, rand) {
  if (candidates.length === 0) return null;
  const top = candidates[0];
  if (candidates.length === 1) return top.h;
  const tie = candidates.filter(
    (x) => x.dev >= top.dev - 1 && x.land.ratio >= top.land.ratio - 1e-3 && x.sea <= top.sea + 1
  );
  return tie[Math.floor(rand() * tie.length)].h;
}
function playerStartCandidatesOnMasses(map, landCache, seaDist, minSeaDist, requireSeaGate) {
  const massOrder = landCache.masses.length > 0 ? [landCache.masses[0], ...landCache.masses.slice(1)] : [];
  const out = [];
...
function pickPlayerClusterCenter(map, landCacheOrMasses, ladowe, mapCenter, rand, seaDist, minSeaDist = 0) {
  ...
  let fallback = ladowe.map((h) => ({
    h,
    land: localLandFraction(map, h.q, h.r),
    dev: developmentSpaceScore(map, h.q, h.r, landCache),
    sea: seaDist ? seaDistAt(seaDist, h.q, h.r) : minSeaDist
  })).filter((x) => passesPlayerStartMassGate(map, x.h.q, x.h.r, landCache)).filter((x) => !seaDist || passesMinSeaDistGate(seaDist, x.h.q, x.h.r, minSeaDist));
  sortSpawnLandCandidates(fallback, mapCenter);
  return fallback[0]?.h ?? null;
}
function pickBestLocalLandSpawn(map, pool, existing, minDist, landCache, rand, seaDist, minSeaDist = 0) {
  const base = pool.filter((h) => existing.every((p) => hexDistanceAxial(h.q, h.r, p.q, p.r) >= minDist)).map((h) => ({
    h,
    land: localLandFraction(map, h.q, h.r),
    dev: developmentSpaceScore(map, h.q, h.r, landCache),
    sea: seaDist ? seaDistAt(seaDist, h.q, h.r) : minSeaDist
  ...
  if (candidates.length > 1 && rand) {
    const tieBand = candidates.filter(
      (x) => x.dev >= top.dev - 1 && x.land.ratio >= top.land.ratio - 1e-3 && x.sea <= top.sea + 1
    );
...
function pickCenterInMassSpread(map, mass, existing, minDist, landCache, rand, seaDist, minSeaDist = 0, landBBox, spreadOpts) {
  const midQ = landBBox ? (landBBox.minQ + landBBox.maxQ) / 2 : 0;
  const midR = landBBox ? (landBBox.minR + landBBox.maxR) / 2 : 0;
  const quadCounts = landBBox ? countCentersPerQuadrant(existing, midQ, midR) : null;
  const base = mass.filter((h) => existing.every((p) => hexDistanceAxial(h.q, h.r, p.q, p.r) >= minDist)).map((h) => ({
    h,
    land: localLandFraction(map, h.q, h.r),
    sea: seaDist ? seaDistAt(seaDist, h.q, h.r) : minSeaDist,
    minExist: existing.length === 0 ? 999 : Math.min(...existing.map((p) => hexDistanceAxial(h.q, h.r, p.q, p.r))),
...
function pickCenterOnLandPoolSpread(map, pool, existing, minDist, landCache, rand, seaDist, minSeaDist = 0, landBBox, spreadOpts) {
...
function balanceClusterSpread(map, centers, ladowe, landCache, minDist, rand, seaDist, minSeaDist, landBBox, maxIterations = 2, spreadOpts) {
  if (centers.length < 2 || maxIterations <= 0) return centers;
  const out = centers.map((c) => ({ ...c }));
  for (let iter = 0; iter < maxIterations; iter++) {
    let farthestHole = null;
    for (const h of ladowe) {
    ...
    for (let i = 0; i < out.length; i++) {
      const others2 = out.filter((_, j) => j !== i);
      const sep = minCapitalSeparationDistance(out[i], others2) ?? Infinity;
      if (sep < tightestSep) {
        tightestSep = sep;
        tightestIdx = i;
      }
    ...
    if (towardHole < currentToward - 2 || farthestHole.minCenterDist > minDist * 1.25 || spreadAfter > spreadBefore) {
...
function enforceQuarterSpreadOnCenters(map, centers, ladowe, landCache, minDist, rand, seaDist, minSeaDist, landBBox, nNeeded) {
  if (nNeeded < 5 || centers.length < 3) return centers;
  const required = minimumSpreadQuartersRequired(nNeeded);
  const out = centers.map((c) => ({ ...c }));
  const midQ = (landBBox.minQ + landBBox.maxQ) / 2;
  const midR = (landBBox.minR + landBBox.maxR) / 2;
  const spreadOpts = { nNeeded };
  for (let iter = 0; iter < 10; iter++) {
...
function placeClusterCentersAcrossLandmasses(map, ladowe, landCache, nNeeded, minDistBase, mapCenter, rand, marginBrzeg, bounds, seaDist, minSeaDist = 0) {
  ...
  function countCentersOnMass(massIdx) {
    return centers.filter((c) => massContainingHex(hexIndex, c.q, c.r) === massIdx).length;
  }
  function tryPlace(c, minDist, relaxMargin, forPlayer = false, softLocalLand = false) {
  ...
  const playerOnMass0 = playerCenter && massContainingHex(hexIndex, playerCenter.q, playerCenter.r) === 0;
  const qualifyingOrder = qualifyingIndices.sort((a, b) => masses[b].length - masses[a].length);
  for (const mi of qualifyingOrder) {
    ...
    while (centers.length < nNeeded) {
      ...
      const candidate = masses.length === 1 ? pickCenterOnLandPoolSpread(
        map,
        placementPool,
        centers,
        hardMin,
  ...
  if (centers.length < nNeeded) {
    ...
    while (centers.length < nNeeded && guard++ < nNeeded * 16) {
      const before = centers.length;
      const candidate = placementPool.filter((h) => centers.every((p) => hexDistanceAxial(h.q, h.r, p.q, p.r) >= hardMin)).map((h) => ({
        h,
        minExist: centers.length === 0 ? 999 : Math.min(...centers.map((p) => hexDistanceAxial(h.q, h.r, p.q, p.r))),
  ...
  if (centers.length < nNeeded && typeof console !== "undefined") {
    console.warn(
      `[clusters] placeClusterCenters: tylko ${centers.length}/${nNeeded} \u015Brodk\xF3w (twardy minSep=${hardMin}; bez luzowania \u2014 lepiej mniej civ ni\u017C kupa)`
    );
  }
...
function capitalMinSeparationForMap(rozmiar, mapW, mapH) {
  const short = Math.min(mapW, mapH);
  if (short < 80) return 0;
  return capitalMinSeparation(rozmiar);
}
function clusterBodySeparation(rozmiar) {
  return capitalMinSeparation(rozmiar);
}
function clusterCenterPlacementSep(rozmiar) {
  return capitalMinSeparation(rozmiar) + 2;
}
function clusterBodySeparationForMap(rozmiar, mapW, mapH) {
  const short = Math.min(mapW, mapH);
  if (short < 80) return 0;
  return clusterBodySeparation(rozmiar);
}
function clusterCenterPlacementSepForMap(rozmiar, mapW, mapH) {
  const short = Math.min(mapW, mapH);
  if (short < 80) return 0;
  return clusterCenterPlacementSep(rozmiar);
}
function clusterBodyBufferRadius(sCluster) {
  if (!Number.isFinite(sCluster) || sCluster <= 0) return 0;
  return Math.ceil(sCluster / 2);
}
function passesForeignClusterBufferGate(h, buffers) {
  for (const b of buffers) {
    if (hexDistanceAxial(h.q, h.r, b.q, b.r) < b.radius) return false;
  }
  return true;
}
function landQuadrantIndex(q, r, midQ, midR) {
  const east = q >= midQ ? 1 : 0;
  const south = r >= midR ? 1 : 0;
  return east + south * 2;
}
function countCentersPerQuadrant(centers, midQ, midR) {
...
function minimumSpreadQuartersRequired(nNeeded) {
  if (nNeeded >= 6) return 3;
  if (nNeeded >= 5) return 3;
  if (nNeeded >= 3) return 2;
  return 1;
}
function preferredQuadrantsForSpread(quadCounts, nNeeded) {
...
function clusterCenterQuadrantSpread(centers, landBBox) {
  if (centers.length === 0) {
    return { occupiedQuadrants: 0, qSpan: 0, rSpan: 0 };
  }
  const midQ = (landBBox.minQ + landBBox.maxQ) / 2;
  const midR = (landBBox.minR + landBBox.maxR) / 2;
  const quads = /* @__PURE__ */ new Set();
  let minQ = Infinity;
  let maxQ = -Infinity;
  let minR = Infinity;
  let maxR = -Infinity;
...
function passesMinCapitalSeparationGate(c, priorCapitals, minSep) {
...
function minCapitalSeparationDistance(c, others) {
...
function seaDistRescueLadder(minSeaDist) {
  ...
  const steps = [
    ...
    2,
    0
    // absolutna ostatnia szansa — wolny ląd przy brzegu > drop civ
  ];
  return [...new Set(steps)];
}
function pickSpawnHexWithCapitalGates(map, pool, landCache, mapCenter, rand, opts = {}) {
  const {
    seaDist,
    minSeaDist = 0,
    priorCapitals = [],
  ...
  const maxSeaDist = opts.maxSeaDist != null ? opts.maxSeaDist : minSeaDist > 0 ? minSeaDist + 5 : 0;
  const seaSteps = progressiveSoftSea ? seaDistRescueLadder(minSeaDist) : [minSeaDist];
  for (const seaMin of seaSteps) {
    ...
    if (preferred && passes(preferred)) {
      if (!seaDist || maxSeaDist <= 0 || seaMin < minSeaDist || passesCapitalSeaBandGate(seaDist, preferred.q, preferred.r, seaMin, maxSeaDist)) {
        return preferred;
      }
    }
    let candidates = pool.filter(passes).map((h) => ({
      h,
      land: localLandFraction(map, h.q, h.r),
      dev: developmentSpaceScore(map, h.q, h.r, landCache),
...
function computeSameTypeRivalHalfPlaneAxis(capital, mapCenter, seed) {
...
function isInSameTypeRivalHalfPlane(hex, capital, axis, margin = -0.01) {
...
function packCityStatesAroundCapital(allLand, region, capital, stateCityCount, minDist, seed, opts) {
  ...
  const minDistLevels = halfPlaneAxis ? [...new Set([
    minDist,
    Math.max(3, minDist - 1),
    Math.max(3, minDist - 2)
  ].filter((d) => d >= 3))] : [minDist];
  const ringDistances = opts?.ringDistances?.length ? [...opts.ringDistances] : [CLUSTER_CITY_STATE_MAX_HEX];
  let best = [];
...
function packRivalCitiesAroundCore(landHexes, core, rivalCount, minDist, seed, mapCenter) {
  if (rivalCount <= 0) return [];
  const halfPlaneAxis = mapCenter ? computeSameTypeRivalHalfPlaneAxis(core, mapCenter, seed) : void 0;
...
function pickCapitalHexInRegion(region, blobCenter, rand, map, seaDist, minSeaDist = 0, anchor, priorCapitals = [], minCapitalSep = 0) {
...
function buildClusterLayoutWithEdgeCapital(region, centrum, stateCityCount, minDist, rand, anchor, growthReserve = CLUSTER_GROWTH_RESERVE, map, seed = 42, seaDist, minSeaDist = 0, priorCapitals = [], minCapitalSep = 0) {
...
function buildClusterCities(region, centrum, stateCityCount, minDist, rand, anchor, seed, map, seaDist, minSeaDist = 0, priorCapitals = [], minCapitalSep = 0) {
...
function buildClusterCitiesSimpleFallback(region, centrum, stateCityCount, minDist, anchor, seed, map, seaDist, minSeaDist = 0, priorCapitals = [], minCapitalSep = 0) {
  let pool = region;
  if (anchor) {
...
function assignTypesToClusterCenters(centrumy, masses, playerKlucz, rosterBezGracza, rand) {
  const hexIndex = buildMassHexIndex(masses);
  const nForeign = Math.max(0, centrumy.length - 1);
  const foreignAllocation = allocateTypyToMasses(nForeign, masses);
  const centersByMass = /* @__PURE__ */ new Map();
  ...
  const aktywneKlucze = new Array(centrumy.length);
  aktywneKlucze[0] = playerKlucz;
  let typeIdx = 0;
  const massOrder = qualifyingMassIndicesForSpawn(masses).sort((a, b) => masses[b].length - masses[a].length);
...
function enforceLocalLandDominance(map, centrumy, regiony, aktywneKlucze, ladowe, landCache, rand, minDist, mapCenter, seaDist, minSeaDist = 0) {
  const masses = landCache.masses;
  let relocated = false;
  for (let ci = 0; ci < centrumy.length; ci++) {
    const center = centrumy[ci];
    const ok = ci === 0 ? passesPlayerStartMassGate(map, center.q, center.r, landCache) : passesLocalLandGate(map, center.q, center.r);
    if (ok) continue;
    ...
    if (ci === 0) {
      ...
      if (!newCenter) {
        const qualifying = qualifyingMassIndicesForSpawn(masses).sort((a, b) => masses[b].length - masses[a].length);
  ...
  const spreadOpts = { nNeeded: centrumy.length };
  ...
  for (let ci = 1; ci < centrumy.length; ci++) {
    ...
    let rescued = pickSpawnHexWithCapitalGates(
      ...
      rand,
      {
        seaDist,
        minSeaDist,
        priorCapitals: others,
        minCapitalSep: minDist,
        requireLocalLand: true,
  ...
  if (!passesLocalLandGate(map, centrumy[0].q, centrumy[0].r) || !passesPlayerStartMassGate(map, centrumy[0].q, centrumy[0].r, landCache)) {
  ...
  if (dropped > 0 && typeof console !== "undefined") {
    ...
    console.warn(
      `[clusters] enforceLocalLandDominance: odrzucono ${dropped} typ(\xF3w) (${droppedKeys.join(", ")}) \u2014 bramka lokalnego l\u0105du \u2265${LOCAL_LAND_DOMINANCE_FRAC * 100}%`
    );
  }
...
function buildClusterCitiesWithLandGate(map, region, centrum, stateCityCount, minDist, rand, anchor, seed, landCache, existingCenters, minClusterDist, seaDist, minSeaDist = 0, priorCapitals = [], minCapitalSep = 0) {
  const masses = landCache.masses;
...
function clusterCapitalHex(k) {
  const m = k.miasta.find((x) => x.isCapital) ?? k.miasta[0];
  return m ? { q: m.q, r: m.r } : null;
}
function enforceHardCapitalSeparationOnKlastry(klastry, clusterRegions, map, rand, seaDist, minSeaDist, minCapitalSep, minDystObcyOdGracza, ladowe, landCache, seed, stateCityCount, mapCenter, minDystMiastaPanstwa, spreadGateOpts) {
  if (!Number.isFinite(minCapitalSep) || minCapitalSep <= 0) return;
  const dropCluster = (ki) => {
    ...
    outer:
      for (let i = 0; i < caps.length; i++) {
        for (let j = i + 1; j < caps.length; j++) {
          ...
          const priorCaps = caps.filter((_, idx) => idx !== fixIdx).map((c) => ({ q: c.q, r: c.r }));
          const playerCap = caps.find((c) => c.ki === 0);
          const anchor = fixKi === 0 || !playerCap ? void 0 : {
            q: playerCap.q,
            r: playerCap.r,
          ...
          const { stateCities, growthSlot } = packCityStatesAroundCapital(
            ...
            stateCityCount,
            minDist,
            seed + fixKi * 2654435761 >>> 0,
            { excludeHex: newCap, anchor }
          );
          const cities = [{ q: newCap.q, r: newCap.r, isCapital: true }];
          for (const s of stateCities) {
            cities.push({ q: s.q, r: s.r, isCapital: false });
          }
          k.miasta = cities;
...
function allClusterCities(k) {
  return k.miasta.map((m) => ({ q: m.q, r: m.r }));
}
function foreignBuffersForCluster(klastry, excludeKi, bufferRadius) {
  const out = [];
  for (let ki = 0; ki < klastry.length; ki++) {
...
function foreignRepackAnchor(ki, cap, playerCap, minDystObcyOdGracza) {
  if (ki === 0 || !playerCap) return void 0;
  const capDist = hexDistanceAxial(cap.q, cap.r, playerCap.q, playerCap.r);
  if (capDist + CLUSTER_CITY_STATE_MAX_HEX < minDystObcyOdGracza) return void 0;
  return { q: playerCap.q, r: playerCap.r, minDist: minDystObcyOdGracza };
}
function repackClusterStateCitiesIfSparse(klastry, clusterRegions, ki, stateCityCount, minDystObcyOdGracza, ladowe, seed, bufferRadius, mapCenter, minDystMiastaPanstwa) {
  const k = klastry[ki];
  if (!k) return;
  ...
  const { stateCities, growthSlot } = packCityStatesAroundCapital(
    ...
    stateCityCount,
    minDist,
    seed + ki * 2654435761 >>> 0,
    {
      excludeHex: cap,
      anchor,
      foreignBuffers: foreignBufs,
      growthReserve: CLUSTER_GROWTH_RESERVE,
      halfPlaneAxis: ki === 0 ? computeSameTypeRivalHalfPlaneAxis(cap, mapCenter, seed) : void 0
    }
  );
  let validMp = stateCities.filter(
    (s) => passesForeignClusterBufferGate(s, foreignBufs)
  );
  ...
  if (validMp.length === 0) {
    ...
    const retry = packCityStatesAroundCapital(
      ...
      seed + ki * 2246822507 >>> 0,
      {
        ...
        halfPlaneAxis: ki === 0 ? computeSameTypeRivalHalfPlaneAxis(cap, mapCenter, seed + 17 >>> 0) : void 0
      }
    );
    validMp = retry.stateCities.filter(
      (s) => passesForeignClusterBufferGate(s, looseBufs)
    );
  ...
  const cities = [{ q: cap.q, r: cap.r, isCapital: true }];
...
function repackAllSparseClusterStateCities(klastry, clusterRegions, stateCityCount, minDystObcyOdGracza, ladowe, seed, bufferRadius, mapCenter, minDystMiastaPanstwa) {
  for (let ki = 0; ki < klastry.length; ki++) {
...
function enforceClusterBodySeparationOnKlastry(klastry, clusterRegions, map, rand, seaDist, minSeaDist, sCluster, minCapitalSep, minDystObcyOdGracza, ladowe, landCache, seed, stateCityCount, mapCenter, minDystMiastaPanstwa, bufferRadius, spreadGateOpts) {
  if (!Number.isFinite(sCluster) || sCluster <= 0) return;
  function bodyViolation(i, j) {
        ...
        for (let j = i + 1; j < klastry.length; j++) {
          ...
          const { stateCities, growthSlot } = packCityStatesAroundCapital(
            ...
            seed + fixKi * 2654435761 >>> 0,
            {
              ...
              anchor,
              foreignBuffers: foreignBufs,
              halfPlaneAxis: fixKi === 0 ? computeSameTypeRivalHalfPlaneAxis(newCap, mapCenter, seed) : void 0
            }
          );
          const cities = [{ q: newCap.q, r: newCap.r, isCapital: true }];
          for (const s of stateCities) {
...
function enforceQuarterSpreadOnKlastry(klastry, clusterRegions, map, landCache, ladowe, landBBox, nNeeded, rand, seaDist, minSeaDist, minCapitalSep, minDystObcyOdGracza, seed, stateCityCount, mapCenter, minDystMiastaPanstwa, bodyBufferRadius) {
  ...
  for (let iter = 0; iter < 10; iter++) {
    ...
    const playerCap = caps.find((c) => c.ki === 0);
    const anchor = relocateKi === 0 || !playerCap ? void 0 : {
      q: playerCap.q,
      r: playerCap.r,
      minDist: minDystObcyOdGracza
    };
...
function computeClusters(map, opts) {
  ...
  const minDystKlastrow = opts?.minDystansKlastrow ?? (centerPlacementSep > 0 ? centerPlacementSep : minCapitalSepEarly > 0 ? minCapitalSepEarly : Math.max(4, Math.floor(shortSide / 15)));
  ...
  const placeholderKlucze = activeCentrumy.map(
    (_, i) => i === 0 ? playerKlucz : `typ${i}`
  );
  ...
  if (seaDist && minSeaDist > 0 && !passesMinSeaDistGate(seaDist, playerCapitalPos.q, playerCapitalPos.r, minSeaDist)) {
    ...
    if (fixed) {
      ...
      if (relocatedLayout && passesMinSeaDistGate(seaDist, relocatedLayout.cities.find((m) => m.isCapital)?.q ?? fixed.q, relocatedLayout.cities.find((m) => m.isCapital)?.r ?? fixed.r, minSeaDist) && passesMinCapitalSeparationGate(
      ...
      } else if (passesMinSeaDistGate(seaDist, fixed.q, fixed.r, minSeaDist) && passesMinCapitalSeparationGate(fixed, placedCapitals, minCapitalSep)) {
  ...
  if (minCapitalSep > 0 && !passesMinCapitalSeparationGate(playerCapitalPos, placedCapitals, minCapitalSep)) {
    resolvePlayerCapitalPos();
  }
  ...
  for (let ci = 1; ci < activeCentrumy.length; ci++) {
    ...
    if (!foreignLayoutResult || foreignLayoutResult.cities.length === 0) {
      if (typeof console !== "undefined") {
        console.warn(
          `[clusters] Pomini\u0119to typ '${activeKlucze[ci] ?? `typ${ci}`}' \u2014 brak hexu startowego z bramk\u0105 l\u0105du (region=${region.length} hex)`
        );
      }
      continue;
    }
    ...
    if (seaDist && softSeaFloor > 0 && !passesMinSeaDistGate(seaDist, foreignCap.q, foreignCap.r, softSeaFloor)) {
      if (typeof console !== "undefined") {
        console.warn(
          `[clusters] Pomini\u0119to typ '${activeKlucze[ci] ?? `typ${ci}`}' \u2014 stolica zbyt blisko morza (seaDist=${seaDistAt(seaDist, foreignCap.q, foreignCap.r)}, floor=${softSeaFloor})`
        );
      }
      continue;
    }
    if (!passesMinCapitalSeparationGate(
      ...
      if (typeof console !== "undefined") {
        console.warn(
          `[clusters] Pomini\u0119to typ '${activeKlucze[ci] ?? `typ${ci}`}' \u2014 stolica zbyt blisko innej stolicy (minSep=${minCapitalSep})`
        );
      }
      continue;
    }
  ...
  }
  {
    ...
    while (klastry.length < nTypy) {
      ...
      if (hexDistanceAxial(cap.q, cap.r, playerCapitalPos.q, playerCapitalPos.r) < minDystObcyOdGracza) {
        ...
        if (!alt) break;
        const typ2 = unusedKeys[keyIdx++] ?? `typ${klastry.length}`;
        klastry.push({
          typIndex: klastry.length,
          typ: typ2,
          centrum: alt,
        ...
        clusterRegions.push(ladowe.filter(
          (h) => hexDistanceAxial(h.q, h.r, alt.q, alt.r) <= Math.max(8, Math.floor(minDystKlastrow / 2))
        ));
      ...
      clusterRegions.push(ladowe.filter(
        (h) => hexDistanceAxial(h.q, h.r, cap.q, cap.r) <= Math.max(8, Math.floor(minDystKlastrow / 2))
      ));
      placedCapitals.push(cap);
    }
    if (klastry.length < nTypy && typeof console !== "undefined") {
      console.warn(
        `[clusters] FALA 193 top-up: nadal ${klastry.length}/${nTypy} klastr\xF3w po dope\u0142nieniu`
      );
    }
  }
  if (klastry.length > 0 && klastry[0].miasta.length > 0) {
    ...
    if (playerRepackFinal.stateCities.length > 0 || playerRepackFinal.growthSlot) {
      const cities = [{ q: playerCap.q, r: playerCap.r, isCapital: true }];
...
function landHexesFromMap(map) {
  ...
  for (const h of Object.values(map.hexes)) {
    if (h.terenBazowy === "morze" /* Morze */ || h.terenBazowy === "gory" /* Gory */ || h.terenBazowy === "wybrzeze" /* Wybrzeze */ || h.terenBazowy === "polarny" /* Polarny */) continue;
    out.push({ q: h.coords.q, r: h.coords.r });
  }
  return out;
}
function mapCenterFromGameMap(map) {
  return {
...
function buildClusterSpawnPlan(input) {
  ...
  if (resolvedPlayerCap) {
    ...
    } else if (minCapitalSep > 0 && !passesMinCapitalSeparationGate(capPos, plannedForeignCapitals, minCapitalSep)) {
      if (typeof console !== "undefined") {
        console.warn(
  ...
  function validateForeignCapital(q, r) {
    return passesLocalLandGate(map, q, r);
  }
  const slots = [];
  const clusterCapitalOwnerIds = [];
  let nextOwnerId = 1;
  const placedCapitals = [{ q: capPos.q, r: capPos.r }];
  ...
  const pendingSameTypeRivalOwnerIds = [];
  for (let i = 0; i < pendingSameTypeRivals; i++) {
    pendingSameTypeRivalOwnerIds.push(nextOwnerId++);
  }
  const hardPlacedCapitals = [{ q: capPos.q, r: capPos.r }];
  const hardAcceptedSlots = [];
  const droppedForeignTyps = /* @__PURE__ */ new Set();
  for (const slot of slots) {
  ...
  const hardClusterCapitalOwnerIds = hardAcceptedSlots.filter((s) => s.isClusterCapital).map((s) => s.ownerId);
  return {
...
0 && (module.exports = {
  ...
  ISLAND_FALLBACK_MASS_FRAC,
  LOCAL_LAND_DOMINANCE_FRAC,
  LOCAL_LAND_DOMINANCE_RADIUS,
  MIN_DEVELOPMENT_HEX_PER_CIV,
  MIN_DIST_FOREIGN_FROM_PLAYER,
  MIN_DIST_FOREIGN_IN_CLUSTER,
  MIN_DIST_START_CITY_STATE,
  MIN_MASS_HEXES_FOR_SPAWN,
  PLAYER_START_MASS_MIN_ABSOLUTE,
  PLAYER_START_MIN_MASS_HEXES,
  REGION_MASS_DOMINANCE_FRAC,
  SMALL_MASS_CAP_THRESHOLD,
  allocateTypyToMasses,
  ...
  clusterCohesionMaxHex,
  clusterPackRadius,
  computeClusters,
  computeSameTypeRivalHalfPlaneAxis,
  developmentSpaceScore,
  generateMap,
