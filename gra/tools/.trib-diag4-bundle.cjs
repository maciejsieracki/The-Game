...
// tools/.trib-diag4-entry.ts
var SEEDS = [42, 7];
var TYPES = ["kontynenty", "pangea", "wyspy", "ziemia"];
var fail = 0;
for (const seed of SEEDS) {
  for (const typ of TYPES) {
    const map = generujSwiat(seed, "standardowy", typ, {
      worldDensity: { rivers: "medium", forest: "medium", desert: "medium", relief: "medium" }
    });
    const W = map.szerokoscQ;
    const H = map.wysokoscR;
    const net = verifyRiverNetworkConnectivity(map.hexes, map.riverPaths ?? [], map.riverPathKinds ?? [], W, H);
    const j = checkTributaryJunctions(map.riverPaths ?? [], map.riverPathKinds ?? [], map.hexes, W, H);
    if (net.orphanCount > 0 || !j.ok) {
      fail++;
      console.log(`FAIL seed=${seed} typ=${typ} orphans=${net.orphanCount} trib=${j.violations}`);
    }
  }
}
console.log(fail === 0 ? "PASS all" : `FAIL ${fail} cases`);
