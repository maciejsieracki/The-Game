...
function formatWyzwienieLabel(level) {
...
function cityMapBadgeKey(a, population) {
  ...
  const prod = a.prodActive ? `${a.prodKind ?? "b"}:${(a.prodId || "").trim()}` : "-";
  const growth = a.growthLevel != null ? `g${formatWyzwienieLabel(a.growthLevel)}` : "g-";
  const cs = a.isCityState ? "cs1" : "cs0";
  const cap = a.isCapital ? "k1" : "k0";
  const era = a.era != null ? `e${Math.max(1, Math.round(a.era) || 1)}` : "e-";
  return [
    (a.cityName || "").trim(),
    pop,
    `d${a.defenseTier}`,
