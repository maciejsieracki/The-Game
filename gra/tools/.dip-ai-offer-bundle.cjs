...
// tools/.dip-ai-offer-entry.ts
var dip_ai_offer_entry_exports = {};
__export(dip_ai_offer_entry_exports, {
  AI_OFFER_PW_BALANCE_TOLERANCE_PN: () => AI_OFFER_PW_BALANCE_TOLERANCE_PN,
  adjustZaplataPerTuraForZeroBalance: () => adjustZaplataPerTuraForZeroBalance,
  aiAllowsOneSidedGoldGift: () => aiAllowsOneSidedGoldGift,
  aiAllowsTradeAgreementSweetener: () => aiAllowsTradeAgreementSweetener,
  aiOfferTargetsZeroBalance: () => aiOfferTargetsZeroBalance,
  aiProposalPlayerBenefitSurplus: () => aiProposalPlayerBenefitSurplus,
  computePlayerAcceptanceSides: () => computePlayerAcceptanceSides,
  diplomacyPnZloto: () => diplomacyPnZloto,
  pickMinimalSweetenerGold: () => pickMinimalSweetenerGold,
  responderPwSurplus: () => responderPwSurplus,
  targetResourceTradePaymentPn: () => targetResourceTradePaymentPn,
  trimProposalForZeroBalance: () => trimProposalForZeroBalance,
  trimProposalGoldForZeroBalance: () => trimProposalGoldForZeroBalance,
  trimQuickDealGiveToTolerance: () => trimQuickDealGiveToTolerance,
  trimResourcePaymentTradeForZeroBalance: () => trimResourcePaymentTradeForZeroBalance
});
module.exports = __toCommonJS(dip_ai_offer_entry_exports);
...
function aiOfferPwUndershootAllowance(difficulty = "normal") {
...
function pnToPaymentAmount(paymentTyp, targetPn) {
  if (targetPn <= 0) return 0;
  if (paymentTyp === "zloto") {
    const perUnit2 = diplomacyPnZloto(1);
    if (perUnit2 <= 0) return targetPn;
    return Math.max(1, Math.ceil(targetPn / perUnit2));
  }
  const perUnit = diplomacyPnPraca(1);
  if (perUnit <= 0) return targetPn;
  return Math.max(1, Math.ceil(targetPn / perUnit));
}
function targetResourceTradePaymentPn(resourceGivePn, relTotal, difficulty = "normal") {
...
function adjustZaplataPerTuraForZeroBalance(zaplataPerTura, zaplataTyp, resourceGivePn, relTotal, difficulty = "normal") {
...
function pickMinimalSweetenerGold(basePayload, relTotal, _difficulty, stepGold, maxSteps, isAccepted, _pnOpts) {
  let bestGold = null;
  for (let step = 1; step <= maxSteps; step++) {
    const extra = step * stepGold;
    const items = [...basePayload.giveItems ?? []];
    ...
    const candidate = { ...basePayload, giveItems: items };
    if (!isAccepted(candidate)) continue;
    if (bestGold == null || extra < bestGold) bestGold = extra;
  }
  return bestGold;
}
function trimQuickDealGiveToTolerance(giveItems, receivePn, relTotal, difficulty = "normal") {
