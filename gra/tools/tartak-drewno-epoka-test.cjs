...
// Keep the mutation executable and independent: mutate the emitted resolver formula,
// load the mutated bundle, and prove the same expectations turn red. This recreates
// the 1.5 -> 1.0 mutation without leaving production changes behind.
const mutatedBundle = `${bundle}.mutated.cjs`;
const originalBundleText = fs.readFileSync(bundle, 'utf8');
const mutationNeedle = 'Math.pow(1.5, era - 1)';
if (!originalBundleText.includes(mutationNeedle)) {
  throw new Error(`mutation target missing: ${mutationNeedle}`);
}
fs.writeFileSync(mutatedBundle, originalBundleText.replace(mutationNeedle, 'Math.pow(1.0, era - 1)'), 'utf8');
const { computeTerritoryResourceYieldByCity: mutatedResolver } = require(mutatedBundle);
...
function woodWithResolver(resolver, tartakCount, era, includeQuarry = false) {
...
function wood(tartakCount, era, includeQuarry = false) {
  return woodWithResolver(computeTerritoryResourceYieldByCity, tartakCount, era, includeQuarry);
}
...
// Invalid-era guard: non-positive eras clamp to the epoch-1 production rate.
eq(wood(1, 0), 200, 'epoka 0 (niepoprawna) = bezpieczny fallback epoki 1');

// Formula mutation proof: 1.5 -> 1.0 must violate the era-2/3 expectations.
let mutationFailures = 0;
for (const era of [1, 2, 3]) {
  for (const tartakCount of [1, 2]) {
    const expected = [200, 300, 450][era - 1] * tartakCount;
    if (woodWithResolver(mutatedResolver, tartakCount, era) !== expected) mutationFailures += 1;
  }
}
eq(mutationFailures > 0, true, 'mutacja formuły 1.5→1.0 czerwieni oczekiwania epok');
console.log(`  [INFO] mutacja formuły: ${mutationFailures} czerwonych przypadków`);
...
for (const file of [entry, bundle, mutatedBundle]) {
  try { fs.unlinkSync(file); } catch (_) { /* best effort cleanup */ }
}
