/**
 * .kamien-inka-preview-entry.ts — entry TYLKO do podgladu porownawczego
 * (dyspozycje/PODGLAD-KAMIEN-INKA.html). NIE jest czescia gry.
 *
 ...
 *
 *   node tools/build-kamien-inka-preview.cjs
 *
 * (Szkielet HTML — style, podpisy, przyciski — jest w samym pliku podgladu;
 * skrypt podmienia tylko wklejony inline bundle.)
 */
import * as THREE from 'three';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';
import { buildMaceWarrior, buildInkaJavelineer } from '../src/render/jednostki-p2-inka';
import { buildMaceWarriorOpus5, buildInkaJavelineerOpus5 } from '../src/render/kamien-inka-opus5';
...
function makePanel(canvas: HTMLCanvasElement, key: string, build: (c: number) => THREE.Group): Panel {
  ...
  // heks pod figurka — pokazuje czytelnosc w realnej skali pola (R = 1)
  ...
  return { scene, renderer, pivot, model, build, hex, canvas, key };
}
...
function updateStats(): void {
  for (const p of panels) {
    ...
    setText(`stat-${p.key}-mesh`, String(c.mesh));
    setText(`stat-${p.key}-tri`, String(c.tri));
    setText(`stat-${p.key}-mat`, String(c.mat));
    setText(`stat-${p.key}-h`, (box.max.y / HEX_R).toFixed(3));
  }
}
...
function setView(kind: string): void {
  ...
  const dist = kind === 'gra' ? 1.62 : kind === 'oko' ? 1.44 : 1.36;
...
// ── init ──────────────────────────────────────────────────────────────────
const ids: [string, string, (c: number) => THREE.Group][] = [
  ['cv-a1', 'a1', buildMaceWarrior],
  ['cv-a2', 'a2', buildMaceWarriorOpus5],
  ['cv-b1', 'b1', buildInkaJavelineer],
  ['cv-b2', 'b2', buildInkaJavelineerOpus5],
];
for (const [cid, key, build] of ids) {
  const cv = document.getElementById(cid) as HTMLCanvasElement | null;
  if (cv) panels.push(makePanel(cv, key, build));
}
