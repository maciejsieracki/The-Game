...
// tools/_tmp-pangea-perf-entry.ts
...
function massRiverCoveragePasses(massSize, pangeaSingleMass = false) {
  const base = Math.max(6, Math.min(24, 6 + Math.floor(Math.sqrt(massSize / 300))));
  if (!pangeaSingleMass) return base;
  return Math.max(4, Math.min(8, 4 + Math.floor(Math.sqrt(massSize / 1200))));
}
function riverProximityMaxRounds(massSize, pangeaSingleMass = false) {
...
function effectiveTopUpPasses(basePasses, pangeaSingleMass) {
  ...
  return Math.max(3, Math.ceil(basePasses * 0.45));
}
...
function ensureRiverGridAndProximity(hexes, massSet, cellSize, seaDist, gridCtx, maxProximityDist = RIVER_PROXIMITY_MAX_DIST, pangeaSingleMass = false) {
  const maxRounds = riverProximityMaxRounds(massSet.size, pangeaSingleMass);
  const forceCtx = {
    ...gridCtx,
    acceptLen: 3,
    sourceSep: 0,
...
function generateRivers(hexes, width, height, rand, opts = {}) {
  ...
  let stage2Total = 0;
  for (const mass of masses) {
    stage2Total += massRiverCoveragePasses(mass.length, pangeaSingleMass);
  }
  ...
  for (const mass of masses) {
    const passes = massRiverCoveragePasses(mass.length, pangeaSingleMass);
    let idleRounds = 0;
...
function bench(label, w, h, typ, seed) {
  const t0 = import_node_perf_hooks.performance.now();
  const map = generateMap(w, h, seed, typ, {
    mapSizeMenuLabel: w >= 200 ? "Du\u017Cy" : "Ma\u0142y",
    worldDensity: DENSITY
  });
  const ms = import_node_perf_hooks.performance.now() - t0;
  console.log(`${label}: ${(ms / 1e3).toFixed(2)}s | rzek=${map.riverPaths.length} hash=${map.hash?.slice(0, 8) ?? "?"}`);
  return ms;
}
console.log("=== perf kontynenty vs pangea (seed 42) ===");
bench("Ma\u0142y kontynenty 84\xD760", 84, 60, "kontynenty", 42);
bench("Ma\u0142y pangea 84\xD760", 84, 60, "pangea", 42);
console.log("--- Du\u017Cy (mo\u017Ce trwa\u0107) ---");
bench("Du\u017Cy pangea 240\xD7168", 240, 168, "pangea", 42);
