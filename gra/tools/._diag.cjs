...
function buildGridRouteCandidates(ctx, sq, sr, massSet) {
  ...
  const seaBufferOk = riverPathRespectsSeaBuffer(
    hexes,
    seaPath,
    seaDist,
  ...
  if (seaPath.length >= acceptLen && pathEndsAtSea(hexes, seaPath, width, height, oceanConnected) && (seaBufferOk || ctx.relaxSeaBuffer)) {
  ...
  if (riverKeys.size > 0) {
    ...
    for (const j of rankNetworkJunctionCandidates(sq, sr, riverKeys, seaDist, traceMax, rand)) {
      const p = traceTributary(hexes, sq, sr, j.q, j.r, traceMax, seaDist, rand, acceptLen);
  ...
  if (!massHasRiver()) {
    ...
    if (bootstrapLand) {
      ...
      for (const c of ranked.slice(0, 8)) {
        ...
        const seaPath = traceRiver(gridCtx.hexes, c.q, c.r, traceMax, {
