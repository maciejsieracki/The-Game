...
// tools/.dip-peace-lock-entry.ts
var dip_peace_lock_entry_exports = {};
__export(dip_peace_lock_entry_exports, {
  PEACE_TREATY_LOCK_TURNS: () => PEACE_TREATY_LOCK_TURNS,
  filterAllianceObligationsRespectingPeaceLock: () => filterAllianceObligationsRespectingPeaceLock,
  freshDiploPairMeta: () => freshDiploPairMeta,
  isPeaceTreatyLocked: () => isPeaceTreatyLocked,
  startPeaceTreatyLock: () => startPeaceTreatyLock
});
module.exports = __toCommonJS(dip_peace_lock_entry_exports);

// src/game/diplomacy-peace-lock.ts
var PEACE_TREATY_LOCK_TURNS = 10;
function isPeaceTreatyLocked(meta, currentTurn) {
  const until = meta?.peaceUntilTurn;
  if (until == null || !Number.isFinite(until)) return false;
  return currentTurn < until;
}
function startPeaceTreatyLock(meta, currentTurn, lockTurns = PEACE_TREATY_LOCK_TURNS) {
...
function filterAllianceObligationsRespectingPeaceLock(obligations, isPeaceLocked) {
  const out = [];
