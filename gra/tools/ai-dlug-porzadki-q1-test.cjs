...
console.log('\n-- (a) eliminateOwner() czyści aiSurplusRedirectedOwners + aiSliderStateByOwner --');
{
  ...
  const ctxNames = [
    ...
    // R-HOTSEAT-DYPLO-KONTAKT-PER-FOTEL-Q1: te 3 zmigrowane Set<number> -> Map<number,
    // Set<number>> per-fotel (main.ts) -- nazwy i typ atrapy (emptyMap() niżej) zaktualizowane,
    // sama migracja nie zmienia tego, co ten test dowodzi (a)/(b).
    'clusterCapitalOwnerIds', 'diplomaticContactEstablishedByHuman', 'diplomaticallyDiscoveredOwnersByHuman',
    'diplomaticDiscoveryPopupShownByHuman', 'battlePowerPtsByOwner', 'aiSurplusRedirectedOwners',
  ...
  if (cleanupSnippet) {
    ...
    fn(
      ...
      emptySet(), emptySet(), emptySet(), emptyMap(), emptyMap(), emptySet(), emptyMap(),
