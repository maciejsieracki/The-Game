...
// tools/.border-march-entry.ts
var border_march_entry_exports = {};
__export(border_march_entry_exports, {
  collectUnauthorizedBorderPairs: () => collectUnauthorizedBorderPairs,
  defaultIsMilitaryUnit: () => defaultIsMilitaryUnit,
  territoryOwnerAt: () => territoryOwnerAt
});
module.exports = __toCommonJS(border_march_entry_exports);
...
// src/game/border-march-scan.ts
function pairKey(intruder, owner) {
  return `${intruder}->${owner}`;
}
function collectUnauthorizedBorderPairs(units, allCityNodesByOwner, _isMilitaryUnit) {
...
function defaultIsMilitaryUnit(unit) {
