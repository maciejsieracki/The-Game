...
// tools/_tmp-duzy-pangea-audit-entry.ts
...
function massRiverCoveragePasses(massSize) {
  return Math.max(6, Math.min(24, 6 + Math.floor(Math.sqrt(massSize / 300))));
}
...
function ensureRiverGridAndProximity(hexes, massSet, cellSize, seaDist, gridCtx, maxProximityDist = RIVER_PROXIMITY_MAX_DIST) {
  const maxRounds = Math.max(16, Math.min(48, 12 + Math.floor(massSet.size / 350)));
  const forceCtx = {
    ...gridCtx,
    acceptLen: 3,
    sourceSep: 0,
  ...
  for (let round = 0; round < maxRounds; round++) {
    let roundPlaced = 0;
    const unfilled = listUnfilledRiverGridCells(massSet, hexes, cellSize, seaDist);
    for (const land of unfilled) {
      if (cellHasRiverHex(land, hexes)) continue;
      if (tryForceCellRiverConnection(forceCtx, land, massSet)) {
        roundPlaced++;
...
function generateRivers(hexes, width, height, rand, opts = {}) {
  ...
  for (const mass of masses) {
    const passes = massRiverCoveragePasses(mass.length);
...
console.log(`Du\u017Cy Pangea ${w}\xD7${h} seed=42`);
console.log("params:", JSON.stringify({ mainCell: params.mainCell, topUpPasses: params.topUpPasses, feederPasses: params.feederPasses, maxLen: params.maxLen, areaScale: +params.areaScale.toFixed(2) }));
var tLow0 = import_node_perf_hooks.performance.now();
generateMap(w, h, SEED, "pangea", { worldDensity: DENSITY_LOW, mapSizeMenuLabel: "Du\u017Cy" });
var lowMs = import_node_perf_hooks.performance.now() - tLow0;
console.log(`rivers=low: ${(lowMs / 1e3).toFixed(1)}s`);
var tFull0 = import_node_perf_hooks.performance.now();
var map = generateMap(w, h, SEED, "pangea", { worldDensity: DENSITY_MEDIUM, mapSizeMenuLabel: "Du\u017Cy" });
var fullMs = import_node_perf_hooks.performance.now() - tFull0;
var kinds = map.riverPathKinds ?? [];
console.log(`rivers=medium (tylko ten krok): ${(fullMs / 1e3).toFixed(1)}s`);
console.log(`delta rzek vs low: ${((fullMs - lowMs) / 1e3).toFixed(1)}s`);
console.log(`total szac.: ${((lowMs + (fullMs - lowMs)) / 1e3).toFixed(1)}s (low + delta)`);
console.log(`rzeki: total=${map.riverPaths?.length} main=${kinds.filter((k) => k === "main").length} med=${kinds.filter((k) => k === "medium").length}`);
