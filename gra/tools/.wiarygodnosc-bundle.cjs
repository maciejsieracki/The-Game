...
__export(wiarygodnosc_entry_exports, {
  ...
  napWygasaTuraFromPayload: () => napWygasaTuraFromPayload,
...
function napWygasaTuraFromPayload(turn, payload) {
  if (payload.bezterminowy === true) return null;
  const turns = clamp3(payload.turns ?? 15, 10, 20);
  return turn + turns;
}
...
function evaluateProposal(proposal, ctx) {
  ...
  switch (actionId) {
    case "nap": {
      ...
      const reason = wygasaTura === null ? "Pakt nieagresji bezterminowy zawarty" : `Pakt nieagresji na ${wygasaTura - ctx.turn} tur`;
