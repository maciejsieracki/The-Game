...
// tools/.wonder-map-build-entry.ts
var wonder_map_build_entry_exports = {};
__export(wonder_map_build_entry_exports, {
  advanceWonderMapBuilds: () => advanceWonderMapBuilds,
  listQualifyingWonderHexesForCity: () => listQualifyingWonderHexesForCity,
  listQualifyingWonderHexesForOwner: () => listQualifyingWonderHexesForOwner,
  ownerHasWonderBuildInProgress: () => ownerHasWonderBuildInProgress
});
module.exports = __toCommonJS(wonder_map_build_entry_exports);

// src/game/wonder-map-build.ts
function advanceWonderMapBuilds(sites, ownerId, pracaAvailable, kosztFn) {
...
function ownerHasWonderBuildInProgress(sites, ownerId, wonderId) {
  return sites.some(
    (s) => s.ownerId === ownerId && (wonderId == null || s.wonderId === wonderId)
  );
}
...
// src/map/wonder-placement.ts
function isLandBuildable(hex) {
  const t = hex.terenBazowy;
  return t !== "morze" /* Morze */ && t !== "gory" /* Gory */ && t !== "wybrzeze" /* Wybrzeze */;
}
function isOccupied(q, r, wonders, cityHexes) {
...
function listQualifyingWonderHexesForCity(input) {
...
function listQualifyingWonderHexesForOwner(input) {
