...
// tools/.gold-deficit-entry.ts
var gold_deficit_entry_exports = {};
__export(gold_deficit_entry_exports, {
  advanceGoldDeficit: () => advanceGoldDeficit,
  applyGoldDeficitHpLoss: () => applyGoldDeficitHpLoss,
  applyGoldDeficitStatMultToCombatUnit: () => applyGoldDeficitStatMultToCombatUnit,
  buildGoldDeficitParams: () => buildGoldDeficitParams,
  clearLastGoldDeficitTicks: () => clearLastGoldDeficitTicks,
  freshGoldDeficitState: () => freshGoldDeficitState,
  getGoldDeficitCountdown: () => getGoldDeficitCountdown,
  isGoldDeficit: () => isGoldDeficit,
  isGoldDeficitStarving: () => isGoldDeficitStarving
});
module.exports = __toCommonJS(gold_deficit_entry_exports);
...
// src/game/gold-deficit.ts
function pick(row, d, fallback) {
  if (!row) return fallback;
  const v = row[d];
  return Number.isFinite(v) ? v : fallback;
}
function buildGoldDeficitParams(raw, difficulty = "normal") {
...
function freshGoldDeficitState() {
  return { turyDeficytuZlota: 0 };
}
function advanceGoldDeficit(ownerIds, states, getOwnerTreasury, params) {
  const perOwner = [];
...
function _setLastGoldDeficitTicks(ticks) {
  _lastTicks = ticks;
}
function clearLastGoldDeficitTicks() {
  _lastTicks = /* @__PURE__ */ new Map();
}
function isGoldDeficit(ownerId) {
  return _lastTicks.get(ownerId)?.deficytZlota ?? false;
}
function isGoldDeficitStarving(ownerId) {
  return _lastTicks.get(ownerId)?.zlotoDeficytAtrycjaAktywna ?? false;
}
function getGoldDeficitCountdown(ownerId, karencjaTur) {
...
function applyGoldDeficitStatMultToCombatUnit(cu, mult) {
...
function applyGoldDeficitHpLoss(units, ownerId, hpFrac, getMaxHp) {
