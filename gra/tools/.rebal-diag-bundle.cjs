...
// tools/.rebal-diag-entry.ts
var rebal_diag_entry_exports = {};
__export(rebal_diag_entry_exports, {
  TerenBazowy: () => TerenBazowy,
  countLandSeaHexes: () => countLandSeaHexes,
  generateMap: () => generateMap,
  rebalanceLandSeaRatio: () => rebalanceLandSeaRatio
});
module.exports = __toCommonJS(rebal_diag_entry_exports);
...
function applyLandFractionByScore(hexes, landScores, targetLandFraction) {
  ...
  let adjusted = 0;
  if (land < targetLand) {
    const morseCandidates = keys.filter((k) => hexes[k].terenBazowy === "morze" /* Morze */).sort((a, b) => (landScores.get(b) ?? 0) - (landScores.get(a) ?? 0));
    ...
    const landCandidates = keys.filter((k) => hexes[k].terenBazowy !== "morze" /* Morze */).sort((a, b) => {
...
function rebalanceLandSeaRatio(hexes, width, height, targetLandFraction) {
  const clamped = Math.max(0.15, Math.min(0.85, targetLandFraction));
        ...
        while (land > targetLand) {
          ...
          for (let r = 0; r < height; r++) {
            for (let q = 0; q < width; q++) {
              ...
              if (!hex) continue;
              const score = erodeTerrainRank(hex.terenBazowy) * 10 - countMorseNeighbors(hexes, q, r);
              if (score < bestScore) {
                bestScore = score;
                bestKey = hexKey(q, r);
              }
...
var e_start_params_default = {
  ...
  skala_mapy: {
    ...
    "Super Huge": { rywale_ai: 10, miasta_panstwa: 9, typy_cywilizacji: 9, hex_w: 336, hex_h: 238 }
  },
  generator_e2: {
...
function generateMap(width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, seed = 42, typ = "kontynenty", genOpts) {
  ...
  const continentCenters = buildContinentCenters(
    ...
    nCenters,
    typ === "kontynenty" ? {
      radiusMin: 0.14 + radiusBoost,
      radiusMax: 0.22 + radiusBoost,
      minCenterDist: landFraction >= 0.55 ? 0.24 : 0.3
    } : void 0
  );
  const hexes = {};
  ...
  applyLandFractionByScore(hexes, landScores, landFraction);
  finalizeCoastAndInlandWater(hexes, width, height, 3, coastOpts);
