...
var terrain_improvements_default = {
  _meta: {
    ...
    klucze_surowcow_ASCII: "drewno | kamien | glina | ruda | zelazo | stal | bydlo | owce | lama | kon | sol | luksus"
  },
  farma: {
    nazwa: "Farma",
    epoka: 1,
  ...
  pastwisko: {
    nazwa: "Pastwisko / zagroda",
    epoka: 1,
    bonus: {
      zywnosc: 1,
      praca: 1
    },
    surowiecOdblokowany: ["bydlo", "owce", "lama", "kon"],
    surowiecOdblokowany_uwaga: "odblokowuje dostep do surowcow hodowlanych: bydlo LUB owce LUB lama LUB kon \u2014 zalezne od zloza na polu; klucze wg resources.json (brak id \u2014 propozycja EKONOMIA, wymaga uzgodnienia z DANE)",
    teren: "\u0141\u0105ka, R\xF3wnina, Wzg\xF3rza",
    warunek: "zas\xF3b zwierz\u0119cy: Ko\u0144 / Krowa / Owca / Lama",
    koszt_praca: 20,
    tech: "Oswojenie zwierz\u0105t",
    odblokowuje: "Konie (rydwany / kawaleria)"
  },
  kopalnia: {
    nazwa: "Kopalnia",
    epoka: 1,
  ...
  tarasy: {
    ...
    surowiecOdblokowany: null,
    teren: "Wzg\xF3rza",
    warunek: "UNIKALNE kulturowe (Inkowie); +\u017Cywno\u015B\u0107 na wzg\xF3rzach",
    koszt_praca: 25,
    tech: "-",
    odblokowuje: "",
    uwagi: "tylko cyw. Inkowie; bonus \u017Cywno\u015Bciowy, NIE kultura"
  },
  lodzie_rybackie: {
  ...
  plantacja: {
    ...
    surowiecOdblokowany: "luksus",
    surowiecOdblokowany_uwaga: "klucz 'luksus' \u2014 surowce luksusowe (winogrona/oliwki/przyprawy) nie maja osobnych wpisow w resources.json v0.1; propozycja EKONOMIA: jeden klucz 'luksus' jako typ; wymaga decyzji DANE czy to 1 klucz zbiorczy czy osobne klucze (winogrona/oliwki/przyprawy)",
    teren: "\u0141\u0105ka, R\xF3wnina",
    warunek: "surowiec luksusowy (winogrona/oliwki/przyprawy) \u2014 hex.zloze=luksus",
    koszt_praca: 22,
    tech: "Kalendarz",
    odblokowuje: "Luksus (+zadowolenie po\u015Brednio)"
  },
  warzelnia_soli: {
  ...
  fort: {
    ...
    tech: "Wojskowosc",
    odblokowuje: "",
    uwagi: "\u017Belazo (epoka 3); zasieg_terytorium=10; +100% Obrona w trybie obozowania (decyzja Naster 2026-06-25)"
  },
  droga: {
    nazwa: "Droga",
    epoka: 1,
...
function normalizeImprovementKey(raw) {
  ...
  const key = raw === "tartak" ? "tartak" : raw;
...
function tileYield(tile) {
  ...
  if (tile.ulepszenieKey) {
    applyImprovementBonus(out, tile.ulepszenieKey);
...
function okolicaTiles(centerQ, centerR, radius, map2, isWorkable) {
  const out = [];
  const rad = Number.isFinite(radius) && radius > 0 ? Math.floor(radius) : 1;
  for (const key of Object.keys(map2.hexes)) {
...
function assignWorkedTiles(centerQ, centerR, population, map2, yieldOf, opts = {}) {
  const radius = opts.radius ?? cityRangeForPopulation(population);
  const tiles = okolicaTiles(centerQ, centerR, radius, map2, opts.isWorkable);
...
function resolveWorkedTiles(city2, map2, yieldOf, opts = {}) {
  const pop = Math.max(0, Math.floor(city2.population ?? 0));
  const radius = opts.radius ?? cityRangeForPopulation(pop);
  const tryb = opts.tryb ?? city2.okolicaTryb ?? DEFAULT_OKOLICA_TRYB;
  const focus = opts.focus ?? city2.okolicaFocus ?? DEFAULT_OKOLICA_FOCUS;
  const isWorkable = opts.isWorkable;
  if (tryb === "reczny") {
    const reczne = opts.reczne ?? city2.okolicaReczne ?? {};
    const tiles = okolicaTiles(city2.q, city2.r, radius, map2, isWorkable);
  ...
  return assignWorkedTiles(city2.q, city2.r, pop, map2, yieldOf, {
    radius,
    isWorkable,
...
// src/game/playtestMiastoEkonomia.ts
var PLAYTEST_MIASTO_SEED = 271828;
function isLandWorkable(map2, q, r) {
  const hex = map2.hexes[keyOf(q, r)];
  if (!hex) return false;
  const t = hex.terenBazowy;
  return t !== "morze" /* Morze */ && t !== "gory" /* Gory */;
}
function findCityCenter(map2) {
  const cq = Math.round((map2.szerokoscQ - 1) / 2);
  const cr = Math.round((map2.wysokoscR - 1) / 2);
  for (let dq = -6; dq <= 6; dq++) {
    for (let dr = -6; dr <= 6; dr++) {
      const q = cq + dq;
      const r = cr + dr;
      if (!isLandWorkable(map2, q, r)) continue;
      let landInRange = 0;
      for (const key of Object.keys(map2.hexes)) {
        const parts = key.split(",");
        ...
        if (isLandWorkable(map2, hq, hr)) landInRange++;
      }
...
function paintOkolicaSandbox(map2, cq, cr) {
  const pattern = [
    "laka" /* Laka */,
    "laka" /* Laka */,
    "rownina" /* Rownina */,
  ...
  for (const key of Object.keys(map2.hexes)) {
    ...
    if (dist === 0) {
      const center = map2.hexes[key];
      if (center) {
        center.terenBazowy = "rownina" /* Rownina */;
        center.nakladka = "brak" /* Brak */;
        center.ulepszenie = "brak" /* Brak */;
      }
      continue;
    }
    if (dist > 9) continue;
    const hex = map2.hexes[key];
    if (!hex) continue;
    if (hex.terenBazowy === "morze" /* Morze */ || hex.terenBazowy === "gory" /* Gory */) continue;
    hex.terenBazowy = pattern[idx % pattern.length];
    hex.nakladka = "brak" /* Brak */;
    hex.ulepszenie = "brak" /* Brak */;
    idx++;
    if (dist === 2 && q === cq + 2 && r === cr) {
      hex.terenBazowy = "laka" /* Laka */;
      hex.ulepszenie = "farma" /* Farma */;
    }
    if (dist === 3 && q === cq - 2 && r === cr + 1) {
      hex.nakladka = "las" /* Las */;
    }
  }
}
function grantSandboxTechs(data2) {
  const ids = [];
  for (const row of data2.tech) {
    const ep = row.Epoka ?? "";
...
function buildPlaytestMiastoEkonomia(map2, data2) {
  const center = findCityCenter(map2);
  if (!center) return null;
  paintOkolicaSandbox(map2, center.q, center.r);
  const city2 = makePlayerCity(center.q, center.r);
  const sight = citySightRadius(city2.population, city2.kulturaSkumulowana ?? 0);
  const visible = Array.from(computeVisibleAt(center.q, center.r, map2, sight));
  ...
  return {
    ...
    focusQ: center.q,
    focusR: center.r,
    playerCityId: city2.id,
    initialProduction,
    grantedTechIds: grantSandboxTechs(data2)
  };
}
...
function generujSwiat(seed, rozmiar2, typ = "kontynenty") {
  const effectiveSeed = seed && seed !== 0 ? seed : (Date.now() ^ 3735928559) >>> 0 || 42;
  const [w, h] = ROZMIAR_DIMS[rozmiar2];
...
var tech_default = {
  ...
  technologie: [
    ...
    },
    {
      ...
      "Odblokowuje ulepszenie terenu": "Farma",
      "Koszt nauki": 8,
...
var civ_ai_default = {
  ...
  _meta: {
    zrodlo: "Cywilizacje.xlsx/AI-zachowanie"
  }
};
...
var civ_params_default = {
  cywilizacje: [
    {
      Cywilizacja: "Grecy",
      preferowaneBudynki: "Biblioteka",
      preferowaneJednostki: "piechota, elita",
      modWzrostu: 1,
      modEkonomii: 1.02,
      uwagi: "seed CYW 2026-06-27"
    },
    ...
    {
      Cywilizacja: "Chi\u0144czycy",
      preferowaneBudynki: "Biblioteka, Rynek",
      preferowaneJednostki: "\u0142ucznicy, kawaleria lekka",
      modWzrostu: 1.05,
      modEkonomii: 1.03,
      uwagi: "seed CYW 2026-06-27"
    },
    ...
    },
    {
      Cywilizacja: "Sumerowie",
      preferowaneBudynki: "Biblioteka",
      preferowaneJednostki: "rydwany, \u0142ucznicy",
      modWzrostu: 1,
      modEkonomii: 1.01,
      uwagi: "seed CYW 2026-06-27"
    },
    {
      Cywilizacja: "Celtowie",
      preferowaneBudynki: "Koszary, Mur",
      preferowaneJednostki: "piechota, elita",
      modWzrostu: 1,
      modEkonomii: 0.98,
      uwagi: "seed CYW 2026-06-27"
  ...
  _meta: {
    zrodlo: "Cywilizacje.xlsx/Parametry-cyw"
  }
};
...
function loadGameData() {
  return {
    ...
    civAi: civ_ai_default,
    civParams: civ_params_default,
...
function hexToWorkedTile(hex) {
  const uKey = normalizeImprovementKey(String(hex.ulepszenie ?? "brak"));
  return {
