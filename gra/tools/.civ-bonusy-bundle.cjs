...
function applyArmyHungerStatMultToCombatUnit(cu, mult) {
  ...
  const progRaw = cu["Prog dezercji (% health)"];
  const progScaled = progRaw === null || progRaw === void 0 ? progRaw : round42(progRaw * (2 - mult));
  return {
    ...cu,
...
function applyGoldDeficitStatMultToCombatUnit(cu, mult) {
  if (mult >= 1 || mult <= 0) return cu;
  const progRaw = cu["Prog dezercji (% health)"];
  const progScaled = progRaw === null || progRaw === void 0 ? progRaw : round43(progRaw * (2 - mult));
  return {
    ...cu,
    meleeAttack: cu.meleeAttack * mult,
    meleeDefence: cu.meleeDefence * mult,
...
function parseCounterBonusFrac(raw) {
