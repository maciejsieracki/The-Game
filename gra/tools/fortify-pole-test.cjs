...
const CITY_BUNDLE = path.join(os.tmpdir(), `fortify-pole-city-bundle-${TMPDIR_RUN_ID}.cjs`);
esbuild.buildSync({
...
// ---------------------------------------------------------------------------
const COMBAT_BUNDLE = path.join(os.tmpdir(), `fortify-pole-combat-bundle-${TMPDIR_RUN_ID}.cjs`);
esbuild.buildSync({
