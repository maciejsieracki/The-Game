...
// tools/._tmp-spawn-bugs-entry.ts
var tmp_spawn_bugs_entry_exports = {};
__export(tmp_spawn_bugs_entry_exports, {
...
module.exports = __toCommonJS(tmp_spawn_bugs_entry_exports);
...
function computeClusters(map, opts) {
  ...
  const minSeaDist = capitalMinSeaDist(rozmiarMapy);
...
function buildClusterSpawnPlan(input) {
  ...
  const seaDist = buildSeaDistanceField(map.hexes);
  const minSeaDist = capitalMinSeaDist(placement.rozmiarMapy);
