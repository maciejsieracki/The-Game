/**
 * build-kamien-inka-preview.cjs — regeneracja podgladu porownawczego
 * dyspozycje/PODGLAD-KAMIEN-INKA.html (Chaska + Estolica: OBECNY vs OPUS 5).
 *
 ...
 *
 * Uruchamiac z katalogu gra/:   node tools/build-kamien-inka-preview.cjs
 */
...
const ENTRY = path.join(ROOT, 'tools', '.kamien-inka-preview-entry.ts');
const HTML = path.join(ROOT, '..', 'dyspozycje', 'PODGLAD-KAMIEN-INKA.html');
...
// '$&', ktora w stringu zastepczym String.replace znaczy "cale dopasowanie"
// i podmienia fragmenty bundla na sam tag <script>.
