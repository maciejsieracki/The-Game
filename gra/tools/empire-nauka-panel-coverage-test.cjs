'use strict';
/**
 * empire-nauka-panel-coverage-test.cjs — Panel 11 zakladek Faza 2 (Nauka).
 ...
 * `nauka`: żadna INNA bramka nie dowodzi, że `if (block === 'nauka') body += nauka;` w
 ...
 * `tsc` zielono, panel renderowałby się CAŁKOWICIE PUSTY dla klika "Nauka".
 *
 ...
 *      `if (block === 'nauka') body += nauka;` istnieje w bloku składania `body`.
 ...
 *      `nauka` ustawionym na marker zawierający dokładnie ten sam tekst, jaki
 *      `renderNaukaSection()` faktycznie emituje (`<div class="civ-emp-eyebrow">NAUKA
 *      IMPERIUM</div>`, zweryfikowane osobną asercją source-text) — dla `block='nauka'` body
 ...
 *      parametrach (block='nauka') — dowodzi, że test B faktycznie łapie regresję (panel
 ...
 *
 * Run from gra/: node tools/empire-nauka-panel-coverage-test.cjs
 */
...
console.log('========================================================================');
console.log('Faza 2 (Nauka) -- render(): blok "nauka" faktycznie dolewa tresc do body (nie tylko routing)');
console.log('========================================================================\n');

// ---------------------------------------------------------------------------
// A. Weryfikacja strukturalna na prawdziwym pliku (source-text).
// ---------------------------------------------------------------------------
console.log('A. render(): linia "if (block === \'nauka\') body += nauka;" istnieje w bloku skladania body (source-text)');

const marker = '<div class="civ-emp-eyebrow">NAUKA IMPERIUM</div>';
assert(
  'renderNaukaSection() faktycznie emituje marker "NAUKA IMPERIUM" (kotwica markera dla dowodu B nie ucieka od prawdziwego kodu)',
...
const naukaLine = "if (block === 'nauka') body += nauka;";
assert(
  'wycinek zawiera "' + naukaLine + '"',
  assemblySnippet.includes(naukaLine),
);
console.log('');

// ---------------------------------------------------------------------------
// B. Dowod wykonywalny -- dosłowny wycinek skladania body URUCHOMIONY (new Function) z
//    zaslepkami tresci per blok; `nauka` niesie realny marker sekcji.
// ---------------------------------------------------------------------------
console.log('B. Dowod wykonywalny -- wyciety wycinek skladania body uruchomiony z realnym markerem "NAUKA IMPERIUM"');
...
const naukaMock = marker + '<div class="civ-emp-hero pos">+17 PN / turę</div>';
const mockArgsFor = (block) => [
  block, '<div>PARAMS</div>', '<div>MOC</div>', '<div>ZASOBY</div>', '<div>SKARBIEC</div>', '<div>PRACA</div>',
  naukaMock, '<div>RELIGIA</div>', '<div>SPICHLERZ</div>', '<div>ARMIA</div>',
  '<div>KULT</div>', '<div>SUR</div>', '<div>HANDEL</div>',
];

const bodyNauka = safeRun('B1', runReal, mockArgsFor('nauka'));
assert(
  'RDZEN: block="nauka" -> body NIEPUSTE i zawiera marker "NAUKA IMPERIUM" (dokladnie luka opisana przez wzorzec M2/Evaluatora)',
  typeof bodyNauka === 'string' && bodyNauka.length > 0 && bodyNauka.includes(marker),
  { bodyNauka },
);

for (const otherBlock of ['moc', 'all', 'praca', 'religia', 'skarbiec']) {
  const bodyOther = safeRun('B-' + otherBlock, runReal, mockArgsFor(otherBlock));
  assert(
    `block="${otherBlock}" -> body NIE zawiera marker Nauki (bloki sie nie mieszaja)`,
...
// ---------------------------------------------------------------------------
// C. Weryfikacja negatywna (mutacyjna) -- linia "if (block === 'nauka') body += nauka;"
//    usunieta -> panel calkowicie pusty. Trwaly odpowiednik zleconej recznej mutacji
//    kontrolnej (usun -> czerwono -> przywroc).
// ---------------------------------------------------------------------------
console.log('C. Weryfikacja negatywna -- mutacja usuwajaca "body += nauka;", lapana przez B1');

const mutatedSnippet = assemblySnippet.replace(
  naukaLine,
  "// MUTANT: linia 'body += nauka;' usunieta -- regresja analogiczna do M2 (Skarbiec, Faza 1)",
);
...
const bodyMutant = safeRun('C1', runMutant, mockArgsFor('nauka'));
assert(
  'MUTANT (linia usunieta) lapany CZERWONO: block="nauka" -> body PUSTE (""), NIE zawiera marker',
  bodyMutant === '',
  { bodyMutant },
);
console.log('');
