...
function buildOceanReachableRiverHexKeys(hexes, paths2, kinds2, width, height, oceanConnected) {
  ...
  const reached2 = /* @__PURE__ */ new Set();
  const queue = [];
  const seed = (k) => {
    if (!riverHexes.has(k) || reached2.has(k)) return;
    reached2.add(k);
    queue.push(k);
  };
  ...
  while (queue.length > 0) {
    ...
    for (const edgeIdx of h.rzeka.krawedzie) {
      ...
      if (riverHexes.has(nk) && !reached2.has(nk)) {
        reached2.add(nk);
        queue.push(nk);
      }
    }
...
function pruneOrphanRiverPaths(hexes, paths2, kinds2, width, height) {
  ...
  for (let iter = 0; iter < 10; iter++) {
    const reached2 = buildOceanReachableRiverHexKeys(hexes, curPaths, curKinds, width, height);
    const ocean = oceanConnectedWaterKeys(hexes, width, height);
    ...
    for (let i = 0; i < curPaths.length; i++) {
      ...
      const connected = p.every((c) => {
        ...
        return reached2.has(hexKey(c.q, c.r));
      });
      if (!connected) {
        dropped = true;
        continue;
...
function checkTributaryJunctions(paths2, kinds2, hexes, width, height) {
  const dims = width != null && height != null ? { width, height } : inferMapDimsFromHexes(hexes);
  const ocean = oceanConnectedWaterKeys(hexes, dims.width, dims.height);
  const hexToPaths = /* @__PURE__ */ new Map();
  for (let pi = 0; pi < paths2.length; pi++) {
    for (const p of paths2[pi] ?? []) {
      const k = hexKey(p.q, p.r);
  ...
  let firstFail = null;
  for (let pi = 0; pi < paths2.length; pi++) {
    if (kinds2[pi] !== "tributary") continue;
    const path = paths2[pi];
...
function generateRivers(hexes, width, height, rand, opts = {}) {
  ...
  const tryPlaceFeederRiver = (sq, sr, junctionKeysOverride) => {
    ...
    for (const j2 of rankNetworkJunctionCandidates(sq, sr, junctionKeys, seaDist, traceMax, rand)) {
      const p = traceTributary(hexes, sq, sr, j2.q, j2.r, traceMax, seaDist, rand);
...
function placeVillages(hexes, cities, existingCamps, seed, opts) {
  ...
  for (let i = candidates.length - 1; i > 0; i--) {
    let rnd;
    [lcg, rnd] = lcgNext(lcg);
    const j2 = Math.floor(rnd * (i + 1));
    const tmp = candidates[i];
    candidates[i] = candidates[j2];
    candidates[j2] = tmp;
  }
...
function generujSwiat(seed, rozmiar, typ = "kontynenty", genOpts, onProgress) {
  ...
  const map2 = generateMap(w, h, effectiveSeed, typ, genOpts);
  onProgress?.("Pozycje startowe", 100, 6, 7);
  return map2;
}

// tools/.trib-diag3-entry.ts
var map = generujSwiat(42, "standardowy", "ziemia", {
  worldDensity: { rivers: "medium", forest: "medium", desert: "medium", relief: "medium" }
});
var W = map.szerokoscQ;
var H = map.wysokoscR;
var paths = map.riverPaths ?? [];
var kinds = map.riverPathKinds ?? [];
var reached = buildOceanReachableRiverHexKeys(map.hexes, paths, kinds, W, H);
for (let i = 0; i < paths.length; i++) {
  const p = paths[i];
  const allOrphan = p.every((c) => !reached.has(`${c.q},${c.r}`));
  if (!allOrphan) continue;
  console.log(`orphan path #${i} kind=${kinds[i]} len=${p.length} start=${p[0].q},${p[0].r} end=${p[p.length - 1].q},${p[p.length - 1].r}`);
  if (i > 5) break;
}
var j = checkTributaryJunctions(paths, kinds, map.hexes, W, H);
console.log("junction ok", j.ok, "violations", j.violations, j.firstFail);
