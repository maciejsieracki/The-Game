...
// tools/.budowa-lista-entry.ts
var budowa_lista_entry_exports = {};
__export(budowa_lista_entry_exports, {
  dedupeBudowaLista: () => dedupeBudowaLista,
  defaultBudowaListaNazwa: () => defaultBudowaListaNazwa,
  loadBudowaListaBiblioteka: () => loadBudowaListaBiblioteka
});
module.exports = __toCommonJS(budowa_lista_entry_exports);
...
// src/game/cities.ts
var _budowaListaSzablonSeq = 0;
function newBudowaListaSzablonId() {
  _budowaListaSzablonSeq += 1;
  return `budowa-lista-${Date.now()}-${_budowaListaSzablonSeq}`;
}
function defaultBudowaListaNazwa(biblioteka) {
...
function isLegacyBudowaListaSzablony(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return false;
  const o = raw;
  return "A" in o || "B" in o || "C" in o;
}
function isBudowaListaSzablonArray(raw) {
  return Array.isArray(raw) && raw.every(
    (item) => item && typeof item === "object" && typeof item.id === "string" && typeof item.nazwa === "string" && Array.isArray(item.budynki)
  );
}
function loadBudowaListaBiblioteka(meta) {
...
function dedupeBudowaLista(budynki) {
