...
// tools/_tmp-quick-perf-entry.ts
...
function resolveRiverTraceForMap(mapMenuLabel, riversTier) {
  const { w: w2, h: h2 } = menuLabelToDims(mapMenuLabel);
  const params = resolveRiverMapParams(riversTier, w2, h2);
...
function continentCenterCount(width, height, typ2) {
  if (typ2 === "kontynenty") return KONTYNENTY_ZONE_COUNT;
...
function reapplyForestOverlay(hexes, scratch, thresholds, typ2, forestTier, continentOf, nContinents, mapHeight) {
  ...
  let assigned = 0;
  const partitions = landPartitionKeysForDistribution(hexes, typ2, continentOf, nContinents);
  for (const part of partitions) {
    ...
    for (const land of landHexesByCoverageCell(massSet, cellSize).values()) {
      ...
      const minForest = typ2 === "pangea" ? 0 : 1;
...
function reapplyLandTerrain(hexes, scratch, seed, thresholds, mapHeight, reliefTier = "medium") {
  ...
  for (const [key, hex] of Object.entries(hexes)) {
    ...
    if (fullTb === "gory" /* Gory */ || fullTb === "wzgorza" /* Wzgorza */) {
      ...
      reliefCandidates.push({ key, n: s2.mtnNoise, want: fullTb });
    } else {
...
function landPartitionKeysForDistribution(hexes, typ2, continentOf, nContinents) {
  if ((typ2 === "kontynenty" || typ2 === "wyspy") && continentOf && nContinents > 0) {
...
function applyReliefByNoiseRank(hexes, scratch, tier, width, height, typ2 = "pangea", continentOf = null, nContinents = 0) {
  const partitions = landPartitionKeysForDistribution(hexes, typ2, continentOf, nContinents);
  for (const part of partitions) {
...
function defaultLandFractionForTyp(typ2) {
  switch (typ2) {
    case "pangea":
      return 0.6;
    case "kontynenty":
...
function applyLandFractionByContinent(hexes, landScores, continentOf, nContinents, targetLandFraction, width, height) {
  ...
  const scoreSums = zoneKeys.map(
    (keys) => keys.reduce((s2, k) => s2 + (landScores.get(k) ?? 0), 0)
  );
...
function minTinyIslandHexesForTyp(typ2) {
  switch (typ2) {
    case "pangea":
      return 10;
    case "kontynenty":
...
function finalizeLandMassAfterCoast(hexes, typ2, width, height, coastOpts, coastPasses = 2) {
  const minHexes = minTinyIslandHexesForTyp(typ2);
  let total = 0;
...
function resetRiverProfileStats() {
  _riverProfile = null;
}
function getRiverProfileStats() {
...
function buildRiverFieldCache(hexes, width, height) {
  const t02 = RIVER_PROFILE_ON ? rpNow() : 0;
...
function trimRiverPathRings(hexes, path) {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    for (const m of marks) {
      const s2 = laid.get(m.key) ?? /* @__PURE__ */ new Set();
      s2.add(m.edge);
      laid.set(m.key, s2);
    }
  }
...
function collectPathHexKeysForKinds(paths2, kinds2, allowed) {
  const allow = allowed instanceof Set ? allowed : new Set(allowed);
  const keys = /* @__PURE__ */ new Set();
  for (let i = 0; i < paths2.length; i++) {
    if (!allow.has(kinds2[i] ?? "main")) continue;
    for (const p of paths2[i] ?? []) keys.add(hexKey(p.q, p.r));
  }
  return keys;
}
...
function buildOceanReachableRiverHexKeys(hexes, paths2, kinds2, width, height, oceanConnected) {
  const ocean = oceanConnected ?? oceanConnectedWaterKeys(hexes, width, height);
...
function verifyRiverNetworkConnectivity(hexes, paths2, kinds2, width, height) {
  const riverHexes = collectRiverHexKeys(hexes);
  const reached = buildOceanReachableRiverHexKeys(hexes, paths2, kinds2, width, height);
...
function pruneOrphanRiverPaths(hexes, paths2, kinds2, width, height) {
  let curPaths = paths2.slice();
  let curKinds = kinds2.slice();
...
function pruneRiversNotReachingRealSea(hexes, paths2, kinds2, width, height) {
  const goal = oceanConnectedWaterKeys(hexes, width, height);
  const drop = /* @__PURE__ */ new Set();
  for (let i = 0; i < paths2.length; i++) {
    if (kinds2[i] !== "main") continue;
    if (!pathReachesRealSea(hexes, paths2[i] ?? [], width, height, goal)) drop.add(i);
  }
  if (drop.size === 0) return { paths: paths2, kinds: kinds2 };
  const keptPaths = paths2.filter((_, i) => !drop.has(i));
  const keptKinds = kinds2.filter((_, i) => !drop.has(i));
  clearRiverMarks(hexes);
  for (const p of keptPaths) markRiverPath(hexes, p);
  return pruneOrphanRiverPaths(hexes, keptPaths, keptKinds, width, height);
}
function ensureRiverOutlets(hexes, paths2, kinds2, width, height) {
  let result = pruneOrphanRiverPaths(hexes, paths2, kinds2, width, height);
...
function scrubStrayRiverHexMarks(hexes, paths2) {
...
function pruneInvalidShortRiverPaths(hexes, paths2, kinds2, width, height, oceanConnected) {
  const mediumKeys = collectPathHexKeysForKinds(paths2, kinds2, ["medium"]);
  if (mediumKeys.size === 0) return { paths: paths2, kinds: kinds2 };
  const drop = /* @__PURE__ */ new Set();
  for (let i = 0; i < paths2.length; i++) {
    if (kinds2[i] !== "short") continue;
    const p = paths2[i] ?? [];
    const p0 = p[0];
    if (!p0) {
      drop.add(i);
...
function collectRiverPathHexKeys(paths2) {
  const keys = /* @__PURE__ */ new Set();
  for (const path of paths2) {
...
function pathHasValidRiverOutlet(hexes, path, paths2, kinds2, width, height) {
  if (!path?.length) return false;
  const ocean = oceanConnectedWaterKeys(hexes, width, height);
  if (pathEndsAtSea(hexes, path, width, height, ocean)) return true;
  const reached = buildOceanReachableRiverHexKeys(hexes, paths2, kinds2, width, height, ocean);
...
function landMassHasMainRiver(massLandKeys, paths2, kinds2) {
  const massSet = new Set(massLandKeys);
  for (let i = 0; i < paths2.length; i++) {
    if (kinds2[i] !== "main") continue;
    for (const p of paths2[i] ?? []) {
      if (massSet.has(hexKey(p.q, p.r))) return true;
    }
  }
  return false;
}
function cellHasMainRiverSource(cellLand, paths2, kinds2) {
...
function enforceHardRiverGridStarts(hexes, massSet, cellSize, seaDist, riverPaths, gridCtx, maxLen, reliefSourceBonus, expandSourceRadius, minInlandFromSea, baseSourceSep, acceptLen) {
  ...
  const cellAvgSeaDist = (cells) => {
    let s2 = 0;
    for (const [q, r] of cells) s2 += seaDist.get(hexKey(q, r)) ?? 0;
    return cells.length > 0 ? s2 / cells.length : 0;
  };
...
function ensureDepositGridCoverage(hexes, tier, typ2, continentOf, nContinents, rand) {
...
var LIVESTOCK_IMPROVEMENT_KEYS = IMPROVEMENT_KEYS.filter((k) => {
  const s2 = IMPROVEMENTS[k]?.surowiecOdblokowany;
  return typeof s2 === "string" && LIVESTOCK_SUROWIEC_KEYS.has(s2);
});
...
function generateMap(width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, seed = 42, typ2 = "kontynenty", genOpts, onProgress) {
  const effectiveSeed = seed || 42;
  const wgn = resolveWorldGenNumbers(genOpts);
  const landFraction = resolveLandFraction(genOpts, typ2);
  const terrainTh = {
  ...
  const nCenters = continentCenterCount(width, height, typ2);
  ...
  if (typ2 === "kontynenty") {
    ...
    zoneCenters = buildContinentCenters(
      rand,
      nCenters,
      { width, height, anchorCenter: typ2 === "pangea" }
    );
  }
  const nZones = typ2 === "kontynenty" || typ2 === "wyspy" ? zoneCenters.length : 0;
  const zoneOf = typ2 === "kontynenty" ? assignContinentIndices(width, height, zoneCenters) : typ2 === "wyspy" ? assignIslandGridIndices(width, height) : null;
  const hexes = {};
  ...
  for (let r = 0; r < height; r++) {
    for (let q = 0; q < width; q++) {
      ...
      if (isInLatitudinalOceanBuffer(r, height, typ2 === "ziemia")) {
        landMask = 0;
      }
      ...
      const { terenBazowy, nakladka } = classifyTerrain(
        ...
        desNoise,
        terrainTh,
        climateBandAt(q, r, height, typ2 === "ziemia"),
  ...
  const coastOpts = typ2 === "pangea" ? { maxInlandPoolSize: 24 } : typ2 === "kontynenty" ? { maxInlandPoolSize: 8 } : void 0;
  if (typ2 === "kontynenty") {
  ...
  if (typ2 === "kontynenty" || typ2 === "pangea") {
    removeTinyLandIslands(hexes, typ2 === "kontynenty" ? 8 : 10);
    if (typ2 === "pangea") {
      trimDeepOceanBays(hexes, width, height);
    }
    finalizeCoastAndInlandWater(hexes, width, height, 3, coastOpts);
  ...
  enforceMapBorderOcean(hexes, width, height);
  enforceLatitudinalOceanBuffer(hexes, width, height, typ2 === "ziemia");
  if ((typ2 === "kontynenty" || typ2 === "wyspy") && zoneOf) {
    ...
    removeTinyLandIslands(hexes, typ2 === "wyspy" ? 4 : 5);
    trimEnclosedOceanOnly(hexes, width, height);
  } else if (typ2 === "ziemia") {
  ...
  applyReliefByNoiseRank(hexes, terrainScratch, reliefTier, width, height, typ2, zoneOf, nZones);
  enforceMapBorderOcean(hexes, width, height);
  enforceLatitudinalOceanBuffer(hexes, width, height, typ2 === "ziemia");
  if (typ2 !== "pangea") {
    purgeInlandWaterForMultiLandTyp(hexes, width, height);
  ...
  finalizeLandMassAfterCoast(hexes, typ2, width, height, coastOpts, 2);
  ...
  applyClimateBandsToHexes(hexes, height, effectiveSeed, typ2 === "ziemia");
  enforceLatitudinalOceanBuffer(hexes, width, height, typ2 === "ziemia");
  purgeInlandWaterForMultiLandTyp(hexes, width, height);
  ...
  reapplyForestOverlay(hexes, terrainScratch, terrainTh, typ2, forestTier, zoneOf, nZones, height);
  ensureForestGridCoverage(hexes, terrainScratch, forestTier, typ2, zoneOf, nZones, rand);
  ...
  ensureDepositGridCoverage(hexes, reliefTier, typ2, zoneOf, nZones, rand);
...
// tools/_tmp-quick-perf-entry.ts
var SEED = 42;
var typ = process.env.CIV_WORLD_TYP ?? "pangea";
var { w, h } = rozmiarToDims("duzy");
resetRiverProfileStats();
var t0 = import_node_perf_hooks.performance.now();
var map = generateMap(w, h, SEED, typ, {
  worldDensity: { rivers: "medium", forest: "medium", desert: "medium", relief: "medium" },
  mapSizeMenuLabel: "Du\u017Cy"
});
var ms = import_node_perf_hooks.performance.now() - t0;
var s = getRiverProfileStats();
var kinds = map.riverPathKinds ?? [];
var paths = map.riverPaths ?? [];
var badOutlet = 0;
for (let i = 0; i < paths.length; i++) {
  const path = paths[i];
  if (!path?.length || !pathHasValidRiverOutlet(map.hexes, path, paths, kinds, w, h)) badOutlet++;
}
var net = verifyRiverNetworkConnectivity(map.hexes, paths, kinds, w, h);
console.log(JSON.stringify({
  typ,
  totalS: +(ms / 1e3).toFixed(2),
  rivers: paths.length,
