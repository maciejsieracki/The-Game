'use strict';
/**
 * .dip-run2.cjs -- TEMP fresh runner (mirrors diplomacy-test.cjs incl. loader + remap).
 * Run:  DIP_SRC_DIR=/tmp/diptest/src node tools/.dip-run2.cjs
...
const esbuild = (() => {
  ...
  catch (e) { console.error('[dip-run2] esbuild not found.'); process.exit(1); }
})();

const ENTRY_FILE  = path.resolve(__dirname, '.dip-entry2.ts');
const BUNDLE_FILE = path.resolve(__dirname, '.dip-bundle2.cjs');
...
} catch (e) { console.error('[dip-run2] bundling failed:\n', e.message || e); process.exit(1); }
...
eq(DIPLOMACY_PARAMS.progPoboczneWojna,       15, 'prog Poboczne Wojna = 15 (remap)');
...
// 8. loadDiplomacyParams -- panel (json.params) -> overrides
...
eq(applyDiplomaticEvent(rel(20, 30), 'dar', loadDiplomacyParams({ params: { dar_zaufanie: 9 } })).zaufanie, 29, 'override z loadera -> applyDiplomaticEvent (+9)');

// 9. progPoboczneWojna remap -- minor war branch reachable
