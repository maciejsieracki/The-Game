/**
 * .braz-rydwan-woly-preview-entry.ts — entry TYLKO do podgladu porownawczego
 * (dyspozycje/PODGLAD-BRAZ-RYDWAN-WOLY.html). Nie jest czescia gry.
 *
 * Pokazuje: DZIS (fallback buildUnitModel('rydwan', ..., 'Rydwan (woly)') —
 * kon + uprzaz rydwanowa) obok NOWEGO buildRydwanWolyBrazOpus5 (4-kolowy woz
 * sumeryjski ze Sztandaru z Ur, para wolow w jarzmie karkowym).
 *
 * REGENERACJA (z katalogu gra/, BEZ npm run build — nie odpala
 * prebuild/export-data.py, wiec nie nadpisuje JSON-ow w gra/data):
 *
 *   node tools/build-braz-rydwan-woly-preview.cjs
 */
...
// Środek bbox nowego rydwanu (mierzone traversem): x∈[-0.19,0.19],
// y∈[0,0.79], z∈[-0.26,0.45] — środek z ≈ +0.09 (woły ciągną wóz do przodu,
// więc geometryczny środek leży bliżej wołów niż skrzyni). Cel kamery i
// dystans dobrane pod TEN układ (dłuższy niż Taran), nie pod stary wzorzec.
const TARGET = new THREE.Vector3(0, 0.36, 0.09);

function setView(kind: string): void {
  const el = THREE.MathUtils.degToRad(kind === 'gra' ? 52 : kind === 'oko' ? 12 : 28);
  const dist = kind === 'gra' ? 2.30 : kind === 'oko' ? 2.05 : 1.95;
...
const specs: [string, (c: number) => THREE.Group][] = [
  ['old', (c) => buildUnitModel('rydwan', c, 'Rydwan (woły)')],
  ['new', buildRydwanWolyBrazOpus5],
];
