...
// tools/_tmp-river-phase-timing-entry.ts
...
function normPlMenuLabel(label2) {
  return label2.toLowerCase().replace(/ł/g, "l").replace(/[ó]/g, "o").replace(/[ąà]/g, "a").replace(/[ę]/g, "e").replace(/[żź]/g, "z").replace(/[^a-z0-9]/g, "");
}
...
function normMenuLabel(label2) {
  return normPlMenuLabel(label2);
}
...
function formatRiverProfileReport(totalMs2) {
  const s = getRiverProfileStats();
  if (!s) return "(profiler off \u2014 set CIV_RIVER_PROFILE=1)";
  const riverMs2 = s.generateRiversMs + s.topUpMs;
  const denom = totalMs2 ?? riverMs2;
  const pct2 = (ms) => denom > 0 ? (ms / denom * 100).toFixed(1) : "0.0";
  return [
    "=== RIVER PROFILE (CIV_RIVER_PROFILE=1) ===",
    `fieldCache:        ${s.fieldCacheMs.toFixed(0)}ms (${pct2(s.fieldCacheMs)}%) \xD7${s.fieldCacheCalls}`,
    `generateRivers:    ${s.generateRiversMs.toFixed(0)}ms (${pct2(s.generateRiversMs)}%)`,
    `  stage1 (main):   ${s.genStage1Ms.toFixed(0)}ms (${pct2(s.genStage1Ms)}%)`,
    `  stage2 (medium): ${s.genStage2Ms.toFixed(0)}ms (${pct2(s.genStage2Ms)}%) rounds=${s.genStage2Rounds}`,
    `  stage3 (short):  ${s.genStage3Ms.toFixed(0)}ms (${pct2(s.genStage3Ms)}%)`,
    `  decor tributary: ${s.genDecorMs.toFixed(0)}ms (${pct2(s.genDecorMs)}%)`,
    `  gen dry-patch:   ${s.genDryPatchMs.toFixed(0)}ms (${pct2(s.genDryPatchMs)}%)`,
    `topUp total:       ${s.topUpMs.toFixed(0)}ms (${pct2(s.topUpMs)}%)`,
    ...s.topUpPassMs.map((ms, i) => `  pass ${i + 1}:        ${ms.toFixed(0)}ms (${pct2(ms)}%)`),
    `  hardStarts:      ${s.topUpHardStartsMs.toFixed(0)}ms (${pct2(s.topUpHardStartsMs)}%)`,
    `  dryPatch:        ${s.topUpDryPatchMs.toFixed(0)}ms (${pct2(s.topUpDryPatchMs)}%)`,
    `  grid+proximity:  ${s.topUpGridProxMs.toFixed(0)}ms (${pct2(s.topUpGridProxMs)}%)`,
    `traceRiver:        ${s.traceRiverMs.toFixed(0)}ms (${pct2(s.traceRiverMs)}%) \xD7${s.traceRiverCalls}`,
    `aStarRiverToSea:   ${s.aStarMs.toFixed(0)}ms (${pct2(s.aStarMs)}%) \xD7${s.aStarCalls}`,
    `forceFill:         \xD7${s.forceFillCalls}`,
    `rivers subtotal:   ${riverMs2.toFixed(0)}ms`
  ].join("\n");
}
...
function collectPathHexKeysForKinds(paths, kinds2, allowed) {
...
function buildOceanReachableRiverHexKeys(hexes, paths, kinds2, width, height, oceanConnected) {
  const ocean = oceanConnected ?? oceanConnectedWaterKeys(hexes, width, height);
...
function pruneOrphanRiverPaths(hexes, paths, kinds2, width, height) {
...
function pruneRiversNotReachingRealSea(hexes, paths, kinds2, width, height) {
  ...
  if (drop.size === 0) return { paths, kinds: kinds2 };
...
function ensureRiverOutlets(hexes, paths, kinds2, width, height) {
  let result = pruneOrphanRiverPaths(hexes, paths, kinds2, width, height);
...
function pruneInvalidShortRiverPaths(hexes, paths, kinds2, width, height, oceanConnected) {
  const mediumKeys = collectPathHexKeysForKinds(paths, kinds2, ["medium"]);
  if (mediumKeys.size === 0) return { paths, kinds: kinds2 };
  const drop = /* @__PURE__ */ new Set();
  for (let i = 0; i < paths.length; i++) {
...
function pathHasValidRiverOutlet(hexes, path, paths, kinds2, width, height) {
  if (!path?.length) return false;
  const ocean = oceanConnectedWaterKeys(hexes, width, height);
  if (pathEndsAtSea(hexes, path, width, height, ocean)) return true;
  const reached = buildOceanReachableRiverHexKeys(hexes, paths, kinds2, width, height, ocean);
...
function landMassHasMainRiver(massLandKeys, paths, kinds2) {
...
function cellHasMainRiverSource(cellLand, paths, kinds2) {
...
function topUpRiverGridCoverage(hexes, width, height, riverPaths, riverKinds, rand, riversTier = "medium", minLen = 4, maxLen = 40, riverParams, onProgress) {
  ...
  const masses = groupLandMassKeys(hexes).filter((m) => m.length >= 8).sort((a, b) => b.length - a.length);
  const riverPerf = buildRiverPerfCtx(masses, params2.areaScale);
  const topUpPasses = effectiveTopUpPasses(params2.topUpPasses, riverPerf);
  if (topUpPasses === 0) {
    onProgress?.(100);
...
function normMenuLabel2(label2) {
  return normPlMenuLabel(label2);
}
function rozmiarFromMenuLabel(label2) {
  const n = normMenuLabel2(label2);
...
function menuLabelToDims(label2) {
  return rozmiarToDims(rozmiarFromMenuLabel(label2));
}

// tools/_tmp-river-phase-timing-entry.ts
var SEED = 42;
var SIZE = (process.env.CIV_MAP_SIZE ?? "standardowy").toLowerCase();
var ROZMIAR = SIZE === "duzy" || SIZE === "du\u017Cy" ? "duzy" : "standardowy";
var DENSITY = {
  rivers: "medium",
  forest: "medium",
  desert: "medium",
...
function msSince(t02) {
  return import_node_perf_hooks.performance.now() - t02;
}
function fmtMs(ms) {
...
var label = ROZMIAR === "duzy" ? "Du\u017Cy" : "Standardowy";
console.log("=== PARAMETRY RZEK (" + label + " " + w + "\xD7" + h + ", tier medium, seed=" + SEED + ") ===");
...
var map = generateMap(w, h, SEED, "kontynenty", {
  worldDensity: DENSITY,
  mapSizeMenuLabel: label
});
var totalMs = msSince(t0);
var kinds = map.riverPathKinds ?? [];
var riverCount = map.riverPaths?.length ?? 0;
console.log("\n=== MAP GEN TOTAL ===");
console.log("  czas: " + (totalMs / 1e3).toFixed(2) + "s");
console.log("  rzeki: total=" + riverCount + " main=" + kinds.filter((k) => k === "main").length + " medium=" + kinds.filter((k) => k === "medium").length + " short=" + kinds.filter((k) => k === "short").length + " tributary=" + kinds.filter((k) => k === "tributary").length);
var stats = getRiverProfileStats();
if (!stats) {
  console.error("\nBRAK PROFILU \u2014 ustaw CIV_RIVER_PROFILE=1");
  process.exit(1);
}
var riverMs = stats.generateRiversMs + stats.topUpMs;
var nonRiverMs = totalMs - riverMs;
...
var rows = [
  ...
  ["  etap2 medium (\xD7" + stats.genStage2Rounds + " rund)", stats.genStage2Ms],
  ...
  ["traceRiver (\xD7" + stats.traceRiverCalls + ")", stats.traceRiverMs],
  ["aStarRiverToSea (\xD7" + stats.aStarCalls + ")", stats.aStarMs]
];
