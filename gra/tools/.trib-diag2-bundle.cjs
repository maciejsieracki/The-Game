...
for (let i = 0; i < paths.length; i++) {
  ...
  if (!pathEndsAtSea(map.hexes, paths[i], W, H)) badMain++;
}
console.log("badMain", badMain, "totalMain", kinds.filter((k) => k === "main").length);
var net = verifyRiverNetworkConnectivity(map.hexes, paths, kinds, W, H);
console.log("orphanCount", net.orphanCount, "riverHexCount", net.riverHexCount);
