'use strict';
/** battle-roster-test.cjs — scout/civil exclusion from battle rosters (Teby x3 fix) */
const fs = require('fs');
const path = require('path');
const esbuild = require(path.resolve(__dirname, '..', 'node_modules', 'esbuild'));

const ENTRY = path.join(__dirname, '.battle-roster-entry.ts');
const OUT = path.join(__dirname, '.battle-roster-bundle.cjs');
...
console.log('battle-roster-test');

// Pole: kotwica + sąsiad dist≤1
